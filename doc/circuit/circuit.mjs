...
/*    Last change :  Wed Jan 14 13:39:09 2026 (serrano)                */
...
function k0(attrs) {
   ...
   const e2 = gate.label({ stroke, class: "E2", baseline: "text-bottom" }, "E'", e2x, e2y);
   const e_gw = 30;
...
function seq(attrs, P, Q) {
   ...
   const height = (p.height + q.height) + km*9;
   const connectm = 8;
   ...
   const ep_w = gate.wireM({ stroke, class: classE}, [e.x, e.y + 2], [null, p.y - km], [p.e.x, null], [null, p.y]);
   ...
   const e_w = gate.wireM({ stroke, class: classE }, [p.e2.x, p.y], [null, p.y - km], [e_g.x + 8, e_g.y + connectm]);
   const ep2_w = gate.wireM({ stroke, class: classE }, [e_g.lx, e_g.outy], [e2.x, null], [null, e2.y + 2]);

   const eq_w = gate.wireM({ stroke, class: classE, dot: "branch" }, [p.x - 2*km, p.y - km], [null, q.y - km], [q.e.x, null], [null, q.y]);
   const eq2_w = gate.wireM({ stroke, class: classE, id: "q-to-E'" }, [q.e2.x, q.y], [null, q.y - km], [q.lx + 2*km, null], [null, e_g.ly - connectm], [e_g.x + 8, null]);

   // go
   const gox = x;
   const goy = p.go.Y;
   const go_w = gate.wireM({ stroke, class: classGO, label: "GO" }, [x + lm, p.go.Y], [p.x, null]);

   // res
   const resx = x;
   const resy = p.res.Y;
   const pRes_w = gate.wireM({ stroke, class: classRES, label: "RES" }, [x + lm, p.res.Y], [p.x, null]);
   const qRes_w = gate.wireM({ stroke, class: classRES, dot: "branch" }, [p.x - 3*km, p.res.Y], [null, q.res.Y], [q.x, null]);

   // susp
   const suspx = x;
   const suspy = p.susp.Y;
   const pSusp_w = gate.wireM({ stroke, class: classSUSP, label: "SUSP" }, [x + lm, p.susp.Y], [p.x, null])
   const qSusp_w = gate.wireM({ stroke, class: classSUSP, dot: "branch" }, [p.x - 4*km, p.susp.Y], [null, q.susp.Y], [q.x, null]);

   // kill
   const killx = x;
   const killy = p.kill.Y;
   const pKill_w = gate.wireM({ stroke, class: classKILL, label: "KILL" }, [x + lm, p.kill.Y], [p.x, null]);
   const qKill_w = gate.wireM({ stroke, class: classKILL, dot: "branch" }, [p.x - 5*km, p.kill.Y], [null, q.kill.Y], [q.x, null]);
   ...
   const sel_w = gate.wireM({ stroke, class: getClass(attrs, "sel-wire"), label: "SEL", anchor: "r" }, [sel_g.lx, sel_g.outy], [selx - lm, null]);
   
   // p.sel -> sel
   const pSel_w = gate.wireM({ stroke, class: getClass(attrs, "p-to-sel") }, [p.lx, p.sel.Y], [sel_g.x + 8, null]);
   // q.sel -> sel
   const qSel_w = gate.wireM({ stroke, class: getClass(attrs, "q-to-sel") }, [q.lx, q.sel.Y], [q.sel.X + km*3, null], [null, sel_g.ly - connectm], [sel_g.x + 8, null]);

   // p.k0 -> q.go
   const k0x = x + width;
   const k0y = p.k0.Y + connectm;
   const pk0_w = gate.wireM({ stroke, class: getClass(attrs, "k0-to-go") },
			    [p.lx, p.k0.Y],
			    [p.lx + km, null],
			    [null, p.ly + km],
			    [q.x - km, null],
   ...
   // k0
   const qk0_w = gate.wireM({ stroke, class: classK0, label: "K0", anchor: "r" },
			    [q.lx, q.k0.Y],
			    [q.lx + km * 4, null],
			    [null, k0y],
			    [k0x - lm, null]);
   ...
   const k1_w = gate.wireM({ stroke, class: classK1, label: "K1", anchor: "r" }, [k1_g.lx, k1y], [k1x - lm, null]);
   const pk1_w = gate.wireM({ stroke, class: classK1 }, [p.lx, p.k1.Y], [k1_g.x + 8, null]);
   const qk1_w = gate.wireM({ stroke, class: classK1 },
			    [q.lx, q.k1.Y],
			    [q.lx + 5*km, null],
   ...
   const k2_w= gate.wireM({ stroke, class: classK2, label: "K2", anchor: "r" }, [k2_g.x + k2_g.width, k2_g.y + k2_g.height/2], [k2x -lm, null]);
   const pk2_w = gate.wireM({ stroke, class: classK2 }, [p.lx, p.k2.Y], [k2_g.x + 8, null]);
   const qk2_w = gate.wireM({ stroke, class: classK2 },
			    [q.lx, q.k2.Y],
			    [q.lx + 6*km, null],
...
function pause(attrs) {
   ...
   const classK1 = getClass(attrs, "K1");
   const classK2 = getClass(attrs, "K2");
   const classREG = getClass(attrs, "REG");
   ...
   const e_w = gate.wireM({ stroke, class: classE }, [e.x, e.y + 2], [null, e_g.outy], [e_g.x, null]);
   const e2_w = gate.wireM({ stroke, class: classE }, [e_g.lx, e_g.outy], [e2.x, null], [null, e2.y + 2]);
   ...
   const go_w = gate.wireM({ stroke, class: classGO, dot: "branch" },
			   [gox + km, goy],
			   [null, go_and_not_kill.ly - connectm],
			   [go_and_not_kill.x, null]);

   const susp_w = gate.wireM({ stroke, class: classSUSP, label: "SUSP", anchor: "l" }, [suspx, suspy], [susp_and_not_kill.x, null]);
   const kill1_w = gate.wireM({ stroke, class: classKILL, label: "KILL", anchor: "l", dot: "not" }, [killx, killy], [susp_and_not_kill.x - km, null], [null, susp_and_not_kill.ly - connectm], [susp_and_not_kill.x, null]);
   const kill2_w = gate.wireM({ stroke, class: classKILL, dot: "branch" }, [killx + 2*km, killy], [null, killy + km + connectm]);
   const kill3_w = gate.wireM({ stroke, class: classKILL, dot: "not" }, [kill2_w.x, kill2_w.ly], [go_and_not_kill.x, null]);
   ...
   const OR_w = gate.wireM({ stroke, class: getClass(attrs, "toreg") }, [OR.lx, OR.outy], [OR.lx + km, null]);

   const go_and_not_kill_w = gate.wireM({ stroke, class: getClass(attrs, "go_and_not_kill") },
   ...
   const susp_and_not_kill_w = gate.wireM({ stroke, class: classSUSP },
					  [susp_and_not_kill.lx, susp_and_not_kill.outy],
					  [susp_and_not_kill_and_reg.x, null]);
   const susp_and_not_kill_and_reg_w = gate.wireM({ stroke, class: getClass(attrs, "susp_and_not_kill_and_reg") },
   ...
   const reg_w = gate.wireM({ stroke, class: classREG }, [reg.lx, reg.outy], [reg.lx + km*3, null], [null, k0_g.ly - connectm], [k0_gx, null]);
   const reg2_wh = (reg.outy) - (susp_and_not_kill.ly) + km;
   const reg2_wx = reg.lx + km;
   const reg2_wy = reg.outy
   const reg2_w = gate.wireM({ stroke, class: classSUSP, dot: "branch" },
			     [reg2_wx, reg2_wy],
   ...
   const selx = x + (width - lm);
   const sely = goy;
   const sel_w = gate.wireM({ stroke, class: classSEL, label: "SEL", anchor: "r", dot: "branch" }, [reg.lx + km*2, reg.outy], [null, sely], [selx, null]);
   
   // k0
   const k0_w = gate.wireM({ stroke, class: classK0, label: "K0", anchor: "r" }, [k0_g.lx, k0_g.outy], [k0x, null]);
   const res_w = gate.wireM({ stroke, class: classRES, label: "RES", anchor: "l" }, [resx, resy], [k0_g.x, null]);

   // k1
   const k1x = selx;
   const k1y = k0_g.outy + lm;
   const assig_g = gate.assig({ stroke, class: classGO, height: assigSize }, ex + esl, goy - assigSize/2);
   const go2_w = gate.wireM({ stroke, class: classGO, label: "GO", anchor: "l" }, [gox, goy], [assig_g.x, null]);
   const k1_w = gate.wireM({ stroke, class: classK1, label: "K1", anchor: "r" }, [assig_g.lx, assig_g.outy], [assig_g.lx + lm, null], [null, k0_g.outy + lm], [k1x, null]);
   ...
   const k2_w = gate.wireM({ stroke, class: classK2, label: "K2", anchor: "r" }, [k2_g.lx, k2_g.outy], [k2x, null]);
   const k20_w = gate.wireM({ stroke, class: classK2, label: "0", anchor: "l" }, [k2_g.x - km, k2_g.outy], [k2_g.x, null]);
...
function emit(attrs) {
   ...
   const esr = 1*width/2 - assigSize/2;
   const e = gate.label({ stroke, class: "E", baseline: "text-bottom" }, "E", ex, ey);
   const e2 = gate.label({ stroke, class: "E2", baseline: "text-bottom" }, "E'", e2x, ey);
   const e_gw = 30;
   const e_g = gate.assig({ stroke, class: "E", width: e_gw }, assigx, ey + e_gw/2);
   ...
   const s = gate.label({ stroke, class: "s", baseline: "text-bottom" }, "s", e2x + lm/3, ey);
...
function loop(attrs, P) {
   ...
   const selww = 50;
   const width = p.width + km*18;
   const height = (p.height) + km*6;
   const connectm = 8;
   const assigSize = 30;
   ...
   const e = gate.label({ stroke, class: classE, baseline: "text-bottom" }, "E", ex, ey);
   const ep_w = gate.wireM({ stroke, class: classE}, [e.x, e.y + 2], [null, p.y - 2*km], [p.e.x, null], [null, p.y]);
   
   const e2 = gate.label({ stroke, class: classE, baseline: "text-bottom" }, "E'", e2x, e2y);
   const ep2_w = gate.wireM({ stroke, class: classE }, [p.e2.x, p.y], [null, p.y - 2*km], [e2.x, null], [null, e2.y + 2]);
   ...
   const go_w = gate.wireM({ stroke, class: classGO, label: "GO" }, [gox, goy], [or_g.x + 8, or_g.ly - connectm]);
   const or_w = gate.wireM({ stroke, class: classGO }, [or_g.lx, or_g.outy], [or_g.lx + lm/2, null], [null, p.go.Y], [p.go.X, null]);

   // res
   const resx = gox;
   const resy = p.res.Y;
   const pRes_w = gate.wireM({ stroke, class: classRES, label: "RES" }, [resx, resy], [p.x, null]);

   // susp
   const suspx = gox;
   const suspy = p.susp.Y;
   const pSusp_w = gate.wireM({ stroke, class: classSUSP, label: "SUSP" }, [suspx, suspy], [p.x, null]);

   // kill
   const killx = gox;
   const killy = p.kill.Y;
   const pKill_w = gate.wireM({ stroke, class: classKILL, label: "KILL" }, [killx, killy], [p.x, null]);

   // sel
   const selx = x + width - lm;
   const sely = p.sel.Y;
   const sel_w = gate.wireM({ stroke, class: classSEL, label: "SEL", anchor: "r" }, [p.sel.X, sely], [selx, null]);
   ...
   const k0_w = gate.wireM({ stroke, class: classK0, label: "K0", anchor: "r" }, [k0_g.lx, k0_g.outy], [k0x, null]);
   const k00_w = gate.wireM({ stroke, class: classK0, label: "0", anchor: "l" }, [k0_g.x - km, k0_g.outy], [k0_g.x, null]);

   const k0loop_w = gate.wireM({ stroke, class: classK0 },
			       [p.k0.X, p.k0.Y],
			       [p.k0.X + km, null],
			       [null, p.y - km],
			       [or_g.x - km, null],
   ...
   const k1x = k0x;
   const k1y = p.k1.Y;
   const k1_w = gate.wireM({ stroke, class: classK1, label: "K1", anchor: "r" }, [p.k1.X, k1y], [k1x, null]);
   
   // k2
   const k2x = k0x;
   const k2y = p.k2.Y;
   const k2_w = gate.wireM({ stroke, class: classK2, label: "K2", anchor: "r" }, [p.k2.X, k2y], [k2x, null]);
