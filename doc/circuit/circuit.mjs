...
function par(attrs, P, Q) {
   ...
   function xemNew() {
      ...
      const pQem_w = gate.wire({ stroke, class: getClass(attrs, "par sel xem q"), dot: "*-" },
			       ...
			       [null, qem_g.outy],
			       [qem_g.xx, null]);
      const pem_w = gate.wire({ stroke, class: getClass(attrs, "par res xem p"), dot: "*-" },
			      [pRes_w.lx, p.ly + km],
			      [pem_g.x - km, null],
			      [null, pem_g.yy],
			      [pem_g.yx, null]);
      const qem_w = gate.wire({ stroke, class: getClass(attrs, "par res xem q"), dot: "*-" },
			      [qem_g.x - km, p.ly + km],
			      [null, qem_g.yy],
			      [qem_g.yx, null]);
   ...
   function xem() {
      return (attrs.synchronizer === "new") ? xemNew() : xemOrig();
   }
   ...
   const qSel_w = gate.wire({ stroke, class: getClass(attrs, "par sel q"), dot: "*-" },
			    [pem_g.x - 4*km, q.sel.Y], [null, sel_g.yy], [sel_g.yx, null]);
...
function prog(attrs, P) {
   ...
   const res = gate.and({ stroke, class: getClass(attrs, "prog res") }, boot.x + 8*km, p.res.Y - dummy_g.height/2);
   const res_w = gate.wire({ stroke, class: getClass(attrs, "prog res") }, [res.lx, res.outy], [p.x - km, null], [null, p.res.Y], [p.x, null]);

   const go_res_w = gate.wire({ stroke, class: getClass(attrs, "prog boot res" ), dot: "*-o" },
			      [res.x - 2*km, boot.outy],
			      [null, res.xy],
			      [res.x, null]);

   const sel = gate.or({ stroke, class: getClass(attrs, "prog globalsel") }, boot.x + 2*km, res.y + 2*km);
