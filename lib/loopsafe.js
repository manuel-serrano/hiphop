...
/*    Last change :  Wed Nov 12 08:59:13 2025 (serrano)                */
...
function unsafeLoop(node) {
   ...
   function walk(node) {
	 ...
	 if (dumpLoops) {
	    console.log(
	       "loop", `${basename(node.loc.filename)}:${node.loc.pos}`,
...
ast.Every.prototype.K = function(env) {
   if (!this.immediate) {
      this.children.forEach(c => c.K());
      return this.$K = new Set([1]);
   } else {
      this.$K = Kseq(this.children, [this.trapName].concat(env));
   }
}
