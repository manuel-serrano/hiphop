...
/*    Last change :  Wed Jul 15 10:51:44 2026 (serrano)                */
...
if (!("union" in Set.prototype)) {
   Set.prototype.union = function(...sets) {
      ...
      this.forEach(e => news.add(e));
      sets.forEach(s => s.forEach(e => news.add(e)));
...
function unsafeLoop(node) {
   ...
   function walk(node) {
	 ...
	 if (dumpLoops) {
	    console.log(
	       node.constructor.name, `${node.loc.filename}:${node.loc.pos}`,
