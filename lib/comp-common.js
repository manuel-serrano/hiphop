...
/*    Last change :  Wed Jul 15 09:32:46 2026 (serrano)                */
...
export {
   assert, assertCircuitInterfaceConnections, assertCircuitInterface,
   debugConnect, debug, debugTrace, debugTraceCircuit, debugSynchronizer,
   Circuit, makeCircuitInterface,
...
function basename(path) {
   ...
   if (j >=0) {
      return path.substring(j + 1, j.length);
   } else {
      return path;
   }
}
