...
/*    Last change :  Fri Sep 18 16:11:52 2026 (serrano)                */
...
function traceNode(astNode) {
   if (debug) {
      return `${astNode.constructor.name}[${astNode.depth}|${astNode.trapDepth + 2}]@${astNode.loc.pos} ${astNode.id}`
   }
}
...
/*    isCyclic ...                                                     */
/*    -------------------------------------------------------------    */
/*    Returns true is the generated circuit is cylic, returns false    */
/*    otherwise.                                                       */
/*---------------------------------------------------------------------*/
function isCyclic(nets) {
...
function compile(mach) {
   ...
   // check is the circuit is cyclic
   mach.cyclic = isCyclic(mach.nets);
