...
/*    Last change :  Fri Feb  7 10:00:17 2025 (serrano)                */
...
import { LogicalNet, SignalNet, ActionNet, RegisterNet, SignalExpressionNet,
	 TestExpressionNet, WireNet, FAN } from "./net.js";
...
const KAP = DEBUG ? parseInt(DEBUG) : 100000;
let K = 0;
...
function debugNet(u, msg) {
   if (DEBUG) {
      consoleLog(u.id, msg,
		 "in:", u.fanin_list.map(f => f.net.id).join(", "),
		 "out:", u.fanout_list.map(f => f.net.id).join(", "));
   }
}
...
function unwire(machine) {
   consoleLog("unwire =====================");
   machine.nets.forEach(u => {
      if (u instanceof WireNet) {
	 const fanin_list = u.fanin_list;
	 const fanout_list = u.fanout_list;
	 
	 disconnectNetFanins(u);
	 disconnectNetFanouts(u);

	 fanin_list.forEach(fpu => {
	    const p = fpu.net;
	    fanout_list.forEach(fus => {
	       const s = fus.net;
	       const polarity =
		  xor(fpu.polarity, fus.polarity) ? FAN.NEG : FAN.STD;

	       p.connectTo(s, polarity);
	    });
	 });
