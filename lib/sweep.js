...
function sweep( machine ) {
   ...
   */
   /*
   netList = machine.nets.filter(net => (net.fanout_list.length === 0 && net.fanin_list.length === 0) ? true : false);
   //console.log(netList.length);
   //netList = machine.nets.filter(net => net.noSweep ? true : false);
   netList.forEach(net=>{
	   //if((net.fanout_list.length === 0) && (net.fanin_list.length === 0)){
		if(net.noSweep === true){
			net.noSweep = false;
			//console.log(net.debug_id)
			removeBuffer(net);
		}   
