...
/*    Last change :  Thu Mar 12 12:52:52 2026 (serrano)                */
...
/*    Debug                                                            */
/*---------------------------------------------------------------------*/
const DEBUG = process.env.HIPHOP_TRACE?.split(",")?.find?.(n => n === "sweep"); 
...
function isTautology(net) {
   ...
   for (let i = 0; i < faninList.length - 1; i++) {
	 ...
	 for (let j = i + 1; j < faninList.length; j++) {
	    if (faninList[j].net === n && !faninList[j].dependency) {
...
function isConnected(from, to, polarity) {
   return from.fanoutList.find(fus => fus.net === to && fus.polarity === polarity && !fus.dependency && !fus.dependency );
}
...
function simplifyPureExpressionNets(machine) {
   ...
   function signalPureEquation(fun) {
	 ...
	 .replace(/}/, "")
	 .trim();
      const e = s.match(/const ([a-zA-Z_$][a-zA-Z0-9_$]*)=this[.]\1;[ ]*return \1[.](now|pre);/);
   ...
   function simplyPureExpressionNet(n) {
	 ...
	 && n.func
	 && n.accessor_list.length === 1
	 && n.faninList.filter(f => f.dependency).length === 1) {
...
/*    unreachableNets ...                                              */
/*---------------------------------------------------------------------*/
function unreachableNets(machine) {
   ...
   function mark(n) {
	 ...
	 // use for instead of foreach as it seems to use significantly
	 // less stack space (3 dec 2025).
	 for (let i = n.fanoutList.length - 1; i >= 0; i--) {
	    mark(n.fanoutList[i].net);
	 }
      }
   }
   ...
   machine.nets.forEach(n => {
	 ...
	 if (n.faninList.length === 0 && n.dependencyCount === 0) {
	    mark(n);
	 }
   ...
   machine.nets = machine.nets.filter(n => n.isInKnownList);
}
...
function sweepNets(machine, fullSweep) {
   ...
   function noFaninNet(u) {
	 ...
	 if (fus.dependency) {
	    return;
	 } else if (s instanceof ActionNet) {
	    return;
	 } else if (s.faninList.length === 0) {
	    // this happens when u is connected twice to s and the previous
	    // connection has fully resolved s
	    ...
	    if ((!u.neutral && fus.polarity) || (u.neutral && !fus.polarity)) {
	       ...
	       consoleLog("   ->", s.id, "disconnect (noFanin)");
	    ...
	    if ((!u.neutral && !fsu.polarity) || (u.neutral && fsu.polarity)) {
	       // transform the target and gate into an and-gate with no fanin
	       // i.e., the constant true
	       consoleLog("   ->", s.id, "->", true);
	       disconnectNetFanins(s);
	       s.neutral = true;
	       res = true;
   ...
   function oneFaninNet(u) {
	 ...
	 && (!isConst0(u.faninList[0].net)
	    || u.faninList[0].antagonist.dependency
	    || !u.faninList[0].antagonist.polarity)) {
	 return;
	 ...
	 consoleLog("   ", p.id, "=>", fanoutList.map(f=> f.net.id));
	 res = true;
      }
   }
   ...
   function cnstToAnd(u) {
      const fusList = u.fanoutList;
      deleteNet(machine, u, `cnstToAnd ${u.id}+[${fusList.map(f => f.net.id).join(", ")}]`)
      disconnectNetFanouts(u);
   }
   ...
   function tautology(u) {
      ...
      consoleLog(K++ + ":", "tautology", "[" + u.id + "] ->", isLogicalOr(u));
   ...
   machine.nets.forEach(u => {
	 ...
	 if (u.fanoutList.find(fus => fus.net === u)) return;
	 ...
	 if (!u.sweeped && isCnstToAnd(u)) cnstToAnd(u);
	 if (!u.sweeped && !machine.cyclic && isTautology(u)) tautology(u);
      }
   });
...
function unwire(machine) {
   machine.nets.forEach(u => {
      if (u instanceof WireNet && u.sweepable && (K < machine.sweep || machine.sweep < 0)) {
...
/*    sweepConstants ...                                               */
/*---------------------------------------------------------------------*/
function sweepConstants(machine) {
   // remove constant for better circuits display
   machine.nets.forEach(u => {
      if (!u.sweepable) return;
      if (!u.sweeped && u.faninList.length === 0 && !u.neutral) {
	 u.fanoutList.forEach(fus => {
	    const s = fus.net;

	    if (s instanceof ActionNet) {
	       // do nothnig
	    } else if (isLogicalAnd(s)) {
	       if (fus.polarity) {
		  consoleLog("   ->", s.id, "->", false);
		  disconnectNetFanins(s);
		  s.neutral = false;
	       } else if (!fus.polarity) {
		  // simply disconnect the two nets
		  consoleLog("   ->", s.id, "disconnect (const)", s.constructor.name);
		  disconnectNetFanin(s, fus);
	       }
	    } else if (isLogicalOr(s)) {
	       const fsu = fus.antagonist;
...
function sweep(machine) {
   ...
   if (machine.sweep !== 0) {
	 ...
	 if (machine.sweep < -1) {
	    // remove constants (for improved circuit display)
	    sweepConstants(machine);
	 }
      }
   }
   
   if (machine.unreachable) {
      unreachableNets(machine);
   }

   if (machine.dumpNets) {
      machine.nets.forEach(n => n.isInKnownList = false);
