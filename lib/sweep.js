...
/*    Last change :  Wed Nov 12 16:04:14 2025 (serrano)                */
...
function simplifyPureExpressionNets(machine) {
   ...
   function signalPureEquation(fun) {
	 ...
	 .trim();
      ...
      if (e) {
	 return { sig: e[1], test: e[2] };
      } else {
	 return { sig: false, test: false };
      }
   }
   ...
   function simplyPureExpressionNet(n) {
	 ...
	 const { sig, test } = signalPureEquation(n.func);
...
function unreachableNets(machine) {
   ...
   function mark(n) {
	 ...
	 n.fanoutList.forEach(fanout => mark(fanout.net));
      }
   }
