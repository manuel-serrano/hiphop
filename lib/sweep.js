...
/*    Last change :  Thu Feb 13 07:24:39 2025 (serrano)                */
...
/*    isOrToOr ...                                                     */
/*    -------------------------------------------------------------    */
/*    True iff NET an or-gate connected to other or-gates.             */
/*---------------------------------------------------------------------*/
...
/*    isAndToAnd ...                                                   */
/*    -------------------------------------------------------------    */
/*    True iff NET an and-gate connected to other and-gates.           */
/*---------------------------------------------------------------------*/
...
function unwire(machine) {
   machine.nets.forEach(u => {
      if (u instanceof WireNet && (K < KAP || KAP < 0)) {
...
function sweep(machine) {
   ...
   machine.status.sweep.wire = machine.nets.length;
      
   unwire(machine);
      
   if (machine.sweep) {
      machine.status.sweep.unwire = machine.nets.length;
	 ...
	 const w = [];
	 machine.nets.forEach(u => {
	    if (u instanceof WireNet) w.push(u.id + ":" + u.debugName);
	    u.fanoutList.forEach(f => {
	       if (f.net instanceof WireNet) {
