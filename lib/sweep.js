...
import { LogicalNet, SignalNet, ActionNet, RegisterNet } from "./net.js";
...
function simplifyBooleanExpressionNets(machine) {
   ...
   function simplyBooleanExpressionNet(n) {
      if (n instanceof ActionNet && n.accessor_list.length === 1) {
	 const sig = signalBooleanEquation(n.func);

	 if (sig === n.accessor_list[0].signame && sig === "A") {
	    let m = new LogicalNet(n.ast_node, n.type, n.debug_name, n.lvl, n.neutral);

	    Object.assign(m, n);

	    m.fanout_list.forEach(f => f.antagonist.net = m);
	    m.fanin_list.forEach(f => f.antagonist.net = m);
	    m.fanin_list.forEach(f => f.antagonist.dependency = false);
   ...
   machine.nets = machine.nets.filter(n => {
      if (n.sweeped) {
	 n.ast_node.net_list = n.ast_node.net_list.filter(x => x !== n);
	 return false;
      } else {
	 return true;
      }
