...
/*    Last change :  Mon Dec  4 07:54:41 2023 (serrano)                */
...
function sweepOrNets(machine) {
   ...
   machine.nets.filter(net => {
      if (net instanceof LogicalNet && net.LogicalNetType() === "OR") {
	 if (net.fanin_list.length === 1
	    && !net.noSweep
	    && net.fanout_list.length === 1
	    && net.fanin_list[0].polarity) {
	    // find the fanin corresponding net in the fanout's net fanin
	    const outnet = net.fanout_list[0].net;
	    const fanin = outnet.fanin_list.find(fan => fan.net === net);

	    if (fanin.polarity) {
	       const innet = net.fanin_list[0].net;
	       // the polarity is positive on both ends, remove the net
	       sweepNet(machine, net, innet, outnet);
