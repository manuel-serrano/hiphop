...
/*    Last change :  Wed Jan  7 07:54:12 2026 (serrano)                */
/*    Copyright   :  2018-26 serrano                                   */
/*    -------------------------------------------------------------    */
/*    HipHop AST                                                       */
/*=====================================================================*/
"use strict"
...
export { Module, Interface, $Run, Run, Frame, Local, $Exec, Exec }; 
export { Abort, WeakAbort, Loop, LoopEach, Every, Suspend, Fork, Sync, If, Sequence };
export { $Delay, DelaySig, DelayUnary, DelayBinary };
...
class $ASTNode {
   ...
   constructor(tag, id, loc, nodebug, children) {
	 ...
	 throw new TypeError(tag + ": bad form", loc);
      ...
      // parent relationship is only used to improve the dump of the
      // nets.
      children.forEach(c => { if (c) c.parent = this });
   ...
   copy(children) {
      // same as clone but receive children
      let msg = `copy not implemented: ${this.constructor.name}`;

      if (this.location?.filename) {
	 msg += ` @ ${this.loc.filename}:${this.loc.pos}`;
      }
   ...
   clone() {
      return this.copy(this.clone_children());
   }
...
class $Emit extends $ActionNode {
   ...
   constructor(tag, id, loc, nodebug,
		...
		if_accessor_list) {
      super(tag, id, loc, nodebug, null, func, accessor_list);
      ...
      //
      // Signal objects of emitted signal. Used by debugger. Not cloned!
      //
      this.signal_map = {};
   }
...
class $Nothing extends $ASTNode {
   constructor(tag, id, loc, nodebug) {
      super(tag, id, loc, nodebug, null);
   }
   
   copy(children) {
      return new Nothing(this.tag, cloneId(this.id), this.loc, this.nodebug);
   }
...
class $Pause extends $ASTNode {
   ...
   copy(children) {
      return new Pause(this.tag, cloneId(this.id), this.loc, this.nodebug);
   }
}
...
class $Trap extends $ASTNode {
   ...
   constructor(tag, id, loc, nodebug, trap_name, children) {
      ...
      if (!trap_name) {
	 throw new Error(`Trap, illegal trapName "${trap_name}"`);
      }
   }
...
class $Abort extends $CountExpressionNode {
   constructor(tag, id, loc, nodebug, func, accessor_list, immediate,
      ...
      super(tag, id, loc, nodebug, children,
	    func, accessor_list, immediate, func_count, accessor_list_count);
   }
...
class $LoopEach extends $CountExpressionNode {
   constructor(tag, id, loc, nodebug, children, func, accessor_list, 
		func_count, accessor_list_count) {
      super(tag, id, loc, nodebug, children,
	    func, accessor_list, false, func_count, accessor_list_count);
   }
...
class LoopEach extends $LoopEach {
   constructor(tag, id, loc, nodebug, children, func, accessor_list, 
		func_count, accessor_list_count) {
      super(tag, id, loc, nodebug, children,
	    func, accessor_list, func_count, accessor_list_count);
   }
}
...
/*    Sync                                                             */
/*    -------------------------------------------------------------    */
/*    This _dummy_ class is only used by the compiler to distinguish   */
/*    nets that belong to a fork synchronizer. No instance of this     */
/*    class is directly created from a user program.                   */
/*---------------------------------------------------------------------*/
class Sync extends $ASTNode {
   astNode;
   
   constructor(tag, depth, astNode) {
      super(tag, undefined, astNode.loc, true, []);
      this.machine = astNode.machine;
      this.parent = astNode;
      this.astNode = astNode;
      this.depth = depth;
      astNode.children.push(this);
   }
   
   copy(children) {
      return new Sync(this.tag, this.depth, this.astNode);
   }
}
...
function getSelValue(sel) {
   ...
   } else if (sel instanceof net.RegisterNet) {
      return sel.nextValue;
   } else {
      return sel.value;
   }
}
...
class DelaySig extends $Delay {
   ...
   constructor(id, prop) {
      ...
      this.id = id;
      this.prop = prop;
      this.accessor_list = [ new SignalAccessor(id, prop === "pre", false) ];
   } 
}
...
/*    DelayUnary ...                                                   */
/*---------------------------------------------------------------------*/
class DelayUnary extends $Delay {
   op;
   delay;
   
   constructor(op, delay) {
      super();
      this.op = op;
      this.delay = delay;
      this.accessor_list = delay.accessor_list;
   }
}
...
/*    DelayBinary ...                                                  */
/*---------------------------------------------------------------------*/
class DelayBinary extends $Delay {
   op;
   lhs;
   rhs;
   ...
   constructor(op, lhs, rhs) {
      ...
      this.lhs = lhs;
      this.rhs = rhs;
      this.accessor_list = lhs.accessor_list.concat(rhs.accessor_list);
   }
}
