...
/*    Last change :  Mon Oct  1 14:04:54 2018 (serrano)                */
...
class Fork extends ASTNode {
   ...
   appendChild( ast_node, oneshot_reg=true ) {
	 ...
	 let sel = (function check_sel( ast_node ) {
	    if( ast_node.circuit &&
   		ast_node.circuit.sel &&
   		ast_node.circuit.sel.value == true ) {
   	       return true;
	    }
...
class Run extends ASTNode {
   constructor( tag, id, loc, name, nodebug, sigdecllist, body ) {
   ...
   clone() {
      return new Run( this.tag, this.id, this.loc, this.name, this.nodebug,
		      clone_list( this.sigDeclList ), this.clone_children() );
   }
}
...
function computeNodeRegisterId( ast_node, seq ) {
   ...
   for( let i = 0; i < ast_node.children.length; i++ ) {
      ...
      // the dynamic node insertion won't work, for a a reason that I
      // don't uninderstand
