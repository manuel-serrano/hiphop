/*=====================================================================*/
/*    serrano/prgm/project/hiphop/1.3.x/lib/ast.js                     */
/*    -------------------------------------------------------------    */
/*    Author      :  Colin Vidal                                       */
/*    Creation    :  Mon Jul 16 18:04:46 2018                          */
/*    Last change :  Thu Dec 14 07:56:53 2023 (serrano)                */
/*    Copyright   :  2018-23 serrano                                   */
/*    -------------------------------------------------------------    */
/*    HipHop AST                                                       */
/*=====================================================================*/
"use strict"
...
export { IN, OUT, INOUT };
export { $ASTNode, ASTNode, ActionNode, ExpressionNode, CountExpressionNode };
export { Emit, Sustain };
export { Nothing, Pause, Trap, Exit, Halt, Atom, Await };
export { Module, Interface, Run, Frame, Local, Exec }; 
...
class $Atom extends $ActionNode {
   ...
   clone() {
      const a= new Atom(this.tag, this.id, this.loc, this.nodebug, 
...
class $Fork extends $ASTNode {
   ...
   appendChild(ast_node, oneshot_reg=true) {
	 ...
	 throw new SyntaxError('Child cannot be added to sweeped machine, use the "seep: false" option', this.loc);
      }
...
class $Sequence extends $ASTNode {
   ...
   appendChild(ast_node, oneshot_reg=true) {
	 ...
	 throw new SyntaxError('Child cannot be added to sweeped machine, use the "sweep: false" option', this.loc);
      ...
      this.machine.action(machine => {
	 // Check if there is a selected register in parallel children: 
	 // checking only the sel gate is wrong if the parallel was
	 // started at the previous reaction (sel is not yet propagated).
	 //
	 // See tests/run-add-par.js
	 let sel = (function check_sel(node) {
