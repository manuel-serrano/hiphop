...
ExpressionNode.prototype.pretty_print_param = function() {
   function print_accessor(arg) {
      let accessor_buf = arg.signal_name;

      if (arg.get_value)
	 accessor_buf = "?" + accessor_buf;
      if (arg.get_pre)
	 accessor_buf = "pre(" + accessor_buf + ")"
      return accessor_buf;
   }
   ...
   if (this.func) {
      ...
      buf += func.name + "("
      for (let i = 0; i < len; i++) {
	 let arg = this.args_list[i];

	 if (arg instanceof lang.SignalAccessor)
	    buf += print_accessor(arg)
	 else
	    buf += arg;
	 ...
	 if (i + 1 < len)
      ...
      buf += print_accessor(this.args_list[0]);
   }
...
function Await(id, loc, test_pre, immediate, func, args_list, count_func,
	       count_args_list) {
   CountExpressionNode.call(this, id, loc, undefined, func, args_list,
			    count_func, count_args_list);
...
___DEFINE_INHERITANCE___(ExpressionNode, Abort);

exports.Abort = Abort;

Abort.prototype.clone = function() {
   return new Abort(this.id, this.loc, this.test_pre, this.immediate,
