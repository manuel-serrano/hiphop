...
ActionNode.prototype.pretty_print_get_args_list = function() {
   return this.args_list;
}

ActionNode.prototype.pretty_print_args = function() {
   let args_list = this.pretty_print_get_args_list();
   let len = args_list.length;
   let buf = "(";

   for (let i = 0; i < len; i++) {
      buf += this.pretty_print_arg(args_list[i])
      if (i + 1 < len)
	 buf += ", ";
   }
   buf += ")";
...
ActionNode.prototype.pretty_print_arg = function(arg) {
   let arg_buf = "";
...
ActionNode.prototype.pretty_print_param = function() {
   ...
   if (this.func)
      buf += this.func.name + this.pretty_print_args();
   else
      buf += this.pretty_print_arg(this.pretty_print_get_args_list()[0]);
...
LocalSignal.prototype.clone = function() {
   return new LocalSignal(this.id, this.loc, this.signal_name,
			  this.clone_children(), this.init_value,
			  this.reinit_func, this.combine_with, this.valued);
}
...
InputSignal.prototype.clone = function() {
   return new InputSignal(this.id, this.loc, this.signal_name,
			  ...
			  this.combine_with, this.valued, this.task_signal);
}
...
function TaskSignal(id, loc, signal_name) {
   /* Task signal are implicit valued signal, as we don't known if start
      function of exec that uses this signal will return a value */
   InputSignal.call(this, id, loc, signal_name, undefined, undefined, undefined,
		    true, true);
}

st.___DEFINE_INHERITANCE___(InputSignal, TaskSignal);

exports.TaskSignal = TaskSignal;

TaskSignal.prototype.clone = function() {
   return new TaskSignal(this.id, this.loc, this.signal_name);
}

function Exec(id, loc, signal_name, exec_interface, args_list, start_args_list) {
   CountExpressionNode.call(this, id, loc, undefined, undefined, args_list,
			    false, undefined, []);
   this.exec_interface = exec_interface;
   this.start_args_list = start_args_list;
   this.signal_name = signal_name;
}

/* Exec is not a CountExpressionNode, but it contains an implicit Await
   statement (which is a CountExpressionNode). It will break things because
   some methods used to build implicit abort in await use methods only on
   expression node. Not big issue, however, yep, it's dirty, but this is the
   *only* case that dosen't fit very well in this design, so... */
st.___DEFINE_INHERITANCE___(CountExpressionNode, Exec);

exports.Exec = Exec;

Exec.prototype.clone = function() {
   return new Exec(this.id, this.loc, this.signal_name, this.exec_interface,
		   this.args_list, this.start_args_list);
}

Exec.prototype.pretty_print_get_args_list = function() {
   return this.start_args_list;
}

Exec.prototype.pretty_print_param = function() {
   let buf = "";

   if (this.args_list.length > 0)
      buf += " start" + this.pretty_print_args();
   buf += " return " + this.signal_name;
   return buf;
}
