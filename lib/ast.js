...
function ASTNode(id, name, loc, children) {
   ...
   /* unset here, use in compiler to store circuit interface on make_circuit
      phase, and connect on others circuit on connect_circuit phase */
   this.circuit;

   /* use on some instrucation (like present, emit, await) to avoid to
      get the signal objet each time we want to access on it */
   this.signal;

   /* use on instructions that contains expression (and could use signal
      access. Each elements is a pair {accessor:Obj, signal:Obj}  */
   this.signal_accessors;
}
