...
/*    Last change :  Sat Feb 15 08:33:21 2025 (serrano)                */
...
export { computeNodeRegisterId, computeNodeDepth };
...
class $Emit extends $ActionNode {
   ...
   clone() {
      const clone = new Emit(this.tag, this.id, this.loc, this.nodebug, 
			     this.signame_list,
...
class $Sustain extends $Emit {
   ...
   clone() {
      const clone = new Sustain(this.tag, this.id, this.loc, this.nodebug, 
				this.signame_list,
				this.func, clone_list(this.accessor_list),
				this.if_func, clone_list(this.if_accessor_list));
      clone.machine = this.machie;
      return clone;
   }
}
...
class $Nothing extends $ASTNode {
   ...
   clone() {
      const clone = new Nothing(this.tag, this.id, this.loc, this.nodebug);
      clone.machine = this.machine;
      clone.depth = this.depth;
      return clone;
   }
...
class $Pause extends $ASTNode {
   ...
   clone() {
      const clone = new Pause(this.tag, this.id, this.loc, this.nodebug);
      clone.machine = this.machine;
      clone.depth = this.depth;
      return clone;
   }
...
class $Trap extends $ASTNode {
   ...
   clone() {
      const clone = new Trap(this.tag, this.id, this.loc, 
...
class $Exit extends $ASTNode {
   ...
   clone() {
      const clone = new Exit(this.tag, this.id, this.loc, 
		       this.nodebug, this.trapName);
      clone.machine = this.machine;
      clone.depth = this.depth;
      return clone;
...
class $Halt extends $ASTNode {
   ...
   clone() {
      const clone = new Halt(this.tag, this.id, this.loc, this.nodebug);
      clone.machine = this.machine;
      clone.depth = this.depth;
      return clone;
   }
...
class $Atom extends $ActionNode {
   ...
   clone() {
      const clone = new Atom(this.tag, this.id, this.loc, this.nodebug, 
...
class $Await extends $CountExpressionNode {
   ...
   clone() {
      const clone = new Await(this.tag, this.id, this.loc, this.nodebug, this.func,
...
class $Module extends $ASTNode {
   ...
   clone() {
      const clone = new Module(this.tag, this.id, this.loc, this.name,
...
class $Run extends $ASTNode {
   ...
   clone() {
      const clone = new Run(this.tag, this.id, this.loc, this.name, this.nodebug,
		      clone_list(this.sigDeclList), 
...
class $Local extends $ASTNode {
   ...
   //
   clone() {
      const clone = new Local(this.tag, this.id, this.loc, this.nodebug,
                              (this.in_run_context ?
				 this.sigDeclList :
				 clone_list(this.sigDeclList)),
                              this.clone_children());
...
class $Exec extends $ASTNode {
   ...
   clone() {
      const clone =new Exec(this.tag, this.id, this.loc, this.nodebug, 
...
class $Abort extends $CountExpressionNode {
   ...
   clone() {
      const clone = new Abort(this.tag, this.id, this.loc, this.nodebug, this.func,
...
class $WeakAbort extends $CountExpressionNode {
   ...
   clone() {
      const clone = new WeakAbort(this.tag, this.id, this.loc, this.nodebug, 
			    this.func, clone_list(this.accessor_list), 
			    this.immediate,
			    this.func_count, 
			    clone_list(this.accessor_list_count),
...
class $Loop extends $ASTNode {
   ...
   clone() {
      const clone = new Loop(this.id, this.loc, this.nodebug,
			     this.clone_children());
      clone.machine = this.machine;
      clone.depth = this.depth;
      return clone;
...
class $LoopEach extends $CountExpressionNode {
   ...
   clone() {
      const clone = new LoopEach(this.tag, this.id, this.loc, this.nodebug, 
			   this.clone_children(),
			   this.func, clone_list(this.accessor_list),
			   this.func_count, 
			   clone_list(this.accessor_list_count));
...
class $Every extends $CountExpressionNode {
   ...
   clone() {
      const clone = new Every(this.tag, this.id, this.loc, this.nodebug, 
			this.clone_children(),
			this.func, clone_list(this.accessor_list), 
			this.immediate,
			this.func_count, 
...
class $Suspend extends $ExpressionNode {
   ...
   clone() {
      const clone = new Suspend(this.tag, this.id, this.loc, this.nodebug, 
			  this.clone_children(),
			  this.func, clone_list(this.accessor_list),
			  this.immediate);
      clone.machine = this.machine;
...
class $Fork extends $ASTNode {
   ...
   clone() {
      const clone = new Fork(this.tag, this.id, this.loc, 
   ...
   appendChild(ast_node, oneshot_reg=true) {
	 ...
	 throw new SyntaxError("Child can't be a Module (you may want use RUN?)", 
      ...
      if (this.machine.sweep) {
	 throw new SyntaxError('Child cannot be added to sweeped machine, use the "sweep: false" option', this.loc);
      }
...
class $If extends $ExpressionNode {
   ...
   clone() {
      const clone = new If(this.tag, this.id, this.loc, 
		    this.nodebug, this.clone_children(),
...
class $Sequence extends $ASTNode {
   ...
   clone() {
      const clone = new Sequence(this.tag, this.id, this.loc, this.nodebug, 
			   this.clone_children());
      clone.machine = this.machine;
      clone.depth = this.depth;
      return clone;
...
class SignalProperties {
   ...
   clone() {
      const clone = new SignalProperties(this.id, this.loc, this.name,
...
function computeNodeDepth(ast_node, depth, in_loop, in_par_sig) {
   ...
   if (ast_node instanceof Loop || ast_node instanceof LoopEach) {
      ...
      || ast_node instanceof Trap
	     ) {
      // MS 15feb2025, add Trap
      in_par_sig = true;
      if (in_loop) {
	 depth++;
	 in_loop = false;
...
function getSelValue(sel) {
   ...
   if (sel instanceof net.WireNet) {
      if (sel.fanoutList.length > 0) {
	 return getSelValue(sel.fanoutList[0].net);
      } else {
	 return false;
      }
   } else {
