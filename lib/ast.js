...
function Sustain(loc, signal_name, func, exprs) {
   Statement.call(this, "SUSTAIN", loc);
   this.signal_name = signal_name;
   this.func = func;
   this.exprs = exprs;
}
Sustain.prototype = new Statement(undefined, undefined);
...
function Await(loc, signal_name, test_pre, immediate, count) {
...
function Abort(loc, signal_name, test_pre, immediate, count, subcircuit) {
...
function LoopEach(loc, subcircuit, signal_name, test_pre, count) {
   Circuit.call(this, "LOOPEACH", loc, subcircuit);
   this.signal_name = signal_name;
   this.test_pre = test_pre;
   this.count = count;
}
LoopEach.prototype = new Circuit(undefined, undefined, undefined);
...
function Every(loc, subcircuit, signal_name, count, immediate) {
   Circuit.call(this, "EVERY", loc, subcircuit);
   this.signal_name = signal_name;
   this.count = count;
   this.immediate = immediate;
}
Every.prototype = new Circuit(undefined, undefined, undefined);
...
Parallel.prototype.factory = function() {
   return new rk.Parallel(this.machine, this.loc, this.subcircuit);
}
