...
ASTNode.prototype.pretty_print = function (format=null) {
   function default_format(indent, instruction, depth, sel) {
      let buf = "[" + depth + "] ";
   ...
   return this._pretty_print(0, -1, format ? format : default_format);
}

ASTNode.prototype._pretty_print = function(indent, sel, format) {
   function are_child_sel(ast_node) {
      for (let i in ast_node.children) {
	 let cinterface = ast_node.children[i].cinterface;

	 if (cinterface && cinterface.sel && cinterface.sel.value)
	    return true;
      }
   ...
   if (sel == -1 &&
       this.cinterface && this.cinterface.sel && this.cinterface.sel.value
       && !are_child_sel(this))
      sel = true;

   let buf = format(indent,
		    this.constructor.name + this.pretty_print_param(),
		    this.depth,
		    sel == true);
   ...
   for (let i in this.children)
      buf += this.children[i]._pretty_print(indent + 3, sel, format);
...
function ActionNode(loc, children, func, args_list) {
   ASTNode.call(this, loc, children);
...
function ExpressionNode(loc, children, func, args_list, immediate) {
   ActionNode.call(this, loc, children, func, args_list);
...
function CountExpressionNode(loc, children, func, args_list, immediate,
			     func_count, args_list_count ) {
   ExpressionNode.call(this, loc, children, func, args_list, immediate);
...
function Emit(loc, signal_name, func, args_list) {
   ActionNode.call(this, loc, undefined, func, args_list);
...
Emit.prototype.clone = function() {
   return new Emit(this.loc, this.signal_name, this.func, this.args_list);
}
...
function Sustain(loc, signal_name, func, args_list) {
   Emit.call(this, loc, signal_name, func, args_list);
}
...
Sustain.prototype.clone = function() {
   return new Sustain(this.loc, this.signal_name, this.func, this.args_list);
}

function Nothing(loc) {
   ASTNode.call(this, loc, undefined);
}
...
Pause.prototype.clone = function() {
   return new Pause(this.loc);
}

function Trap(loc, trap_name, children) {
   ASTNode.call(this, loc, children);
...
Trap.prototype.clone = function() {
   return new Trap(this.loc, this.trap_name, this.clone_children());
}
...
Exit.prototype.clone = function() {
   return new Exit(this.loc, this.trap_name);
}
...
Halt.prototype.clone = function() {
   return new Halt(this.loc);
}

function Atom(loc, func, args_list) {
   ActionNode.call(this, loc, undefined, func, args_list);
}
...
Atom.prototype.clone = function() {
   return new Atom(this.loc, this.func, this.args_list);
}

function Await(loc, func, args_list, immediate, func_count, args_list_count) {
   CountExpressionNode.call(this, loc, undefined, func, args_list, immediate,
			    func_count, args_list_count);
}
...
Await.prototype.clone = function() {
   return new Await(this.loc, this.func, this.args_list, this.immediate,
...
function Abort(loc,
	       ...
	       args_list_count,
	       children) {
   CountExpressionNode.call(this, loc, children, func, args_list, immediate,
			    func_count, args_list_count);
}
...
Abort.prototype.clone = function() {
   return new Abort(this.loc, this.func, this.args_list, this.immediate,
		    this.func_count, this.args_list_count,
		    this.clone_children());
}


function WeakAbort(loc,
		   func,
		   args_list,
		   immediate,
		   func_count,
...
WeakAbort.prototype.clone = function() {
   return new WeakAbort(this.loc, this.func, this.args_list, this.immediate,
			this.func_count, this.args_list_count,
			this.clone_children());
}

function Loop(loc, children) {
   ASTNode.call(this, loc, children);
}
...
Loop.prototype.clone = function() {
   return new Loop(this.loc, this.clone_children());
}

function LoopEach(loc, children, func, args_list, immediate, func_count,
		  args_list_count) {
   CountExpressionNode.call(this, loc, children, func, args_list, immediate,
			    func_count, args_list_count);
}
...
LoopEach.prototype.clone = function() {
   return new LoopEach(this.loc, this.get_children(), this.func,
		       this.args_list, this.immediate, this.func_count,
		       this.args_list_count);
}

function Every(loc, children, func, args_list, immediate, func_count,
	       args_list_count) {
   CountExpressionNode.call(this, loc, children, func, args_list, immediate,
			    func_count, args_list_count);
}
...
Every.prototype.clone = function() {
   return new Every(this.loc, this.clone_children(), this.func, this.args_list,
		    this.immediate, this.func_count, this.args_list_count);
}

function Suspend(loc, children, func, args_list, immediate) {
   ExpressionNode.call(this, loc, children, func, args_list, immediate);
}
...
Suspend.prototype.clone = function() {
   return new Suspend(this.loc, this.clone_children(), this.func,
...
Parallel.prototype.appendChild = function(ast_node) {
...
function If(loc, children, not, func, args_list) {
   if (children[1] == undefined)
      children[1] = new Nothing(loc);

   ExpressionNode.call(this, loc, children, func, args_list, false);
...
If.prototype.clone = function() {
   return new If(this.loc, this.clone_children(), this.not, this.func,
		 this.args_list)
}

function Sequence(loc, children) {
   ASTNode.call(this, loc, children);
}
...
Sequence.prototype.clone = function() {
   return new Sequence(this.loc, this.clone_children());
}

/* sigs_assoc must be checked before the call of this constructor,
   see lang.js */

function Run(loc, module_stmts) {
   ASTNode.call(this, loc, module_stmts);
}
...
LocalSignal.prototype.clone = function() {
   return new LocalSignal(this.loc, this.signal_name, this.clone_children(),
