...
/*    Last change :  Wed Dec  3 12:56:55 2025 (serrano)                */
...
function findCausalityError(mach) {
   if (mach.causalityErrorTrace === "shallow") {
      ...
      const deep = findCausalityErrorDeep(mach);
...
function findCausalityErrorShallow(machine) {
   ...
   function tarjan(net) {
	 ...
	 if (v.LogicalNetType !== "REG") {
	    ...
	    // each for loop to save stack frames
	    for (let i = v.fanoutList.length - 1; i >= 0; i--) {
	       const f = v.fanoutList[i];
	       const w = f.net;
	       if (w.num === undefined) {
		  walk(w);
...
function findCausalityErrorDeep(machine, compilePhase = false) {
	...
	function visit(v, partition) {
	   if (v.LogicalNetType !== "REG") {
		 ...
		 if (loop) {
		    ...
		    // MS fix 6jun2025
		    subComponent(v).forEach(e => partition.push(e));
		 } else {
		    partition.push(v);
		 }
	      }
    ...
    for (let i = 0 ; i < nets.length ; i++) {
       if (nets[ i ].dfn === 0) {
	  const components = bourdoncleTarjan(nets[ i ]);
