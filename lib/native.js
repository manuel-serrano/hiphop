...
/*    Last change :  Fri Sep 18 15:51:12 2026 (serrano)                */
...
/*    conditional import                                               */
/*---------------------------------------------------------------------*/
let readFileSync = p => undefined;
let writeFileSync = (p, s) => undefined;

if (typeof window === "undefined") {
   const { default: fs } = await import("node:fs");
   readFileSync = fs.readFieSync;
...
/*    DEBUG ...                                                        */
/*---------------------------------------------------------------------*/
const DEBUG = config.Process.env.HIPHOP_TRACE
   && config.Process.env.HIPHOP_TRACE.split(",").find(n => n === "native");
...
/*    this function with all the actual thunks which yield to a        */
...
function compileJS(mach) {
   ...
   buf += "      mach.global_signal_map[signame].nowval = sig.value;\n";
   buf += "      if (callbacks) {\n";
   ...
   nets.filter(n => n instanceof net.ActionNet)
      .forEach(n => {
	 const arr = `\n      new Array(${n.lvl + 1}).fill(undefined)\n         `;
   ...
   const regs = nets.filter(n => n instanceof net.RegisterNet)
   ...
   for (let k in mach.input_signal_map) {
      ...
      const g = s.netSigList[0];

      if (g.sweeped && !s.netSigList[0].sweeped) {
	 buf += `      const ${s.netSigList[0].toJSVar()} = ("${k}" in signals);\n`;
      }
   }

   // reset machine public signals
   buf += ("      for (let i in this.signals) {\n");
   buf += ("         let sig = this.signals[i];\n");
   buf += ("         sig.pre = sig.now;\n");
   buf += ("         sig.now = false;\n");
   buf += ("      }\n\n");
   
   // declare gate registers
   const cnsts = nets.filter(isConstantNet).map(n => n.toJSDecl());
   const vars = nets.filter(n => !isConstantNet(n)).map(n => n.toJSDecl());
   if (cnsts.length > 0) {
      buf += ("      const " + cnsts.join(", ") + ";\n");
   }
   ...
   while (nets.length > 0) {
	 ...
	 if (n.astNode?.loc?.filename
	    ...
	    const fn = n.astNode.loc.filename;
	    const j = fn.lastIndexOf("/");
	    com += ` @ ${purify(j >= 0 ? fn.substring(j+1, fn.length) : fn)}:${n.astNode.loc.pos}\n`;
	 } else {
	    com += "\n";
	 }
   ...
   // output listeners
   buf += "      // signal listeners and react result\n";
   buf += "      const emitted = {};\n";
   ...
   for (let k in mach.output_signal_map) {
      ...
      const g = s.netSigList[0];
      buf += `      // global output signals\n`
      buf += `      if (${g.toJSVar()}) {\n`;
      buf += `         invokeListeners(this, "${s.name}", emitted, emittedDebug);\n`;
      buf += `         allEmittedSignals["${s.name}"]=this.signals["${s.name}"].nowval;\n`;
      buf += '      }\n\n';
   }
   for (let k in mach.input_signal_map) {
      const s = mach.input_signal_map[k];
      const g = s.netSigList[0];
      buf += `      // global input signals\n`
      buf += `      if (${g.toJSVar()}) allEmittedSignals["${s.name}"]=this.global_signal_map["${s.name}"].nowval;\n`;
   }
   buf += "\n";
   buf += `      // local signals (debug only)\n`
   buf += "      if (this.allEmittedFunc) {\n";
   buf += "         let li = -1;\n";
   for (let k in mach.local_signal_list) {
      const s = mach.local_signal_list[k];
      buf += `         li = this.signalsById[${s.id}]?.emitted.findLastIndex(x => x);\n`;
      buf += `         if (li >= 0) { allEmittedSignals["${s.name}"]=this.signalsById[${s.id}].values[li]; }\n`;
   }
...
/*    compileJSDebug ...                                               */
/*---------------------------------------------------------------------*/
function compileJSDebug(buf) {  
   console.log('native succeeded (see "/tmp/native.mjs"), to re-execute:');
   console.log('');
   console.log(`NODE_OPTIONS="${config.Process.env?.NODE_OPTIONS ?? "--enable-source-maps --no-warnings --loader @hop/hiphop/lib/hiphop-loader.mjs"}" HIPHOP_DEBUG_NATIVE_SOURCE="/tmp/native.mjs" ${config.Process.argv.join(" ")}`);
