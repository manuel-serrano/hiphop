...
/*    Last change :  Tue Dec 16 10:59:47 2025 (serrano)                */
...
function HHSyntaxError(msg, loc) {
   ...
   if (loc && loc.filename && config.isServer) {
...
/*    HHLoopError ...                                                  */
/*---------------------------------------------------------------------*/
class HHLoopError extends TypeError {
   constructor(msg, prog, node) {
      const loc = node.loc;
      super(loc?.filename ? `${msg} (${loc.filename}:${loc.pos})` : msg);
      this.location = loc

      // use non enumerable properties not to minimize error messages
