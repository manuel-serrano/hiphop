...
function parser(buffer) {
   parser_raw += buffer.toString("utf8", 0).trim() + " ";

   var i;
   while ((i = parser_raw.indexOf(";")) > -1)  {
      var cmd = parser_raw.substring(0, i).trim();
      parser_raw = parser_raw.slice(i + 1);
	 ...
	 cmd = cmd.split(" ");
	 if (cmd[1] != "") {
	    fs.readFile(cmd[1], 'utf8', function (err, data=undefined) {
	       if (err) {
		  batch_error("Unable to read file " + cmd[1]);
	       } else {
		  if (include_dependencies.indexOf(cmd[1]) > -1) {
		     console.log(include_dependencies);
   ...
   if (!eval_mode && parser_raw.indexOf(".") > -1) {
      console.log("Bye.");
      process.exit(0);
   }
}
