...
/*    Last change :  Thu Sep 17 10:54:39 2026 (serrano)                */
...
export { IN, OUT, INOUT, $Delay, DelaySig, DelayBinary, DelayUnary } from "./ast.js";
...
function machineDumpNets(machine, sweep, suffix) {
   ...
   const dump = {
      ...
      sweep: sweep,
      age: machine.age(),
      nets: machine.nets.map(net => net.toJSON())
   };
...
/*---------------------------------------------------------------------*/
ReactiveMachine.setConfiguration( {
   CausalityHandler: reportCausalityError,
   DumpNetsProc: machineDumpNets
});
