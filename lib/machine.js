...
/*    Last change :  Tue Sep  4 08:46:08 2018 (serrano)                */
...
ReactiveMachine.prototype._react = function() {
   ...
   if (nonpropagated_nets > 0) {
      ...
      // MS (2018/07/31): find a location for that error
      for( let i in this.nets ) {
	 if( !nets[ i ].hasBeenPropagated() &&
	     nets[ i ].fanin_list.length > 0 &&
	     nets[i].ast_node.loc ) {
	    eloc = nets[i].ast_node.loc;
	    break;
	 }
      }
      
      // MS (2018/07/31): not called anymore (see CausalityError@error.js)
      let listing = () => {
	 for (let i in this.nets) {
      ...
      throw error.CausalityError( "causality error", eloc );
      //throw error.CausalityError(eloc, nonpropagated_nets, listing);
   }
