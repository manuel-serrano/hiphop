...
/*    Last change :  Tue Jan 28 08:00:25 2025 (serrano)                */
...
function toJS(mach) {
   ...
   // invokeListeners
   buf += "   function invokeListeners(mach, signame) {\n";
   ...
   buf += "      mach.react_in_progress = false;\n";

   while (nets.length > 0) {
      const l = nets.length;

      nets = nets.filter(n => {
	 if (n.fanin_list_mirror.every(f => f.net instanceof net.RegisterNet)) {
	    // true is fanin_list_mirror is empty
