/*=====================================================================*/
/*    serrano/prgm/project/hiphop/hiphop/machine.js                    */
/*    -------------------------------------------------------------    */
/*    Author      :  Manuel Serrano                                    */
/*    Creation    :  Tue Jul 17 08:19:22 2018                          */
/*    Last change :  Fri Mar  7 16:01:16 2025 (serrano)                */
...
class ReactiveMachine {
   ...
   toJS() {
      try {
	 if (process.env.HIPHOP_DEBUG_NATIVE_SOURCE) {
	    // debug, substitute the actual circuit with a user file
	    console.log(`*** WARNING, using precompile file ${process.env.HIPHOP_DEBUG_NATIVE_SOURCE}`);
	    return readFileSync(process.env.HIPHOP_DEBUG_NATIVE_SOURCE).toString();
	 } else {
	    return toJS(this);
	 }
      } catch(e) {
...
function toJS(mach) {
   ...
   while (nets.length > 0) {
	 ...
	 if (n.faninList_mirror.every(f => f.net instanceof net.RegisterNet)) {
      ...
      if (nets.length === l) {
	 throw new Error("error, cyclic net list"
