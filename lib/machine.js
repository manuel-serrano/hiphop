...
function ReactiveMachine(ast_node, name="") {
   ...
   // Debugger infos :
   //
   // If started, it contains a Hop.js service.
   //
   this._debugger = null;
...
ReactiveMachine.prototype.debuggerOn = function(path) {
   if (hh.on_client)
      return;
   ...
   this._debugger = debug.start(this, path);
}

ReactiveMachine.prototype.debuggerUpdate = function() {
   if (this._debugger)
      debug.update(this);
}
...
ReactiveMachine.prototype.pretty_print = function() {
   ...
   let modules = this.ast.dump().modules;
   let buf = "";

   if (modules.length > 0)
   modules.forEach(function(module, idx, array) {
      buf += "\nMODULE" + idx + "\n";
      buf += module;
   });
   return buf + "\n\n";
