...
/*    Last change :  Mon Nov 12 17:20:54 2018 (serrano)                */
...
export class ReactiveMachine {
   constructor( ast_node, opts={} ) {
	 ...
	 this.name = opts.name;
      ...
      if( hop.isServer ) {
	 this.nowval = new hop.reactProxy( {} );
	 this.preval = new hop.reactProxy( {} );
	 this.now = new hop.reactProxy( {} );
	 this.pre = new hop.reactProxy( {} );
      } else {
	 this.nowval = {};
	 this.preval = {};
	 this.now = {};
	 ...
	 this.nowval[ k ] = undefined;
	 this.preval[ k ] = undefined;
	 this.now[ k ] = false;
	 this.pre[ k ] = false;
      }
      
      // Generate the API for input/output signals (proxies, promises, etc.)
      this.promise = {};
      this.value = {};
      this.present = {};
      makeSignalAPI( this );
...
/*    Exec ...                                                         */
/*    -------------------------------------------------------------    */
/*    Objects reifying execs (spawns) executions.                      */
/*---------------------------------------------------------------------*/
class Exec extends signal.ActionArg {
   ...
   react( value = undefined ) {
      this.machine.react( { [ this.state.astNode.signame ]: value } );
   }
}
...
function react( mach ) {
   ...
   for (let i in mach.output_signal_map) {
	 ...
	 mach.preval[ sig.name ] = mach.nowval[ sig.name ];
	 mach.pre[ sig.name ] = mach.now[ sig.name ];
	 mach.nowval[ sig.name ] = sig.value; 
	 mach.now[ sig.name ] = true;
   ...
   for( let i in mach.exec_status_list ) {
      ...
      if( state.kill || state.start ) {
	    ...
	    if( state.func_kill ) state.func_kill.call( state.exec );
	    ...
	    state.exec = new Exec( mach, state );
