...
/*    Last change :  Tue Sep 11 07:22:31 2018 (serrano)                */
...
ReactiveMachine.prototype._react = function() {
   ...
   if (nonpropagated_nets > 0) {
      ...
      throw error.CausalityError( "causality error involving signals: " + signals, loc );
   }
...
function findCausalityError( machine ) {
   ...
   if( head ) {
      return { loc: head.ast_node.loc, signals: findCycleSignals( head ) }
   } else {
      return { loc: false, signals: [] };
   }
}
