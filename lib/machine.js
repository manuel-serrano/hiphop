...
function ReactiveMachine(ast_node, name) {
   ...
   */
   ...
   /* exec_state structure in compiler.js:ast.Exec.prototype.make_circuit.
      Mapped by task signal name. */
   this.exec_state_map = {};
...
ReactiveMachine.prototype.react = function() {
   ...
   for (let signal_name in this.exec_state_map) {
      let state = this.exec_state_map[signal_name];
      let sig = state.signal;
      if (state.prev_active && !state.suspended) {
   	 if (!state.kill) {
   	    sig.gate_list[0].value = true;
   	    if (state.value)
   	       sig.set_value(state.value);
   	 }
      ...
      if (state.kill || state.start) {
   	 ...
   	 if (state.start) {
   	    ...
   	    for (let i in state.ast_node.start_args_list) {
   	       let arg = state.ast_node.start_args_list;
