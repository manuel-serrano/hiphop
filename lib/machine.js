...
/*    Last change :  Mon Mar 17 10:30:24 2025 (serrano)                */
...
class ReactiveMachine {
   ...
   constructor(ast_node, opts={}) {
	 ...
	 throw new Error("HipHop, cannot use bothdynamic and sweep: " + opts);
      ...
      if (opts.native === true || config.Process.env.HIPHOP_NATIVE === "true") {
	 if (!this.dynamic) {
	    try {
...
function toJS(mach) {
   ...
   while (nets.length > 0) {
	 ...
	 if (n.faninList.length === 0) {
	    ...
	    buf += "      undefined;\n";
	    return false;
	 } else if (n instanceof net.RegisterNet) {
	    n.propagated = true;
	    buf += ("      " + n.toJS() + ";\n");
	    return false;
	    ...
	    console.log("ELSE", n.id, n.faninList.filter(f => !f.net.propagated).map(f => f.net.id));
	    console.log("NETS=", nets.map(n=>n.id));
	    return true;
	 }
      });
