...
/*    Last change :  Fri Dec 19 08:15:37 2025 (serrano)                */
...
class ReactiveMachine {
   ...
   constructor(ast_node, opts={}) {
	 ...
	 this.$$name = opts?.name || "";
      ...
      this.sweep = opts?.sweep ?? (this.dynamic ? false: ReactiveMachine.Configuration.Sweep);
      this.sigpre = opts?.sigpre ?? ReactiveMachine.Configuration.Sigpre;
      this.unreachable = opts.unreachable ?? ReactiveMachine.Configuration.Unreachable;
	 ...
	 throw new Error("HipHop, cannot use both dynamic and sweep: " + opts);
      ...
      this.compiler = opts?.compiler ?? ReactiveMachine.Configuration.Compiler;
      this.native = opts?.native ?? (this.dynamic ? false : ReactiveMachine.Configuration.Native);

      // unroll loops
      this.loopUnroll = opts?.loopUnroll ?? (ReactiveMachine.Configuration.LoopUnroll === undefined ? (this.compiler === "new") : ReactiveMachine.Configuration.LoopUnroll);

      // expand loops
      this.loopExpand = opts?.loopExpand ?? ReactiveMachine.Configuration.LoopExpand;

      // reincarnation
      this.reincarnation = opts?.reincarnation ?? (ReactiveMachine.Configuration.Reincarnation === undefined ? (this.compiler === "int") : ReactiveMachine.Configuration.Reincarnation);
      
      // loop duplication
      this.loopDup = opts.loopDup ?? ReactiveMachine.Configuration.LoopDup;
      
      // loop safe
      this.loopSafe = opts?.loopSafe ?? ReactiveMachine.Configuration.LoopSafe;

      // forkOrKill (see compiler.js)
      this.forkOrKill = opts?.forkOrKill ?? ReactiveMachine.Configuration.ForkOrKill;
      
      // Display the propagation of gates value
      this.tracePropagation = !!opts?.tracePropagation;
      this.dumpNets = config.isServer && (!!opts?.dumpNets || ReactiveMachine.Configuration.DumpNets) ? ReactiveMachine.dumpNets : false;
   ...
   static Configuration = {
      ...
      Native: getenv("HIPHOP_NATIVE", undefined),
      Sweep: getenv("HIPHOP_SWEEP", true),
      Sigpre: getenv("HIPHOP_SIGPRE", true),
      Unreachable: getenv("HIPHOP_UNREACHABLE", true),
      LoopSafe: getenv("HIPHOP_LOOPSAFE", true),
      ForkOrKill: getenv("HIPHOP_FORKORKILL", false),
      LoopUnroll: getenv("HIPHOP_LOOPUNROLL", undefined),
      LoopExpand: getenv("HIPHOP_LOOPEXPAND", false),
      LoopDup: getenv("HIPHOP_LOOPDUP", undefined),
      Reincarnation: getenv("HIPHOP_REINCARNATION", undefined),
      CausalityHandler: x => undefined,
      DumpNets: process.env.HIPHOP_TRACE?.split(",").find(n => n === "nets")
   }

   static getConfiguration() {
      return Object.assign({}, RetactiveMachine.Configuration);
   }
