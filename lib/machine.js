...
/*    Last change :  Fri Jan 17 07:39:19 2025 (serrano)                */
...
/*    }                                                                */
   ...
   mach.nets.forEach((n,i,a) => {
	 ...
	 if ((n.accessibility & ast.INOUT !== 0) && n.signal.emitted[n.lvl]) {
	    n.value = true;
	    n.isInKnownList = true;
	    known_list.push(n);
...
function toJS(mach) {
   ...
   // declare all the gate registers
   console.log("let", nets.map(n => n.toJSVar()).join(", "), ";");
...
/*                                                                     */
   ...
   nets.forEach(n => n.fanin_list_mirror = n.fanin_list);
   
   while (nets.length > 0) {
      const l = nets.length;

      nets = nets.filter(n => {
	 if (n.fanin_list_mirror.length === 0) {
	    console.log(`// ${n.id} ${n.debug_name}`);
	    console.log(`// ${n.ast_node.loc.filename}:${n.ast_node.loc.pos}`);
	    console.log(n.toJSVar(), "=", n.toJS() + ";", "\n");
	    
	    n.fanout_list.forEach(f => {
	       f.net.fanin_list_mirror =
		  f.net.fanin_list_mirror.filter(g => g !== f.antagonist);
	    });
	    return false;
	 } else {
	    return true;
      ...
      if (nets.length === l) {
	 throw new Error("error, cannot generate JS code: " + nets.length);
      }
   }
}
