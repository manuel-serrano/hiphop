...
/*    Last change :  Fri Oct 12 07:22:56 2018 (serrano)                */
...
class ReactiveMachine {
   constructor( ast_node, opts={} ) {
	 ...
	 this.name = opts.name;
      ...
      //
      this.sweep = opts.sweep === undefined ? false : !!opts.sweep;
...
function react( mach ) {
   ...
   function doUpdate( state, value, reactp ) {
      const machine = state.astNode.machine;
   ...
   for( let i in mach.exec_status_list ) {
	 ...
	 const react = function( value = undefined ) {
	    state.astNode.machine.react( { [state.astNode.signame]: value } );
	 }
	 const self = signal.makeActionArg( mach, state.astNode.accessor_list );
	 
	 self.notify = notify;
	 self.react = react;
	 self.notifyAndReact = notifyAndReact;
	 self.terminateExec = notify;
	 self.terminateExecAndReact = notifyAndReact;
	 self.complete = notify;
	 self.completeAndReact = notifyAndReact;

	 Object.defineProperty( self, "id", { value: state.id, writable: false } );

	 signal.fillActionArg( self, state.astNode.accessor_list, state.lvl );

	 // payload objet allows to add properties in EXEC handlers
	 // that can be used in other handlers of the same EXEC
	 // instance.
	 //
	 // MACH -> mach.payload
	 // MACH.something -> mach.payload.something
	 if( newPayload ) {
	    state.payload = {
	 ...
	 return self;
      ...
      if( state.kill || state.start ) {
   	 if( state.kill ) {
	 console.log( "state.kill id=", state.id );
   	    state.kill = false;
	    state.prev_killed = true;

	    if( state.func_kill ) {
   	       state.func_kill.call( get_exec_handler_receiver() );
	    }
   	 }
	 ...
	 console.log( "state.start id=", state.id );
   	    state.func_start.call( get_exec_handler_receiver( true ) );
	 }
      } else if( state.suspended && !state.prev_suspended ) {
   	 state.prev_suspended = true;
   	 if( state.func_susp ) {
   	    state.func_susp.call( get_exec_handler_receiver() );
	 }
      } else if( !state.suspended && state.prev_suspended ) {
   	 state.prev_suspended = false;
   	 if( state.func_res ) {
   	    state.func_res.call( get_exec_handler_receiver() );
	 }
      }
