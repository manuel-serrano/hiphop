      ...
      if (state.kill || state.start) {
	    ...
	    /* As start_func is not embedded in a ActionNet, we have
	       ...
	       BUT, the good part is as the reaction is over, we can
   	       ...
   	       about dependencies.
	    */
	    let accessor_list = state.ast_node.func_start_accessor_list;
	    for (let i in accessor_list) {
		  ...
		  acc.runtime = [];
	       ...
	       if (acc.get_value) {
   		  acc.runtime[state.lvl] = sig;
   	       } else {
		  if (acc.get_pre) {
   		     acc.runtime[state.lvl] = sig.pre_gate;
   		  } else {
   		     acc.runtime[state.lvl] = sig.gate_list[lvl];
   		  }
	       }
	    }
