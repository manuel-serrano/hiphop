...
export class ReactiveMachine {
   ...
   react( signals ) {
      //console.log("inside machine.react");
   ...
   save() {
	 ...
	 for (let i in map_out) {
      ...
      console.log("machine net length inside save",this.nets.length);
      let i=0;
      this.nets.forEach( n => {
	 if( n instanceof net.RegisterNet ) {
       machine_state.registers[ n.stable_id ] = n.value;
       console.log("save", n.debug_id);
       i++;
	 }
      } );

     
      console.log("registers saved",i,"registerstore",this.registerNetMach.length);
    */
      
      this.registerNetMach.forEach(n => {
         machine_state.registers[ n.stable_id ] = n.value;
      });
   ...
   restore( machine_state ) {
	 ...
	 if( n instanceof net.RegisterNet ) {
       ...
       if (value !== undefined){ n.value = value;
         console.log("restore",n.debug_id);
      }
	 }
      } );
      this.registerNetMach.forEach(n =>{
         console.log("restorenet",n.debug_id);
      });
      */
...
function react( mach ) {
   ...
   //console.log("inside function react");
   ...
   //let nonpropagated_nets = mach.nets.length + mach.registerNetMach.length ;
   
   //let nonpropagated_nets = mach.nets.length + mach.registerNetMach.length + mach.actionNetMach.length;
...
/*    }                                                                */
   ...
   mach.registerNetMach.forEach(n=>{n.reset(false)});
   mach.actionNetMach.forEach(n=>{n.reset(false)});
   // optimization on register nets
   
   mach.registerNetMach.forEach(n=>{
   //   n.isInKnownList = true;
   //   known_list.push(n);    
   });
   
   mach.actionNetMach.forEach(n=>{
   //   n.isInKnownList = true;
   //   known_list.push(n);
   });
...
function compile( mach ) {
   ...
   //   }
	 ...
	 console.timeEnd( "traceCompileDuration" );
      ...
      //}
      /*
      for( let i=0; i<mach.nets.length; i++ ){
         let deleted = false; 
         for( let j=0; j < mach.registerNetMach.length; j++ ){
            if( mach.nets[i].debug_id === mach.registerNetMach[j].debug_id){
               mach.nets.splice(i,1);
               deleted = true;
            }
         }
         if( !deleted ){
            for(let j=0; j < mach.actionNetMach.length; j++ ){
               if(mach.nets[i].debug_id === mach.actionNetMach[j].debug_id){
                  mach.nets.splice(i,1);
               }
            }
         }
...
ReactiveMachine.prototype.batch_reset = function() {
   ...
   // for register nets
   for (let i in this.registerNetMach)
      this.registerNetMach[i].reset(true);

   for (let i in this.actionNetMach)
      this.actionNetMach[i].reset(true);
