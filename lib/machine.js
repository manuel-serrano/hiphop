...
/*    Last change :  Tue Nov 18 15:55:18 2025 (serrano)                */
...
function conf(key, def) {
   return (key in config.Process.env) ? config.Process.env[key] === "true": def;
}

export const Compiler = config.Process.env?.HIPHOP_COMPILER ?? "new";
export const Native = config.Process.env?.HIPHOP_NATIVE ?? "try";
...
class ReactiveMachine {
   ...
   constructor(ast_node, opts={}) {
	 ...
	 throw new Error("HipHop, cannot use both dynamic and sweep: " + opts);
      ...
      this.native = opts?.native ?? (this.dynamic ? "no" : Native);
