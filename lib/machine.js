...
/*    Last change :  Mon Dec 13 05:50:23 2021 (serrano)                */
...
import * as signal from "./net.js";
import { Queue } from "./queue.js";
import { debug as configDebug } from "hop:config";
...
class ReactiveMachine {
   ...
   constructor(ast_node, opts) {
	 ...
	 this.name = opts.name;
      ...
      this.sweep = opts.sweep === undefined ? configDebug === 0 : !!opts.sweep;
...
/*    }                                                                */
   ...
   if (nonpropagated_nets > 0) {
      let loc, size, signals, result;
      
      if (mach.CausalityErrorTrace === "shallow") {
	 const { _loc, _size, _signals } = causality.findCausalityError(mach);
	 loc = _loc;
	 size = _size;
	 signals = _signals;
      } else if (mach.CausalityErrorTrace === "deep") {
	 const { _loc, _size, _signals, posFlag, _result } = causalityDeep.findCausalityErrorDeep(mach);
	 loc = _loc;
	 size = _size;
	 signals = _signals;
	 result = _result;
      } else {
	 const { _loc, _size, _signals } = findCausalityError(mach);
	 loc = _loc;
	 size = _size;
	 signals = _signals;
      }
