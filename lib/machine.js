...
function ReactiveMachine(ast_node, name="") {
   ...
   //
   // Allow to delay inputs/notify/modifications and reactions if they
   // append during a reaction. update_queue contains the
   // modifications to make to the machine before the next
   // reaction. When react() is called during a reaction, the
   // update_queue is pushed in react_queue and is reinitialized.
   //
   // Each element of update_queue is a function, which can do the
   // following things:
   //
   //   - emit a signal
   //   - notify of exec termination (happens when the exec terminates
   //     instantaneously)
   //   - append/remove child of parallel
   //
   this.react_queue = [];
   this.update_queue = [];
...
ReactiveMachine.prototype.stepperNext = function() {
   if (this._debugger) {
      this._debugger.stepper_next();
   }
}
...
ReactiveMachine.prototype.react = function() {
   if (this.react_in_progress) {
      this.react_queue.push(this.update_queue);
      this.update_queue = [];
      return;
   }

   if (this._debugger && this._debugger.stepper) {
      this._debugger.stepper_react();
      return;
   }
   ...
   for (let i in this.exec_status_list) {
      ...
      if (state.kill || state.start) {
	    ...
	    let self = {
	       id: state.id,
	       notify: function(value=undefined) {
		  ...
		  if (state.id == self.id) {
			...
			state.prev_active = true;
		     ...
		     let debug = machine._debugger;
		     if (debug && debug.stepper) {
			debug.stepper_notify(_notify);
		     } else if (machine.react_in_progress) {
			//
			// Happend if the exec call returns immediatly
			//
			machine.update_queue.push(_notify);
		     } else {
			_notify();
		     }
		     return true;
		  ...
		  return false;
	       ...
	       notifyAndReact: function(value=undefined) {
   ...
   //
   // Applies pending modifications and reactions
   //
   if (this.react_queue.length == 0) {
      this.update_queue.forEach(update => update());
      this.update_queue = [];
   } else {
      while (this.react_queue.length > 0) {
	 this.react_queue.shift().forEach(update => update())
	 this.react();
      }
   }
   
   //
   // Does not mean anything if there are waiting reactions in waiting
   // queue, and should be removed.
   //
   return emitted;
}
...
ReactiveMachine.prototype.input = function(signal, value=undefined) {
   ...
   if (this.react_in_progress) {
      this.update_queue.push(() => this.input(signal, value));
      return;
   }

   if (this._debugger && this._debugger.stepper) {
      this._debugger.stepper_input(signal, value);
      return;
   }
...
const makeSignalAPI = function(m) {
   function signalProxyBuilder(signal) {
      return function() {
	 let signalName = signal.name;
	 let proxy = m.DOMproxies[signalName];

	 if (!proxy) {
	    proxy = new hop.reactProxy({value: signal.init_value});
	    m.DOMproxies[signalName] = proxy;
   ...
   for (let signalName in m.global_signal_map) {
	 ...
	 getterProxy = signalProxyBuilder(signal);
