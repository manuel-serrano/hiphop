...
/*    Last change :  Wed Jan 15 16:29:08 2025 (serrano)                */
...
function toJS(mach) {
   ...
   mach.nets.forEach((n,i,a) => {
	 ...
	 n.isInKnownList = true;
	 known_list.push(n);
      } else if (n instanceof net.ActionNet) {
	 if (n.trueFaninCount === 0) {
	    n.isInKnownList = true;
	    known_list.push(n);
	 }
      } else if (n instanceof net.LogicalNet) {
    ...
    while (known_list.length > 0) {
      known_list.shift().propagateJS(known_list);
   }
}
