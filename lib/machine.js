...
/*    Last change :  Tue May 28 18:11:40 2019 (serrano)                */
...
//const tccqueue = require( "./TCCQueue.js" );
...
/*    }                                                                */
   ...
   if (nonpropagated_nets > 0) {
      ...
      if( size >= 0 ) {
      //  console.log(loc);
	 throw error.CausalityError( `causality cycle of length ${size} detected, involving signals ${signals.toString()} at position: `, loc );
      } else {
...
function compile( mach ) {
   if( mach.needCompile ) {
	 ...
	 console.timeEnd( "traceCompileDuration" );
      ...
      mach.queue = new queue( mach.nets.length, n => n.isInKnownList = true );
   }
}
...
function findCausalityError( machine ) {
 /*  for(let k=0;k<machine.nets.length;k++){


     console.log(machine.nets[k].debug_id, machine.nets[k].fanout_list.length);

     for(let l=0;l<machine.nets[k].fanout_list.length;l++){
       let m=machine.nets[k].fanout_list[l];
       console.log("fanout", m.net.debug_id);
     }
     console.log("-----");
   ...
   let components = tarjan( nets );
    ...
    //console.log( "COMP.tarjan=", components[j].map( n => n.length ) );
  //  console.log(components[0]);
    if(components[j].length >1){
    let tcc_debug_id = components[j].map(n=>n.debug_id);
    let tcc_loc = components[j].map(n=>n.ast_node.loc);
  //  let ex_faninlist= components[j].map(n=>n.fanin_list);
  ...
  //  console.log(tcc_loc);
    ...
    for(let i=0;i<tcc_debug_id.length;i++){
      //    let tcc_node = components[j][i].debug_name.split(":",2);
      //    console.log(tcc_debug_id[i]);
          console.log(tcc_loc[i]); // to see the positions
          //console.log(components[j][i].debug_name);
      //    fanin_id=ex_faninlist[i].map(n => n.net.debug_id);
          fanout_id=tcc_fanoutlist[i].map(n => n.net.debug_id);

      //      console.log("Fanin", fanin_id);
        //    console.log("Fanout", fanout_id);
         tcc_dict[tcc_debug_id[i]]=fanout_id;
        //  JSON.stringify(fanin_id)
    }
  //  console.log(tcc_dict);//to check cycles
    shortestCycle(tcc_dict);
    console.log("---------------")
  }
  }

    function shortestCycle(ex_dict){
    ...
    let   mykey=[];
     ...
     for(let key in ex_dict) {
      ...
      //   console.log(key);
         ...
         let t=  findcycle(ex_dict,key);
         console.log(key, "cycle length ",t);
         console.log("--------------");
   ...
   function findcycle(ex_dict,my_key){
        ...
        //   console.log(bfsKey_q);
            let found = bfsKey_q.findIndex(function(element){

                return element == my_key;});
      //      console.log(my_key,found);
            if(found>-1){

              done = true;break;
            }
            count+=1;
            let new_v = bfsKey_q[0];
            item=ex_dict[new_v];


            while(item===undefined && bfsKey_q.length > 1){
        //    console.log(bfsKey_q.length,"length of the queue");
              bfsKey_q.shift();
              new_v = bfsKey_q[0];
              item=ex_dict[new_v];
            }
             ...
             bfsKey_q.shift();
          //  console.log(bfsKey_q,"length of the queue outside",item);
          }
