...
/*    Last change :  Sun Dec  3 19:05:25 2023 (serrano)                */
...
import * as causalityDeep from "./bourdoncle.js";
...
let debug = false;
let configDebug = process.env?.HIPHOP_DEBUG ?? true;
...
class ReactiveMachine {
   ...
   init(...vals) {
	 ...
	 if (!f) {
	    throw new TypeError("Program accepts no parameters");
	 } else {
...
/*    }                                                                */
   ...
   if (nonpropagated_nets > 0) {
	 ...
	 if (cycles && hop.isServer) {
	    await reportCausalityCycles(cycles);
	 }
...
function findCausalityError(mach) {
   if (mach.CausalityErrorTrace === "shallow") {
      return causality.findCausalityError(mach);
   } else if (mach.CausalityErrorTrace === "deep") {
      return causalityDeep.findCausalityErrorDeep(mach);
   } else {
...
/*    dumpCausalityError ...                                           */
/*---------------------------------------------------------------------*/
async function dumpCausalityError(json) {
...
/*---------------------------------------------------------------------*/
async function reportCausalityCycles(cycles) {
   const fs = await import("fs");
   ...
   cycles.forEach(src => {
	 ...
	 if (fs.existsSync(filename)) {
	    ...
	    try {
	       ...
	       let pos = 0, locIndex = 0, lineIndex = 0, lastLine = -1;

	       console.log("\x1B[34m\x1B[1m" + fname + "\x1B[0m:");
	       lines.forEach(line => {
		  ...
		  if (loc >= pos && loc <= pos + line.length) {
			...
			console.log("...");
		     ...
		     console.log(lineNumber(lineIndex), lineError(line, loc - pos));
		     locIndex++;
		     lastLine = lineIndex;
		  }
		  pos += line.length + 1;
