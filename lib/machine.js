...
ReactiveMachine.prototype.react = function() {
   ...
   this.reset_valued_signals(false);
   if (this.trace_on)
      console.log("SEL:" + (this.sel ? this.sel.value : false),
...
ReactiveMachine.prototype.reset = function() {
   ...
   this.reset_valued_signals(true);
}

ReactiveMachine.prototype.reset_valued_signals = function(reset_machine) {
   function _reset_valued_signals(map) {
      for (let i in map)
	 if (map[i] instanceof signal.ValuedSignal)
	    map[i].reset(reset_machine);
   }

   _reset_valued_signals(this.input_signal_map);
   _reset_valued_signals(this.output_signal_map);
   _reset_valued_signals(this.local_signal_map);
}
