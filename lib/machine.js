...
/*    Last change :  Mon Sep 10 15:35:46 2018 (serrano)                */
...
ReactiveMachine.prototype.compile = function() {
   ...
   this.queue = new queue( this.nets.length, n => n.ispropagated = true );
}
...
ReactiveMachine.prototype._react = function() {
   ...
   if (nonpropagated_nets > 0) {
      let { loc, signals } = findCausalityError( this );
      throw error.CausalityError( "causality error", loc );
   }
...
function findCausalityError( machine ) {
   ...
   function recursive( net, stack ) {
      if( stack.indexOf( net ) >= 0 ) {
	 return true;
      } else {
	 stack.push( net );
	 return net.fanin_list.find( f => recursive( f.net, stack ) );
      }
   }
      
   let nets = machine.nets.filter( n => !n.ispropagated );
   let signals = [];
   let stack = [];
   let head = nets.find( n => n.fanin_list.find( f => recursive( f.net, [ n ] ) ) );
   let loc = head ? head.ast_node.loc : false;
