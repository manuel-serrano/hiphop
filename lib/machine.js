...
/*    Last change :  Tue Jun 17 17:42:15 2025 (serrano)                */
...
class ReactiveMachine {
   ...
   constructor(ast_node, opts={}) {
	 ...
	 throw new Error("HipHop, cannot use both dynamic and sweep: " + opts);
      ...
      // sync register
      this.syncReg = ("syncReg" in opts) ? opts.syncReg : undefined;
...
/*    }                                                                */
   ...
   if (nonpropagated_nets > 0) {
	 ...
	 throw error.TypeError("hiphop: causality error", mach.nets.find(n => !n.isInKnownList).astNode.loc);
      } finally {
