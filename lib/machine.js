...
function react( mach ) {
   ...
   let nonpropagated_nets = mach.nets.length + mach.registerNetMach ;
...
/*    }                                                                */
  ...
  /*
   // optimisation - step 1 - attempt 1
   mach.nets.forEach((n,i,a) => {
      n.reset(false);
      if (n.LogicalNetType() === "REG") {
	 n.isInKnownList = true;
    known_list.push(n);
      } else if (n.LogicalNetType() === "ACTION" ) {
	 if( n.trueFaninCount === 0 ) {
	    n.isInKnownList = true;
       known_list.push(n);
	 }
      } else if (n.LogicalNetType() === "AND"  || n.LogicalNetType() === "OR" ) {
	 if (n.isGlobalInputSignalNet && n.signal.emitted[ n.lvl ]) {
	    n.value = true;
	    n.isInKnownList = true;
       known_list.push(n);
   ...
   */
   ...
   known_list.data.forEach(v => {
      console.log(v.debug_id);
   });
   const nbpush = known_list.length;
   console.log(nbpush," nbpush ",known_list.data.length);
