      ...
      if (state.kill || state.start) {
	    ...
	    state.id = get_exec_id();
		  ...
		  return: function(value=undefined) {
			...
			if (!state.kill && !state.suspended) {
			   let machine = state.ast_node.machine;
			   let exec_interface = state.exec_interface;

			   if (!exec_interface.autoreact ||
			       exec_interface.autoreact == hh.OFF) {
			      return;
			   }

			   if (exec_interface.autoreact == hh.ANY) {
			      machine.react();
			      return;
			   }

			   if (exec_interface.autoreact == hh.ALL) {
			      for (let s in machine.exec_status_list) {
				 let state2 = machine.exec_status_list[s];

				 if (state2.exec_interface != exec_interface)
				    continue;
				 if (state2.kill || state2.suspended)
				    continue;
				 if (state2.active)
				    return;
			      }
...
ReactiveMachine.prototype.inputAndReact = function(signal,
   ...
   if (signal instanceof Object) {
      ...
      this.setInput(signal.signalName, signal.signalValue);
   } else {
      this.setInput(signal, value);
   }
   return this.react();
}
