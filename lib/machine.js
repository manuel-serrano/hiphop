...
ReactiveMachine.prototype._react = function() {
   ...
   if (this.debugTimeTrace) {
      console.time("debugTimeTrace");
   }
   ...
   if (nonpropagated_nets > 0) {
	 ...
	 for (let i in this.nets) {
		...
		this.nets[i].fanin_list.length > 0) {
		  ...
		  if (fin.net.has_been_propagated)
		     return;
