...
/*    Last change :  Mon Mar  3 13:01:32 2025 (serrano)                */
...
class ReactiveMachine {
   ...
   #age;
   $$status = { sweep: {} };
   sweep = true;    // enable sweep optimization
   dynamic = false; // enable dynamic program modification
   tracePropagation;
   dumpNets;
   traceReactDuration;
...
function toJS(mach) {
   ...
   buf += "      const preval = mach.sgnals[signame].nowval;\n";
   ...
   buf += "      } else {\n";
   buf += "         mach.signals[sigame].pre = mach.signals[signame].now;\n";
   buf += "         mach.signals[sigame].now = false;\n";
   buf += "      }\n";
   buf += "   }\n\n";
   ...
   buf += "      const argf = arg.signals[signame];\n";
