...
/*    Last change :  Thu Sep 17 13:27:28 2026 (serrano)                */
...
import * as uncycle from "./uncycle.js";
...
/*---------------------------------------------------------------------*/
const MachineOpts = [
   "name", "dynamic", "sweep", "sigPre", "unreachable", "compiler", "native",
   "loopExpand", "reincarnation", "loopDup",
   "loopSafe", "tracePropagation", "experimental", "uncycle",
   "traceReactDuration", "dumpNets", "synchronizer",
   "causalityErrorTrace", "debuggerName", "verbose", "reincarnationTrap",
   "needCompile" ];
...
function getenvBool(key, def) {
   switch (config.Process.env[key]) {
      ...
      default: {
	 if (key in config.Process.env) {
	    console.error(`*** WARNING:Illegal env "${key}" value: ${config.Process.env[key]}, using ${def}`);
	 }
	 return def;
      }
   }
...
function getenvInt(key, def) {
   if (key in config.Process.env) {
      return parseInt(config.Process.env[key])
   } else {
      return def;
   }
}
...
class ReactiveMachine {
   ...
   synchronizer; // fork synchornizer ("new", "old")
   tracePropagation;
   dumpNets;
   traceReactDuration;
   ...
   constructor(ast_node, opts={}) {
	 ...
	 this.opts = { namme: opts };
      } else {
	 for (let k in opts) {
	    if (MachineOpts.indexOf(k) < 0) {
	       throw new TypeError("wrong machine options `" + k + "'");
	    }
	 }
	 ...
	 this.$$name = opts?.name || "";
      ...
      // nets init
      this.nets = [];
      this.needCompile = opts?.needCompile ?? true;
      ...
      this.sweep = get(opts, "sweep", (this.dynamic ? false: ReactiveMachine.Configuration.Sweep));
      if ((typeof this.sweep) !== "number") {
	 // backward compatibility
	 if (this.sweep === false) {
	    this.sweep = 0;
	 } else if (this.sweep === true) {
	    this.sweep = Number.MAX_SAFE_INTEGER;
	 } else {
	    throw new TypeError(`ReactiveMachine, bad sweep value ${this.sweep} (integer expected)`);
	 }
      }

      // static detection of .pre and .preval properties to optimize signals
      this.sigPre = get(opts, "sigPre", ReactiveMachine.Configuration.SigPre);

      // remove unreachable nets
      this.unreachable = get(opts, "unreachable", ReactiveMachine.Configuration.Unreachable);

      // net sweping optimization
      if (this.dynamic && this.sweep) {
	 throw new Error("HipHop, cannot use both dynamic and sweep: " + opts);
      }
      
      // default machine compiler
      this.compiler = get(opts, "compiler", ReactiveMachine.Configuration.Compiler);
      this.native = get(opts, "native", (this.dynamic ? false : ReactiveMachine.Configuration.Native));
      this.synchronizer = get(opts, "synchronizer", ReactiveMachine.Configuration.Synchronizer);

      // reincarnation (compile with reincarnation level)
      this.reincarnation = get(opts, "reincarnation", (ReactiveMachine.Configuration.Reincarnation === undefined ? (this.compiler === "new") : ReactiveMachine.Configuration.Reincarnation));
      
      // reincarnation (compile with reincarnation level)
      this.reincarnationTrap = this.reincarnation && (get(opts, "reincarnationTrap", (ReactiveMachine.Configuration.ReincarnationTrap === undefined ? (this.compiler === "new") : ReactiveMachine.Configuration.ReincarnationTrap)));
      
      // expand loops (expand implicit loops)
      this.loopExpand = get(opts, "loopExpand", ReactiveMachine.Configuration.LoopExpand);

      // loop duplication
      this.loopDup = get(opts, "loopDup", ReactiveMachine.Configuration.LoopDup);
      
      // loop safe (static instantaneous loop detection )
      this.loopSafe = get(opts, "loopSafe", ReactiveMachine.Configuration.LoopSafe);

      // remove circuit cycles
      this.uncycle = get(opts, "uncycle", ReactiveMachine.Configuration.Uncycle);

      // unused compiler option used to conduct experiments
      this.experimental = get(opts, "experimental", ReactiveMachine.Configuration.Experimental);

      // Display the propagation of gates value
      this.tracePropagation = !!opts?.tracePropagation;
      this.dumpNets = config.isServer && (!!opts?.dumpNets || ReactiveMachine.Configuration.DumpNets) ? ReactiveMachine.Configuration.DumpNetsProc : false;
      // verbosity and debugging
      this.verbose = get(opts, "verbose", 1);
	 ...
	 this.ast = ast_node;
      ...
      // remove cycles
      if (this.cyclic && this.uncycle) {
	 uncycle.uncycle(this);
      }
      
      // native code compilation
      if (this.native !== false && !this.cyclic) {
	 if (this.dynamic) {
   ...
   static Configuration = {
      Compiler: config.Process.env?.HIPHOP_COMPILER ?? "new",
      Synchronizer: config.Process.env?.HIPHOP_SYNCHRONIZER ?? "new",
      Native: getenvBool("HIPHOP_NATIVE", undefined),
      Sweep: getenvInt("HIPHOP_SWEEP", Number.MAX_SAFE_INTEGER),
      SigPre: getenvBool("HIPHOP_SIGPRE", true),
      Unreachable: getenvBool("HIPHOP_UNREACHABLE", true),
      Uncycle: getenvBool("HIPHOP_UNCYCLE", false),
      Experimental: getenvBool("HIPHOP_EXPERIMENTAL", false),
      LoopSafe: getenvBool("HIPHOP_LOOPSAFE", true),
      LoopExpand: getenvBool("HIPHOP_LOOPEXPAND", false),
      LoopDup: getenvBool("HIPHOP_LOOPDUP", undefined),
      Reincarnation: getenvBool("HIPHOP_REINCARNATION", true),
      ReincarnationTrap: getenvBool("HIPHOP_REINCARNATION_TRAP", true),
      CausalityHandler: x => undefined,
      DumpNets: config.Process.env.HIPHOP_TRACE?.split(",")?.find?.(n => n === "nets"),
      DumpNetsProc: (machine, sweep, suffix) => undefined
   }

   static getConfiguration() {
      return Object.assign({}, ReactiveMachine.Configuration);
   }

   static setConfiguration(conf) {
      for (let k in conf) {
	 ReactiveMachine.Configuration[k] = conf[k];
      }
   }

   static fromJSON(json, opts) {
      const nets = net.fromJSON(JSON.parse(json).nets);
      const loc = { filename: opts?.name ?? "json", pos: 0 };
      const ast_node = new ast.Module("module", 0, loc, "dummy", [], null);
      const m = new ReactiveMachine(ast_node, { ... opts, needCompile: false });

      // link together the newly built machine, its nets, and its ast
      m.nets = nets;
      m.ast.machine = m;
      ast_node.machine = m;
      nets.forEach(n => n.astNode = ast_node);

      if (m.dumpNets) {
	 m.dumpNets(m, false, ".nets-.json");
      }
      
      if (circuit.isCyclic(nets)) {
	 uncycle.uncycle(m);
      }
      return m;
   }
   ...
   setInputSignal(signame, value) {
	 ...
	 throw new ReferenceError(`unbound input signal "${signame}"`);
      ...
      this.global_signal_map[signame].nowval = value;
   }
...
/*    }                                                                */
   ...
   mach.nets.forEach((n,i,a) => {
	 ...
	 if (mach.tracePropagation || net.DEBUG_PROPAGATE) {
	    console.error(`[${age}] register ${n.id} v=${n.value} (${n.debugName})`);
	 }
	 ...
	 if (((n.accessibility & ast.IN) !== 0) && n.signal?.emitted[n.lvl]) {
	    if (mach.tracePropagation || net.DEBUG_PROPAGATE) {
	       console.error(`[${age}] signal ${n.id} (${n.debugName})`);
	    }
	    n.value = true;
	    n.isInKnownList = true;
	    known_list.push(n.trace(age));
	    ...
	    if (mach.tracePropagation || net.DEBUG_PROPAGATE) {
	       console.error(`[${age}] constant ${n.id} v=${n.neutral} (${n.debugName})`);
	    }
	 ...
	 if (mach.tracePropagation || net.DEBUG_PROPAGATE) {
	    console.error(`[${age}] wire ${n.id} (${n.debugName})`);
	 }
   ...
   if (nonpropagated_nets > 0 && mach.nets.find(n => !n.isInKnownList && !n.maybeBottom)) {
      try {
	 ReactiveMachine.Configuration.CausalityHandler(mach);
