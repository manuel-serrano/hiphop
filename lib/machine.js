...
function ReactiveMachine(ast_node, name="") {
   ...
   //
   // Allow to delay inputs and reactions if they append during a
   // reaction. input_queue can contain:
   //
   //   - a pair {signal_name, signal_value}
   //   - a function (immediate notification of exec termination
   //     when the exec is started)
   //
   this.react_queue = [];
   this.input_queue = [];
...
ReactiveMachine.prototype.react = function() {
   if (this.react_in_progress) {
      this.react_queue.push(this.input_queue);
      this.input_queue = [];
      return;
   }
      ...
      if (state.kill || state.start) {
	    ...
	    let self = {
		  ...
		  if (state.id == self.id) {
			...
			//
			machine.input_queue.push(_notify);
		     } else {
			_notify();
		     }
		     return true;
   ...
   while (this.react_queue.length > 0) {
      let inputs = this.react_queue.pop();

      inputs.forEach(input => {
	 if (input instanceof Function) {
	    input();
	 } else {
	    this.input(input.signal_name, input.signal_value);
	 }
      });
      this.react();
   }
...
ReactiveMachine.prototype.input = function(signal, value=undefined) {
   ...
   if (this.react_in_progress) {
      this.input_queue.push({ signal_name: signal, signal_value: value });
      return;
   }
