...
/*    Last change :  Tue Nov 28 20:27:23 2023 (serrano)                */
...
//import { debug as configDebug } from "config";
...
let debug = false;
let configDebug = process.env.HIPHOP_DEBUG;
...
class ReactiveMachine {
   ...
   constructor(ast_node, opts={}) {
	 ...
	 this.name = opts?.name || "";
      ...
      this.sweep = opts?.sweep || (configDebug === 0);
...
/*    }                                                                */
   ...
   if (nonpropagated_nets > 0) {
      const { loc, size, signals, json, result } = findCausalityError(mach);
      
      if (configDebug && hop.isServer) {
	 await dumpCausalityError(json);
      ...
      if (size > 0) {
	 throw error.CausalityError(`causality cycle of length ${size} detected, involving signals "${signals.join(', ')}"`, loc);
      } else {
