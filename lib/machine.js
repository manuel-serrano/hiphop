...
/*    Last change :  Thu Oct  4 09:30:45 2018 (serrano)                */
...
function react( mach ) {
   ...
   for( let i in mach.exec_status_list ) {
	 ...
	 let vals = signal.generate_this( state.astNode.machine,
					  state.astNode.accessor_list,
					  state.lvl );
	 Object.assign( self, vals );
...
function findCausalityError( machine ) {
   ...
   let XXhead = nets.find( n => isCyclic( n ) );
