...
/*    Last change :  Thu Nov  6 10:11:09 2025 (serrano)                */
...
export let Sigpre = config.Process.env?.HIPHOP_SIGPRE ?? true;
export let Unreachable = config.Process.env?.HIPHOP_UNREACHABLE ?? true;
...
class ReactiveMachine {
   ...
   constructor(ast_node, opts={}) {
	 ...
	 this.$$name = opts?.name || "";
      ...
      this.sigpre = ("sigpre" in opts) ? opts.sigpre : Sigpre;
      this.unreachable = ("unreachable" in opts) ? opts.unreachable : Unreachable;
	 ...
	 throw new Error("HipHop, cannot use both dynamic and sweep: " + opts);
      ...
      // sync duplication
      this.syncDup = ("syncDup" in opts) ? opts.syncDup : undefined;
      
      // unroll loops
      this.loopUnroll = ("loopUnroll" in opts) ? opts.loopUnroll : undefined;
      
      // reincarnation
      this.reincarnation = ("reincarnation" in opts) ? opts.reincarnation : this.compiler === "int";
      
      // loop duplication
      this.loopDup = ("loopDup" in opts) ? opts.loopDup : undefined;
      
      // loop safe
      this.loopSafe = ("loopSafe" in opts) ? opts.loopSafe : undefined;
      ...
      if (opts.native !== false && config.Process.env?.HIPHOP_NATIVE === "true") {
	 if (!this.dynamic) {
	    try {
   ...
   toJS() {
	 ...
	 console.log(`*** WARNING, using precompile file ${config.Process.env.HIPHOP_DEBUG_NATIVE_SOURCE}`);
...
function toJS(mach) {
   ...
   function isConstantNet(n) {
      return n.faninList.length === 0 && n.__proto__ === net.LogicalNet.prototype;
   }
   ...
   nets.forEach(n => n.propagated = false);
   ...
   while (nets.length > 0) {
	 ...
	 buf += `      // ${purify(n.astNode.loc.filename)}:${n.astNode.loc.pos}\n`;
	 if (n.faninList.length === 0) {
	    // a constant automatically propagated
	    n.propagated = true;
	    buf += "      undefined;\n";
	    return false;
   ...
   for (let k in mach.output_signal_map) {
      ...
      const g = s.gate_list[0];
      buf += `      if (${g.toJSVar()}) invokeListeners(this, "${s.name}", emitted, emittedDebug);\n`;
   }
   buf += "\n";
   buf += "      // trigger execs\n";
