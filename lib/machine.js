...
/*    Last change :  Tue Mar  4 11:14:34 2025 (serrano)                */
...
class ReactiveMachine {
   ...
   constructor(ast_node, opts={}) {
	 ...
	 this.$$name = opts?.name || "";
      ...
      this.sweep = ("sweep" in opts) ? opts.sweep : Sweep;
      ...
      if (opts.native !== false && !this.dynamic ) {
	 try {
   ...
   react(signals) {
	 ...
	 this.init_values = [];
      ...
      if (this.#nativeReact) {
	 if (this.react_in_progress) {
	    this.actions.push(() => this.#nativeReact(signals));
	 } else {
	    return this.#nativeReact(signals);
	 }
      } else {
...
function toJS(mach) {
   ...
   buf += "      const res = {};\n";
   buf += "      for (let n in this.output_signal_map) {\n";
   buf += "          const s = this.output_signal_map[n];\n";
   buf += "          if (s.emitted) res[n] = s.value;\n";
   buf += "      }\n";
   buf += "      return res;\n";
   buf += ("   };\n");
   buf += ("   return react;\n");
