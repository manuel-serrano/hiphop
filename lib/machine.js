...
/*    Last change :  Thu Mar 12 08:47:45 2026 (serrano)                */
...
const MachineOpts = [
   ...
   "loopUnroll", "loopTrapUnroll", "loopExpand", "reincarnation", "loopDup",
   "loopSafe", "tracePropagation", "forkOrKill", 
...
class ReactiveMachine {
   ...
   constructor(ast_node, opts={}) {
	 ...
	 throw new Error("HipHop, cannot use both dynamic and sweep: " + opts);
      ...
      // unroll loops (compile with loop duplication)
      this.loopUnroll = get(opts, "loopUnroll", (ReactiveMachine.Configuration.LoopUnroll === undefined ? (this.compiler === "new") : ReactiveMachine.Configuration.LoopUnroll));

      // unroll trap loops (unroll trap in loops even with reincarnation)
      this.loopTrapUnroll = get(opts, "loopTrapUnroll", (ReactiveMachine.Configuration.LoopTrapUnroll === undefined ? (this.compiler === "new") : ReactiveMachine.Configuration.LoopTrapUnroll));
      ...
      this.reincarnation = get(opts, "reincarnation", (ReactiveMachine.Configuration.Reincarnation === undefined ? (this.compiler === "int") : ReactiveMachine.Configuration.Reincarnation));
      
      // reincarnation (compile with reincarnation level)
      this.reincarnationTrap = get(opts, "reincarnationTrap", (ReactiveMachine.Configuration.ReincarnationTrap === undefined ? (this.compiler === "int") : ReactiveMachine.Configuration.ReincarnationTrap));
      
      // loop duplication (experimental compilation feature)
      ...
      // unused compiler option used to work around a bug
      this.forkOrKill = get(opts, "forkOrKill", ReactiveMachine.Configuration.ForkOrKill);
   ...
   static Configuration = {
      ...
      ForkOrKill: getenvBool("HIPHOP_FORKORKILL", false),
      LoopUnroll: getenvBool("HIPHOP_LOOPUNROLL", undefined),
      LoopTrapUnroll: getenvBool("HIPHOP_LOOPTRAPUNROLL", true),
      LoopExpand: getenvBool("HIPHOP_LOOPEXPAND", false),
      LoopDup: getenvBool("HIPHOP_LOOPDUP", undefined),
      Reincarnation: getenvBool("HIPHOP_REINCARNATION", undefined),
      ReincarnationTrap: getenvBool("HIPHOP_REINCARNATION_TRAP", undefined),
      CausalityHandler: x => undefined,
      DumpNets: process.env.HIPHOP_TRACE?.split(",")?.find?.(n => n === "nets"),
