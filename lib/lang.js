...
//
const KEYWORDS = /(\d|^)(signal|immediate|pre|value|not|apply|countValue|countApply|res|susp|kill|ifApply|ifValue|id|from|fromValue|fromApply|applyAccessors|countAccessors|to|toValue|toApply|toggleSignal|toggleValue|toggleApply|emitWhenSuspended|noDebug|name)(\s|$)/i;
...
// `direction` is an alias for `accessibility`
...
const get_signal_declaration_list = function(attrs, loc) {
   ...
   for (let name in attrs) {
      if (name == "%location" || name.toLowerCase() == "__hh_reserved_debug_name__")
	 continue;
	 ...
	 throw new error.SyntaxError("Accessibility/Direction of a signal must be " +
	 ...
	 init_accessor_list = get_accessor_list(init_func,
						prop.initAccessors,
						loc);
      }
	 ...
	 reinit_accessor_list = get_accessor_list(reinit_func,
...
//     - this.type["s"]
//
// It now (June 2018) accepts a second parameter: accessors which have
// been already parsed by preprocessor.
//
const get_accessor_list = function(func, accessors, loc) {
   if (accessors) {
      return accessors.map(acc => makeAccessor(acc.name, acc.type));
   }
...
const get_arguments = function(attrs, loc) {
   ...
   if (raw.apply) {
	 ...
	 throw new error.SyntaxError("apply must be a function.", loc);
      args.func = raw.apply
      args.accessor_list = get_accessor_list(raw.apply,
					     raw.applyaccessors,
					     loc);
   ...
   if (raw.countapply) {
	 ...
	 throw new error.SyntaxError("countApply must be a function.", loc);
      ...
      args.accessor_list_count = get_accessor_list(raw.countapply,
						   raw.countapplyaccessors,
						   loc);
...
function getEmitNodeArguments(attrs, exec_context=false) {
   ...
   if (!exec_context) {
	 ...
	 if (name.toLowerCase() == "ifapply") {
	    ...
	    args.if_accessor_list = get_accessor_list(ifapply, undefined, loc);
	 }
...
function SUSPEND(attrs={}) {
   ...
   let condToggle = attrs.togglesignal || attrs.toggleapply || attrs.togglevalue;
   ...
   if (sig_list.length == 0 && !condFrom && !condRegular && !condToggle) {
      throw new error.SyntaxError("Missing suspend condition.",
				  loc);
   } else if ((condFrom && condToggle) || (condFrom && condRegular)
	      || (condToggle && condRegular)) {
      throw new error.SyntaxError("Cannot use suspend-from/-toggle/apply/value"
				  + " together", loc);
      ...
      throw new error.SyntaxError("Cannot use suspend-toggle with a signal.",
				  loc);
	...
	<hh.trap END_BODY nodebug>
	  <hh.parallel nodebug>
	    ...
	    <hh.every signal=${attrs.from} immediate=${attrs.immediate}
	              apply=${attrs.fromapply} applyAccessors=${attrs.fromAccessors}
		      value=${attrs.fromvalue}
                      nodebug>
	      <hh.abort signal=${attrs.to}
			apply=${attrs.toapply} applyAccessors=${attrs.toAccessors}
