...
/*    Last change :  Thu Dec 18 08:04:52 2025 (serrano)                */
...
function getSignalDeclList(tag, attrs, nodes, loc) {
   ...
   for(let name in attrs) {
	 ...
	 throw error.SyntaxError(tag + ": bad signal declaraison (" + name + ").",
				loc);
      }
...
function getArguments(tag, attrs, loc, raise) {
   ...
   if (raw["%value"] && raw.apply) {
	 ...
	 tag + ": apply and %value can't be used at same time.",
	 loc);
   ...
   if (raise && raw.immediate && (raw["%value"] || raw.apply)) {
      throw error.SyntaxError(
	 tag + ": immediate cannot be used with an expression.",
	 loc);
   }

   if (raise && raw.pre && (raw["%value"] || raw.apply)) {
      throw error.SyntaxError(
	 tag + "pre cannot be used with an expression.", loc);
   }

   if (raw.apply) {
      if (raw.apply instanceof ast.$Delay) {
	 args.func = raw.apply;
	 args.accessor_list = [];
      } else if (raw.apply instanceof Function) {
	 args.func = raw.apply;
	 args.accessor_list =
...
function getArgumentsExpressionNode(tag, attrs, loc, axs) {
   ...
   if (false && attrs?.apply instanceof ast.$Delay) {
      const delay = attrs.apply;
      if (!delay.accessor_list || !delay.accessor_list_count) {
	 console.error("*** ERROR: wrong delay", JSON.stringify(delay));
	 throw TypeError("Illegal delay");
      }
      return {
	 func: delay,
	 accessor_list: delay.accessor_list,
	 accessor_list_count: delay.accessor_list_count
      };
   } else {
      const raise = (axs.length == 0) && !(attrs?.apply instanceof ast.$Delay);
...
function getArgumentsCountExpressionNode(tag, attrs, axs, cntaxs) {
   ...
   if (args.immediate && args.func_count) {
      throw error.SyntaxError(tag + ": cannot use immediate with count expression.",
			      loc);
   }
...
function checkArgumentsAccessors(tag, args, loc) {
   if (!(args.func instanceof ast.$Delay)
      &&
      (args.accessor_list.length == 0 && args.accessor_list_count.length == 0)) {
...
function AWAIT(attrs={}, ...nodes) {
   ...
   return new ast.Await(tag, args.id, loc, args.nodebug, args.func,
...
function ABORT(attrs={}, ...nodes) {
   ...
   return new ast.Abort(
      tag, attrs.id, loc, args.nodebug, args.func,
      args.accessor_list, args.immediate,
      args.func_count,
      args.accessor_list_count,
      makeImplicitSequence(attrs, children));
...
function WEAKABORT(attrs={}, ...nodes) {
   ...
   checkArgumentsAccessors(tag, args, loc);
   return new ast.WeakAbort(
      tag, attrs.id, loc, args.nodebug,
...
function SUSPEND(attrs={}, ...nodes) {
   ...
   if (("from" in attrs || "fromapply" in attrs || "fromvalue" in attrs) &&
	 ...
	 TRAP({ END_BODY: "", nodebug: "", "%location": loc },
      ...
      if (axs.length == 0 && signame) attrs[signame] = "";
      const args = getArgumentsExpressionNode(tag, attrs, loc, axs);
      const children = checkChildrenMin(tag, attrs, nodes, 1, loc);
      checkArgumentsAccessors(tag, args, loc);
...
function LOOPEACH(attrs={}, ...nodes) {
   ...
   return new ast.LoopEach(tag, args.id, loc, args.nodebug,
			    makeImplicitSequence(attrs, children),
			    args.func, args.accessor_list,
			    args.func_count,
			    args.accessor_list_count);
...
function EVERY(attrs={}, ...nodes) {
   ...
   return new ast.Every(tag, args.id, loc, args.nodebug,
			 makeImplicitSequence(attrs, children),
			 args.func, args.accessor_list,
			 args.immediate,
			 args.func_count,
