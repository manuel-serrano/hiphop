...
function HIPHOP(attrs) {
   ...
   children = children.slide(0, i);
   children.push(implicit_sequence(attrs, stmts);
...
function INPUTSIGNAL(attrs) {
   var loc = format_loc(attrs);
   var sigdecl = init_signal_declaration(attrs, loc, ["!id", "name"],
...
function LOCALSIGNAL(attrs) {
   var children = get_children(arguments);
   var loc = format_loc(attrs);
   var sigdecl = init_signal_declaration(attrs, loc, ["name"], ["id",
								"type",
								"init_value",
								"combine_with",
								"valued"]);
