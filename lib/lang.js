...
//
const KEYWORDS = /(\d|^)(signal|immediate|pre|value|not|apply|countValue|countApply|res|susp|kill|ifApply|ifValue|id|from|fromValue|fromApply|to|toValue|toApply|emitWhenSuspended|noDebug|name)(\s|$)/i;
...
function SUSPEND(attrs={}) {
   ...
   if (sig_list.length == 0 && !condFrom && !condRegular) {
      throw new error.SyntaxError("Missing suspend condition.",
				  loc);
   } else if (condFrom && condRegular) {
      throw new error.SyntaxError("Cannot use suspend-from with an apply of value.",
				  loc);
	...
	<hh.trap END_BODY nodebug>
	  <hh.parallel nodebug>
	    ...
	    <hh.sequence nodebug>
	      ...
	      <hh.atom apply=${() => console.log("will exit")}/>
