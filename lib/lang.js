...
/*    Last change :  Fri Dec 24 06:24:43 2021 (serrano)                */
...
function SUSPEND(attrs={}, ...nodes) {
   ...
   if (("from" in attrs || "fromapply" in attrs || "fromvalue" in attrs) &&
      ...
      return LOCAL({ SUSPEND_CONTINUOUS: true, nodebug: true },
	 TRAP({ END_BODY: true, nodebug: true },
	    FORK({ nodebug: true },
	       SEQUENCE({ nodebug: true, "%location": attrs['%location']},
		  SUSPEND_({immediate: true,
			    SUSPEND_CONTINUOUS: true },
		     SEQUENCE.apply(this, [{}].concat(nodes))),
		  EXIT({END_BODY: true, nodebug: true })),
		  ...
		  ABORT({ signal: attrs.to,
			...
			: NOTHING({}),
		     FORK({ nodebug: true },
			SUSTAIN({ SUSPEND_CONTINUOUS: true, 
				  nodebug: true }),
			attrs.emitwhensuspended
			   ? SUSTAIN({ [attrs.emitwhensuspended]: true,
				       nodebug: true })
			   : NOTHING({})))))));
	 ...
	 TRAP({ END_BODY: true, nodebug: true },
	    FORK({ nodebug: true },
		  ...
		  AWAIT({ immediate: attrs.immediate,
			  ...
			  applyAccessor: attrs.toggleAccessors,
