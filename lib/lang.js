/*=====================================================================*/
/*    serrano/prgm/project/hiphop/1.3.x/lib/lang.js                    */
/*    -------------------------------------------------------------    */
/*    Author      :  Colin Vidal                                       */
/*    Creation    :  Mon Jul 16 11:12:00 2018                          */
/*    Last change :  Thu Feb 22 15:01:47 2024 (serrano)                */
...
function FORK(attrs={}, ...nodes) {
   ...
   return new ast.Fork(tag, attrs.id, loc,
			has_nodebug_kwd(attrs),
			children);
}
...
function SUSPEND(attrs={}, ...nodes) {
   ...
   if (("from" in attrs || "fromapply" in attrs || "fromvalue" in attrs) &&
	 ...
	 TRAP({ END_BODY: "", nodebug: "", "%location": loc },
      ...
      return LOCAL({ SUSPEND_CONTINUOUS: "", nodebug: "" },
	 TRAP({ END_BODY: "", nodebug: "" },
	    FORK({ nodebug: "" },
	       SEQUENCE({ nodebug: "", "%location": loc},
		  SUSPEND_({immediate: "",
			    "%location": loc,
      ...
      const args = getArgumentsExpressionNode("SUSPEND", attrs, loc, axs);
      const children = checkChildrenMin("SUSPEND", attrs, nodes, 1, loc);
...
function LOOP(attrs={}, ...nodes) {
   ...
   return new ast.Loop(attrs.id, loc, has_nodebug_kwd(attrs),
			makeImplicitSequence(attrs, children));
}
