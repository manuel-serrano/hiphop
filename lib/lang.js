...
/*    Last change :  Sun Dec 23 08:02:14 2018 (serrano)                */
...
function getModule( id, loc ) {
   if( allModules[ id ] ) {
      ...
      throw new error.ReferenceError( `module "${id}" unbound`, loc );
   }
}
...
function registerModule( id, mod, loc ) {
   if( allModules[ id ] ) {
      throw new error.TypeError( `module "${id}" already defined`, loc );
   } else {
      return allModules[ id ] = mod;
   }
}
