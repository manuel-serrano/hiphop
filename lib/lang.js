...
function expression_node(loc, attrs, signal_name_in_expr=true) {
   ...
   if (attrs.signal_name && signal_name_in_expr) {
	 ...
	 throw new error.SyntaxError("Can't use `signal_name` with " +
      ...
      let accessor = attrs.test_pre ?
	  presentPre(attrs.signal_name) :
	  present(attrs.signal_name);
      attrs.args[0] = accessor;
   } else if (attrs.immediate && signal_in_expr) {
...
function EMIT(attrs) {
   ...
   expression_node(loc, attrs, false);
