...
/*    Last change :  Mon Jul 16 17:45:43 2018 (serrano)                */
...
//
function getArguments( tag, attrs, loc ) {
   let raw = {};
   let args = {
      loc: loc,
      id: "",
...
/*---------------------------------------------------------------------*/
function getArgumentsActionNode( tag, attrs ) {
   let args = getArguments( tag, attrs, attrs[ "%location" ] );

   if( !args.func ) {
      throw new SyntaxError( tag + ": missing expression.", loc );
   }
...
/*    validity.                                                        */
/*---------------------------------------------------------------------*/
function getArgumentsExpressionNode( tag, attrs, loc=null ) {
   if( !loc ) loc = attrs[ "%location" ];

   let args = getArguments( tag, attrs, loc );
   ...
   if( signal_name && args.func ) {
      throw new SyntaxError( tag + ": an expression node can take a signal or " +
...
/*    validity.                                                        */
/*---------------------------------------------------------------------*/
function getArgumentsCountExpressionNode( tag, attrs ) {
   let loc = attrs[ "%location" ];
   let args = getArgumentsExpressionNode( tag, attrs, loc );
...
function LOCAL( attrs={}, ...nodes ) {
   const loc = format_loc( attrs );
   const children = getChildrenMin( "LOCAL", attrs, nodes, 1 );
...
function EMIT( attrs={}, ...nodes ) {
   ...
   const children = checkNoChildren( "EMIT", attrs, nodes );
   
   return new ast.Emit( args.id, args[ "%location" ],
...
function SUSTAIN( attrs={}, ...nodes ) {
   ...
   const children = checkNoChildren( "SUSTAIN", attrs, nodes );
...
function IF( attrs={}, ...nodes ) {
   const loc = format_loc( attrs );
   const args = getArgumentsExpressionNode( "IF", attrs );
...
function AWAIT( attrs={}, ...nodes ) {
   let args = getArgumentsCountExpressionNode( "AWAIT", attrs );
...
function ABORT( attrs={}, ...nodes ) {
   const args = getArgumentsCountExpressionNode( "ABORT", attrs );
...
function WEAKABORT( attrs={}, ...nodes ) {
   const args = getArgumentsCountExpressionNode( "WEAKABORT", attrs );
...
function SUSPEND_( attrs={}, ...nodes ) {
   const args = getArgumentsExpressionNode( "SUSPEND", attrs );
...
function LOOPEACH( attrs={}, ...nodes ) {
   const args = getArgumentsCountExpressionNode( "LOOPEACH", attrs );
   const children = getChildrenMin( "LOOPEACH", attrs, nodes, 1 );
...
function EVERY( attrs={}, ...nodes ) {
   const args = getArgumentsCountExpressionNode( "EVERY", attrs );
   const children = getChildrenMin( "EVERY", attrs, nodes, 1 );
   
   return new ast.Every( args.id, args.loc, args.nodebug,
			 makeImplicitSequence(attrs, children), args.func,
...
function ATOM( attrs={}, ...nodes ) {
   const args = getArgumentsActionNode( "ATOM", attrs );
   const children = checkNoChildren( "ATOM", attrs, nodes );
...
function TRAP( attrs={}, ...nodes ) {
   const children = getChildrenMin( "TRAP", attrs, nodes, 1 );
...
function EXIT( attrs={}, ...nodes ) {
   const children = checkNoChildren( "EXIT", attrs, nodes );
