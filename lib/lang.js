...
/*    Last change :  Sun Sep 30 00:36:40 2018 (serrano)                */
...
function getArguments( tag, attrs, loc, raise ) {
   ...
   args.signame_list = getSignalNameList( attrs, loc );
   args.id = raw.id;
...
function getArgumentsExpressionNode( tag, attrs, loc, axs ) {
   ...
   if( signame ) {
	 ...
	 makeAccessor( signame, (args.pre ? "prePresent" : "present"), loc );
...
function RUN( attrs={}, ...nodes ) {
   ...
   sigdecllist.forEach( (sigdecl, d, a) => {
      ...
      if( sigalias ) {
	 if( typeof( sigalias ) != "string" && !(sigalias instanceof String) ) {
   ...
   let body = new ast.Local( attrs.id, loc, false, sigdecllist,
			     makeImplicitSequence( attrs, module.children ) );
   body.in_run_context = true;
   return new ast.Run( attrs.id, loc, module.name, has_nodebug_kwd(attrs), sigdecllist, body );
}
...
exports.get_signame_list = getSignalNameList;
