...
function check_action_node(loc, attrs) {
...
function check_expression_node(loc, attrs) {
   ...
   if (immediate) {
	 ...
	 throw new error.SyntaxError("`immediate` must be boolean.", loc);
...
function check_one_children(node_arguments, loc) {
   check_children(node_arguments, loc, 1, 1);
}
...
function ABORT(attrs) {
   let loc = format_loc(attrs);
   let children = check_one_children(arguments, loc);
...
function SEQUENCE(attrs) {
   let loc = format_loc(attrs);
   let children = check_children(arguments, loc, 2, -1);
...
function TRAP(attrs) {
   let loc = format_loc(attrs);
   let children = get_one_children(arguments, loc);
