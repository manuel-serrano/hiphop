...
function EMIT(attrs) {
   ...
   return new ast.Emit(loc, attrs.signal_name, attrs.func, attrs.args);
}
...
function SUSTAIN(attrs) {
   ...
   attrs = check_attributes(attrs, ["signal_name"], ["func", "args"]);
   check_emit_node(loc, attrs);
   return new ast.Sustain(loc, attrs.signal_name, attrs.func, attrs.args);
}
...
function AWAIT(attrs) {
   ...
   attrs = check_attributes(attrs,
			    ...
			    ["test_pre", "immediate", "func", "args",
			     "args_count", "func_count", "count"]);
   check_count_expression_node(loc, attrs);
   return new ast.Await(loc,
			attrs.func,
			attrs.args,
...
function PARALLEL(attrs) {
   ...
   attrs = check_attributes(attrs, undefined, ["id"]);
...
function LOOP(attrs) {
   ...
   attrs = check_attributes(attrs, undefined, undefined);
   return new ast.Loop(loc, implicit_sequence(attrs, children));
}
...
function SEQUENCE(attrs) {
   ...
   attrs = check_attributes(attrs, undefined, undefined);
   return new ast.Sequence(loc, children);
}
...
function ATOM(attrs) {
   ...
   return new ast.Atom(loc, attrs.func, attrs.args);
}
...
function TRAP(attrs) {
   ...
   return new ast.Trap(loc, attrs.trap_name,
		       implicit_sequence(attrs, children));
}
...
function EXIT(attrs) {
   ...
   check_no_children(arguments, loc);
   check_attributes(attrs, ["trap_name"], undefined);
   return new ast.Exit(loc, attrs.trap_name);
}
...
function LOCALSIGNAL(attrs) {
   ...
   return new ast.LocalSignal(loc,
			      sigdecl.name,
			      implicit_sequence(attrs, children),
			      sigdecl.type,
			      sigdecl.init_value,
