...
function check_expressions(loc, func, args) {
   ...
   for (let i in args) {
      if (args[i] instanceof SignalAccessor) {
	 if (args[i].used)
	    throw new error.SyntaxError("A signal accessor is used more than " +
					"one time. You shouldn't keeps the " +
					"object return by an accessor.", loc);
	 args[i].used = true;
      }
   }
...
function SignalAccessor(signal_name, get_pre, get_value) {
   ...
   /* following fields filled at compile time

      `gate` holds the gate only if !get_value, which we must test the value
      at runtime. If we need to test pre, there is nothing to do (the compiler
      put the door of pre)

      `signal` holds the valued signal object only is get_value, which we must
      get the value (or pre value) at runtime.

      `used` is a flag used by frontend, to trigger an error is a user
      uses more than one time the same signal accessor.
   */
   this.gate = null;
   this.signal = null;
   this.used = false;
