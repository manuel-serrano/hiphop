...
function EMIT(attrs) {
   ...
   attrs = check_attributes(attrs, ["signal_name"], ["func", "arg"]);
...
function SUSTAIN(attrs) {
   ...
   attrs = check_attributes(attrs, ["signal_name"], ["func", "arg"]);
   check_emit_node(loc, attrs);
   return new ast.Sustain(attrs.id, loc, attrs.signal_name,
			  attrs.func, attrs.args);
}
...
function TASKSIGNAL(attrs) {
   let loc;

   attrs = check_attributes(attrs, ["name"], undefined);
   loc = format_loc(attrs);
   check_no_children(arguments, loc);
   check_signal_name(loc, attrs.name);
   return new ast.TaskSignal(attrs.id, loc, attrs.name);
}
...
function EXEC(attrs) {
   ...
   attrs = check_attributes(attrs, ["signal_name", "interface"], ["arg"]);
   ...
   return new ast.Exec(attrs.id, loc, attrs.signal_name, exec_interface,
		       [present(attrs.signal_name)], attrs.args);
}
