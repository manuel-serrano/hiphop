...
function ABORT(attrs) {
   ...
   if (!func && args.length == 0 && !attrs.signal_name)
      throw new error.SyntaxError("`signal_name` must be defined.", loc)
   else if (!attrs.signal_name && attrs.test_pre)
      throw new error.SyntaxError("`test_pre` is useless here.", loc);
