...
/* TODO:
   - rewrite generic code for testing arguments given to statments
   - write generic code whose check that signal are in allowed position
     in the whole code (and not just on top-level or a let top-level
*/
...
function check_count_expression_node(loc, attrs) {
   ...
   attrs.args_count = check_func_args_list(loc, attrs.func_count, attrs,
					   "arg_count");
...
/* This function return an empty attrs object if it didn't exists,
   that can happen if a node is defined without any attribute.
   Them it check that all given attribute are allowed, and mandatory attributes
   are given.

   It's old an terrible mess function, maybe not mandatory, it must be
   think again and rewritten... */
function check_attributes(attrs, mandatory, optional) {
   function check_prefix(name) {
      let splited;

      splited = name.split("arg_count");
      if (splited[0] == "" && (parseInt(splited[1]) == splited[1] ||
			       splited[1] == ""))
	 return "arg_count";

      splited = name.split("arg");
      if (splited[0] == "" && (parseInt(splited[1]) == splited[1] ||
			       splited[1] == ""))
	 return "arg";
   ...
   for (let name in attrs) {
      if (name == "%location" || name == "id")
	 ;
      else if (check(mandatory, name))
	 ;
      else if (check(optional, name))
	 ;
      else
...
function MODULE(attrs) {
   ...
   while(children[i] instanceof ast.InputSignal
	 ...
	 if (inputs.indexOf(child) > -1
	     || outputs.indexOf(child) > -1)
...
function LET(attrs) {
   ...
   function mk_tree(children) {
      ...
      if (car instanceof ast.LocalSignal) {
   ...
   for (i = 0; i < children.length && children[i] instanceof ast.Signal; i++) {
      let child = children[i];

      if (child instanceof ast.InputSignal ||
	  child instanceof ast.OutputSignal) {
	 throw new error.SyntaxError("Can't use global signal in a let", loc);
      }

      if (local_signals.indexOf(child) > -1) {
	 throw new error.SignalError("Name of local signal already used.",
				     child.name, loc);
      }
      local_signals.push(child);
   }

   /* define j in for stmt make hop tell that `i` is not defined */
   let j = 0;
   for (j = i; j < children.length; j++) {
      let child = children[j];

      if (child instanceof ast.Signal) {
	 throw new error.SyntaxError("Signal forbidden in this context.", loc);
      }
      stmt_list.push(child);
   }

   if (stmt_list.length > 1) {
      let seq = implicit_sequence(attrs, children.slice(i));

      children = children.slice(0, i);
      children.push(seq);
   } else if (stmt_list == 0) {
      throw new error.SyntaxError("Missing statements in Let.", loc);
   }

   return mk_tree(children);
}
...
function EMIT(attrs) {
   ...
   attrs = check_attributes(attrs, ["signal"], ["func", "arg"]);
...
function IF(attrs) {
   ...
   attrs = check_attributes(attrs, ["arg"], ["func", "not"]);
   check_expression_node(loc, attrs);
...
function PRESENT(attrs) {
   ...
   attrs = check_attributes(attrs, ["signal"], ["test_pre", "not"]);
   check_expression_node(loc, attrs);
   return new ast.If(attrs.id,
		     loc,
		     [children[0], children[1]],
...
function AWAIT(attrs) {
   ...
   attrs = check_attributes(attrs,
			    ["signal"],
			    ["test_pre", "immediate", "func", "arg",
			     "arg_count", "func_count", "count"]);
...
function ABORT(attrs) {
   ...
   attrs = check_attributes(attrs, undefined,
			    ["signal", "test_pre", "immediate",
			     "func", "arg",
			     "count", "func_count", "arg_count"]);
   check_count_expression_node(loc, attrs);
...
function SUSPEND(attrs) {
   ...
   attrs = check_attributes(attrs, ["signal"], ["test_pre"]);
   check_expression_node(loc, attrs)
...
function LOOPEACH(attrs) {
   ...
   attrs = check_attributes(attrs, ["signal"], ["test_pre", "count"]);
   check_count_expression_node(loc, attrs);
...
function EVERY(attrs) {
   ...
   attrs = check_attributes(attrs, ["signal"], ["immediate", "count"]);
   check_count_expression_node(loc, attrs);
...
function ATOM(attrs) {
   ...
   check_no_children(arguments, loc);
   attrs = check_attributes(attrs, ["func"], ["arg"]);
...
function TRAP(attrs) {
   ...
   check_attributes(attrs, ["name"], undefined);
...
function EXIT(attrs) {
   ...
   check_no_children(arguments, loc);
   check_attributes(attrs, ["trap"], undefined);
...
function init_signal_declaration(attrs, loc, mandatory, optional) {
   ...
   if (attrs.combine_with) {
      if (!(attrs.combine_with instanceof Function))
	 throw new error.TypeError("Function", typeof(attrs.combine_with), loc);
      combine = true;
   }

   if (attrs.reinit_func) {
      if (!combine)
	 throw new error.SyntaxError("`reinit_func` must always be used " +
				      "with a combinaison function", loc);
      if (!(attrs.reinit_func instanceof Function))
	 throw new error.TypeError("Function", typeof(attrs.reinit_func), loc)
      if (attrs.reinit_func.length != 0)
	 throw new error.TypeError("Function with arity 0", "Function with" +
				   "arity " + attrs.reinit_func.length, loc)
   }
...
function SIGNAL(attrs) {
   let loc = format_loc(attrs);
   let sigdecl = init_signal_declaration(attrs, loc, ["name"], ["init_value",
								"combine_with",
								"valued",
								"reinit_func"]);
   check_no_children(arguments, loc);
...
function EXEC(attrs) {
   ...
   attrs = check_attributes(attrs, ["interface"], ["signal", "arg"]);
   ...
   /* See Exec comment in ast.js. As Exec is typed by CountExpressionNode,
      args_list must contains an accessor to signal to wait. */
