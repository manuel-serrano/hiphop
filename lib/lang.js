...
/*    Last change :  Fri Sep 28 14:26:50 2018 (serrano)                */
...
/*    getSignalNameList ...                                            */
/*---------------------------------------------------------------------*/
function getSignalNameList( attrs, loc ) {
...
function getSignalDeclList( tag, attrs, nodes, loc ) {
   ...
   for( let name in attrs ) {
	 ...
	 throw error.SyntaxError( tag + ": bound must be a string ("
				+ name + ").",
				loc);
      }
...
function getArguments( tag, attrs, loc, raise ) {
   ...
   args.signal_name_list = getSignalNameList( attrs, loc );
   args.id = raw.id;
...
function mergeAccessors( l1, l2 ) {
   ...
   const l = l2.filter( a2 => !l1.find( a1 => a1.signal_name === a2.signal_name ) );
   return l1.concat( l );
}
...
function checkAccessorList( tag, loc, sns, axs, exec_context=false ) {
   axs.forEach( acc => {
      sns.forEach( sn => {
	 if( acc.signal_name == sn && !acc.get_pre && !exec_context ) {
...
function getEmitNodeArguments( tag, attrs, axs, cntaxs, exec_context=false ) {
   ...
   const signal_name_list =
	 args.signal_name_list
	 .concat( axs.map( a => a.signal_name ) )
	 .concat( cntaxs.map( a => a.signal_name ) );
   ...
   args.accessor_list =
      checkAccessorList( tag, loc, args.signal_name_list,
   ...
   if( args.if_accessor_list ) {
	 ...
	 checkAccessorList( tag, loc, args.signal_name_list,
      ...
      args.if_accessor_list =
	 checkAccessorList( tag, loc, args.signal_name_list, cntaxs );
   }
...
function MODULE( attrs={}, ...nodes ) {
   ...
   return new ast.Module( attrs.id, attrsLoc( attrs ),
			  attrs.__hh_reserved_debug_name__,
			  getSignalDeclList( "MODULE", attrs, nodes, attrsLoc( attrs ) ),
...
function LOCAL( attrs={}, ...nodes ) {
   ...
   return new ast.Local( attrs.id, attrsLoc( attrs ),
			 has_nodebug_kwd( attrs ),
			 getSignalDeclList( "LOCAL", attrs, nodes, attrsLoc( attrs ) ),
			 makeImplicitSequence( attrs, children ) );
}
...
function EMIT( attrs={}, ...nodes ) {
   ...
   const args = getEmitNodeArguments( "EMIT", attrs, axs, cntaxs );
   
   checkNoChildren( "EMIT", attrs, nodes );

   return new ast.Emit( args.id, "EMIT", loc,
...
function SUSTAIN( attrs={}, ...nodes ) {
   ...
   const args = getEmitNodeArguments( "SUSTAIN", attrs, axs, cntaxs );
   const loc = attrsLoc( attrs );

   checkNoChildren( "SUSTAIN", attrs, nodes );

   return new ast.Sustain( args.id, loc, args.nodebug,
...
function IF( attrs={}, ...nodes ) {
   let axs = filterFlat( nodes, isValSignalAccessor );
   const loc = attrsLoc( attrs );
   const args = getArgumentsExpressionNode( "IF", attrs, loc, axs );
...
function AWAIT( attrs={}, ...nodes ) {
   const axs = filterFlat( nodes, isValSignalAccessor );
   const cntaxs = filterFlat( nodes, isCntSignalAccessor );
   const args = getArgumentsCountExpressionNode( "AWAIT", attrs, axs, cntaxs );
   
   checkNoChildren( "AWAIT", attrs, nodes );
   checkArgumentsAccessors( "AWAIT", args, args.loc );
...
function FORK( attrs={}, ...nodes ) {
   ...
   return new ast.Fork( attrs.id, attrsLoc( attrs ),
			has_nodebug_kwd( attrs ),
			children );
}
...
function ABORT( attrs={}, ...nodes ) {
   ...
   const axs = filterFlat( nodes, isValSignalAccessor );
   const cntaxs = filterFlat( nodes, isCntSignalAccessor );
   const args = getArgumentsCountExpressionNode( "ABORT", attrs, axs, cntaxs );
   const loc = attrsLoc( attrs );

   checkArgumentsAccessors( "ABORT", args, loc );
   
   var r = new ast.Abort( attrs.id, loc, args.nodebug, args.func,
			  args.accessor_list, args.immediate,
			  args.func_count,
			  args.accessor_list_count,
			  makeImplicitSequence( attrs, children ) );
...
function WEAKABORT( attrs={}, ...nodes ) {
   ...
   const axs = filterFlat( nodes, isValSignalAccessor );
   const cntaxs = filterFlat( nodes, isCntSignalAccessor );
   const args = getArgumentsCountExpressionNode( "WEAKABORT", attrs, axs, cntaxs );
   const loc = attrsLoc( attrs );
   
   checkArgumentsAccessors( "WEAKABORT", args, loc );
   
   return new ast.WeakAbort( attrs.id, loc, args.nodebug,
...
function SUSPEND( attrs={}, ...nodes ) {
   let loc = attrsLoc( attrs );
   let sig_list = getSignalNameList( attrs, loc );
   ...
   if( ("from" in attrs || "fromapply" in attrs || "fromvalue" in attrs) &&
	...
	<hh.trap END_BODY nodebug>
      ...
      if( axs.length == 0 ) attrs[ signal_name ] = "";
      const args = getArgumentsExpressionNode( "SUSPEND", attrs, loc, axs );
      const children = checkChildrenMin( "SUSPEND", attrs, nodes, 1 );
      checkArgumentsAccessors( "SUSPEND", args, loc );

      return new ast.Suspend( args.id, loc, args.nodebug,
			      makeImplicitSequence( attrs, children ),
			      args.func, args.accessor_list,
			      args.immediate );
   }
...
function LOOPEACH( attrs={}, ...nodes ) {
   let axs = filterFlat( nodes, isValSignalAccessor );
   let cntaxs = filterFlat( nodes, isCntSignalAccessor );
   const args = getArgumentsCountExpressionNode( "LOOPEACH", attrs, axs, cntaxs );
   const children = checkChildrenMin( "LOOPEACH", attrs, nodes, 1 );
   checkArgumentsAccessors( "LOOPEACH", args, args.loc );
...
function EVERY( attrs={}, ...nodes ) {
   let axs = filterFlat( nodes, isValSignalAccessor );
   let cntaxs = filterFlat( nodes, isCntSignalAccessor );
   const args = getArgumentsCountExpressionNode( "EVERY", attrs, axs, cntaxs );
   
   const children = checkChildrenMin( "EVERY", attrs, nodes, 1 );
   checkArgumentsAccessors( "EVERY", args, args.loc );
...
function RUN( attrs={}, ...nodes ) {
   ...
   if( !attrs.module || !(attrs.module instanceof ast.Module) )
      throw error.SyntaxError( "RUN: module must be a Hiphop.js module", loc );
   ...
   for( let sig in signal_declaration_list ) {
	 ...
	 if( typeof( sigbound ) != "string" && !(sigbound instanceof String) ) {
	    throw error.SyntaxError( "RUN: string expected on signal " +
...
function EXEC( attrs={}, ...nodes ) {
   let axs = filterFlat( nodes, isValSignalAccessor );
   let args = getEmitNodeArguments( "EXEC", attrs, axs, [], true );

   checkNoChildren( "EXEC", attrs, nodes );
   if( attrs.value )
      throw error.SyntaxError( "EXEC: must takes an apply argument.",
			     attrsLoc( attrs ) );

   if (attrs.res && !(attrs.res instanceof Function))
      throw error.TypeError( "Function", typeof attrs.res, attrsLoc( attrs ) );

   if (attrs.susp && !(attrs.susp instanceof Function))
      throw error.TypeError( "Function", typeof attrs.susp, attrsLoc( attrs ) );

   if (attrs.kill && !(attrs.kill instanceof Function))
      throw error.TypeError( "Function", typeof attrs.kill, attrsLoc( attrs ) );
...
exports.get_signal_name_list = getSignalNameList;
