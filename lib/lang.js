...
/*    Last change :  Thu Jul 26 12:43:02 2018 (serrano)                */
...
function EMIT( attrs={}, ...nodes ) {
   const loc = attrsLoc( attrs );
   const axs = nodes.filter( isValSignalAccessor );
   const cntaxs = nodes.filter( isCntSignalAccessor );
   const args = getEmitNodeArguments( "EMIT", attrs, axs, cntaxs );
   
   checkNoChildren( "EMIT", attrs, nodes );

   return new ast.Emit( args.id, loc,
