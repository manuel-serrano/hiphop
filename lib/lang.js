...
function RUN(attrs) {
   ...
   let current_stmt = run_module.children[0];

   while (current_stmt instanceof ast.InputSignal ||
	  current_stmt instanceof ast.OutputSignal) {
   }

   for (let i in run_machine.input_signals)
      if (sigs_assoc[i] == undefined)
	 reactive.fatal_error("Signal " + i + " not mapped.", loc);

   for (let i in run_machine.output_signals)
      if (sigs_assoc[i] == undefined)
	 reactive.fatal_error("Signal " + i + " not mapped.", loc);

   return new ast.Run(attrs.id, format_loc(attrs), run_module, sigs_assoc);
}
