...
/*    Last change :  Wed Oct  3 09:00:30 2018 (serrano)                */
...
function INTERFACE( attrs={}, ...nodes ) {
   ...
   const intf = new ast.Interface( attrs.id, sig );
   
   if( id ) {
      if( id.value in allInterfaces ) {
	 throw error.ReferenceError( "illegal interface duplicate declaration `" + 
				     id.value + "'",
				     loc );
      } else {
	 allInterfaces[ id ] = intf;
      }
   }
...
function RUN( attrs={}, ...nodes ) {
   ...
   if( !attrs.module || !(attrs.module instanceof ast.Module) )
      throw error.SyntaxError( tag + ": module must be a Hiphop.js module", loc );
