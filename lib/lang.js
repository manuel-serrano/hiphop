/*=====================================================================*/
/*    serrano/prgm/project/hiphop/reactive-js/lib/lang.js              */
/*    -------------------------------------------------------------    */
/*    Author      :  Colin Vidal                                       */
...
/*---------------------------------------------------------------------*/
function getSignalDeclarationList( tag, attrs, loc ) {
   ...
   for( let name in attrs ) {
	 ...
	 throw new SyntaxError( tag + ": " + name + " is a reserved keyword." +
...
//
function getEmitNodeArguments(attrs, exec_context=false) {
   ...
   if (!exec_context) {
	 ...
	 if (name.toLowerCase() == "ifapply") {
	    ...
	    args.if_accessor_list =
	       getAccessorList( ifapply, undefined, loc );
	 }
...
function checkChildren( tag, attrs, args, min, max ) {
   let children = args.filter( isHiphopInstruction );
...
/*    MODULE ...                                                       */
/*---------------------------------------------------------------------*/
function MODULE(attrs={}, ...children) {
   let loc = format_loc( attrs );
   console.log( "children" );
   checkChildrenMin( "MODULE", attrs, children, 1 );

   return new ast.Module( attrs.id, loc, attrs.__hh_reserved_debug_name__,
			  getSignalDeclarationList( "MODULE", attrs, attrs[ "%location" ] ),
...
/*    LOCAL ...                                                        */
/*---------------------------------------------------------------------*/
function LOCAL( attrs={}, ...children ) {
   let loc = format_loc( attrs );
   let children = getChildrenMin( children, 1 );
   
   return new ast.Local( attrs.id, loc, has_nodebug_kwd( attrs ),
			 getSignalDeclarationList( "LOCAL", attrs, attrs[ "%location" ] ),
			 make_implicit_sequence( attrs, children ) );
}
...
/*    EMIT ...                                                         */
/*---------------------------------------------------------------------*/
function EMIT( attrs={}, ...children ) {
   let args = getEmitNodeArguments( attrs );
   checkNoChildren( "EMIT", attrs, children );
...
/*    SUSTAIN ...                                                      */
/*---------------------------------------------------------------------*/
function SUSTAIN( attrs={}, ...children ) {
   let args = getEmitNodeArguments( attrs );
   checkNoChildren( "SUSTAIN", attrs, children );
...
function IF( attrs={}, ...children ) {
   ...
   checkChildren( "IF", attrs, children, 1, 2 );
...
/*    NOTHING ...                                                      */
/*---------------------------------------------------------------------*/
function NOTHING( attrs={}, ...children ) {
   checkNoChildren( "NOTHING", attrs, children );
...
/*    PAUSE ...                                                        */
/*---------------------------------------------------------------------*/
function PAUSE( attrs={}, ...children ) {
   checkNoChildren( "PAUSE", attrs, children );
...
/*    HALT ...                                                         */
/*---------------------------------------------------------------------*/
function HALT( attrs={}, ...children ) {
   checkNoChildren( "HALT", attrs, children );
...
/*    AWAIT ...                                                        */
/*---------------------------------------------------------------------*/
function AWAIT( attrs={}, ...children ) {
   let args = get_arguments_count_expression_node( attrs );
   checkNoChildren( "AWAIT", attrs, children );
...
/*    FORK ...                                                         */
/*---------------------------------------------------------------------*/
function FORK( attrs={}, ...children ) {
   checkChildrenMin( "FORK", attrs, children, 1 );
   
   return new ast.Fork( attrs.id, format_loc( attrs ),
...
/*    ABORT ...                                                        */
/*---------------------------------------------------------------------*/
function ABORT( attrs={}, ...children ) {
   let args = get_arguments_count_expression_node( attrs );
   checkChildrenMin( "ABORT", attrs, children, 1);
...
/*    WEAKABORT ...                                                    */
/*---------------------------------------------------------------------*/
function WEAKABORT( attrs={}, ...children ) {
   let args = get_arguments_count_expression_node( attrs );
   checkChildrenMin( "WEAKABORT", attrs, children, 1 );
...
/*    SUSPEND_ ...                                                     */
/*---------------------------------------------------------------------*/
function SUSPEND_( attrs={}, ...children ) {
   let args = get_arguments_expression_node( attrs );
   checkChildrenMin( "SUSPEND", attrs, children, 1 );
...
/*    SUSPEND ...                                                      */
/*---------------------------------------------------------------------*/
function SUSPEND( attrs={}, ...children ) {
   ...
   if( ("from" in attrs || "fromapply" in attrs || "fromvalue" in attrs) &&
	...
	<hh.trap END_BODY nodebug>
	  <hh.fork nodebug>
		...
		${SEQUENCE.apply( children.unshift( {} ) ) }
	      </suspend_>
...
/*    LOOP ...                                                         */
/*---------------------------------------------------------------------*/
function LOOP( attrs={}, ...children ) {
   checkChildreMin( "LOOP", attrs, children, 1 );
...
/*    LOOPEACH ...                                                     */
/*---------------------------------------------------------------------*/
function LOOPEACH( attrs={}, ...children ) {
   let args = get_arguments_count_expression_node( attrs );
   getChildrenMin( "LOOPEACH", attrs, children, 1 );
...
/*    EVERY ...                                                        */
/*---------------------------------------------------------------------*/
function EVERY( attrs={}, ...children ) {
   let args = get_arguments_count_expression_node( attrs );
   getChildrenMin( "EVERY", attrs, children, 1 );
...
/*    SEQUENCE ...                                                     */
/*---------------------------------------------------------------------*/
function SEQUENCE( attrs={}, ...children ) {
   checkChildrenMin( "SEQUENCE", attrs, children, 1);
   return new ast.Sequence(attrs.id, format_loc( attrs ),
			   has_nodebug_kwd( attrs ),
			   children );
}
...
/*    ATOM ...                                                         */
/*---------------------------------------------------------------------*/
function ATOM( attrs={}, ...children ) {
   let args = get_arguments_action_node( attrs );
   checkNoChildren( "ATOM", attrs, children );
...
/*    TRAP ...                                                         */
/*---------------------------------------------------------------------*/
function TRAP( attrs={}, ...children ) {
   getChildrenMin( "TRAP", attrs, children, 1 );
...
/*    EXIT ...                                                         */
/*---------------------------------------------------------------------*/
function EXIT( attrs={}, ...children ) {
   checkNoChildren( "EXIT", attrs, children );
...
function RUN( attrs={}, ...children ) {
   ...
   checkNoChildren( "RUN", attrs, children );
...
/*    EXEC ...                                                         */
/*---------------------------------------------------------------------*/
function EXEC( attrs={}, ...children ) {
   let args = getEmitNodeArguments( attrs, true );

   checkNoChildren( "EXEC", attrs, children );
   if( attrs.value )
