...
//
const KEYWORDS = /(\d|^)(signal|immediate|pre|value|not|apply|countValue|countApply|res|susp|kill|ifApply|ifValue|id|until|untilValue|untilApply|noDebug)(\s|$)/i;
...
const get_arguments = function(attrs, loc) {
   ...
   args.nodebug = raw.hasOwnProperty("nodebug");
...
function MODULE(attrs) {
   ...
   return new ast.Module(attrs.id, loc, get_signal_declaration_list(attrs, loc),
...
function LET(attrs) {
   ...
   return new ast.Let(attrs.id, loc, has_nodebug_kwd(attrs),
...
function RUN(attrs) {
   ...
   let body = new ast.Let(attrs.id, loc, false, signal_declaration_list,
			  make_implicit_sequence(attrs, module.children));
   body.in_run_context = true;
   return new ast.Run(attrs.id, loc, has_nodebug_kwd(attrs), body);
}
