...
function expression_node(loc, attrs, signal_name_in_expr) {
   ...
   if (attrs.signal_name && signal_name_in_expr) {
      if (attrs.func || attrs.args.length > 0)
	 throw new error.SyntaxError("Can't use `signal_name` with " +
				     "expression here.", loc);
      attrs.args[0] = present(attrs.signal_name);
   }

   if (!attrs.signal_name && !attrs.func && attrs.args.length == 0)
      throw new error.SyntaxError("Missing expression/signal.", loc);
}
...
function count_expression_node(loc, attrs) {
   ...
   attrs.args_count = parse_args(attrs.func_args);
...
function ABORT(attrs) {
   ...
   count_expression_node(loc, attrs);
