...
/*    Last change :  Sun Sep 30 19:35:51 2018 (serrano)                */
...
function RUN( attrs={}, ...nodes ) {
   ...
   sigdecllist.forEach( (sigdecl, d, a) => {
	 ...
	 if( typeof( sigalias ) !== "string" ) {
	    throw error.SyntaxError( tag + ": string expected on signal " +
   ...
   return new ast.Run( tag, attrs.id, loc, module.name, has_nodebug_kwd(attrs), sigdecllist, body );
}
