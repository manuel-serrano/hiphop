...
/* Function that checks and prepares attributes for ExpressionNode.
   If `signal_name_in_expr` is set, that means there is `signal_name` attribute
   that must be put in the expression (and using expression with signal_name
   is not allowed) */
function expression_node(loc, attrs, signal_name_in_expr) {
   check_expression(loc, attrs.func, attrs.args);

   if (attrs.signal_name) {
      if (signal_name_in_expr && (attrs.func || attrs.args))
	 throw new error.SyntaxError("Can't use `signal_name` with " +
				     "expression here.", loc);

      attrs.args = [present(attrs.signal_name)];
   } else if (!attrs.args) {
	 attrs.args = [];
   }
}
...
function count_expression_node(loc, attrs) {
   expression_node(loc, attrs, true);
   check_expression(loc, attrs.func_count, attrs.args_count);
   ...
   if (attrs.count && (attrs.func_count || attrs.args_count))
   ...
   if (attrs.count)
      attrs.args_count = [attrs.count];
   else if (!attrs.args_count)
      attrs.args_count = [];
}

function check_expression(loc, func, args) {
...
function check_exclusif_keywords(attrs, loc) {
   if (attrs.immediate) {
	 ...
	 throw new error.SyntaxError("`immediate` can't be used with " +
				     "`count` or expression.", loc);
   } else if (attrs.count) {
      if (attrs.immediate || attrs.func_count || attrs.args_count)
	 throw new error.SyntaxError("`count` can't be used with " +
				     "`immediate`, `func_count`, `args_count`.",
				     loc);
   } else if (attrs.args || attrs.func) {
      if (attrs.immediate)
	 throw new error.SyntaxError("Expression can't be used with " +
				     "`ìmmediate` or `signal_name`.", loc);
   } else if (attrs.func_count || attrs.args_count)
      if (attrs.count || attrs.immediate)
	 throw new error.SyntaxError("`func_count` and `args_count` can't " +
				     "be used with `count` or `immediate`.",
				     loc);
}
...
function ABORT(attrs) {
   ...
   attrs = check_attributes(attrs, undefined,
			    ["signal_name", "test_pre", "weak", "immediate",
			     "func", "args",
			     "count", "func_count", "args_count"]);

   if (attrs.signal_name) {
      args = [present(attrs.signal_name)];
   } else {
      func = attrs.func;
      args = parse_args(attrs.args);
      check_expressions(loc, func, args);
   ...
   if (count) {
      ...
      check_expressions(loc, func_count, args_count)
      func_count = attrs.func_count;
      args_count = parse_args(attrs.args_count);
   }
