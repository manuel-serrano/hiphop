/*=====================================================================*/
/*    serrano/prgm/project/hiphop/0.2.x/lib/net.js                     */
/*    -------------------------------------------------------------    */
/*    Author      :  Colin Vidal                                       */
/*    Creation    :  Sun Sep  9 06:19:12 2018                          */
/*    Last change :  Tue Sep 11 05:42:24 2018 (serrano)                */
...
class LogicalNet extends Net {
   ...
   receive( value, fromDep ) {
	 ...
	 // if value is nkown not-neutral, has already been propagated
	 ...
	 if( value != this.neutral ) {
	    // value received is not-neutral, immediately becomes the net's value,
	    // always return true because dependencies are not relevant anymore
	    this.value = value;
	    ret = true;
	 } else {
	    //  neutral value recieved, value becomes neutral if no more true
	    // fanin, but then return true only if there are no more dependencies
	    if( this.trueFaninCount == 0 ) {
	       this.value = this.neutral;
