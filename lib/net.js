...
/*    Last change :  Thu Jan 16 07:27:48 2025 (serrano)                */
...
class Net {
   ...
   propagate(knownList) {
      ...
      if (trace) {
	 console.error(`propagate ${this.id} v=${this.value} (${this.debug_name})`);
      }
...
class LogicalNet extends Net {
   ...
   receiveTraceExit() {
      console.error("    < " + this.id + " this.v=" + this.value);
   }
   ...
   receive(value, fromDep) {
	 ...
	 return false;
      ...
      if (fromDep) {
	 // MS 16jan2025, I think the following comment is wrong.
...
class SignalNet extends LogicalNet {
   constructor(ast_node, type, debug_name, accessibility, lvl, neutral) {
   ...
   toJS() {
	 ...
	 return `machine["${this.signal.name}"].emitted[${this.lvl}] || ${super.toJS()}`;
      } else {
	 return super.toJS();
      }
   }
...
/*    makeSig ...                                                      */
/*---------------------------------------------------------------------*/
function makeSig(ast_node, type, debug_name, accessibility, lvl=0) {
  return new SignalNet(ast_node, type, debug_name, accessibility, lvl, false);
}
