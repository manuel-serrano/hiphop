...
/*    Last change :  Thu Jan 16 05:26:43 2025 (serrano)                */
...
class Net {
   ...
   propagate(knownList) {
      ...
      if (trace) {
	 console.error(`propagate ${this.id} value:${this.value} (${this.debug_name})`);
      }
   ...
   propagateJS(knownList) {
      const fanout_list = this.fanout_list;
      const len = fanout_list.length;
      
      console.log(`// ${this.id} ${this.debug_name}`);
      console.log(`// ${this.ast_node.loc.filename}:${this.ast_node.loc.pos}`);
      console.log(this.toJSVar(), "=", this.toJS() + ";", "\n");
      
      for(let i = 0; i < len; i++) {
	 const fanout = fanout_list[i];
	 const net = fanout.net;

	 if (!net.isInKnownList) {
	    if (fanout.dependency) {
	       if (--net.dependencyCount === 0) {
		  fanout.net.isInKnownList = true;
		  knownList.push(fanout.net);
	       }
	    } else if (--net.trueFaninCount === 0) {
	       fanout.net.isInKnownList = true;
	       knownList.push(fanout.net);
	    }
	 }
...
class LogicalNet extends Net {
   ...
   LogicalNetType () {
      if (this.neutral == true) {
         ...
         return this.fanin_list.length === 0 ? "FALSE" : "AND";
      }
   }
   ...
   receiveTraceEnter(fromDep) {
	 ...
	 + (fromDep ? "~> " : "-> ")
	 + this.id + " "
	 + (this.fanin_list.length + 1 - (this.dependencyCount + this.trueFaninCount))
	 + "/" + this.fanin_list.length
	 + " v=" + this.value + " n=" + this.neutral);
   }

   receiveTraceExit() {
      console.error("    < " + this.id + " v=" + this.value);
   }
   ...
   receive(value, fromDep) {
	 ...
	 console.log("non-null", this.id, " this.value=", this.value, " this.neutral=", this.neutral, " value=", value, "fromDep=", fromDep);
	 ...
	 if (value !== this.neutral) {
	    console.log("dep != neutral", this.id, " this.value=", this.value, " this.neutral=", this.neutral, " value=", value);
	    this.value = value;
	    return true;
	 }
   ...
   toJS() {
	 ...
	 const deps = this.fanin_list.map(f => f.net.toJSVar());
...
class SignalNet extends LogicalNet {
   ...
   constructor(ast_node, type, debug_name, lvl, neutral) {
      super(ast_node, type, debug_name, lvl, neutral);
      this.sweepable = false;
   }
   ...
   toJS() {
      if (this.isGlobalInputSignalNet) {
	 return `machine["${this.signal.name}"].emitted[${this.lvl}]`;
      } else {
	 return this.super();
      }
   }
...
class SignalExpressionNet extends ActionNet {
   ...
   dump() {
      ...
      if (this.ast_node.machine.verbose > 0) {
	 return Object.assign(d, {
	    signals: [ `!${this.signal.name}:${this.signal_depth}` ]
	 });
      } else {
	 return d;
      }
...
/*    makeSig ...                                                      */
/*---------------------------------------------------------------------*/
function makeSig(ast_node, type, debug_name, lvl=0) {
  return new SignalNet(ast_node, type, debug_name, lvl, false);
}
