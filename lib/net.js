...
/*    Last change :  Tue Jan  6 07:54:37 2026 (serrano)                */
...
class ActionNet extends LogicalNet {
   ...
   toJSprepareThisArg(ax) {
      const lvl = this.lvl;
      const sig = ax.signal;
      const now = sig.netList[Math.min(lvl, sig.astNode.depth)].toJSVar();
      const pre = lvl >= sig.depth ? sig.preNet.toJSVar?.() : false;
   ...
   toJSCall() {
	 ...
	 if (tmp.get_pre) {
	    return sig.preNet.toJSVar();
	 } else {
	    const lvl = this.lvl;
	    const min_lvl = lvl > sig.astNode.depth ? sig.astNode.depth : lvl;
	    
	    return sig.netList[min_lvl].toJSVar();
	 }
      }
   }
}
...
class SignalExpressionNet extends ActionNet {
   ...
   constructor(astNode, signal, debugName, lvl, func, accessor_list) {
      ...
      this.connectTo(signal.dependency_netList[this.min_lvl], FAN.DEP);
   }
