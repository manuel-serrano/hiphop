...
/*    Last change :  Sat Feb 22 07:12:25 2025 (serrano)                */
...
class Net {
   ...
   constructor(astNode, debugName, lvl) {
	 ...
	 throw new Error(`bad "lvl" (${typeof lvl}) for Net ${this.constructor.name} from ast node ${astNode.ctor}`);
      ...
      this.debugName = astNode.machine.verbose > 0 ? `${debugName} [${astNode.ctor}:${lvl}/${astNode.depth}]` : undefined;
      this.id = globalId++;
   ...
   setDebugTrace(trace = false) {
	 ...
	 this.#debugTrace = function() {
	    return(`${this.id} ${this.debugName} <= ${this.value}`);
	 }
      }
   }
   ...
   propagate(knownList, age) {
	 ...
	 if (fanout.net.value !== undefined) {
	    // debug see compiler.js
	    if (fanout.net.#debugTrace) {
	       console.error(`[${age}] ${fanout.net.#debugTrace()}`);
	    }
	 }
...
class WireNet extends Net {
   constructor(astNode, debugName, lvl) {
      ...
      this.debugName = astNode.machine.verbose > 0 ? `${debugName} [${astNode.ctor}]` : undefined;
      this.lvl = lvl;
