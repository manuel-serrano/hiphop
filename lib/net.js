...
function ActionNet(ast_node, type, debug_name, lvl, func, accessor_list) {
   ...
   /* Extends SignalAccessor by adding attribute array"runtime" indexed by
      incarnation level and containing:
      - EITHER a gate (present test);
      - OR a signal object (value access)

      Incarnation level is meaningless for prePresence and (pre)Value,
      but we have at least an uniform way to access to signal state/values
      at runtime.

      The following code also add dependency to the accessor if needed */

   for (let i in this.accessor_list) {
      let acc = this.accessor_list[i]
      let gate = null;
	 ...
	 gate = acc.get_pre ? sobj.pre_gate : sobj.dependency_gate;
      } else {
	 if (acc.get_pre)
...
ActionNet.prototype.apply_func = function() {
   ...
   if (this.func) {
      ret = this.func.call(signal.generate_this(this.accessor_list,
						this.lvl));
   } else {
      /* No function is provided to the ActionNet, that means that it
	 try to access to a signal (pre)presence. So accessor_list
	 contains only one element, which is the (pre) gate */
      ret = this.accessor_list[0].runtime[this.lvl].value;
   }
