...
Net.prototype._removeFanin = function(fanin) {
   let idx = this.fanin_list.indexOf(fanin);
   if (idx >= 0) {
      this.fanin_list.splice(idx, 1);
      fanin.net.removeFanout(fanin.antagonist);
   }
}

Net.prototype._removeFanout = function(fanout) {
   let idx = this.fanout_list.indexOf(fanout);
   if (idx >= 0) {
      this.fanout_list.splice(idx, 1);
      fanout.net.removeFanin(fanout.antagonist);
   }
}

Net.prototype.removeFan = function(fan) {
   if (this.fanin_list.indexOf(fan) > -1) {
      this._removeFanin(fan);
   } else if (this.fanout_list.indexOf(fan) > -1) {
      this._removeFanout(fan);
   } else {
      throw new error.InternalError("Can't remove fan", undefined);
   }
}
