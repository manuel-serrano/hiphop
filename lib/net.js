...
Net.prototype.power = function() {
   // console.log("power", this.constructor.name, this.state, this.ast_node.name,
   // 	       this.ast_node.loc);
   this.action();
...
Emit.prototype.action = function() {
   ...
   if (this.callee || this.args)
      this.signal.set(get_runtime_value(this));
   else
      this.signal.set();
}
...
function get_runtime_value(stmt) {
   ...
   if (stmt.func)
      value = func.apply(null, runtime_args);
   else
...
function get_runtime_args(stmt) {
   ...
   for (var i in stmt.args) {
	 ...
	 let accessor = stmt.args[i].accessor;
	 let sig = stmt.sigal_accessors[i_sig++].signal;
