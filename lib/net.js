...
/*    Last change :  Tue Feb 11 05:26:32 2025 (serrano)                */
...
const DEBUG_ACTION = process.env.HIPHOPTRACE
   && process.env.HIPHOPTRACE.split(",").find(n => n === "action");
...
class Net {
   ...
   constructor(astNode, debugName, lvl) {
	 ...
	 throw new Error(`bad "debugName" (${typeof debugName}) for Net ${this.constructor.name} from ast node ${astNode.constructor.name}`);
      if (typeof lvl !== "number")
	 throw new Error(`bad "lvl" (${typeof lvl}) for Net ${this.constructor.name} from ast node ${astNode.constructor.name}`);
      
      astNode.machine.nets.push(this);
      astNode.net_list.push(this);

      this.debugName = astNode.machine.verbose > 0 ? `${debugName} [${astNode.constructor.name}/${this.constructor.name}]:${lvl}/${astNode.depth}` : undefined;
      this.id = globalId++;
   ...
   dump() {
	 ...
	 fanin: this.faninList
	    .map(fan => { return { id: fan.net.id, polarity: fan.polarity, dep: fan.dependency }}),
	 $name: this.debugName,
	 $ast: this.astNode.constructor.name,
   ...
   propagate(knownList, age) {
      const trace = this.astNode.machine.tracePropagation || DEBUG;
      
      if (trace) {
	 console.error(`[${age}] propagate ${this.id} v=${this.value} (${this.debugName})`);
      }
   ...
   connectTo(net, connType = FAN.STD) {
	 ...
	 if (net.faninList.length > 1) {
	    throw new error.TypeError("too many fanin for wite",
				      net.astNode.loc);
	 }
      }
...
class WireNet extends Net {
   constructor(astNode, debugName) {
      super(astNode, debugName, -1);
      this.debugName = astNode.machine.verbose > 0 ? `${debugName} [${astNode.constructor.name}/${this.constructor.name}]` : undefined;
   ...
   propagate(knownList, age) {
	 ...
	 console.error(`[${age}] propagate ${this.id} v=${this.value} (${this.debugName})`);
      ...
      // Mostly used for debugging circuit construction. This method is
      // not used in normal execution as wire nets are _all_ removed
      // by the sweep optimization
      if (this.faninList.length === 0) {
	 this.fanoutList.forEach(fanout => {
	    if (trace) fanout.net.receiveTraceEnter(false);
	    if (fanout.net.receiveNoValue()) {
	       fanout.net.isInKnownList = true;
	       knownList.push(fanout.net);
	    }
	    if (trace) fanout.net.receiveTraceExit();
...
class SignalNet extends LogicalNet {
   ...
   toJSDecl() {
	 ...
	 return `${this.toJSVar()} = signals["${this.signal.name}"]`;
...
class ActionNet extends LogicalNet {
   ...
   apply_func() {
	 ...
	 if (this.astNode.machine.tracePropagation || DEBUG || DEBUG_ACTION) {
	    console.error(`[${this.astNode.machine.age()}] exec ${this.id} ${this.debugName}`);
	 }
   ...
   toJS() {
	 ...
	 .map(ax => `fillThisArg(${this.toJSArg()}[${l}], "${ax.signame}", ${ax.signal.gate_list[0].toJSVar()}, ${ax.signal.pre_gate ? ax.signal.pre_gate.toJSVar() : undefined})`);
...
class SignalExpressionNet extends ActionNet {
   ...
   constructor(astNode, signal, debugName, lvl, func, accessor_list) {
      ...
      this.signal = signal;
      this.signal_depth = this.signal.astNode.depth;
   ...
   toJS() {
      ...
      const axs = this.accessor_list
	 .map(ax => `fillThisArg(${this.toJSArg()}[${l}], "${ax.signame}", ${ax.signal.gate_list[0].toJSVar()}, ${ax.signal.pre_gate ? ax.signal.pre_gate.toJSVar() : undefined})`);
      return `${d}; if (${this.toJSVar()}) { ${axs.join("; ")}; this.global_signal_map["${this.signal.name}"].set_value(${this.toJSAction()}.call(${this.toJSArg()}[${l}]), ${this.min_lvl}, ${JSON.stringify(this.astNode.loc)}); }\n`;
   }
}
