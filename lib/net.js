...
/*    Last change :  Fri Oct  5 07:55:44 2018 (serrano)                */
...
class ActionNet extends LogicalNet {
   constructor( ast_node, type, debug_name, lvl, func, accessor_list ) {
      ...
      // the "this of user actions
      for( let i = 0; i <= lvl; i++ ) {
	 this.actionArgs[ i ] = 
	    signal.makeActionArg( ast_node.machine, accessor_list );
      }
   }
   ...
   apply_func() {
	 ...
	 signal.fillActionArg( functhis, this.accessor_list, this.lvl );
	 ret = this.func.call( functhis );
      } else {
	 //
