...
/*    Last change :  Fri Dec 19 10:21:09 2025 (serrano)                */
...
class ActionNet extends LogicalNet {
   ...
   toJSprepareThisArg(ax) {
      const lvl = this.lvl;
      const sig = ax.signal;
      const now = sig.gate_list[Math.min(lvl, sig.astNode.depth)].toJSVar();
      const pre = lvl >= sig.depth ? sig.pre_gate.toJSVar?.() : false;
