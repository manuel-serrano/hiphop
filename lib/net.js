...
/*    Last change :  Tue Jan 28 09:13:45 2025 (serrano)                */
...
class ActionNet extends LogicalNet {
   ...
   toJS() {
      const d = super.toJS();
      return d + (this.neutral ? " && " : " || ")
	 + `(${this.func.toString()}(), true)`;
   }
}
...
class SignalExpressionNet extends ActionNet {
   ...
   toJS() {
	 ...
	 .filter(f => !f.dependency)
	 .map(f => (f.polarity === false ? "!" : "") + f.net.toJSVar());
      return `${this.toJSVar()} = (${deps.join(this.neutral ? " && " : " || ")} && (this.global_signal_map["${this.signal.name}"].set_value((${this.func.toString()})(), ${this.min_lvl}, ${JSON.stringify(this.ast_node.loc)})), true)`;
   }
}
