...
/*    Last change :  Wed Jun  5 10:03:00 2019 (serrano)                */
...
class Net {
   constructor( ast_node, type, debug_name, lvl ) {
      ...
      this.debug_name = ast_node.loc.filename ? `[${debug_id}]:${ast_node.constructor.name}[${path.relative(process.cwd(), ast_node.loc.filename)}:${ast_node.loc.pos}](${this.constructor.name})_${debug_name}:${lvl}/${ast_node.depth}`
...
class LogicalNet extends Net {
   ...
   receiveTraceEnter() {
      console.error( "   >>> receive " + this.debug_id + " " +
		     ...
		     this.value );
      // GB too heavy... console.error( "      value-before:" + this.value );
   }
