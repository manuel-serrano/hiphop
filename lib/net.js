...
/*    Last change :  Fri Feb  7 09:34:04 2025 (serrano)                */
...
export { resetNetId };
export { FAN };
export { WireNet, RegisterNet, LogicalNet, SignalNet, ActionNet };
...
class WireNet extends Net {
   constructor(ast_node, type, debug_name) {
      super(ast_node, type, debug_name, -1);
      this.debug_name = ast_node.machine.verbose > 0 ? `${debug_name} [${ast_node.constructor.name}/${this.constructor.name}]` : undefined;
   }
...
class SignalNet extends LogicalNet {
   ...
   toJSDecl() {
	 ...
	 return `${this.toJSVar()} = signals["${this.signal.name}"]`;
      } else if (this.fanin_list.length === 0) {
...
class SignalExpressionNet extends ActionNet {
   ...
   toJS() {
      ...
      const axs = this.accessor_list
	 .map(ax => `fillThisArg(${this.toJSArg()}[${l}], "${ax.signame}", ${ax.signal.gate_list[0].toJSVar()}, ${ax.signal.pre_gate ? ax.signal.pre_gate.toJSVar() : undefined})`);
      return `${d}; if (${this.toJSVar()}) { ${axs.join("; ")}; this.global_signal_map["${this.signal.name}"].set_value(${this.toJSAction()}.call(${this.toJSArg()}[${l}]), ${this.min_lvl}, ${JSON.stringify(this.ast_node.loc)}); }\n`;
   }
}
