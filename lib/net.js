...
/*    Last change :  Tue Sep 11 08:57:23 2018 (serrano)                */
...
class Net {
   constructor( ast_node, type, debug_name, lvl ) {
      ...
      this.debug_name =
	 `${ast_node.constructor.name}(${this.constructor.name})_${debug_name}:${lvl}/${ast_node.depth}`;
...
function make_or( ast_node, type, debug_name, lvl=0 ) {
  return new LogicalNet( ast_node, type, debug_name, lvl, false );
}

function make_and( ast_node, type, debug_name, lvl=0 ) {
...
class ActionNet extends LogicalNet {
   ...
   apply_func() {
	 ...
	 if( acc.get_pre ) {
	    ...
	    ret = sig.gate_list[ min_lvl ].value;
	 }
      }
...
class SignalExpressionNet extends ActionNet {
   constructor( ast_node, type, signal, debug_name, lvl ) {
      ...
      this.connect_to( signal.dependency_gate_list[ min_lvl ], FAN.DEP );
   }
