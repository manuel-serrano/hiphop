...
ActionNet.prototype.get_runtime_args = function() {
   function get_value(el) {
	 ...
	 return el.pre ? el.signal.pre_value : el.signal.value;
      } else {
   ...
   for (let i in this.args)
      runtime_args.push(get_value(this.args[i]));
...
function TestExpressionNet(ast_node, debug_name, func, args_list, lvl) {
   ...
   /* call super before translate args list because connect_to function needs
      to use properties of this that are initialized in super constructors */
   ActionNet.call(this, ast_node, debug_name, func, new_args_list);
   ...
   for (let i in args_list) {
	 ...
	 if (arg.get_value) {
	    let sobj = get_signal_object(ast_node.machine, arg.signal_name);
