...
/*    Last change :  Tue Dec 16 08:26:04 2025 (serrano)                */
...
class TestExpressionNet extends ActionNet {
   ...
   toJS() {
      ...
      const act = deleteDuplicates(this.accessor_list)
	 .map(ax => `prepareThisArg(${this.toJSArg()}[${l}], "${ax.signame}", ${ax.signal.id}, ${ax.signal.gate_list[Math.min(l, ax.signal.astNode.depth)].toJSVar()}, ${ax.signal.pre_gate.toJSVar?.()})`)
