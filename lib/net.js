...
/*    Last change :  Sat Jun 14 08:12:41 2025 (serrano)                */
...
class Net {
   ...
   dump() {
      const $ast = { ctor: this.ctor || this.astNode.ctor, loc: this.astNode.loc, id: this.astNode.key };
      const parent = this.getNonSweepedParent();
      const $parent = parent
	 ? { ctor: parent.ctor, loc: parent.loc, id: parent.key }
	 : undefined;
...
class ActionNet extends LogicalNet {
   ...
   apply_func() {
	 ...
	 if (this.astNode.machine.tracePropagation || DEBUG_PROPAGATE || DEBUG_ACTION) {
	    console.error(`   exec ${this.id} ${this.debugName} -> ${res}`);
	 }
	 ...
	 if (tmp.get_pre) {
	    return tmp.signal.pre_gate.value;
	 } else {
	    let sig = tmp.signal;
	    let lvl = this.lvl;
   ...
   toJS() {
	 ...
	 .map(ax => `prepareThisArg(${this.toJSArg()}[${l}], "${ax.signame}", ${ax.signal.id}, ${ax.signal.gate_list[Math.min(l, ax.signal.astNode.depth)].toJSVar()}, ${ax.signal.pre_gate ? ax.signal.pre_gate.toJSVar() : undefined})`)
   ...
   toJSCall() {
	 ...
	 if (tmp.get_pre) {
	    return tmp.signal.pre_gate.toJSVar();
	 } else {
	    let sig = tmp.signal;
	    let lvl = this.lvl;
	    let min_lvl = lvl > sig.astNode.depth ? sig.astNode.depth : lvl;
...
class SignalExpressionNet extends ActionNet {
   ...
   dup() {
      return new SignalExpressionNet(this.astNode, this.signal, this.debugName,
...
function makeAnd(astNode, debugName, lvl = 0, ctor = false) {
   const net = new LogicalNet(astNode, debugName, lvl, true, ctor);
   net.ctor = ctor;
   return net;
}
