...
/*    Last change :  Thu Sep 17 10:54:35 2026 (serrano)                */
...
class Net {
   ...
   ctor = false;
   maybeBottom = false;
   origin; // used only for the cycle removal
   #dependencyCount;
   #trueFaninCount;
   #len;
   #debugTrace;
   ...
   constructor(astNode, debugName, lvl) {
      if (astNode && astNode.machine.verbose > 0 && typeof debugName !== "string") {
   ...
   toJSON() {
	 ...
	 : undefined;
      ...
      const $value = $propagated ? this.value : undefined;
   ...
   connectTo(net, connType) {
	 ...
	 throw new error.TypeError("illegal fan type", this.astNode.loc);
      }
   ...
   toJS() {
      throw new TypeError(`toJS not implemented for net ${this.constructor.name}`);
   }
}
...
class RegisterNet extends Net {
   ...
   constructor(astNode, debugName, lvl) {
	 ...
	 this.stable_id = RegisterNet.next_register_id++;
      }
...
class SignalNet extends LogicalNet {
   ...
   constructor(astNode, signal, signame, accessibility, lvl, neutral) {
      ...
      //console.log("SIGNAME=", signame, "N=", neutral);
   }

   reset(reset_machine) {
      super.reset(reset_machine);
      this.value = this.neutral;
      //console.log("RESET ", this.signame, this.neutral, this.value);
   }
   
   dup() {
      const net = new SignalNet(this.astNode, this.signal, this.signame, this.accessibility, this.lvl, this.neutral);
      net.debugName += "'";
      return net;
   }
...
class ActionNet extends LogicalNet {
   ...
   apply_func() {
	 ...
	 if (this.astNode.machine.tracePropagation || DEBUG_PROPAGATE || DEBUG_ACTION) {
	    console.error(`   ## action ${this.id} ${this.debugName}/${this.lvl} -> ${res}`);
	 }
	 ...
	 if (tmp.get_pre) {
	    return sig.preSigNet.value;
	 } else {
	    const lvl = this.lvl;
	    const min_lvl = lvl > sig.astNode.depth ? sig.astNode.depth : lvl;
	    
	    return sig.netSigList[min_lvl].value;
	 }
      }
   }
   ...
   toJSprepareThisArg(ax) {
      const lvl = this.lvl;
      const sig = ax.signal;
      const now = sig.netSigList[Math.min(lvl, sig.depth)].toJSVar();
      const pre = lvl >= sig.depth ? sig.preSigNet.toJSVar?.() : false;
      return `prepareThisArg(${this.toJSArg()}[${lvl}], "${ax.signame}", ${sig.id}, ${now}, ${pre})`;
   }
   ...
   toJS() {
	 ...
	 .map(ax => this.toJSprepareThisArg(ax))
   ...
   toJSCall() {
	 ...
	 if (tmp.get_pre) {
	    return sig.preSigNet.toJSVar();
	 } else {
	    const lvl = this.lvl;
	    const min_lvl = lvl > sig.astNode.depth ? sig.astNode.depth : lvl;
	    
	    return sig.netSigList[min_lvl].toJSVar();
	 }
      }
   }
}
...
class SignalExpressionNet extends ActionNet {
   ...
   constructor(astNode, signal, debugName, lvl, func, accessor_list) {
      ...
      this.connectTo(signal.dependency_netSigList[this.min_lvl], FAN.DEP);
   }
...
/*    fromJSON ...                                                     */
/*---------------------------------------------------------------------*/
function fromJSON(preNets) {
   const nets = new Map();
   
   function jsonToNet(preNet) {
      let net;

      switch (preNet.type) {
	 case "REG":
	    net = new RegisterNet(null, preNet.$name, preNet.lvl);
	    break;

	 case "FALSE":
	    net = new LogicalNet(null, preNet.$name, preNet.lvl, false);
	    break;
	    
	 case "TRUE":
	    net = new LogicalNet(null, preNet.$name, preNet.lvl, true);
	    break;
	    
	 case "SIG":
	    net = new SignalNet(null, preNet.signame, preNet.signame, preNet.accessibility, preNet.lvl, preNet.neutral);
	    break;
	 ...
	 default: {
	    throw new Error(`jsonToNet: net not implemented ${preNet.type}`);
	 }
      }
   ...
   // create the nets
   preNets.forEach(jsonToNet);

   // connect the nets
   preNets.forEach(pn => {
      const net = pn.net;
      pn.fanout.forEach(fan => {
	 const tgt = nets.get(fan.id);

	 if (!tgt) {
	    throw new Error(`Cannot find net: ${fan.id}`);
	 }
	 const ct = fan.dep ? FAN.DEP : (fan.polarity ? FAN.STD : FAN.NEG);
	 
	 net.connectTo(tgt, ct);
      });
   });

   return preNets.map(pn => pn.net);
}
