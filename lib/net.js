...
function Hamlet(ast_node) {
   Net.call(this, ast_node);
}

Hamlet.prototype = new Net(undefined);
Hamlet.prototype.constructor = Hamlet;

exports.Hamlet = Hamlet;

Hamlet.prototype.action = function() {
   if (!this.state)
      this.state = true;
}
...
function get_runtime_args(stmt) {
   ...
   for (var i in stmt.args) {
      if (stmt.args[i] instanceof signal.SignalAccessor) {
