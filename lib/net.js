...
/*    Last change :  Fri Feb 14 07:20:43 2025 (serrano)                */
...
/*    DEBUG                                                            */
/*---------------------------------------------------------------------*/
const DEBUG_PROPAGATE = process.env.HIPHOPTRACE
...
class Net {
   ...
   constructor(astNode, debugName, lvl) {
	 ...
	 throw new Error(`bad "lvl" (${typeof lvl}) for Net ${this.constructor.name} from ast node ${astNode.constructor.name}`);
      ...
      this.debugName = astNode.machine.verbose > 0 ? `${debugName} [${astNode.constructor.name}:${lvl}/${astNode.depth}]` : undefined;
      this.id = globalId++;
...
class WireNet extends Net {
   constructor(astNode, debugName) {
      ...
      this.debugName = astNode.machine.verbose > 0 ? `${debugName} [${astNode.constructor.name}]` : undefined;
