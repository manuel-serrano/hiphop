...
function connect(net1, net2, neg) {
   net1.fanout.push(new Plug(net2, neg));
   net2.fanin.push(new Plug(net1, false));
}
...
Net.prototype.reset = function(reset_machine) {
   ...
   console.log("waiting", this.ast.name, this.waiting);
...
Net.prototype.power = function() {
   this.action();
   console.log("power on", this.ast.name, this.ast.loc);
