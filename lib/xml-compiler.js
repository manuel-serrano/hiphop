...
function SUSTAIN(attrs) {
   check_signal_name(attrs.signal_name, attrs);
   return new ast.Sustain(format_loc(attrs),
			  attrs.signal_name,
			  attrs.func,
			  parse_exprs(attrs));
}
...
function ABORT(attrs) {
   ...
   return new ast.Abort(format_loc(attrs),
			...
			attrs.immediate != undefined,
			typeof(count) == "number" ? count : 0,
			implicit_sequence(attrs, arguments));
}
...
function LOOP(attrs) {
   return new ast.Loop(format_loc(attrs), implicit_sequence(attrs, arguments));
}
...
function EVERY(attrs) {
   ...
   check_signal_name(signame, attrs);
   return new ast.Every(format_loc(attrs),
			implicit_sequence(attrs, arguments),
			signame,
			count,
			attrs.immediate != undefined)
...
function TRAP(attrs) {
   ...
   return new ast.Trap(format_loc(attrs),
		       attrs.trap_name,
		       implicit_sequence(attrs, arguments));
}
...
function implicit_sequence(attrs, args) {
   var children = get_children(args);
