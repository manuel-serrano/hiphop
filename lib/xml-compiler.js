...
function EMIT(attrs) {
   ...
   return new ast.Emit(loc, attrs.signal_name, func, exprs);
}
...
function ATOM(attrs) {
   ...
   if (!(func instanceof Function))
      reactive.fatal_error("Atom must have a func attribute which is a function.",
	    format_loc(attrs));
...
function RUN(attrs) {
   ...
   for (var i in run_machine.input_signals)
	 ...
	 reactive.fatal_error("Signal " + i + " not mapped.", format_loc(attrs));
