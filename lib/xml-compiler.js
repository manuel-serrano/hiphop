...
function check_attributes(attrs, mandatory, optional) {
   ...
   for (var name in attrs) {
      if (name == "id" && mandatory.indexOf("!id") > -1)
	 reactive.fatal_error("Can't set `id` field here.", format_loc(attrs));
      else if (name == "%location" || name == "id")
	 ;
      else if (mandatory.indexOf(name) > -1)
	 ;
      else if (optional.indexOf(name) > -1)
	 ;
      else
...
function ATOM(attrs) {
   attrs = check_attributes(attrs, ["func"], undefined);
   var func = attrs.func;
...
function check_signal_type(attrs) {
   if (attrs.type && typeof(attrs.type) != "string")
      reactive.fatal_error("Attribute `type` must by typed by a string, not "
			   + typeof(attrs.type) + ".", format_loc(attrs));
}
