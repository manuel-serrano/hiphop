...
/*    Last change :  Thu Nov 20 11:25:03 2025 (serrano)                */
...
function reportCausalityError(mach) {
   ...
   if (size > 0) {
	 ...
	 console.error("unpropagated nets", mach.nets.filter(n => !n.isInKnownList).map(n => n.id));
      ...
      if (signals.length === 0) {
	 throw error.TypeError("Causality error.", mach.nets.find(n => !n.isInKnownList).astNode.loc);
      }
   ...
   cycles.forEach(src => {
	 ...
	 if (fs.existsSync(filename)) {
	    ...
	    try {
		  ...
		  locIndex = locations.findIndex(l => l > end);
