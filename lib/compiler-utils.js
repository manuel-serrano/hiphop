...
/*    Last change :  Sun Sep 30 16:41:54 2018 (serrano)                */
...
SignalVisitor.prototype.visit = function( ast_node ) {
   ...
   if( ast_node instanceof ast.Module ) {
	 ...
	 this.global_names.push( prop.name );
      ...
      for( let i = ast_node.sigDeclList.length - 1; i >= 0; i-- ) {
	 let sigprop = ast_node.sigDeclList[ i ];
	 ...
	 if( sigprop.alias ) {
	    ...
	    if( sigprop.alias == -1 ) {
		  ...
		  sigprop.alias = sigprop.name;
	       ...
	       throw error.SyntaxError( "Signal `" + sigprop.name + "' is " +
