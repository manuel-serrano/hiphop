...
/*    Last change :  Thu May 21 14:08:12 2026 (serrano)                */
...
function uncycleNets(machine, nets) {
   ...
   getBottomNet(machine).connectTo(src, FAN.STD);
...
function uncycle(machine) {
   ...
   if (userCycle) {
      ...
      let cycle = findCycle(machine.nets);
      
      while (cycle) {
	 uncycleNets(machine, cycle);
	 cycle = findCycle(machine.nets);
	 if (cycle) {
	    console.error("C=", cycle.length, cycle.map(net => net.id));
	    machine.nets.forEach(net => net.inCycle = false);
	    cycle.forEach(net => net.inCycle = true);
	    if (machine.dumpNets) {
	       machine.dumpNets(machine, true, ".nets~.json");
	    }
	 }
