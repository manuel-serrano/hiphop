/*=====================================================================*/
/*    serrano/prgm/project/hiphop/hiphop/lib/comp-new.js               */
/*    -------------------------------------------------------------    */
/*    Author      :  Colin Vidal, Manuel Serrano                       */
/*    Creation    :  Mon Jul 16 18:06:28 2018                          */
/*    Last change :  Thu Mar 12 14:41:37 2026 (serrano)                */
...
import { assert, assertCircuitInterfaceConnections,
	 debugTrace, debugTraceCircuit, debugSynchronizer } from "./comp-common.js";
import { Circuit,
	 makeCircuitInterface, connectTo, connectToList, connMode,
	 bindSigAccessorList,
...
function makeForkCircuit(env, astNode, childCircuits) {
   ...
   // debug
   if (debugSynchronizer) {
      kMatrix.forEach(ns => ns[0].setDebugTrace());
   }
   ...
   childCircuits.forEach((c, n, _) => {
      ...
      const forceOldSynchronizer = astNode.machine.synchronizer === "old";

      // MS 12 march 2026 c.astNode.depth === 0 is equivalent to
      // !astNode.machine.reincarnation
      // if (c.astNode.depth === 0) {
      if (!astNode.machine.reincarnation) {
	 // DO NOT CONNECT OR-NOT gate from LEM = !(GO ^ c.SEL) as
	 // shown in Fig 11.10 of the book. 
	 // Instead, use a variant suggested by Gerard:
	 //    LEM = RES ^ SEL ^ !c.SEL
	 // the schema of the book is wrong, it handles incorrectly
	 // programs such as:
	 // loop {
	 //    fork {
	 //       yield;
	 ...
	 //  }
	 //
	 // See the test test/loop-par.hh.js
...
function makeTrapCircuit(env, astNode, childCircuit) {
   ...
   const suspicious_k2_kill_connection = !astNode.machine.reincarnationTrap;
   const experimental = true;
   const superexperimental = true;
   if (suspicious_k2_kill_connection) {
	 ...
	 connectToList(nkill, childCircuit.killList, connMode.CONNECT);
      ...
      // experimental, connect kMatrix[2] only at the depth
      const nkill = Array.from({length: l + 2}, (_, i) =>
         net.makeOr(astNode, "NKILL", i));

      connectToList(killList, nkill, connMode.GO);
      connectToList(childCircuit.kMatrix[2], nkill, connMode.CONNECT);
      connectToList(nkill, childCircuit.killList, connMode.GO);
      console.log("ICI...", l, "t=", astNode.depth, "c=", astNode.children[0].depth);
/*    } else {                                                         */
...
/*---------------------------------------------------------------------*/
function makeLoopCircuit_nodup(env, astNode, childCircuit) {
   // circuits common interface
   const { l, goList, res, susp, killList, kMatrix, sel } =
      makeCircuitInterface(astNode);
...
function makeLoopCircuit(env, astNode, childCircuit) {
   if (astNode.machine.loopDup) {
      ...
      return makeLoopCircuit_nodup(env, astNode, childCircuit);
   }
}
...
function nodeDepth(astNode) {
   ...
   function rec(astNode, depth) {
	 ...
	 || (astNode instanceof ast.Trap && astNode.machine.reincarnationTrap)) {
	 astNode.children.forEach(c => rec(c, depth + 1));
      } else {
	 astNode.children.forEach(c => rec(c, depth));
      }
   }
   return rec(astNode, 0);
}
