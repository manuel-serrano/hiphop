...
recurcively to build strongly connected sub components */
   function bourdoncleTarjan( v ) {
      ...
      function subComponent(vertex){
         let partition = [];
        // console.log(vertex.LogicalNetType());
         /*
         vertex.fanout_list.forEach( f => {
            let w = f.net;
            console.log(w.debug_id,w.LogicalNetType());
            if( w.dfn === 0 ) {
               visit( w, partition );
            }
         } );
   ...
   function findCycleSignalsBeta( src ) {
      let res = [];
   ...
   let concatFlag = false;
      ...
      for(let i=0; i < component.length ; i++) {
         if(!Array.isArray(component[i])){
            tempStack.push(component[i]);
         } else {
            let arrayTemp = readMergeComponents(component[i]);
            if (arrayTemp.length === undefined){
      ...
      if(Array.isArray(components)){
         ...
         for( let j = 0 ; j < tempStack.length ; j++ ) {
            if( Array.isArray(tempStack[ j ])) {
              ...
              let tempSignal = findCycleSignalsBeta( solComponent );
