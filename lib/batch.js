...
function pretty_print_web_debugger() {
   function format_web_debugger(indent, ast_node, sel) {
      let buf = "[" + ast_node.depth + "] ";
      let instruction =  (ast_node.constructor.name +
			  ast_node.pretty_print_param());
...
service debug() {
   var code = <div style="font-family:mono">${pretty_print_web_debugger()}</div>
   return <html>
     <head>
   	     ~{
