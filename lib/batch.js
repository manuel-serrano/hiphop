...
/*    Last change :  Fri Nov 17 11:00:48 2023 (serrano)                */
...
function batch(mach, test = undefined) {
   ...
   if (test) {
	 ...
	 fdin = fs.createReadStream(null, {fd: fs.openSync(input, "r")});
      } else {
   ...
   fdin.on("data", function(buffer) {
	 ...
	 if (test) {
	    ...
	    if (expected !== outbuf) {
	       ...
	       return "expecting [" + expected + "] got [" + outbuf + "]";
	    }
	 } else {
	    return false;
	 }
