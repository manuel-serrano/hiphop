...
/*    Last change :  Sun Feb 16 10:54:57 2025 (serrano)                */
...
/*    checkResult ...                                                  */
/*---------------------------------------------------------------------*/
function checkResult(mach, output) {
   if (output && fs.existsSync(output)) {
      const expected = fs.readFileSync(output).toString();
      const err = output.replace(/[.]out$/, ".err");
	 ...
	 fs.writeFileSync(err, mach.outbuf);
	 return `=== expecting [${expected}] === GOT {${mach.outbuf}}`;
      }
   } else {
      return false;
   }
...
function batch(mach, test = undefined, done = undefined) {
   ...
   if (mach.batchPromise) {
      return mach.batchPromise.then(v => checkResult(mach, output));
   } else {
      return new Promise((res, rej) => res(checkResult(mach, output)));
   }
}
