...
/*    Last change :  Thu Dec 14 07:18:05 2023 (serrano)                */
...
/*    check_result ...                                                 */
/*---------------------------------------------------------------------*/
function check_result(mach, output) {
...
function batch(mach, test = undefined, done = undefined) {
   ...
   if (mach.batchPromise) {
      return mach.batchPromise.then(v => check_result(mach, output));
   } else {
      return new Promise((res, rej) => res(check_result(mach, output)));
   }
}
