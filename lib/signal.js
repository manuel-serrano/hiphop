...
/*    Last change :  Tue Jan 14 13:01:09 2025 (serrano)                */
...
class Signal {
   ...
   constructor(ast_node, prop, type, pre_reg, k1_list = null, kill_list = null, susp = null) {
	 ...
	 this.pre_gate = makeSig(ast_node, type, this.name + "_pre_gate");
	 ...
	 this.pre_reg.connectTo(this.pre_gate, FAN.STD);
      ...
      if (k1_list && kill_list && susp) {
   	 ...
   	 for (let i = 0; i <= ast_node.depth; i++) {
      	    ...
      	    //
      	    this.gate_list[i] = makeSig(ast_node, type, this.name, i);
	 ...
	 if (this.pre_reg) {
   	 ...
   	 //
   	 let signal_gate = makeSig(ast_node, type, this.name);
