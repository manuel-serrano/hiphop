...
/*    Last change :  Tue Jan 28 12:18:46 2025 (serrano)                */
...
class Signal {
   ...
   constructor(ast_node, prop, type, pre_reg, k1_list = null, kill_list = null, susp = null) {
	 ...
	 this.pre_gate = makeSig(ast_node, type, undefined, this.name + "_pre_gate", "LOCAL");
	 ...
	 this.pre_reg.connectTo(this.pre_gate, FAN.STD);
      ...
      if (k1_list && kill_list && susp) {
   	 ...
   	 for (let i = 0; i <= ast_node.depth; i++) {
      	    ...
      	    //
      	    this.gate_list[i] = makeSig(ast_node, type, this, this.name, "LOCAL", i);
...
/* 	       this.name + "_dep", i);                                 */
      	    this.dependency_gate_list[i] = makeAnd(ast_node, type,
	       this.name + "_dep_local", i);
	 ...
	 if (this.pre_reg) {
   	 ...
   	 //
   	 let signal_gate = makeSig(ast_node, type, this, this.name, prop.accessibility);
...
/*    	 this.dependency_gate_list[0] = makeOr(ast_node, type, this.name + "_dep"); */
   	 this.dependency_gate_list[0] = makeAnd(ast_node, type, this.name + "_dep_global");
   ...
   set_value(value, lvl, loc=false) {
      // console.error ("Emitting", this.name, " value=" + value, " lvl=" + lvl, " emitted=" + this.emitted[lvl], typeof this.gate_list[0]); // GB
