...
function Signal(ast_node, type, k1_list, kill_list, susp) {
   ...
   this.gate_list = [];
   this.pre_gate = new net.RegisterNet(ast_node, type, this.name + "_pre_gate");
   this.set_by_host = false;
   let or_to_pre = net.make_or(ast_node, type, this.name + "_or_to_pre");
   let pre_and_susp = net.make_and(ast_node, type, this.name + "_pre_and_susp");
   let to_pre_p = [];
   ...
   for (let i = 0; i <= ast_node.depth; i++) {
      ...
      if (k1_list && k1_list[i])
	 k1_list[i].connect_to(to_pre_p[i], net.FAN.STD);
      if (kill_list && kill_list[i])
   ...
   /* pre AND susp */
   this.pre_gate.connect_to(pre_and_susp, net.FAN.STD);
   if (susp)
   ...
   or_to_pre.connect_to(this.pre_gate, net.FAN.STD);
}
