...
/*    fillActionArg ...                                                */
/*    -------------------------------------------------------------    */
/*    Generate the object passed to user actions.                      */
/*---------------------------------------------------------------------*/
function fillActionArg( argobj, accessor_list, lvl ) {
   accessor_list.forEach( acc => {
      let sig = acc.signal;
      let min_lvl = lvl > sig.ast_node.depth ? sig.ast_node.depth : lvl;
      
      argobj.preValue[ acc.signame ] = acc.signal.pre_value;
      argobj.value[ acc.signame ] = acc.signal.value;
      argobj.prePresent[ acc.signame ] = acc.signal.pre_gate.value;
      argobj.present[ acc.signame ] = sig.gate_list[ min_lvl ].value;
   } );
...
/*    makeActionArg ...                                                */
/*---------------------------------------------------------------------*/
function makeActionArg( machine, accessor_list ) {
   const argobj = {
      machine: machine,
      value: {},
      preValue: {},
   ...
   accessor_list.forEach( acc => {
      argobj.preValue[ acc.signame ] = undefined;
      argobj.value[ acc.signame ] = undefined;
      argobj.prePresent[ acc.signame ] = undefined;
      argobj.present[ acc.signame ] = undefined;
   } );
...
exports.fillActionArg = fillActionArg;
exports.makeActionArg = makeActionArg;
