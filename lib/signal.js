...
/*    Last change :  Thu Dec 18 13:17:46 2025 (serrano)                */
...
class Signal {
   ...
   constructor(astNode, prop, pre, k1_list = null, kill_list = null, susp = null, depth) {
	 ...
	 throw new Error("Signal: illegal call k1_list: " + k1_list);
      ...
      if (pre) {
	 this.pre_reg = new RegisterNet(astNode, prop.name + "_pre", depth);
	 this.pre_gate = makeSig(astNode, undefined, this.name + "_pre", "LOCAL", depth);
	 //
	 // The pre_gate is not the register but a door to which the register
	 ...
	 if (this.pre_reg) {
   	    for (let i = 0; i <= depth; i++) {
      	       ...
      	       this.gate_list[i].connectTo(to_pre[i], FAN.STD);

	       const j = i === k1_list.length ? i - 1 : i;
      	       k1_list[j].connectTo(to_pre[i], FAN.STD);
      	       kill_list[j].connectTo(to_pre[i], FAN.NEG);
		  ...
		  to_pre[i].connectTo(and, FAN.STD);
...
class ActionArg {
   ...
   prepare(accessor_list, lvl) {
	 ...
	 const siglvl = lvl > sig.depth ? sig.astNode.depth : lvl;
         ...
         this[signame].now = sig.gate_list[siglvl].value;
         this[signame].pre = lvl === sig.depth ? sig.pre_gate?.value : undefined;
      });
   }
}
...
function runtimeSignalAccessor(astNode, accessor_list, lvl, dep_target_gate, dep = FAN.DEP) {
   accessor_list.forEach(ax => {
	 ...
	 gate = (ax.get_pre
	    ? sig.pre_gate
	    : sig.dependency_gate_list[siglvl]);
      } else {
	 gate = (ax.get_pre 
	    ? sig.pre_gate
	    : sig.gate_list[siglvl]);
      }
	 ...
	 if (gate.lvl <= dep_target_gate.lvl || !ax.get_pre) {
	    ...
	    console.log("runtimeSignalAccessor lvl=", lvl, "gate.lvl=", gate.lvl, gate.id, "dep_target_gate=", dep_target_gate.lvl, dep_target_gate.id); 
