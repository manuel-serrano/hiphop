...
/*    Last change :  Thu Jan 16 08:55:17 2025 (serrano)                */
...
class Signal {
   ...
   constructor(ast_node, prop, type, pre_reg, k1_list = null, kill_list = null, susp = null) {
	 ...
	 this.pre_gate = makeSig(ast_node, type, this.name + "_pre_gate", "LOCAL");
	 ...
	 this.pre_reg.connectTo(this.pre_gate, FAN.STD);
      ...
      if (k1_list && kill_list && susp) {
   	 ...
   	 for (let i = 0; i <= ast_node.depth; i++) {
      	    ...
      	    //
      	    this.gate_list[i] = makeSig(ast_node, type, this.name, "LOCAL", i);
...
/* 	       this.name + "_dep", i);                                 */
	 ...
	 if (this.pre_reg) {
   	 ...
   	 //
	 const accessibility = (prop.accessibility === IN) ? "IN" : ((prop.accessibility === OUT) ? "OUT" : "INOUT");
   	 let signal_gate = makeSig(ast_node, type, this.name, accessibility);
...
/*    	 this.dependency_gate_list[0] = makeOr(ast_node, type, this.name + "_dep"); */
   	 this.dependency_gate_list[0] = makeAnd(ast_node, type, this.name + "_dep");
