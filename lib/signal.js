...
/*    Last change :  Mon Dec 15 06:36:57 2025 (serrano)                */
...
class Signal {
   ...
   constructor(astNode, prop, pre, k1_list = null, kill_list = null, susp = null, depth) {
	 ...
	 throw new Error("Signal: illegal call k1_list: " + k1_list);
      ...
      this.pre_gate_list = [];
      this.dependency_pre_gate_list = [];

      if (pre) {
	 this.pre_reg = new RegisterNet(astNode, prop.name + "_pre", 0);
	 this.pre_gate_list = [];

   	 for (let i = 0; i <= depth; i++) {
	    this.pre_gate_list[i] =
	       makeSig(astNode, undefined, this.name + "_pre", "LOCAL", i);
	    ...
	    // previous instant.
	    //
	    this.pre_reg.connectTo(this.pre_gate_list[i], FAN.STD);
	 }
      }
	 ...
	 if (this.pre_reg) {
	       ...
	       const j = i === k1_list.length ? i - 1 : i;
      	       k1_list[j].connectTo(to_pre_p[i], FAN.STD);
      	       kill_list[j].connectTo(to_pre_p[i], FAN.NEG);
...
class ActionArg {
   ...
   prepare(accessor_list, lvl) {
	 ...
	 const min_lvl = lvl > sig.depth ? sig.astNode.depth : lvl;
         ...
         this[signame].pre = sig.pre_gate_list[min_lvl]?.value;
      });
   }
}
...
function runtimeSignalAccessor(astNode, accessor_list, lvl, dep_target_gate, dep = FAN.DEP) {
   accessor_list.forEach(ax => {
      ...
      let min_lvl = lvl > sigdepth ? sigdepth : lvl;

      if (ax.get_value) {
	 gate = (ax.get_pre
	    ? sig.pre_gate_list[min_lvl]
