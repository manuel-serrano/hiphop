...
function Signal(ast_node, type, k1_list, kill_list, susp) {
   ...
   /* The pre_gate is not the register but a door which the register is
      connected to. That ensure that pre_reg hold the value of previous
      instant */
...
function ValuedSignal(ast_node, type, k1_list, kill_list, susp) {
   ...
   this.combine_func = ast_node.combine_func;
   this.init_value = ast_node.init_value;
   this.reinit_func = ast_node.reinit_func; /* "reset" keyword in language */
   this.initialized;
...
ValuedSignal.prototype.set_value = function(value) {
   ...
   if (this.emitted_in_react || (this.reinit_func && this.initialized)) {
	 ...
	 value = this.combine_func.call(null, this.value, value);
   }
   this.value = value;
...
exports.generate_this = function(accessor_list, lvl) {
   ...
   /* Local signal are internally renamed.
      This function put the original name as signal name. This is avoid
      to modifiy user expression with internal name */
   function add_value(acc, kind, prop) {
      let name = (acc.original_signal_name ?
		  acc.original_signal_name :
		  acc.signal_name)
      self[kind][name] = acc.runtime[lvl][prop]
   }

   for (let i in accessor_list) {
      let acc = accessor_list[i];

      if (acc.get_value) { /* accessor of value */
	 if (acc.get_pre)
	    add_value(acc, "preValue", "pre_value");
	 else
	    add_value(acc, "value", "value");
      } else { /* accessor of (pre)presence */
	 if (acc.get_pre)
	    add_value(acc, "prePresent", "value");
	 else
	    add_value(acc, "present", "value");
      }
   }
