...
function Signal(ast_node) {
   ...
   this.gate_list = [];
   this.pre_gate = net.make_or(ast_node, this.name + "_pre");
   this.set_by_host = false;

   for (let i = 0; i <= ast_node.depth; i++)
      this.gate_list[i] = net.make_or(ast_node, this.name + "___" + i);
}
...
function ValuedSignal(ast_node) {
   ...
   this.dependency_gate = net.make_or(ast_node, this.name + "_dep");
