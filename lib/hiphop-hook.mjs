...
/*    Last change :  Thu Nov 23 13:34:54 2023 (serrano)                */
...
import { compile as hiphop, $ioParseError } from "./hhc-compiler.mjs";
...
async function compile(url) {
   ...
   if (await newer(target, path)) {
	 ...
	 if (err.obj.loc?.filename) {
	    $ioParseError({
	       "fname": err.obj.loc?.filename,
	       "location": err.obj.loc?.offset,
	       "message": err.msg + "\n"
	    });
	 }
	 throw "syntax error.";
