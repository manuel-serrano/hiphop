...
/*    Last change :  Fri Nov 17 10:28:18 2023 (serrano)                */
...
export async function resolve(url, context, nextResolve) {
   if (extensionsRegex.test(url)) {
      ...
      const transformedSource = await compile(rawSource, url);
...
/*    compile ...                                                      */
/*---------------------------------------------------------------------*/
async function compile(src, url) {
   const source = fileURLToPath(src);
   const target = dirname(source) + "/._" + basename(source).replace(extensionsRegex, ".mjs");

   if (await newer(target, source)) {
      return target;
   } else {
      try {
	 await writeFile(target, hiphop(source));
	 return target;
      } catch (err) {
	 console.error(err);
      }
