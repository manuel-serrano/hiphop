...
rk.ReactiveMachine.prototype.pretty_print = function(indent) {
   var buf = indent_cursor(indent)
       + "reactivemachine " + this.machine_name + " {\n";
   var sig;

   for (var i in this.input_signals) {
      buf += indent_cursor(indent + INDENT_LVL) +"input ";
      buf += signal_init(this.input_signals[i]) + ";\n";
   }

   for (var i in this.output_signals) {
      buf += indent_cursor(indent + INDENT_LVL) + "output ";
      buf += signal_init(this.output_signals[i]) + ";\n";
   }

   buf += this.go_in.stmt_out.pretty_print(indent + INDENT_LVL);
...
rk.LoopEach.prototype.pretty_print = function(indent) {
   return indent_cursor(indent) + "loop {\n"
      + this.go_in.stmt_out.pretty_print(indent + INDENT_LVL)
      + "\n" + indent_cursor(indent) + "} each " + this.signal_name + ";";
}
...
rk.If.prototype.pretty_print = function(indent) {
   var buf = indent_cursor(indent) + "if " + "[NYI]";
   var then_is_seq = this.go_in[0].stmt_out instanceof rk.Sequence;
   var else_is_seq = this.go_in[1].stmt_out instanceof rk.Sequence;
...
rk.Parallel.prototype.pretty_print = function(indent) {
   var buf = indent_cursor(indent) + "fork {\n";

   for (var i in this.go_in) {
      buf += this.go_in[i].stmt_out.pretty_print(indent + INDENT_LVL);
      if (i < this.go_in.length - 1)
