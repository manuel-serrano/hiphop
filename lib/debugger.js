...
ast.Run.prototype._dump_word_list = function() {
   ...
   for (let i in this.children[0].signal_declaration_list) {
	 ...
	 for (let j = 0; j < sig.netSigList.length && !present; j++) {
      	    let gate = sig.netSigList[j];
...
ast.Emit.prototype._dump_word_list = function() {
   ...
   for (let i = 0; i < this.signame_list.length; i++) {
      ...
      let netSigList = sig.netSigList;

      for (let j in netSigList) {
	 if (netSigList[j].value == true) {
	    present = true;
	    break;
	 }
      }
...
ast.Exec.prototype._dump_word_list = function() {
   ...
   if (this.signal) {
      ...
      for (let i in this.signal.netSigList) {
	 if (this.signal.netSigList[i].value == true) {
	    present = true;
	    break;
	 }
      }
...
function dump_action_node_wd(func, accessor_list, word_list) {
   ...
   function get_signal_status_and_value(signal) {
      let netSigList = signal.netSigList;
      let ret = {
	 status: false,
	 value: signal.value
      };

      for (let i in netSigList) {
	 if (netSigList[i].value == true) {
	    ret.status = true;
	    break;
	 }
      }
