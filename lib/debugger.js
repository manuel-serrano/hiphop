...
MachineDebugger.prototype.update = function() {
   ...
   if (this.watchpoint_func) {
      let self = signal.generate_this(this.watchpoint_accessor_list, 0); 
...
MachineDebugger.prototype.stepper_next = function(flush=false) {
   ...
   //
   // Force a reaction if the stepper queue is empty. It will fill in
   // with possibles waiting inputs
   //
   if (this.stepper_queue.length == 0)
      this.machine.react();
   ...
   let _react = input_list => {
	 ...
	 if (input instanceof Function) {
	    ...
	    this.machine.input(input.signal_name, input.signal_value);
	 }
      });
      this.machine.react();
   }
...
MachineDebugger.prototype.stepper_notify = function(notify_func) {
   this.stepper_input_state.push(notify_func);
}
...
MachineDebugger.prototype.watchpoint_enable = function(watchpoint_expr) {
   try {
      let func = eval(`(function() {return (${watchpoint_expr})})`);
      let accessor_list = lang.get_accessor_list(func);

      this.watchpoint_func =  func;
      this.watchpoint_accessor_list = accessor_list;
      accessor_list.forEach(acc => {
	 let sig = this.machine.global_signal_map[acc.signal_name];
