...
ast.Run.prototype._dump_word_list = function() {
   ...
   for (let i in this.children[0].signal_declaration_list) {
	 ...
	 let v = sig.value;
	 let word = word_list.add(sig.name + "=" + sigdecl.name,
...
ast.Emit.prototype._dump_word_list = function() {
   ...
   for (let i = 0; i < this.signal_name_list.length; i++) {
	 ...
	 if (gate_list[j].value == true) {
      ...
      w.signal_value = v instanceof Object ? v.toString() : v;
   }
...
ast.Exec.prototype._dump_word_list = function() {
   ...
   let w = word_list.add_at(1, this.signal.name, DISP_FG_SIGNAL(present));
   w.signal_value = v instanceof Object ? v.toString() : v;
...
function dump_action_node_wd(func, accessor_list, word_list) {
   ...
   //
   // This function is a hack, and it does not works very well: slow and
   // if the accessor checks the presence of a valued signal, we can't get
   // the value. A proper implementatioqqn would be using a reference to
   // signal on gates build for signals objects. TODO!
   // 
   function get_signal_status_and_value(is_gate, runtime) {
   ...
   if (accessor_list.length > 0) {
	 ...
	 if (acc.get_value) {
	    ...
	    s_status_value = get_signal_status_and_value(false, acc.runtime);
	    
	 } else {
	    word_list.add(acc.get_pre ? "pre present" : "present");
	    s_status_value = get_signal_status_and_value(true, acc.runtime);
	 }

	 let word = word_list.add(acc.signal_name,
...
function MachineDebugger(machine, name) {
   ...
   let addr = hh.on_client ? location.host : `localhost:${hop.port}`;
   ...
   this.sdebugger_ws.onmessage = evt => {
	 ...
	 case dcommon.DEBUGGER_OVERRIDEN:
	    if (hh.on_client)
	       alert("Hiphop.js debugger: debugger name overriden.");
   ...
   this.sdebugger_ws.onclose = evt => {
	 ...
	 // Should never happen
	 //
	 throw new error.InternalError("Hiphop.js debugger/server-side: " +
				       "WS closed.", null);
      }
      this.disable(true);
   }
...
MachineDebugger.prototype.stepper_disable = function() {
   this.stepper_next(true);
   this.stepper = false;
}

MachineDebugger.prototype.stepper_next = function(flush=false) {
   if (!this.stepper || this.stepper_queue.length == 0)
      return;

   let _react = input_list => {
      input_list.forEach(input => this.machine.input(input.signal_name,
						     input.signal_value))
      this.machine.react();
   }
   ...
   this.stepper = false;
   if (flush) {
      while (this.stepper_queue.length > 0)
	 _react(this.stepper_queue.shift());
   } else {
      _react(this.stepper_queue.shift());
   }
   this.stepper = true;
...
MachineDebugger.prototype.stepper_input = function(signal_name, signal_value) {
   this.stepper_input_state.push({ signal_name: signal_name,
				   signal_value: signal_value });
}

MachineDebugger.prototype.stepper_react = function() {
   this.stepper_queue.push(this.stepper_input_state);
   this.stepper_input_state = [];
   this.stepper_update();
}
...
MachineDebugger.prototype.stepper_update = function() {
   this.send({ type: dcommon.STEPPER_UPDATE,
	       remains: this.stepper_queue.length });
}
