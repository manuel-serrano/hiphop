...
ast.Run.prototype._dump_word_list = function() {
   ...
   for (let i in this.children[0].signal_declaration_list) {
	 ...
	 for (let j = 0; j < sig.netList.length && !present; j++) {
      	    let gate = sig.netList[j];
...
ast.Emit.prototype._dump_word_list = function() {
   ...
   for (let i = 0; i < this.signame_list.length; i++) {
      ...
      let netList = sig.netList;

      for (let j in netList) {
	 if (netList[j].value == true) {
	    present = true;
	    break;
	 }
      }
...
ast.Exec.prototype._dump_word_list = function() {
   ...
   if (this.signal) {
      ...
      for (let i in this.signal.netList) {
	 if (this.signal.netList[i].value == true) {
	    present = true;
	    break;
	 }
      }
