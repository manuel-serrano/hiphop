...
function MachineDebugger(machine, name) {
   ...
   enable_debugger(this.name, this.wd_list, this);
}
...
MachineDebugger.prototype.stepper_next = function(flush=false) {
   if (!this.stepper || this.react_queue.length == 0)
      return;

   let _react = action => action.func.apply(this.machine, action.args);

   this.stepper = false;
   if (flush) {
      while (this.react_queue.length > 0)
	 _react(this.react_queue.shift());
   } else {
      _react(this.react_queue.shift());
   }
   this.stepper = true;
...
if (hh.on_client) {
   ...
   let ws_cli = new WebSocket(`ws://${host}:${port}/hop/${dcommon.DEBUGGER_WS}`);
   let mdebugger_map = {};
   let send_queue = [];
   ...
   ws_cli.onmessage = function(evt) {
      let data = dcommon.parse_data(evt)

      if (!data)
	 return;

      let md = mdebugger_map[data.d_name];
   ...
   update_debugger = function(d_name, seq, diff_list) {
      send({ type: dcommon.DEBUGGER_UPDATE, d_name: d_name,
	     diff_list: diff_list,
      	     seq: seq });
   };

   enable_debugger = function(d_name, wd_list, mdebugger) {
      mdebugger_map[d_name] = mdebugger;
      send({ type: dcommon.DEBUGGER_ENABLE, d_name: d_name, wd_list: wd_list });
   };

   disable_debugger = function(d_name) {
      delete mdebugger_map[d_name];
      send({ type: dcommon.DEBUGGER_DISABLE, d_name: d_name });
   };
} else {
   //
