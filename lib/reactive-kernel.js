...
ValuedSignal.prototype.apply_react_functions = function() {
   this.react_functions.forEach(function(func, id, arr) {
      func.call(this, this.name, this.get_value());
   }, this)
}
...
ReactiveMachine.prototype.on = function(sig, callback) {
   this.addEventListener(sig, callback);
}
...
function Expression(loc, func, exprs) {
   ...
   if (exprs != undefined && exprs.length != func.length)
      fatal_error("Arity error in expression [expected:" + func.length
		  + " given:" + exprs.length + "]", loc);
}
...
BoxingExpression.prototype = new Expression(undefined, undefined, undefined);
...
SignalExpression.prototype = new Expression(undefined, undefined, undefined);
