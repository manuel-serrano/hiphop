...
ReactiveMachine.prototype.react = function() {
   ...
   for (var i in this.local_signals) {
	 ...
	 if (this.local_signals[i][j].value != undefined) {
	    for (var h in this.local_signals[i]) {
	       this.local_signals[i][h].value = this.local_signals[i][j].value;
	       this.local_signals[i][h].is_pre_value_init = true;
	       this.local_signals[i][h].is_value_init = true;
	    }
	    break;
	 }
      }
...
UnfoldAbstractTreeVisitor.prototype.visit = function(node) {
   ...
   if (node instanceof Circuit) {
      ...
      if (node instanceof MultipleCircuit)
...
exports.Run = Run;
exports.UnfoldAbstractTreeVisitor = UnfoldAbstractTreeVisitor;
