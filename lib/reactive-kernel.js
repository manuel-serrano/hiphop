...
function check_valued_signal_definition(type, combine_with, name) {
   if (COMBINED_VALUED_TYPES[type] == undefined)
      fatal_error("Wrong type on valued signal " + name, undefined);
...
function Emit(machine, loc, signal_name, func, exprs) {
...
function LoopEach(machine, loc, circuit, signal_name, test_pre, count) {
   this.circuit = circuit;
   this.signal_name = signal_name;
   this.count = count;
   this.test_pre = test_pre;
   var sub_seq = new Sequence(machine, loc, [circuit, new Halt(machine, loc)]);
   var sub_ab = new Abort(machine, loc, sub_seq, signal_name, test_pre, false,
			  count);

   Circuit.call(this, machine, loc, "LOOPEACH", new Loop(machine, loc, sub_ab));
}

LoopEach.prototype = new Circuit(undefined, undefined, undefined, undefined);

LoopEach.prototype.get_ast_node = function() {
      return new ast.LoopEach(this.loc,
...
function Sustain(machine, loc, signal_name, func, exprs) {
   this.signal_name = signal_name;
   this.func = func;
   this.exprs = exprs;
   var sub_seq_internal = [ new Emit(machine, loc, signal_name, func, exprs),
			   new Pause(machine, loc) ];
   var sub_seq = new Sequence(machine, loc, sub_seq_internal);

   Circuit.call(this, machine, loc, "SUSTAIN", new Loop(machine, loc, sub_seq));
}

Sustain.prototype = new Circuit(undefined, undefined, undefined, undefined);

Sustain.prototype.get_ast_node = function() {
   return new ast.Sustain(this.loc, this.signal_name, this.func, this.exprs);
}

/* Every */

function Every(machine, loc, circuit, signal_name, count, immediate) {
   this.circuit = circuit;
   this.signal_name = signal_name;
   this.count = count;
   this.immediate = immediate;

   var sub_await = new Await(machine, loc, signal_name, false, immediate,
			     count);
   var sub_loopeach = new LoopEach(machine, loc, circuit, signal_name, false,
				   count);

   Circuit.call(this, machine, loc, "EVERY",
		new Sequence(machine, loc, [sub_await, sub_loopeach]));
}

Every.prototype = new Circuit(undefined, undefined, undefined, undefined);

Every.prototype.get_ast_node = function() {
   return new ast.Every(this.loc,
			this.circuit.get_ast_node(),
			this.signal_name,
			this.count,
			this.immediate);
...
function Abort(machine, loc, circuit, signal_name, test_pre, immediate, count) {
...
function Await(machine, loc, signal_name, test_pre, immediate, count) {
   this.signal_name = signal_name;
   this.count = count;
   this.test_pre = test_pre;
   this.immediate = immediate;
...
function Parallel(machine, loc, branches) {
   MultipleCircuit.call(this, machine, loc, "PARALLEL", branches);
...
Parallel.prototype.run = function() {
   ...
   if (max_code > 1) {
      ...
      this.go_in.forEach(function(el, i, arr) {
      	 el.stmt_out.accept(reset_register)
      });
   }
...
Parallel.prototype.get_ast_node = function() {
   ...
   for (var i in this.go_in)
      subcircuits[i] = this.go_in[i].stmt_out.get_ast_node();
   return new ast.Parallel(this.loc, subcircuits);
}
...
/* only one error method wich take js exception parameter */
