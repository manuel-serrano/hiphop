...
function ReactiveMachine(loc, machine_name, debug) {
   ...
   /* If this instance of reactive machine has been used in a RUN instruction
      of an other reactive machine, we must redirect add input of signals
      on new signal names

      We also must forbid to use the same machine on both RUN statement,
      use have to embed the machine reclaration in a function tu create
      several times the machine

      `map_signals` has original name of original input signals, and the
      new name as value
