...
Signal.prototype.decr_emitters = function() {
   ...
   // if (this.machine.signals_emitters[this.name] < 0)
   //    catchable_error("Assert signals_emitters >= 0 failed. Signal name: "
   // 		      + this.name, current_control.loc);
}
...
Signal.prototype.build_event_object = function() {
   return { machine: this.machine.machine_name,
	    signal: this.name,
	    stopPropagation: function() { this.state.stop = true },
	    state: { stop: false } }
}

/* Used only for output signals  */

Signal.prototype.apply_react_functions = function() {
   var evt = this.build_event_object();

   Object.freeze(evt);
   for (var f in this.react_functions) {
      this.react_functions[f].call(this, evt)
      if (evt.state.stop)
	 break;
   }
...
function ValuedSignal(name,
   ...
   if (this.has_init_value && type != undefined)
      type_test(init_value, type, "ValuedSignal " + name);
}
...
ValuedSignal.prototype.set_value = function(value) {
   ...
   if (this.type != undefined)
      type_test(value, this.type,
		"ValuedSignal " + this.name + "::" + current_control.loc);
...
ValuedSignal.prototype.build_event_object = function() {
   var evt = Signal.prototype.build_event_object.call(this);

   evt.value = this.get_value(false)
   return evt;
}
...
function ReactiveMachine(loc, machine_name, debug) {
   ...
   /* If this instance of reactive machine has been used in a RUN instruction
      of an other reactive machine, we must redirect any access of signal
      (setInput, Accessor), to the caller and with the name of the caller.

      We also must forbid to use the same machine on more than 1 RUN
      statement. To do it, one must to embed the machine reclaration in
      a function, and return it.

      `map_signals` has original name of original signals, and the
      new name as value

      If `map_signals` is null, the machine is never used in clone, and
      must not redirect input. If it is not null, the machine has to redirect
      inputs, and also to raise an error if we try to use it in a new RUN
      statement

      When `map_signals` is not null, `machine_dst` refers to the caller
      machine, to redirect the input

      Note that map_signals is also use for accessors functions */
   this.map_signals = null;
   this.machine_dst = null;
}
...
ReactiveMachine.prototype.react = function() {
   ...
   if (this.map_signals !== null)
      fatal_error("Machine used in RUN statement, can't be use anymore",
		  current_control.loc);
   ...
   this.go_in.stmt_out.accept(new CountSignalEmitters(this, true));
...
ReactiveMachine.prototype.getElementById = function(id) {
   ...
   ast_machine.accept(new compiler.BuildTreeVisitor(ast_machine, this));
...
ReactiveMachine.prototype.setInput = function(name, value) {
   if (this.map_signals !== null) {
      this.machine_dst.setInput(this.map_signals[name], value);
      return;
   }
...
function Emit(machine, id, loc, signal_name, func, exprs) {
   ...
   this.exprs = exprs;
   this.blocked = false;
   this.not_emitter_in_react = false;
}
...
Emit.prototype.run = function() {
   ...
   if (this.go.set) {
	 ...
	 if (!(signal instanceof ValuedSignal))
	    catchable_error("Can't set value to non non valued signal `"
			    + signal.name + "`.",
			    current_control.loc);
...
If.prototype.get_ast_node = function() {
...
Loop.prototype.run = function() {
   ...
   while (loop) {
      ...
      if (!this.go_in.stmt_out.run())
	 return false;
      loop = this.k_in[0].set;

      if (loop && this.machine.reincarnation_lvl > 0) {
	 this.machine.reincarnation_lvl--;
...
function Abort(machine,
   ...
   /* We must make this test here, to avoid to make it on xml-compiler at
      each statement with allow immediate and count attributes */
   if (immediate && count > 0)
      fatal_error("Attribute `immediate` can't be set with"
		  + " `count` delay.", loc);
}
...
Abort.prototype.run = function() {
   ...
   this.go_in.set = this.go.set && (!((this.immediate && !this.weak)
				      && signal_set));
   this.res_in.set = this.res.set && this.sel.set
      && (!signal_set || (signal_set && this.weak));
   this.susp_in.set = this.susp.set;
   this.kill_in.set = this.kill.set;
...
function Run(machine, id, loc, sigs_assoc, subcircuit, runnable_machine) {
...
function CountSignalEmitters(machine, reinit_emitters) {
   this.stop_go = false;
   this.machine = machine;
...
CountSignalEmitters.prototype.visit = function(stmt) {
   if (stmt instanceof Parallel)
      for (var i = 0; i < stmt.go_in.length; i++) {
	 var visitor = new CountSignalEmitters(this.machine, false);

   	 stmt.go_in[i].stmt_out.accept(visitor);
	 if (visitor.stop_go)
	    this.stop_go = true;
      }

   else if (stmt instanceof Abort && stmt.immediate)
     this.ignore_next_pause = true;

   else if (stmt instanceof Pause) {
   ...
   else if (stmt instanceof Emit) {
      if (this.stop_go) {
	 stmt.not_emitter_in_react = true;
      } else {
	 stmt.not_emitter_in_react = false;
   ...
   else if (stmt instanceof MultipleCircuit)
      for (var i in stmt.go_in)
	 stmt.go_in[i].stmt_out.accept(this);

   else if (stmt instanceof Circuit)
      stmt.go_in.stmt_out.accept(this);
}
...
SignalAccessor.prototype._get = function() {
   var sig;

   if (this.machine.map_signals != null) {
      var dst_name = this.machine.map_signals[this.signame];
      sig = this.machine.machine_dst.get_signal(dst_name);
   } else {
      sig = this.machine.get_signal(this.signame);
   }
