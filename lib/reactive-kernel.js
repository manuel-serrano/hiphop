...
ReactiveMachine.prototype._insertStatement = function(id, stmt_ast, before) {
   if (!(stmt_ast instanceof ast.Statement))
      catchable_error("Can't insert statement.", undefined);
   if (stmt_ast.signal_name && !this.get_signal(stmt_ast.signal_name))
      catchable_error("Can't insert statement, unknown signal "
		      + stmt_ast.signal_name + ".", stmt_ast.loc)

   var get_id_visitor = new compiler.GetNodeFromIdVisitor(id)
   var prg_ast = this.go_in.stmt_out.get_ast_node();
   var neighbor;
   prg_ast.accept(new compiler.BuildTreeVisitor(null));
   //prg_ast.accept(new compiler.PrintTreeVisitor());
   prg_ast.accept_auto(get_id_visitor);
   neighbor = get_id_visitor.node;

   stmt_ast.machine = this;
   if (neighbor.parent == null)
      catchable_error("Can't find parent of neighbor node.", stmt_ast.loc);

   var parent = neighbor.parent;
   stmt_ast.parent = neighbor.parent;
   var neighbor_index = parent.subcircuit.indexOf(neighbor)
   var seq_order;

   if (before)
      seq_order = [ stmt_ast, neighbor ]
   else
      seq_order = [ neighbor, stmt_ast ]

   var seq = new ast.Sequence(undefined, neighbor.loc, seq_order);
   seq.parent = parent;
   ...
   var state = this.save();
   compiler.dyn_recompile(this, prg_ast);
   this.restore(state);
   this.boot_reg = true;
}

ReactiveMachine.prototype.insertBefore = function(id, stmt_ast) {
   this._insertStatement(id, stmt_ast, true);
}

ReactiveMachine.prototype.insertAfter = function(id, stmt_ast) {
   this._insertStatement(id, stmt_ast, false);
}
...
function Atom(machine, id, loc, func) {
...
Atom.prototype.get_ast_node = function() {
   return new ast.Atom(this.id, this.loc, this.func);
}
