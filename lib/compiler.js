...
/*    Last change :  Fri Nov 14 07:46:13 2025 (serrano)                */
...
function makeExpressionNet(env, astNode, level) {
   ...
   if (astNode.func || astNode.accessor_list.length > 0) {
      const name = `${basename(astNode.loc.filename)}:${astNode.loc.pos}`;
...
function makePauseCircuit(env, astNode) {
   ...
   // REG := reglhs v regrhs
   const or = net.makeOr(astNode, "(sus ^ reg !kill V(go^!kill)")
   connectTo(reglhs, or);
   connectTo(regrhs, or);
   connectTo(or, reg);
