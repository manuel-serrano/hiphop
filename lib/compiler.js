...
function make_trap(ast_node, child_interface) {
   ...
   let k0_list_child = connect_k_min_incarnation(ast_node, ast.Trap,
						 child_interface, 0);
   let trap_gate_list = connect_k_min_incarnation(ast_node, ast.Trap,
						  child_interface, 2);

   for (let l in k0_list_child)
      k0_list_child[l].connect_to(k0_list, net.FAN.STD);
   ...
   for (let l in trap_gate_list) {
      ...
      trap_gate_list[l].connect_to(kill_list[l], net.FAN.STD);
   }

   k_matrix[1] = connect_k_min_incarnation(ast_node, ast.Trap,
					   child_interface, 1);
   for (let k = 3; k < k_matrix_child.length; k++)
      k_matrix[k - 1] = connect_k_min_incarnation(ast_node, ast.Trap,
						  child_interface, k);
...
function make_loop(ast_node, child_interface) {
   ...
   for (let k = 1; k < child_interface.k_matrix.length; k++)
      k_matrix[k] = connect_k_min_incarnation(ast_node, ast.Loop,
					      child_interface, k);
...
ast.LocalSignal.prototype.make_circuit = function() {
   ...
   for (let k in child.cinterface.k_matrix)
      k_matrix[k] = connect_k_min_incarnation(this, ast.LocalSignal,
					      child.cinterface, k);
