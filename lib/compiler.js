...
function compile(ast_machine) {
   ...
   ast_machine.accept_auto(new CheckIdVisitor());
   //ast_machine.accept(new PrintTreeVisitor());
   ast_machine.accept(new BuildCircuitVisitor(machine));

   if (ast_machine.auto_react > 0)
      machine.auto_start(ast_machine.auto_react);
   return machine;
}

function dyn_recompile(machine, prg_ast) {
   if (!(machine instanceof reactive.ReactiveMachine))
      reactive.fatal_error("Can't dyn_recompile", undefined);

   prg_ast.accept(new SetIncarnationLevelVisitor());
   prg_ast.accept(new SetExitReturnCodeVisitor());
   //prg_ast.accept(new PrintTreeVisitor());

   var build_circuit_visitor = new BuildCircuitVisitor(machine);
   prg_ast.accept(build_circuit_visitor);
   machine.build_wires(build_circuit_visitor.children_stack.pop());
}

exports.compile = compile;
exports.GetNodeFromIdVisitor = GetNodeFromIdVisitor;
exports.BuildTreeVisitor = BuildTreeVisitor;
exports.dyn_recompile = dyn_recompile;
