...
/*    Last change :  Wed Feb 26 17:25:09 2025 (serrano)                */
...
/*    connMode ...                                                     */
/*---------------------------------------------------------------------*/
const connMode = { GO: 101, K: 102, FORK: 103, MERGE: 104, BRANCH: 105, CONNECT: 106 };
...
/*       if (astNode.circuit.goList[astNode.depth] instanceof net.WireNet) { */
/* 	 const old = astNode.circuit.goList[astNode.depth];            */
/* 	 const or = net.makeOr(old.astNode, old.debugName, old.lvl);   */
/* 	 astNode.circuit.goList[astNode.depth] = or;                   */
/* 	 replace(old, or);                                             */
/*       }                                                             */
...
function makeSequenceCircuit_int(env, astNode, childCircuits) {
   ...
   childCircuits.forEach(c => {
      ...
      // c.KILL <- KILL
      if (c.killList) connectToList(killList, c.killList);
   });
...
function makeSequenceCircuit(env, astNode, childCircuits) {
   ...
   } else if (false && intCompiler && origCompilerFuns.indexOf("makeSequenceCircuit") < 0) {
...
function makeAtomCircuit(env, astNode, sigtable) {
   ...
   } else if (false && intCompiler && origCompilerFuns.indexOf("makeAtomCircuit") < 0) {
