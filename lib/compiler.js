...
/* Tests for signal dependencies use by instructions using expression,
   during the make_circuit() phase */

function is_signal_dependent(ast_node) {
...
function make_emit(ast_node) {
   ...
   get_signal(ast_node).add_emitter(ast_node, ast_node.callee || ast_node.args);
...
function make_atom(ast_node) {
   var go = new net.Net(ast_node);
   var dep = is_signal_dependent(ast_node) ? [go] : undefined;
...
function get_signal(ast_node) {
   ...
   sig = machine.input_signals[name] || machine.output_signals[name];
   if (!sig) {
...
ConnectCircuitVisitor.prototype.visit = function(ast_node) {
   if (ast_node instanceof ast.Present
      ...
      let sig = get_signal(ast_node);
      let emitters_deps = make_signal_test(ast_node, sig)
      ...
      for (let i in ast_node.exprs) {
	 let expr = ast_node.exprs[i];
	 if (expr instanceof signal.SignalAccessor) {
	    let dep = make_signal_test(ast_node, get_signal(ast_node));
