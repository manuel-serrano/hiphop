...
/*    Last change :  Tue May 27 22:00:29 2025 (serrano)                */
...
function makeLoopCircuit(env, astNode, childCircuit) {
   ...
   if (l === 0) {
      const instantaneousOr = net.makeOr(astNode, "instantaenousOr", 0);
      const instantaneousAnd = net.makeOr(astNode, "instantaenousAnd", 0);
      const instantaneousXor = net.makeAnd(astNode, "instantaenousXor", 0);
...
function compile(machine, astNode) {
   ...
   if (machine.syncReg === undefined) {
      machine.syncReg = (compiler(astNode) === "new");
   }
