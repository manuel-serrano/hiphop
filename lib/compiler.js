...
ast.Module.prototype.make_circuit = function(env) {
   let boot_reg = new net.Register(this, "boot_register");
   let res = net.make_or(this, "global_res");
   let const0 = net.make_or(this, "const0");
   let sel = net.make_or(this, "global_sel");
   let k0 = [net.make_or(this, "global_k0")];
   let k1 = [net.make_or(this, "global_k1")];

   const0.connect_to(boot_register, net.FAN.STD);
   boot_reg.connect_to(res, net.FAN.NEG);

   this.cinterface = new Interface([boot_reg], res, const0, [const0], sel
				   [k0, k1]);
   this.machine.global_environment_interface = this.cinterface;

   /* because `cinterface` of the module is also the global environment, we
      add lists of signals */

   this.cinterface.input_signal_map = {};
   this.cinterface.output_signal_map = {};
}
...
function get_signal_gate(env, signal_name, lvl) {
   let gate_list =
       env.input_signal_list[signal_name] || env.output_signal_list[signal_name];
