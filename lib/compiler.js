...
/*    Last change :  Fri Oct  3 13:27:35 2025 (serrano)                */
...
function makeForkCircuit_new(env, astNode, childCircuits) {
   ...
   childCircuits.forEach((c, n, _) => {
      ...
      // connect OR-NOT door with GO of parallel and SEL of child
      if (astNode.machine.forkNoGo) {
	 // a variant of Fig 11.10 suggested by Gerard:
	 ...
	 connectTo(xem, cmin[0][l], net.FAN);
      } else {
	 // plain fork circuit as described in fig 11.10
	 const xem = net.makeOr(astNode, "xem[" + n + "]", l, 0);
...
function compile(machine, astNode) {
   ...
   if ("HIPHOP_FORKNOGO" in process.env) {
      machine.forkNoGo = (process.env.HIPHOP_FORKNOGO === "true");
   }
