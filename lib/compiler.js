...
function make_present(ast_node, circuit_then, circuit_else) {
   ...
   res = circuit_then.res.concat(circuit_else.res);
   susp = circuit_then.susp.concat(circuit_else.susp);
   kill = circuit_then.kill.concat(circuit_else.kill);
...
function make_sequence(ast_node, subcircuit) {
   ...
   for (let c = 0; c < len; c++) {
      ...
      seq_interface.res = seq_interface.res.concat(circuit.res);
      seq_interface.susp = seq_interface.susp.concat(circuit.susp);
      seq_interface.kill = seq_interface.kill.concat(circuit.kill);
