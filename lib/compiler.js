...
/*    Last change :  Wed Jan 29 12:34:47 2025 (serrano)                */
...
function makeSuspendCircuit(env, ast_node, childCircuit) {
   ...
   const kills = ccutils.killListChildren(ast_node, ast.Suspend, [childCircuit]);

   return new Circuit(ast_node, ast.Suspend, childCircuit.go_list,
		      and1, or1, kills, childCircuit.sel,
		      k_matrix);
}
...
function makeAbortCircuit(env, ast_node, childCircuit, force_non_immediate) {
   ...
   function makeAbortInner(env, ast_node, childCircuit) {
	 ...
	 decr_list[0] = makeExpressionNet(env, ast_node, ast.Abort,
					  [and1], [], [],
					  ast_node.depth);
	 decr_list[1] = and1;
	 makeCounterNet(env, ast_node, ast.Abort, decr_list, go_list, [and3], [and2]);
      } else {
	 makeExpressionNet(env, ast_node, ast.Abort,
			   [and1], [and3], [and2],
			   ast_node.depth);
      }
	 ...
	 k_matrix[k] = ccutils.getKListChild(ast_node, ast.Abort, childCircuit, k);
      ...
      const kills = ccutils.killListChildren(ast_node, ast.Abort, [childCircuit]);
      return new Circuit(ast_node, ast.Abort, go_list,
			 and1, childCircuit.susp, kills, childCircuit.sel,
			 k_matrix);
   }
