...
/*    Last change :  Sun Feb 23 06:35:05 2025 (serrano)                */
...
function traceNode(astNode) {
   if (debug) {
      return `${astNode.constructor.name}[${astNode.depth}:${astNode.trapDepth}]@${astNode.loc.pos}`
   }
}
...
ast.Module.prototype.makeCircuit = function(env, sigtable) {
   traceEnter(`${this.constructor.name}[${this.depth}:${this.trapDepth}]@${this.loc.pos}...`);
   ...
   if (child.circuit.res) {
      ...
      console.log("HipHop warning: no global_res");
   }

   if (child.circuit.sel) {
      connectTo(child.circuit.sel, global_sel);
   } else {
      console.log("HipHop warning: no global_sel");
   }
