...
ExpressionVisitor.prototype.visit = function(node) {
   if (node instanceof ast.Emit) {
	 ...
	 fatal_error("Emit must have func attribute set if it had more than"
		     + " one expression", node.loc);
   }
}
...
function compile(ast_machine) {
   ...
   for (var i in ast_machine.output_signals) {
      ...
      var arity = sig.signal_ref instanceof reactive.ValuedSignal ? 2 : 1;

      for (func in sig.react_functions)
	 if (sig.react_functions[func].length != arity)
	    reactive.fatal_error("Wrong arity function in react_functions of "
				 + sig.signal_ref.name,
				 machine.loc)
   }
