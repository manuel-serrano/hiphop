...
ast.If.prototype.make_circuit = function() {
   ...
   for (let i = 0; i <= this.depth; i++) {
      ...
      translate_args_list(this.machine, this.args, args, i, test);
...
/* Helper function returns a copy of the argument list `args_list` (used in
   expression) in the argument list `new_args_list` but it replace
   SignalAccessor by gate or signal objects */

function translate_args_list(machine, args_list, new_args_list, lvl, dep_net) {
   for (let i in args_list) {
	 ...
	 if (arg.get_value) {
	    let sobj = get_signal_object(machine, arg.signal_name);

	    new_args_list.push({ pre: arg.get_pre, signal: sobj});
	    gate = arg.get_pre ? sobj.pre_gate : sobj.dependency_gate_list[lvl];
	 } else {
	    if (arg.get_pre)
	       gate = get_signal_pre_gate(machine, arg.signal_name)
	    else
	       gate = get_signal_gate(machine, arg.signal_name, lvl)
	    new_args_list.push(gate);
	 }

	 gate.connect_to(dep_net, net.FAN.DEP);
      } else {
