...
/*    Last change :  Mon Jan  5 17:22:01 2026 (serrano)                */
...
function signalGate(sig, l) {
   const netList = sig.netList;

   if (netList.length <= l) {
      return netList[netList.length - 1];
   } else {
      return netList[l];
   }
}
...
function makeForkCircuit_new(env, astNode, childCircuits) {
   ...
   // c.KILL <- KILL
   childCircuits.forEach(c => connectToList(killList, c.killList, connMode.RNCAKILL));
   
   // SEL <- V c.SEL
   const or_sel = net.makeOr(astNode, "SEL", l);
   childCircuits.forEach(c => connectTo(c.sel, or_sel));
   connectTo(or_sel, sel);
...
/* 	 connectTo(xem, cmin[0][l], net.FAN.NEG);                      */
   ...
   // connect the synchronizer to the cicruit kMatrix
   sync.kMatrix.forEach((m, k, _) =>
      connectToList(m, kMatrix[k], connMode.RNCAKMATRIX));
...
function makeTrapCircuit_new(env, astNode, childCircuit) {
   ...
   const superexperimental = false;
   if (suspicious_k2_kill_connection) {
      if (experimental) {
	 if (superexperimental) {
	    // super experimental, connect kMatrix[2] only at the depth
	    const nkill = Array.from({length: l + 1}, (_, i) =>
               net.makeOr(astNode, "NKILL", i));
	    const nkillK2 = net.makeAnd(astNode, "NKILLK2", l);
	    nkillK2.sweepable = false;
	    
	    connectToList(killList, nkill, connMode.CONNECT);
	    
	    // childCircuit.sel.connectTo(nkillK2, net.FAN.NEG);
	    childCircuit.kMatrix[2][l].connectTo(nkillK2, net.FAN.STD);
	    nkillK2.connectTo(nkill[l], net.FAN.STD);
	    
	    connectToList(nkill, childCircuit.killList, connMode.GO);
	 } else {
	    // experimental, connect kMatrix[2] only at the depth
	    const nkill = Array.from({length: l + 1}, (_, i) =>
               net.makeOr(astNode, "NKILL", i));
   ...
   // K0 <- c.K0 V c.K2
   const k0_or_k2 = Array.from({length: l + 1}, (_, i) =>
      net.makeOr(astNode, `K[0]`, i));

   const kmode = childCircuit.astNode.depth >= 1
      ? connMode.RNCAKMATRIX : connMode.CONNECT;
...
function makeSequenceCircuit_new(env, astNode, childCircuits) {
   ...
   // SEL <- V c.SEL
   const orSel = net.makeOr(astNode, "SEL", l);
   connectTo(orSel, sel);
   childCircuits.forEach(c => connectTo(c.sel, orSel));
...
ast.Module.prototype.makeCircuit = function(env, sigtable) {
   ...
   for (let i = 0; i < this.sigDeclList.length; i++) {
	 ...
	 //
	 s.netList[0].signal = s;

	 // debug
	 if (!(s.netList[0] instanceof net.SignalNet)) {
	    throw new Error(`makeCircuit::Module, netList[0] not a signal ${s.netList[0].constructor.name}`);
	 }
	 if (!(s.netList[0].accessibility & ast.INOUT)) {
	    throw new Error(`makeCircuit::Module, netList[0] bad accessibility ${s.netList[0].accessibility}`);
	 }
      }
...
function computeNodeDepth_new(astNode, depth) {
   
   function rec(astNode, depth) {
      astNode.depth = depth;

      if (astNode instanceof ast.Fork
	 || astNode instanceof ast.Local
/* 	 || astNode instanceof ast.Trap                                */
	 ) {
	 astNode.children.forEach(c => rec(c, depth + 1));
      } else {
	 astNode.children.forEach(c => rec(c, depth));
      }
   }
   return rec(astNode, depth);
}

function computeNodeDepth(astNode) {
   try {
      if (compiler(astNode) === "new") {
	 return computeNodeDepth_new(astNode, 0);
      } else {
...
function compile(machine, astNode) {
   ...
   if (machine.loopUnroll) {
      ...
      if (machine.loopTrapUnroll) {
	 astNode = unrollTrapLoop(astNode);
      }
      if (machine.loopExpand) {
