...
function make_loop(ast_node) {
   ...
   child.cinterface.k_matrix[0][ast_node.depth].connect_to(
...
function make_abort(ast_node) {
   ...
   for (let i = 0; i < ast_node.depth; i++) {
      ...
      /* gate for `test_expr` expression (evaluated each tine RES=1) */
