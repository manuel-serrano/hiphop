...
/*    Last change :  Fri Dec  5 14:47:05 2025 (serrano)                */
...
/*    connMode ...                                                     */
/*---------------------------------------------------------------------*/
const connMode = { GO: 101, K: 102, KGO: 103, KK: 104, FORK: 105, MERGE: 106, BRANCH: 107, CONNECT: 108, NEW: 109 };
...
function connectToList(from_list, to_list, mode = connMode.BRANCH) {
   ...
   switch (mode) {
      case connMode.NEW:
	 if (fl !== tl) {
	    throw new Error(`connectToList(NEW), inconsistent lists ${fl}/${tl}`);
	 } else {
	    for (let i = 0; i < fl; i++) {
...
/*    makeNothingCircuit ...                                           */
/*---------------------------------------------------------------------*/
function makeNothingCircuit_int(env, astNode) {
   const { l, goList, res, susp, killList, kMatrix, sel } =
      makeCircuitInterface(astNode);
...
function makeNothingCircuit_new(env, astNode) {
   ...
   for (let k = 0; k < kMatrix.length; k++) {
      if (k !== 1) {
	 connectToList(goList, kMatrix[k], connMode.NEW);
      }
   }
...
function makeNothingCircuit(env, astNode, childCircuit) {
   if (compiler(astNode) === "new") {
      return makeNothingCircuit_new(env, astNode, childCircuit);
   } else {
      return makeNothingCircuit_int(env, astNode, childCircuit);
   }
}
