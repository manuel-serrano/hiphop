...
ast.CountExpressionNode.prototype.make_counter = function(type,
   ...
   let init_counter_func = function() {
      ...
      let init_val = func_count.apply(this);
...
/* This function must be called *only* in ast.*.make_circuit()
   and *never* in make_*. Otherwise, it could result that the oneshot register
   is push not on the top of dynamically added branch but inside an embded
   instruction */
...
ast.Let.prototype.make_circuit = function() {
   this.children[0].make_circuit();
...
function make_emit(ast_node) {
   ...
   for (let i = 0; i <= ast_node.depth; i++) {
      ...
      /* make special case for exec */
      if (ast_node instanceof ast.Exec) {
	 let sig = get_signal_object(ast_node.machine, ast_node.signal_name);
	 let exec_emission_func = function() {
	    sig.set_value(ast_node.exec_status.value)
	 ...
	 go.connect_to(new net.ActionNet(ast_node, ast.Emit, "_exec_return_sig",
		       ...
		       net.FAN.STD);
      } else if (ast_node.func || ast_node.accessor_list.length > 0) {
...
ast.Exec.prototype.make_circuit = function() {
   let exec_status = {
      ...
      /* abort of await use this wire instead of signal wire to know when
	 user routine is done */
   ...
   /* needed for particular case in Abort (via this nested Await instruction)
      and Emit */
   this.exec_status = exec_status;

   /* make_await/sequence/emit take account of incarnation levels */
   let await = make_await(this);
   ...
   for(let l = 0; l <= this.depth; l++) {
	 ...
	 exec_status.lvl = l;
      ...
      await.go_list[l].connect_to(start, net.FAN.STD);

      let kill = new net.ActionNet(this, ast.Exec, "kill", l, function() {
   	 exec_status.kill = true;
      }, []);
      await.kill_list[l].connect_to(start, net.FAN.NEG);
      await.kill_list[l].connect_to(kill, net.FAN.STD);

      let susp = new net.ActionNet(this, ast.Exec, "susp", l, function() {
   	 exec_status.suspended = true;
      }, []);
      await.susp.connect_to(susp, net.FAN.STD);
      await.sel.connect_to(susp, net.FAN.STD);

      let res = new net.ActionNet(this, ast.Exec, "res", l, function() {
   	 exec_status.suspended = false;
      }, []);
      await.res.connect_to(res, net.FAN.STD);
      await.sel.connect_to(res, net.FAN.STD);
   }

   if (this.signal_name) {
      this.cinterface = make_sequence(this, [await, make_emit(this)]);
   } else {
      this.cinterface = await;
   }
}
...
ast.LocalSignal.prototype.make_circuit = function() {
   ...
   /* As child circuit can use this signal, we need to build it first. As we
      don't know yet if the child circuit use k1, kill or susp, we have to
      use buffers... */
   let k_matrix = [[], []];
   let kill_list = [];
   let susp = net.make_or(this, ast.LocalSignal, "susp_buf");

   for (let i = 0; i <= this.depth; i++) {
      k_matrix[1][i] = net.make_or(this, ast.LocalSignal, "k1_buf", i);
      kill_list[i] = net.make_or(this, ast.LocalSignal, "kill_buf", i);
   }

   sig = ast.Signal.prototype.make_circuit.call(this, ast.LocalSignal,
						k_matrix[1], kill_list, susp);
   this.machine.local_signal_map[this.signal_name] = sig;
   child.make_circuit();

   /* connect child circuit to buffers k1, kill and susp */
   if (child.cinterface.susp)
      susp.connect_to(child.cinterface.susp, net.FAN.STD);

   for (let i in child.cinterface.kill_list)
      if (i <= this.depth)
	 kill_list[i].connect_to(child.cinterface.kill_list[i], net.FAN.STD);

   k_matrix[0] = get_k_list_child(this, ast.LocalSignal, child.cinterface, 0);
   for (let i = 2; i < child.cinterface.k_matrix.length; i++)
      k_matrix[i] = get_k_list_child(this, ast.LocalSignal, child.cinterface, i)
...
ast.IOSignal.prototype.make_circuit = function() {
   let sig = ast.Signal.prototype.make_circuit.call(this, ast.IOSignal);

   this.machine.input_signal_map[this.signal_name] = sig;
   this.machine.output_signal_map[this.signal_name] = sig;
}

ast.Signal.prototype.make_circuit = function(type, k1_list=null, kill_list=null,
					     susp=null) {
   if (this.valued)
      return new signal.ValuedSignal(this, type, k1_list, kill_list, susp);
   else
      return new signal.Signal(this, type, k1_list, kill_list, susp);
}
...
CheckSignalTypeVisitor.prototype.visit = function(ast_node) {
   if (ast_node instanceof ast.Signal) {
      this.signal_map[ast_node.signal_name] = ast_node;
   } else {
      if (ast_node instanceof ast.Emit) {
	 let s = this.signal_map[ast_node.signal_name];

	 if ((ast_node.accessor_list.length > 0 || ast_node.func) && !s.valued)
	    throw new error.SignalError("Pure signal, can't set value of it",
					ast_node.signal_name, ast_node.loc);
      } else if (ast_node instanceof ast.Exec && ast_node.signal_name) {
	 let s = this.signal_map[ast_node.signal_name];

	 if (!s.valued) {
	    throw new error.SignalError("Non-valued signal mapped on Exec.",
					ast_node.signal_name, ast_node.loc);
	 }
      }
	 ...
	 if (acc.get_value && !this.signal_map[acc.signal_name].valued)
	    throw new error.SignalError("Pure signal, can't get value of it",
					acc.signal_name, ast_node.loc);
	 }
   }
}

/*
 * Check names are legals (trap and signals)
 * Rename local signals
 *
 * Nested trap / nested local signal with same name are allowed
 * Local signal with same name that a global signal is allowed
 */

function CheckNameVisitor(machine) {
   this.machine = machine;
   this.trap_stack = [];
   this.signal_names = [];
   this.local_stack = [];
   this.local_stack_renamed = [];
   this.ids = [];
}

CheckNameVisitor.prototype.visit = function(ast_node) {
   function check_accessor_list(sigs, sigslocal, sigslocalremaned, list) {
      for (let i = 0; i < list.length; i++) {
	 let acc  = list[i];
	 let local_id = sigslocal.lastIndexOf(acc.signal_name);

	 if (local_id > -1) {
	    acc.original_signal_name = acc.signal_name;
	    acc.signal_name = sigslocalremaned[local_id];
	 } else if (sigslocalremaned.indexOf(acc.signal_name) == -1 &&
		    sigs.indexOf(acc.signal_name) == -1) {
	    throw new error.SignalError("Unknown signal identifier",
					acc.signal_name, ast_node.loc);
	 }
      }
   }
   ...
   let instanceof_trap = ast_node instanceof ast.Trap;
   let name = ast_node.signal_name || ast_node.trap_name;

   if (ast_node.id && this.ids[ast_node.id])
      throw new error.SyntaxError("`id` must be unique", ast_node.loc);
   this.ids[ast_node.id] = true;

   if (instanceof_local_signal) {
      let new_name = ast_node.signal_name;
      let org_name = ast_node.original_name;

      if (new_name == org_name) {
	 new_name = ("__LOCAL_" + this.machine.local_signal_next_id +
		     "__" + org_name);
	 ast_node.signal_name = new_name;
	 this.machine.local_signal_next_id++;
      }

      this.local_stack.push(org_name);
      this.local_stack_renamed.push(new_name);

   } else if (ast_node instanceof ast.Signal) {
      /* this case happens only for InputSIgnal, OutputSignal */

      if (this.signal_names.indexOf(name) > -1)
	 throw new error.SignalError("Global signal name already used",
				     name, ast_node.loc)
      this.signal_names.push(name);
   } else if (instanceof_trap) {
     /* Nested trap with same name are legal. If ones try to exit,
      * the completion code will be the shortest (exit to the most deep trap)
      */
      this.trap_stack.push(name);
   } else if (ast_node.signal_name) {
      let local_id = this.local_stack.lastIndexOf(name);

      if (local_id > -1) {
	 ast_node.signal_name = this.local_stack_renamed[local_id];
      } else if (this.local_stack_renamed.indexOf(name) == -1 &&
		 this.signal_names.indexOf(name) == -1) {
	 throw new error.SignalError("Unknown signal identifier",
				     name, ast_node.loc)
      }
   } else if (ast_node instanceof ast.Exit) {
      if (this.trap_stack.indexOf(name) == -1)
	 throw new error.TrapError("Unknown trap identifier",
				   name, ast_node.loc);
   }

   if (ast_node.accessor_list)
      check_accessor_list(this.signal_names,
			  this.local_stack,
			  this.local_stack_renamed,
			  ast_node.accessor_list);

   if (ast_node.accessor_list_count)
      check_accessor_list(this.signal_names,
			  this.local_stack,
			  this.local_stack_renamed,
			  ast_node.accessor_list_count);

   /* Test specific for Exec statement */
   if (ast_node.func_start_accessor_list)
      check_accessor_list(this.signal_names,
			  this.local_stack,
			  this.local_stack_renamed,
			  ast_node.func_start_accessor_list);
...
function compute_depth(ast_node, depth, in_loop, in_par_sig) {
   ...
   if (ast_node instanceof ast.Module ||
       ...
       ast_node instanceof ast.IOSignal)
      ast_node.depth = 0;
   else
...
function compiler_format_pretty_print(indent, ast_node, sel) {
   let buf = "[" + ast_node.depth + "] ";
   let instruction = ast_node.constructor.name + ast_node.pretty_print_param();
...
function compile(machine, ast_node) {
   ...
   // console.log(ast_node.pretty_print(compiler_format_pretty_print));
