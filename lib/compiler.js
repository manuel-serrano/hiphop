...
function make_sequence(ast_node, subcircuit) {
   ...
   for (let c = 0; c < len; c++) {
      ...
      res = res.concat(seq_interface.res).concat(circuit.res);
      susp = susp.concat(seq_interface.susp).concat(circuit.susp);
      kill = kill.concat(seq_interface.kill).concat(circuit.kill);
