...
/*    Last change :  Fri Sep 28 18:01:52 2018 (serrano)                */
...
function getSignalObject( env, signame, ast_node ) {
   for( let i = env.length - 1; i >= 0; i-- ) {
      ...
      if( sigprop.name === signame ) {
	 if( !sigprop.alias ) {
	    return sigprop.signal;
	 } else if( sigprop.aliasOf ) {
	    return sigprop.aliasOf;
	 } else {
	    throw error.TypeError( tag + ": unbound signal `" 
				   + sigprop.alias + "', alias of `" 
				   + signame + "'.", ast_node.loc);
	 }
      }
   }
...
/*---------------------------------------------------------------------*/
function resolveSignalList( env, siglist, ast_node ) {
...
function makeExpressionNet( env, ast_node, node, 
   ...
   if( ast_node.func || ast_node.accessor_list.length > 0 ) {
      resolveSignalList( env, ast_node.accessor_list, ast_node );
...
function makeCounterNet( env, ast_node, type, 
   ...
   let init_counter_func = function() {
      let init_val = parseInt( func_count.apply( ast_node ) );
   ...
   resolveSignalList( env, ast_node.accessor_list_count, ast_node );
   ...
   reset_net.forEach( n => n.connectTo( init_or, net.FAN.STD ) );
   true_nets.forEach( n => res.connectTo( n, net.FAN.STD ) );
   false_nets.forEach( n => res.connectTo( n , net.FAN.NEG ) );
...
ast.Fork.prototype.makeCircuit = function( env ) {
   ...
   function makeParallelCircuit( env, ast_node, childCircuits ) {
	 ...
	 for (let c in childCircuits) {
	    let child_go = childCircuits[c].go_list[l];
      ...
      //
      for (let c in childCircuits) {
	 let child_sel = childCircuits[c].sel;
	 ...
	 if (child_sel) {
      ...
      for (let c in childCircuits) {
	 if (childCircuits[c].k_matrix.length > max) {
	    max = childCircuits[c].k_matrix.length;
	 }
      }
      ...
      //
      for (let c in childCircuits) {
	 let iChild = childCircuits[c];
	 for (let k = 0; k < max; k++) {
	    if (!union[k]) {
	       union[k] = [];
	       for (let i = 0; i <= ast_node.depth; i++) {
		  union[k][i] = net.makeOr(ast_node, ast.Fork, "union_k" + k, i);
	       }
	    }
      ...
      //    for (let l = 0; l <= ast_node.depth; l++) {
      // 	 union[k][l] = net.makeOr(ast_node, ast.Fork, "union_k" + k, l)

      // 	 for (let c in childCircuits) {
      // 	    let k_list = childCircuits[c].k_matrix[k]

      // 	    if (k_list && k_list[l])
      // 	       k_list[l].connectTo(union[k][l], net.FAN.STD)
      // 	 }
      //    }
      // }
	 ...
	 for (let k = 2; k < union.length; k++) {
	    for (let l in union[k]) {
	       if (!kill_list[l]) {
		  kill_list[l] = net.makeOr(ast_node, ast.Fork, "pkill", l);
	       }
	       union[k][l].connectTo(kill_list[l], net.FAN.STD);
	    }
	 }
      }
	 ...
	 //
	 for (let l = 0; l <= ast_node.depth; l++) {
	    child_min_matrix[0][l] = net.makeOr(ast_node, ast.Fork,
						"or_min_k0" + "_child" + c, l);
	    if (child_k_matrix[0] && child_k_matrix[0][l])
	       child_k_matrix[0][l].connectTo(child_min_matrix[0][l],
					       net.FAN.STD);
	 }
	 ...
	 for (let k = 1; k < max; k++) {
	    ...
	    for (let l = 0; l <= ast_node.depth; l++) {
	       child_min_matrix[k][l] = net.makeOr(ast_node, ast.Fork,
						   "or_min_k" + k + "_child" + c,
						   l);
	       child_min_matrix[k - 1][l].connectTo(child_min_matrix[k][l],
						     net.FAN.STD);
	       if (child_k_matrix[k] && child_k_matrix[k][l])
		  child_k_matrix[k][l].connectTo(child_min_matrix[k][l],
						  net.FAN.STD);
	    }
	 }
      }
	 ...
	 k_matrix[k] = [];
	 for (let l in union[k]) {
	    k_matrix[k][l] = net.makeAnd(ast_node, ast.Fork, "and_k" + k, l);
...
function makePauseCircuit( env, ast_node ) {
   ...
   reg.connectTo( and_to_reg, net.FAN.ST );
...
function makeAwaitCircuit( env, ast_node ) {
   return makeAbortCircuit( env, ast_node, makeHaltCircuit( env, ast_node ), false );
}
...
ast.Every.prototype.makeCircuit = function( env ) {
   ...
   const cloopeach = makeLoopeach( env, this, this.children[ 0 ].circuit );
...
ast.LoopEach.prototype.makeCircuit = function( env ) {
   this.children[ 0 ].makeCircuit( env );
   this.circuit = makeLoopeach( env, this, this.children[ 0 ].circuit );
...
ast.Atom.prototype.makeCircuit = function( env ) {
   ...
   for( let i = 0; i <= this.depth; i++ ) {
      resolveSignalList( env, this.accessor_list, this );
...
function makeEmitCircuit( env, ast_node ) {
   ...
   function makeEmitIfCircuit( env, ast_node ) {
	 ...
	 emit_node = makeSequenceCircuit( 
	    ast_node, ast_node.signame_list.map(
	       function( el, i, arr ) {
	 ...
	 nothing_circuit = makeNothingCircuit( if_ast );
	 if_circuit = makeIfCircuit(if_ast, [ emit_node, nothing_circuit ] );
...
ast.Sustain.prototype.makeCircuit = function( env ) {
   ...
   const cseq = makeSequenceCircuit( env, this, [ cemit, pause] );
...
ast.Halt.prototype.makeCircuit = function( env ) {
   this.circuit = makeHaltCircuit( this );
...
ast.Exec.prototype.makeCircuit = function( env ) {
   ...
   for(let l = 0; l <= this.depth; l++) {
	 ...
	 this, ast.Exec, "res", l, function() {
      ...
      signal.runtimeSignalAccessor(this, this.accessor_list, l, env);
   }

   if (this.signame) {
      this.circuit = makeSequenceCircuit(env, this, [await_node, makeEmit(this)]);
   } else {
...
ast.Run.prototype.makeCircuit = function( env ) {
   const ast_node = this.astNode;
   
   // resolve the alias signals
   this.sigDeclList.forEach( sigprop => {
      if( sigprop.alias ) {
	 sigprop.aliasOf = getSignalOject( env, sigprop.alias, ast_node );
      } 
   } );
   
   this.circuit = this.children[ 0 ].makeCircuit( this.SigDeclList );
...
ast.Local.prototype.makeCircuit = function( env ) {
   ...
   //
   // Makeeration of signal in the Local scope.  For now, Only not
   // bounded signal are created, and `accessibility` is ignored.
   //
   for( let i = this.sigDeclList.length - 1; i >= 0; i-- ) {
	 ...
	 //
	 for( let l = 0; l <= this.depth; l++ ) {
	    resolveSignalList( env, s.init_accessor_list, this );
	    ...
	    resolveSignalList( env, s.reinit_accessor_list, this );
	 ...
	 //
	 if( !sigdecl.signal ) {
	    sigdecl.signal = getSignalObject( env, sigdecl.alias, this );
	 }
      }
   }
...
function compile( machine, ast_node ) {
   ...
   ast_node.accept_auto( new ccutils.InitVisitor( machine ) );
