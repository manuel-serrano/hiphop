...
function make_signal_access(ast_node, signal) {
   ...
   for (let i in signal.emitters) {
      ...
      signal.emitters[i].ast_node.circuit.k[0].connect_to(hamlet, false)
...
function make_signal_test(ast_node, signal) {
   ...
   if (signal.emitters.length == 0 && !signal.input) {
      k0 = new net.Register(ast_node);
   } else if (signal.emitters.length == 0) {
      k0 = signal.input;
   } else {
	 ...
	 signal.emitters[i].ast_node.circuit.k[0].connect_to(k0, false)
      if (signal.input)
	 signal.input.connect_to(k0, false)
   }
   return k0;
}
...
function make_emit(ast_node) {
   ...
   ast_node.signal.add_emitter(ast_node, emit.func || emit.args.length > 0);
...
ast.Emit.prototype.make_circuit = function() {
   this.signal = get_signal(this, this.signal_name);
