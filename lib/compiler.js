...
function Interface(ast_node, type, go_list, res, susp, kill_list, sel,
   ...
   if (ast_node.dynamically_added_in_sel) {
      let reg = new net.RegisterNet(ast_node, type, "oneshot_register");
      let const0 = net.make_or(ast_node, type, "oneshot_register_reset");

      const0.connect_to(reg, net.FAN.STD);
      reg.connect_to(const0, net.FAN.DEP);
      reg.connect_to(go_list[ast_node.depth], net.FAN.STD);
      reg.oneshot = true;

      ast_node.dynamically_added_in_sel = false;
   }
}
