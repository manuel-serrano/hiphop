...
/* Generic function to connect dependencies of expressions. It return an AND
   door with all dependency connect to it (with a simulation of HAMLET).
   Return null if there is not dependencies */

function connect_expression_dependencies(ast_node) {
   var and = net.make_and(ast_node);

   if (ast_node.args && ast_node.args.length > 0) {
      for (let i in ast_node.args) {
	 let arg = ast_node.args[i];

	 if (arg instanceof lang.SignalAccessor && !arg.get_pre) {
	    let sig = get_signal(ast_node, arg.signal_name);

	    sig.connect_to(and, net.CONNECT.ALWAYS_TRUE);
	 }
      }
   }

   if (and.fanin.length == 0) {
      let machine = ast_node.machine;
      let index = machine.nets.indexOf(and);
      machine.nets.splice(index, 1);
      return null;
   }
   return and;
}

/* generic functions to get value return by an expression node at runtime */
...
function get_runtime_args(stmt) {
   ...
   for (var i in stmt.args) {
      if (stmt.args[i] instanceof lang.SignalAccessor) {
	 let accessor = stmt.args[i];
	 let sig = get_signal(stmt.ast_node, accessor.signal_name);
...
function make_if(ast_node, circuit_then, circuit_else) {
   var deps = connect_expression_dependencies(ast_node);
   var test = new net.Expr(ast_node, function() {
      this.state = this.state && get_runtime_value(this);
   });

   var and_then = net.make_and(ast_node);
   var and_else = net.make_and(ast_node);
   var go = [and_then, and_else];

   if (deps)
      deps.connect_to(test, net.CONNECT.STATE);
   else
      go.push(test);

   test.connect_to(and_then, net.CONNECT.STATE);
   test.connect_to(and_else, net.CONNECT.NEG_STATE);
   ...
   /* connect test output of GO of then and else branches */
   for (let i in circuit_then.go)
      and_then.connect_to(circuit_then.go[i], net.CONNECT.STATE)
   for (let i in circuit_else.go)
      and_else.connect_to(circuit_else.go[i], net.CONNECT.STATE);
   ...
   if (circuit_then.sel) {
	 ...
	 sel = net.make_or(ast_node);
      circuit_then.sel.connect_to(sel, net.CONNECT.STATE);
   }
   if (circuit_else.sel) {
      if (!sel)
	 sel = net.make_or(ast_node);
      circuit_else.sel.connect_to(sel, net.CONNECT.STATE);
   }
   ...
   for (let i in circuit_then.k) {
	 ...
	 k[i] = net.make_or(ast_node)
      circuit_then.k[i].connect_to(k[i], net.CONNECT.STATE);
   }
   for (let i in circuit_else.k) {
      if (!k[i])
	 k[i] = net.make_or(ast_node)
      circuit_else.k[i].connect_to(k[i], net.CONNECT.STATE);
   }
...
function make_pause(ast_node) {
   ...
   and1.connect_to(or1, net.CONNECT.STATE);
   or1.connect_to(and2, net.CONNECT.STATE);
   and2.connect_to(reg, net.CONNECT.STATE);
   reg.connect_to(and3, net.CONNECT.STATE);
   reg.connect_to(and1, net.CONNECT.STATE);
   negkill.connect_to(and2, net.CONNECT.NEG_STATE);

   // and3.action = function() {
   //    net.LogicalDoor.prototype.action.call(and3);
   //    if (this.state && this.max_incarnation > 0)
   // 	 this.machine.current_incarnation++;
   // }
...
function make_sequence(ast_node, subcircuit) {
   ...
   for (let c = 0; c < len; c++) {
	 ...
	 if (!sel)
	    sel = net.make_or(ast_node);
	 circuit.sel.connect_to(sel, net.CONNECT.STATE);
      }
	 ...
	 if (!k[i])
	    k[i] = net.make_or(ast_node);
	 circuit.k[i].connect_to(k[i], net.CONNECT.STATE);
      }

      /* connect K0 of previous stmt to GO of following stmt */
      if (c + 1 < len)
	 for (let i in subcircuit[c + 1].go)
	    circuit.k[0].connect_to(subcircuit[c + 1].go[i], net.CONNECT.STATE);
   }
...
function make_parallel(ast_node, circuit_left, circuit_right) {
   ...
   if (circuit_left.sel) {
      if (!sel)
	 sel = net.make_or(ast_node);
      circuit_left.sel.connect_to(sel, net.CONNECT.STATE);
      circuit_left.sel.connect_to(lem, net.CONNECT.STATE);
   }

   if (circuit_right.sel) {
      if (!sel)
	 sel = net.make_or(ast_node);
      circuit_right.sel.connect_to(sel, net.CONNECT.STATE);
      circuit_right.sel.connect_to(rem, net.CONNECT.STATE);
   }
   ...
   for (let i = 0; i < static_k_max; i++) {
      ...
      k[i] = k_and;
      k_or.connect_to(k_and, net.CONNECT.STATE);

      let l_or = net.make_or(ast_node);
      let r_or = net.make_or(ast_node);

      lem.connect_to(l_or, net.CONNECT.STATE)
      if (circuit_left.k[i]) {
	 circuit_left.k[i].connect_to(l_or, net.CONNECT.STATE);
	 circuit_left.k[i].connect_to(k_or, net.CONNECT.STATE);
      }

      rem.connect_to(r_or, net.CONNECT.STATE)
      if (circuit_right.k[i]) {
	 circuit_right.k[i].connect_to(r_or, net.CONNECT.STATE);
	 circuit_right.k[i].connect_to(k_or, net.CONNECT.STATE);
      }

      l_or.connect_to(k_and, net.CONNECT.STATE);
      r_or.connect_to(k_and, net.CONNECT.STATE);
...
function make_abort(ast_node, subcircuit) {
   ...
   and1.connect_to(and2, net.CONNECT.STATE);
   and1.connect_to(and3, net.CONNECT.STATE);
   and3.connect_to(or, net.CONNECT.STATE);
   if (subcircuit.sel)
      subcircuit.sel.connect_to(and1, net.CONNECT.STATE)
   subcircuit.k[0].connect_to(or, net.CONNECT.STATE);

   for (let i in subcircuit.res)
      and2.connect_to(subcircuit.res[i], net.CONNECT.STATE);
...
function make_trap(ast_node, subcircuit) {
   ...
   for (let i in subcircuit.kill)
      kill.connect_to(subcircuit.kill[i], net.CONNECT.STATE)

   subcircuit.k[0].connect_to(k[0], net.CONNECT.STATE);

   /* We make this test in case of trap statement without any exit inside it */
   if (subcircuit.k[2]) {
      subcircuit.k[2].connect_to(kill, net.CONNECT.STATE);
      subcircuit.k[2].connect_to(k[0], net.CONNECT.STATE);
   }
...
function make_loop(ast_node, subcircuit) {
   ...
   go_or.debug = "go_or loop";

   /* Always send `0` value to K0 */
   var k = [new net.Register(ast_node)];

   for (let i in subcircuit.go)
      go_or.connect_to(subcircuit.go[i], net.CONNECT.STATE);
   for (let i = 1; i < subcircuit.length; i++)
      k[i] = subcircuit.k[i];
   subcircuit.k[0].connect_to(go_or, net.CONNECT.STATE);

   // subcircuit.k[0].action = function() {
   //    this.constructor.prototype.action.call(this);
   //    if (this.machine.current_incarnation > 0)
   // 	 this.machine.current_incarnation--;
   // }
...
function make_emit(ast_node) {
   ...
   var emit = new net.Expr(ast_node, function() {
      if (!this.state)
	 return;
      if (this.func || this.args.length > 0)
	 sig.set_value(get_runtime_value(this));
   });

   if (deps) {
      deps.connect_to(emit, net.CONNECT.STATE);
      go = deps;
   } else {
      go = emit
   }
   emit.connect_to(sig, net.CONNECT.STATE);
   return new Interface(go, undefined, undefined, undefined, undefined, [emit]);
}
...
function make_atom(ast_node) {
   var go;
   var deps = connect_expression_dependencies(ast_node);
   var atom = new net.Expr(ast_node, function() {
      if (this.state && this.waiting == 0)
         this.func.apply(null, get_runtime_args(this));
   });

   if (deps) {
      deps.connect_to(atom, net.CONNECT.STATE);
      go = deps
   } else {
      go = atom;
   }
   return new Interface(go, undefined, undefined, undefined, undefined,	[atom]);
}
...
ast.Module.prototype.make_circuit = function() {
   ...
   if (first_circuit) {
	 ...
	 this.machine.go.connect_to(first_circuit.go[i], net.CONNECT.STATE)
      for (let i in first_circuit.res)
	 this.machine.res.connect_to(first_circuit.res[i], net.CONNECT.STATE);
      for (let i in first_circuit.susp)
	 this.machine.susp.connect_to(first_circuit.susp[i], net.CONNECT.STATE);
      for (let i in first_circuit.kill)
	 this.machine.kill.connect_to(first_circuit.kill[i], net.CONNECT.STATE);
   }
}
...
IncarnationVisitor.prototype.visit = function(node) {
   var instance_loop = node instanceof ast.Loop;
   var instance_local = node instanceof ast.LocalSignal;

   if (instance_loop) {
      this.loop_lvl++
   } else if (instance_local && this.loop_lvl > 0) {
      this.incar_lvl++;
   }

   node.max_incarnation = this.incar_lvl;
   ...
   if (instance_loop) {
      ...
      this.incar_lvl--;
   } else if (node instanceof ast.Pause) {
...
PrintTreeVisitor.prototype.visit = function(node) {
   ...
   else if (node instanceof ast.Pause && node.k0_on_depth)
      buf = buf + " \\./";
   console.log(buf + " [max_reincarnation: " + node.max_incarnation + "]");
