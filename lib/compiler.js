...
function make_parallel(ast_node, children_interface) {
   ...
   for (let c in children_interface) {
      ...
      /* connect all incarnation of K0 of child and sel-not of child */
      for (let l = 0; l <= ast_node.depth; l++) {
	 min[c][0][l] = net.make_or(ast_node, ast.Parallel,
				    "or_min_k0" + "_child" + c, l);
	 if (child_k_matrix[0][l])
	    child_k_matrix[0][l].connect_to(min[c][0][l], net.FAN.STD);
      }

      if (child_interface.sel)
	 child_interface.sel.connect_to(min[c][0][ast_node.depth], net.FAN.NEG);

      /* connect all incarnation of child min Kn-1 to child min Kn */
      for (let k = 1; k < child_k_matrix.length; k++) {
	 min[c][k] = [];

	 for (let l = 0; l <= ast_node.depth; l++) {
	    min[c][k][l] = net.make_or(ast_node, ast.Parallel,
				       "or_min_k" + k + "_child" + c, l);
	    min[c][k - 1][l].connect_to(min[c][k][l], net.FAN.STD);
	    if (child_k_matrix[k][l])
	       child_k_matrix[k][l].connect_to(min[c][k][l], net.FAN.STD);
	 }
      }
   }
   ...
   for (let c in min)
	 ...
	 for (let l in min[c][k]) {
	    if (!k_matrix[k][l])
	       k_matrix[k][l] = net.make_and(ast_node, ast.Parallel,
					     "and_k" + k, l)
...
function make_trap(ast_node, child_interface) {
   ...
   /* Get K2 of child, and connect it to K0 output door, and KILL input door */
   ...
   /* Propagate KILL to child KILL input */
   /* WARNING! BREAK P18 EXAMPLE1 */
   for (let l in kill_list)
      if (child_interface.kill_list[l])
...
ast.WeakAbort.prototype.make_circuit = function() {
   ...
   let abort = make_abort(this, child_interface);
   let ifstmt = make_if(this, [make_exit(this), make_nothing(this)]);
   let par = make_parallel(this, [ifstmt, abort]);

   this.cinterface = make_trap(this, par)
}
...
function get_k_list_child(ast_node, type, child_interface, k) {
   ...
   if (!k_list2)
      throw new error.InternalError("Can't use `get_k_list_child` " +
				    "with undefined K code on child.",
				    cinterface.ast_node.loc);
   if (k_list2.length > 0) {
      for (let l = 0; l <= depth1; l++) {
	 k_list1[l] = net.make_or(ast_node, type, "k" + k, l);
