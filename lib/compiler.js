...
/*    Last change :  Fri Feb  7 08:23:38 2025 (serrano)                */
...
function makeCircuitInterface(ast_node, type) {
   ...
   const go_list = makeWireList(l + 1, ast_node, ast.Pause, "go");
   const res = new net.WireNet(ast_node, ast.Pause, "res");
   const susp = new net.WireNet(ast_node, ast.Pause, "susp");
   const kill_list = makeWireList(l + 1, ast_node, ast.Pause, "kill");
   const k_matrix = [makeWireList(l + 1, ast_node, ast.Pause, "k0"),
		     makeWireList(l + 1, ast_node, ast.Pause, "k1")];
   const sel = new net.WireNet(ast_node, ast.Pause, "sel");
...
function makePauseCircuit_new(env, ast_node) {
   ...
   // K[1, i] <= GO[i]
   for (let i = 0; i <= l; i++) {
      go_list[i].connectTo(k_matrix[1][i]);
   }
...
function makeAbortNonImmediateCircuit_new(env, ast_node, childCircuit) {
   ...
   if (ast_node instanceof ast.Exec) {
      ...
      // connected only on the counter output (and not on expr test)
      const e = makeExpressionNet(env, ast_node, ast.Abort, l);
      const { cnt, reset } = makeCounterNet(env, ast_node, ast.Abort);
   ...
   for (let i = 0; i <= l; i++) {
      go_list[i].connectTo(childCircuit.go_list[i]);
      kill_list[i].connectTo(childCircuit.kill_list[i]);
   }
   susp.connectTo(childCircuit.susp);

   for (let i = 0; i < l; i++) {
      if (childCircuit.k_matrix[0][i]) {
	 childCircuit.k_matrix[0][i].connectTo(k_matrix[0][i]);
      }
   }
   for (let k = 1; k < childCircuit.k_matrix.length; k++) {
      for (let i = 0; i <= l; i++) {
	 childCircuit.k_matrix[k][i].connectTo(k_matrix[k][i]);
      }
   }
