...
/*    Last change :  Thu Apr 17 17:12:26 2025 (serrano)                */
...
function makeForkCircuit_new(env, astNode, childCircuits) {
   ...
   const mins = Array.from({length: childCircuits.length}, (_, n) =>
	 ...
	 Array.from({length: lc + 1}, (_, i) =>
	    net.makeOr(astNode, "min_k" + k + "[" + n + "]", i))));
	 ...
	 for (let i = 0; i <= lc; i++) {
	    if (s.init_func) {
		  ...
		  astNode, "init", i, function() {
	       ...
	       // MS9mar2025 to fib bug-dep-73dc062.hh.js
...
ast.Module.prototype.makeCircuit = function(env, sigtable) {
   ...
   const const0 = net.makeOr(this, "global_const.0");
...
function compile(machine, astNode) {
   ...
   if ("HIPHOP_UNROLL" in process.env) {
      machine.unrollLoops = (process.env.HIPHOP_UNROLL === "true");
   } else {
   ...
   // assertion
   if (!machine.unrollLoops && !machine.reincarnation) {
   ...
   machine.nets.forEach(n => {
	 ...
	 const const0 = net.makeOr(n.astNode, "unconnect.0", n.astNode.depth);
	 connectTo(const0, n);
      }
   });
