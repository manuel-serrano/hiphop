   ...
   for (let i = 0; i < this.depth; i++) {
	 ...
	 if (arg instanceof lang.SignalAccessor) {
	    ...
	    if (arg.get_value) {
	       ...
	       gate = arg.get_pre ? sobj.pre_gate : sobj.gate_list[i];
	    } else if (arg.get_pre) {
	 ...
	 /* todo connect / build others gates */
   }
...
ast.LocalSignal.prototype.make_circuit = function(env) {
   ...
   env.input_signal_pre_gate_map[name] = sig.pre_gate;
   env.output_signal_pre_gate_map[name] = sig.pre_gate;

   this.children[0].make_circuit(env);

   delete env.input_signal_gate_map[name];
   delete env.output_signal_gate_map[name];
   delete env.input_signal_pre_gate_map[name];
   delete env.output_signal_pre_gate_map[name];
}
...
ast.Module.prototype.make_circuit = function(env) {
   ...
   env.input_signal_pre_gate_map = {};
   env.output_signal_pre_gate_map = {};
...
function get_signal_pre_gate(env, signal_name) {
   return (env.input_signal_pre_gate_map[signal_name] ||
	   env.output_signal_pre_gate_map[signal_name]);
}
