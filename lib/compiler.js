...
function BuildTreeVisitor(ast_machine, machine) {
...
BuildTreeVisitor.prototype.visit = function(node) {
   ...
   if (node instanceof ast.Run)
      node.runnable_ast.accept_auto(new MapNameRunVisitor(node.runnable_machine,
							  node.sigs_assoc));
   ...
   if (node.exprs !== undefined)
	 ...
	 if (expr instanceof reactive.SignalAccessor) {
	    /* We must keep the caller machine reference in case of RUN
	       statement, to know on which callee signal redirect
	       the access */
	    if (!expr.machine)
...
/* This visitor is used to map signal of the caller reactive machine to
   ...
   the callee.
   Also, this visitor fill `map_signals` hashmap of the callee machine
   to allow it to redirect signal access to the caller machine */

function MapNameRunVisitor(runnable_machine, sigs_assoc) {
...
MapNameRunVisitor.prototype.visit = function(node) {
   if (node.signal_name != undefined) {
      var callee_name = node.signal_name;
      var caller_name = this.sigs_assoc[callee_name];

      if (this.sigs_assoc[callee_name] != undefined) {
	 this.runnable_machine.map_signals[callee_name] = caller_name;
	 node.signal_name = caller_name;
      }
   }
...
function compile(ast_machine, machine) {
   ...
   ast_machine.accept(new BuildTreeVisitor(ast_machine, machine));
