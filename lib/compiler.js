...
function make_signal_test(ast_node, signal) {
   var constnot = new net.Register(ast_node);
   var or = net.makeOr(ast_node);

   /* constant value of `0` in OR to propagate `0` value if not emit exists
      for this signal */
   constnot.connect_to(or, false);
   for (let i in signal.emitters)
      signal.emitters[i].ast_node.circuit.k[0].connect_to(or, false)
   return new Interface(or, undefined, undefined, undefined, undefined, [or]);
}
...
function make_present(ast_node, circuit_then, circuit_else) {
   ...
   for (let i in circuit_then.res)
      res.push(circuit_then.res[i])
   for (let i in circuit_else.res)
      res.push(circuit_else.res[i])
   for (let i in circuit_then.susp)
      susp.push(circuit_then.susp[i])
   for (let i in circuit_else.susp)
      susp.push(circuit_else.susp[i])
   for (let i in circuit_then.kill)
      kill.push(circuit_then.kill[i])
   for (let i in circuit_else.kill)
      kill.push(circuit_else.kill[i])
...
ConnectCircuitVisitor.prototype.visit = function(ast_node) {
   if (ast_node instanceof ast.Present) {
      ...
      emitters_deps.k[0].connect_to(ast_node.circuit.go[0], false);
      emitters_deps.k[0].connect_to(ast_node.circuit.go[1], true);
   }
}
