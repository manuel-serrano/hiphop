...
function make_trap(ast_node, subcircuit) {
   ...
   for (let i in subcircuit.kill)
      subcircuit.kill[i].connect_to(kill, false)
   ...
   k[1] = subcircuit.k[1];
   for (let i = 2; i < subcircuit.k.length; i++)
...
function make_exit(ast_node) {
   var wire = new net.Wire(ast_node);
   var k = [];

   k[ast_node.return_code] = wire;
   return net Interface(wire, undefined, undefined, undefined, undefined, k,
			undefined);
}
...
ast.Exit.prototype.nake_circuit = function() {
