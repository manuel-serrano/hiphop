...
function make_sequence(ast_node, children_interface) {
   ...
   /* get K0 of list children, taking account a possible superior incarnation
      level */
   k_matrix[0] = get_k_list_child(ast_node, ast.Sequence,
				  children_interface[len - 1], 0)
   // let child_k0_list = children_interface[len - 1].k_matrix[0];

   // for (let l in child_k0_list) {
   //    if (l <= ast_node.depth) {
   // 	 k_matrix[0][l] = net.make_or(ast_node, ast.Sequence, "k0_buffer", l);
   // 	 if (child_k0_list[l])
   // 	    child_k0_list[l].connect_to(k_matrix[0][l], net.FAN.STD);
   //    } else {
   // 	 child_k0_list[l].connect_to(k_matrix[0][ast_node.depth],
   // 				     net.FAN.STD);
   //    }
   // }
