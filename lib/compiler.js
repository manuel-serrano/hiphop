...
/*    Last change :  Fri Jun 13 10:01:34 2025 (serrano)                */
...
/*---------------------------------------------------------------------*/
function connectToWire(from, to) {
...
function makeForkCircuit_int(env, astNode, childCircuits) {
   ...
   for (let k = 2; k < sync.union.length; k++) {
      for (let i = sync.union[k].length - 1; i >= 0; i--) {
...
function makeLoopCircuit_dup(env, astNode, childCircuit) {
   ...
   // duplicate the child circuit
   const dupChildCircuit = childCircuit
   // c.GO[n] < GO[n], for n < l
   for (let i = 0; i < l; i++) {
      connectTo(goList[i], childCircuit.goList[i]);
   }
   ...
   const cgo = childCircuit.goList[l];
   const ck0 = childCircuit.kMatrix[0][l2];
   const nk0 = dup(netenv, ck0, cgo);
   const ngo = netenv.get(cgo.id);

   if (ngo) {
      connectTo(goList[l], ngo);
      connectToWire(const0, ngo);
   }
   connectTo(nk0, cgo);
...
function computeNodeDepth(astNode) {
   ...
   try {
	 ...
	 return computeNodeDepth_int(astNode, 0, false);
      }
   } finally {
...
function compile(machine, astNode) {
   ...
   if ("HIPHOP_UNROLL" in process.env) {
	 ...
	 machine.unrollLoop = true;
      } else if (process.env.HIPHOP_UNROLL === "false") {
	 machine.unrollLoop = false;
      } else {
	 console.error('*** WARNING: illegal HIPHOP_UNROLL value', process.env.HIPHOP_UNROLL);
	 machine.unrollLoop = true;
      }
   }
   if (machine.unrollLoop === undefined) {
      machine.unrollLoop = (compiler(astNode) === "new");
   }
   ...
   if (machine.syncReg === undefined) {
      machine.syncReg = (compiler(astNode) === "new") && !machine.syncDup;
   }

   if ("HIPHOP_SYNCDUP" in process.env) {
      if (process.env.HIPHOP_SYNCDUP === "true") {
	 machine.syncDup = true;
      } else if (process.env.HIPHOP_DYPSYNC === "false") {
	 machine.syncDup = false;
      }
   }
   if (machine.syncDup === undefined && !machine.syncReg) {
      machine.syncDup = (compiler(astNode) === "new");
   }
   ...
   // assertion
   if (!machine.unrollLoop && !machine.reincarnation && !machine.syncReg && !machine.dupLoop) {
      console.error('*** WARNING: suspicious compilation mode, use "unroll" or "reincarnation".');
   }
   if (machine.syncReg && !machine.unrollLoop && !machine.dupLoop) {
      console.error('*** WARNING: suspicious compilation mode, use "syncReg" and "!unrollLoop".');
   }
   
   if (machine.unrollLoop) {
