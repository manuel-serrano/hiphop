...
/*    Last change :  Thu Nov 30 07:02:33 2023 (serrano)                */
...
/*    dumpNets ...                                                     */
/*---------------------------------------------------------------------*/
async function dumpNets(machine) {
   const fs = await import("fs");
   const filename = machine.nets[0].ast_node.loc.filename ?? "nets.json";
   const dump = {
      sweep: machine.sweep,
      nets: machine.nets.map(net => net.dump())
   };

   fs.writeFileSync(filename + ".nets.json", JSON.stringify(dump));
}
