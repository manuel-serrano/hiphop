...
function make_parallel(ast_node, circuit_left, circuit_right) {
   ...
   res = [].res.concat(circuit_left.res).concat(circuit_right.res);
   susp = [].susp.concat(circuit_left.susp).concat(circuit_right.susp);
   kill = [].kill.concat(circuit_left.kill).concat(circuit_right.kill);
   ...
   for (let i = 0; i < static_k_max; i++) {
	 ...
	 circuit_right.k[i].connect_to(k_or, false);
      ...
      l_or.connect_to(k_and);
      r_or.connect_to(k_and);
...
ast.Parallel.prototype.make_circuit = function() {
   function _make_parallel(branches) {
      let len = branches.length;

      if (len == 1)
	 return make_parallel(this, branches[0], make_nothing(ast_node));
      else if (len == 2)
	 return make_parallel(this, branches[0], branches[1]);
      else if (len % 2 == 0)
	 return make_parallel(this,
			      _make_parallel(branches.slice(0, len / 2))
			      _make_parallel(branches.slice(len / 2 + 1, len)));
      else
	 return make_parallel(this,
			      branches[0],
			      _make_parallel(branches.slice(1, len)))
   }
