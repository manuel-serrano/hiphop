...
function Interface(ast_node, type, go_list, res, susp, kill_list, sel,
   ...
   if (kill_list && !(kill_list instanceof Array))
      throw new error.InternalError("`kill_list` must be an array.",
				    ast_node.loc);
   this.kill_list = kill_list || this.get_kill_list();
   ...
   this.res = res || this.get_res();
   this.susp = susp || this.get_susp();
   this.sel = sel;
}

Interface.prototype.get_res = function() {
   for (let i in this.ast_node.children) {
      let res_child = this.ast_node.children[i].cinterface.res;

      if (res_child) {
	 if (!this.res)
	    this.res = net.nake_or(this.ast_node, this.type, "buffer_res");
	 this.res.connect_to(res_child, net.FAN.STD);
      }
   }
...
Interface.prototype.get_susp = function() {
   console.log("get_susp_list of " + this.ast_node.constructor.name +
	       this.type);

   for (let i in this.ast_node.children) {
      let susp_child = this.ast_node.children[i].cinterface.susp;

      if (susp_child) {
	 if (!this.susp)
	    this.susp = net.make_or(this.ast_node, this.type, "buffer_susp");
	 this.susp.connect_to(susp_child, net.FAN.STD);
      }
   }
...
Interface.prototype.get_kill_list = function() {
   for (let i in this.ast_node.children) {
      let child = this.ast_node.children[i];
      let kill_list_child = child.cinterface.kill_list;

      if (!kill_list_child)
	 continue;

      for (let j = 0; j <= this.ast_node.depth; j++) {
	 let lvl = j > child.depth ? child.depth : j;
	 let kill_child = kill_list_child[lvl];

	 if (kill_child) {
	    if (!this.kill_list)
	       this.kill_list = [];
	    if (!this.kill_list[j])
	       this.kill_list[j] = net.make_or(this.ast_node, this.type,
					       "buffer_kill", j);
	    this.kill_list[i].connect_to(kill_child, net.FAN.STD);
	 }
      }
   }
}
...
ast.CountExpressionNode.prototype.make_counter = function(node,
   ...
   if (this.func_count || this.args_list_count.length > 0) {
	 ...
	 return counter == 0;
      ...
      let decr_net = new net.TestExpressionNetNet(this, node, "decr_counter",
...
ast.Sequence.prototype.make_circuit = function() {
   ...
   for (i = 0; i < len; i++) {
	 ...
	 if (!sel)
	    sel = net.make_or(this, ast.Sequence, "sel");
	 ...
	 for (let l = 0; l <= this.depth; l++) {
	    if (!seq_retcode_list[l])
	       seq_retcode_list[l] = net.make_or(this, ast.Sequence, "k" + j,
						 l);
   ...
   this.cinterface = new Interface(this, ast.Sequence,
				   this.children[0].cinterface.go_list,
...
ast.Await.prototype.make_circuit = function() {
   this.cinterface = make_abort(this, make_halt(this));
}
...
function make_loop(ast_node, child_interface) {
   ...
   return new Interface(ast_node, ast.Loop, go_list, null, null, null,
...
function make_abort(ast_node, child_interface) {
   ...
   return new Interface(ast_node, ast.Abort, go_list, and1, null, null,
			child_interface.sel, k_matrix);
}
...
ast.If.prototype.make_circuit = function() {
   ...
   this.cinterface = new Interface(this, ast.If, go_list, null, null, null, sel,
				   k_matrix);
}
...
ast.LocalSignal.prototype.make_circuit = function() {
   ...
   this.cinterface = new Interface(this, ast.LocalSignal, go_list, null, null,
				   null, child.cinterface.sel,
