...
function make_parallel(ast_node, subcircuit) {
   var len = subcircuit.length;
   var go = [];
   var res = [];
   var susp = [];
   ...
   for (let c in subcircuit) {
      let circuit = subcircuit[c];
      let sel_to_max = net.makeNor(ast_node);

      /* broadcast GO, RES, SUSP and KILL on branches */
      go = go.concat(circuit.go);
      res = res.concat(circuit.res);
      susp = susp.concat(circuit.susp);
      kill = kill.concat(circuit.kill);
	 ...
	 if (!sel)
	    sel = net.makeOr(ast_node)
	 circuit.sel.connect_to(sel, false);
	 circuit.sel.connect_to(sel_to_max, false);
      }

      /* prepare inputs of Max for this branch */
      go.push(sel_to_max)
      max_sel[c] = sel_to_max;
      max_k[c] = [];
      for (let i in circuit.k)
	 max_k[c][i] = circuit.k[i];
   }

   /* building the constructive Max circuit */
   for (let i in max_sel) {
   }
...
ast.Parallel.prototype.make_circuit = function() {
   ...
   this.circuit = make_parallel(this, subcircuit);
}
