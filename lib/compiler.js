...
/*    Last change :  Thu Nov  6 10:12:43 2025 (serrano)                */
...
import { loopError } from "./error.js";
...
function makeSynchronizer(astNode, cLength, rLevel) {
   ...
   const union = Array.from({length: kLength}, (_, k) =>
      Array.from({length: rLevel + 1}, (_, i) =>
	 net.makeOr(syncNode, "union_k" + k, i)));
   ...
   const kMatrix = Array.from({length: kLength}, (_, k) =>
      Array.from({length: rLevel + 1}, (_, i) => 
	 net.makeAnd(syncNode, "and_k" + k, i)));

   for (let k = 0; k < kLength; k++) {
      connectToList(union[k], kMatrix[k], connMode.K);
      for (let n = 0; n < cLength; n++) {
	 connectToList(mins[n][k], kMatrix[k], connMode.K);
      }
   }

   return { mins, union, kMatrix };
}
...
function makeForkCircuit_int(env, astNode, childCircuits) {
   ...
   // build and connect the global synchronizer
   const sync = makeSynchronizer(astNode, childCircuits.length, rLevel);

   // Union K of children
   childCircuits.forEach(c => {
      for (let k = 0; k < kLength; k++) {
	 connectToList(c.kMatrix[k], sync.union[k], connMode.K);
      }
   });
   ...
   // Connect K to KILL
   for (let k = 2; k < sync.union.length; k++) {
      const l = orKillList.length;
      for (let i = 0; i < sync.union[k].length; i++) {
	 connectTo(sync.union[k][i], orKillList[i < l ? i : l - 1]);
      }
   }
...
function makeForkCircuit_new(env, astNode, childCircuits) {
   ...
   for (let k = 2; k < sync.union.length; k++) {
      const l = orKillList.length;
      for (let i = l - 1; i >= 0; i--) {
	 connectTo(sync.union[k][i], orKillList[i]);
      }

      if (sync.union[k].length > l) {
	 connectTo(sync.union[k][sync.union[k].length - 1], orKillList[l - 1]);
      }
   }
...
function makeLoopCircuit_int(env, astNode, childCircuit) {
   ...
   if (l === 0) {
      const instantaneousOr = net.makeOr(astNode, "iOr", 0);
      const instantaneousAnd = net.makeOr(astNode, "iAnd", 0);
      const instantaneousXor = net.makeAnd(astNode, "iXor", 0);
      
      connectTo(goList[0], instantaneousOr);
      connectTo(childCircuit.kMatrix[0][0], instantaneousOr);
      connectTo(goList[0], instantaneousAnd);
      connectTo(childCircuit.kMatrix[0][0], instantaneousAnd);

      connectTo(instantaneousOr, instantaneousXor);
      connectTo(instantaneousAnd, instantaneousXor, net.FAN.NEG);
      
      connectTo(instantaneousXor,
		new net.ActionNet(astNode, "error", 0,
				  function() {
				     throw error.TypeError("Instantaneous loop detected.", astNode.loc) },
				  []));
   }
...
function makeLoopCircuit_new(env, astNode, childCircuit) {
   ...
   // c.GO[l] <- GO v c.k0[l]
   const go_or_k0 = net.makeOr(astNode, "go v k0", l2);
   const ngo_and_nsel = net.makeAnd(astNode, "!go ^ !sel", l2);
   const cgo = net.makeAnd(astNode, "childGo", l2);
   ...
   if (l !== 0) {
      ...
      // MS: 23 oct 2025 variant of fig 11.9 that forces a 0 to the 
      // child circuit when the loop is not executed
      // go = (go v c.k0 v c.k1) ^ !(!go ^ !c.sel)
      connectTo(goList[l], ngo_and_nsel, net.FAN.NEG);
      connectTo(childCircuit.sel, ngo_and_nsel, net.FAN.NEG);
      connectTo(ngo_and_nsel, cgo, net.FAN.NEG);

      const go_or_k0_or_k1 = net.makeOr(astNode, "go v k0 v k1", l2);
      connectTo(go_or_k0, go_or_k0_or_k1);
      connectTo(childCircuit.kMatrix[1][l2], go_or_k0_or_k1);
      connectTo(go_or_k0_or_k1, cgo);
      connectTo(cgo, childCircuit.goList[l]);
   }
   ...
   // instantaneous check (c.k0 ^ !c.sel) ^ (go v res)
   if (l === 0) {
      const instantaneousAnd = net.makeAnd(astNode, "k0 ^ !sel ^ (go v res)", 0);
      const instantaneousOr = net.makeOr(astNode, "go v res", 0);

      connectTo(goList[0], instantaneousOr);
      connectTo(res, instantaneousOr);

      connectTo(instantaneousOr, instantaneousAnd);
      
      connectTo(childCircuit.kMatrix[0][0], instantaneousAnd);
      connectTo(childCircuit.sel, instantaneousAnd, net.FAN.NEG);
      
      connectTo(instantaneousAnd,
		new net.ActionNet(astNode, "error", 0,
				  function() { loopError(astNode.loc) },
				  []));
   }
...
function compile(machine, astNode) {
   ...
   if ("HIPHOP_LOOPSAFE" in process.env) {
      machine.loopSafe = (process.env.HIPHOP_LOOPSAFE === "true");
   }
      
   if ("HIPHOP_UNROLL" in process.env) {
      if (process.env.HIPHOP_UNROLL === "true") {
	 machine.loopUnroll = true;
      } else if (process.env.HIPHOP_UNROLL === "false") {
	 machine.loopUnroll = false;
      } else {
   ...
   if ("HIPHOP_SYNCDUP" in process.env) {
      machine.syncDup = (process.env.HIPHOP_SYNCDUP === "true");
   }

   if ("HIPHOP_REINCARNATION" in process.env) {
      machine.reincarnation = (process.env.HIPHOP_REINCARNATION === "true");
   }
   if (machine.reincarnation === undefined) {
      machine.reincarnation = machine.compiler !== "new";
   }
   if ("HIPHOP_LOOPDUP" in process.env) {
      machine.loopDup = (process.env.HIPHOP_LOOPDUP === "true");
   }

   // assertion
   if (!machine.loopUnroll && !machine.reincarnation && !machine.loopDup) {
