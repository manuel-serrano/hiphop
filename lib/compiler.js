...
function make_abort(ast_node) {
   ...
   for (let i = 0; i < this.depth; i++) {
      ...
      if (ast_node.count && ast_node.signal_name) {
	 let counter = 0;
	 let decr_function = function(value) {
	    if (value)
	       counter--;
	    return counter == 0;
	 }
	 let reset_function = function(value) {
	    counter = value;
	 }
	 let reset_net = new net.ActionNet(ast_node, "test_count___" + i,
					   reset_function, [ast_node.count], i);

	 //test 
	 go_list[i].connect_to(reset_test, net.FAN.STD);
...
ast.Emit.prototype.make_circuit = function() {
   ...
   for (let i = 0; i <= this.depth; i++) {
	 ...
	 go.connect_to(new net.SignalExpressionNet(this, "signal_expr___" + i,
						   ast_node.func,
						   ast_node.args_list,
						   get_signal_object, i),
		       net.FAN.STD);
