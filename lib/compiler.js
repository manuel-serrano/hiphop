...
CheckNameVisitor.prototype.visit = function(ast_node) {
   function check_args_list(sigs, sigslocal, list) {
      for (let i = 0; i < list.length; i++) {
	 let arg  = list[i];

	 if (arg instanceof lang.SignalAccessor &&
	     sigs.indexOf(arg.signal_name) == -1 &&
	     sigslocal.indexOf(arg.signal_name) == -1) {
   ...
   if (instanceof_local_signal) {
	 ...
	 throw new error.SignalError("Local signal name already used",
				     name, ast_node.loc)
      if (this.local_stack[name])
	 throw new error.SignalError("Local signal name embedded in another" +
	 ...
	 throw new error.SignalError("Signal name already used",
				     name, ast_node.loc)
   ...
   if (ast_node.args_list) {
      check_args_list(this.signal_names, this.local_stack, ast_node.args_list);
   }

   /* Test specific for Exec statement */
   if (ast_node.start_args_list)
      check_args_list(this.signal_names, this.local_stack,
