...
/*    Last change :  Wed Nov 26 12:31:19 2025 (serrano)                */
...
function makePresentNet(env, astNode, expr, level) {
   ...
   if (expr instanceof ast.DelaySig) {
      const n = net.makeAnd(astNode, "DelaySig", level);
      ...
      const n = (expr.op === "&&")
	 ? net.makeAnd(astNode, "DelayAndBinary", level)
	 : net.makeOr(astNode, "DelayOrBinary", level);
      signal.runtimeSignalAccessor(astNode, expr.accessor_list, level, n, net.FAN.STD);

      lhs.connectTo(n, net.FAN.STD);
      rhs.connectTo(n, net.FAN.STD);
      return n;
   } else if (expr instanceof ast.DelayUnary) {
      const e = makePresentNet(env, astNode, expr.expr, level);
      const n = net.makeAnd(astNode, "DelayUnary", level);
      signal.runtimeSignalAccessor(astNode, expr.accessor_list, level, n, net.FAN.STD);
...
function makeExpressionNet(env, astNode, level) {
   ...
   if (astNode.func instanceof ast.$Delay) {
      const delay = makePresentNet(env, astNode, astNode.func, level);
      n = net.makeAnd(astNode, "delay", level);
      delay.connectTo(n, net.FAN.STD);
