...
/*    Last change :  Thu Feb 20 16:37:48 2025 (serrano)                */
...
function makeCircuitInterface(astNode, klength) {
   ...
   if (assert) {
      ...
      // cannot seal kmatrix because it is extended in makeLocalCircuit
      // Object.seal(kMatrix);
      kMatrix.forEach(r => Object.seal(r));
   }
...
/*    connMode ...                                                     */
/*---------------------------------------------------------------------*/
const connMode = { GO: 101, K: 102, FORK: 103, MERGE: 104, BRANCH: 105 };

/*---------------------------------------------------------------------*/
/*    makeUnconnectedNet ...                                           */
/*---------------------------------------------------------------------*/
function makeUnconnectedNet(net, i) {
   return new net.wire(to_list[tl - 1].astNode, `const0^${n.debugName}`, i);
}
	 ...
	 if (fl > tl) {
	    ...
	    if (fl < tl) {
		  ...
		  throw new Error(`connectToList(FORK), from list too small ${fl}/${tl}`);
	       ...
	       connectTo(from_list[fl - 1], to_list[tl], connType);
	    }
	 }
	 return;
...
function makeForkCircuit_orig(env, astNode, childCircuits) {
   ...
   for (let c = childCircuits.length - 1; c >= 0; c--) {
	 ...
	 child_min_matrix[0][i] =
	    net.makeOr(astNode, "minK0" + "vchild" + c, i);
	 if (child_kMatrix[0] && child_kMatrix[0][i]) {
      ...
      const sel_not = net.makeOr(astNode, "go/lvsel[" + c + "]");
...
function makeForkCircuit_new(env, astNode, childCircuits) {
   ...
   const { l, goList, res, susp, killList, kMatrix, sel } =
      makeCircuitInterface(astNode, kmaxlen);
   ...
   childCircuits.forEach((c, n, _) => {
      ...
      // connect OR-NOT door with GO of parallel and SEL of child
      const sel_not = net.makeOr(astNode, "go/lvsel[" + n +"]", l);

      // xMIN[0,l'] <= !SEL v GO[l]
      connectTo(goList[l], sel_not);
      connectTo(c.sel, sel_not);
      connectTo(sel_not, cmin[0][l], net.FAN.NEG);
   ...
   const akmatrix = Array.from({length: kmaxlen}, (_, k) =>
      Array.from({length: l + 1}, (_, i) => {
	 const a = net.makeAnd(astNode, "and_k" + k, i);
...
function makeEmitCircuit_new(env, astNode, signame) {
   // circuits common interface
   const { l, goList, res, susp, killList, kMatrix, sel } =
      makeCircuitInterface(astNode, 1);
