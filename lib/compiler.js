...
function Interface(go, res, susp, kill, sel, k, test_connectors) {
   if (go)
   ...
   /* test_connectors[0]: send `1` if signal (or value) is ready
      test_connectors[1]: send `1` if signal (or value) is not ready */
   if (test_connectors) {
      if (!(test_connectors instanceof Array))
	 error.InternalError("assert `test_connectors` instanceof Array failed",
			     undefined)
      if (test_connectors.length > 2)
	 error.InternalError("àssert `test_connectors` > 2 failed", undefined)
      this.test_connectors = test_connectors;
   }
}
...
/* During `make_circuit()` phase, this functions fill the array
   ast_node.signal_accessors with signal that needs to access an instruction
   using expression, in the expression */

function fill_exprs_signal_accessors(ast_node) {
   var arr = [];

   for (let i in ast_node.exprs) {
      if (ast_node.exprs[i] instanceof signal.SignalAccessor) {
	 if (!ast_node.signal_accessors)
	    ast_node.signal_accessors = [];
	 arr.push({accessor:ast_node.exprs[i],
		   signal: get_signal(ast_node,
				      ast_node.exprs[i].signal_name)})
      }
   }

   ast_node.signal_accessors = arr;
}
...
function make_atom(ast_node) {
   ...
   /* and door avoid to give `1` to aton net if it depends of signal (with the
...
ast.Atom.prototype.make_circuit = function() {
   fill_exprs_signal_accessors(this);
...
ConnectCircuitVisitor.prototype.visit = function(ast_node) {
   if (ast_node instanceof ast.Present
       || ast_node instanceof ast.Abort
       || ast_node instanceof ast.Await) {
      var emitters_deps = make_signal_test(ast_node, ast_node.signal)
      emitters_deps.connect_to(ast_node.circuit.test_connectors[0], false);
      emitters_deps.connect_to(ast_node.circuit.test_connectors[1], true);
   } else if (ast_node instanceof ast.Atom) {
      for (let i in ast_node.signal_accessors) {
	 if (!ast_node.signal_accessors[i].accessors.get_pre) {
	    let dep = make_signal_test(ast_node, ast_node.signal_accessors[i]);
	    dep.connect_to(ast_node.circuit.test_connectors[0], false);
	 }
      }
   }
}
