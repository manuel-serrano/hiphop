...
function Interface(go, res, susp, kill, sel, k, deps_connectors) {
   if (go)
   ...
   /* deps_connectors[0]: send `1` if signal (or value) is ready
      deps_connectors[1]: send `1` if signal (or value) is not ready */
   if (deps_connectors) {
      if (!(deps_connectors instanceof Array))
	 error.InternalError("assert `deps_connectors` instanceof Array failed",
			     undefined)
      if (deps_connectors.length > 2)
	 error.InternalError("assert `deps_connectors` > 2 failed", undefined)
      this.deps_connectors = deps_connectors;
   } else
      this.deps_connectors = [];
}
...
function make_signal_access(ast_node, sig) {
   var k0 = new net.Net(ast_node);

   for (let i in sig.emitters) {
      let hamlet = new net.Hamlet(ast_node);

      sig.emitters[i].circuit.k[0].connect_to(hamlet, false)
      hamlet.connect_to(k0, false);
   }
   return k0;
}

function make_signal_test(ast_node, sig) {
   var k0;

   if (sig.emitters.length == 0 && !sig.input) {
      k0 = new net.Register(ast_node);
   } else if (sig.emitters.length == 0) {
      k0 = sig.input;
   } else {
      k0 = net.make_or(ast_node);
      for (let i in sig.emitters)
	 sig.emitters[i].circuit.k[0].connect_to(k0, false)
      if (sig.input)
	 sig.input.connect_to(k0, false)
   }
   return k0;
}


/* Store depepdencies on signals for an expression. It must be called during
   the make circuit phase */

ast.ASTNode.prototype.store_dependencies = function() {
   if (this.signal_name)
      this.deps_test.push(get_signal(this, this.signal_name));

   for (let i in this.args) {
      let arg = this.args[i];

      if (arg instanceof signal.SignalAccessor && !arg.get_pre) {
	 if (arg.get_value)
	    this.deps_access.push(get_signal(this, arg.signal_nane))
	 else
	    this.deps_test.push(get_signal(this, arg.signal_name))
      }
   }
}

ast.ASTNode.prototype.has_signal_dependencies = function() {
   return this.deps_access.length > 0 || this.deps_test.length > 0;
}
...
function make_sequence(ast_node, subcircuit) {
   ...
   return new Interface(go, res, susp, kill, sel, k, undefined);
}
...
function make_abort(ast_node, subcircuit) {
   ...
   return new Interface(go, res, susp, kill, sel, k, [and3, and2]);
}
...
function make_loop(ast_node, subcircuit) {
   ...
   /* reset to get brand new reincarnation of local signal inside it */
   go_or.local_to_reincar = ast_node.local_to_reincar;
   go_or.action = function() {
      if (!this.state)
   	 return;
      net.Net.prototype.action.call(ast_node);
      for (let i in ast_node.locals_to_reincar) {
   	 let locals = ast_node.machine.local_signals;
   	 let sig = locals[ast_node.locals_to_reincar[i].signal_name];
   	 sig[ast_node.incarnation].reset(true)
      }
   }
   ...
   return new Interface(go_or, subcircuit.res, subcircuit.susp, subcircuit.kill,
			subcircuit.sel, k, undefined);
}

function make_emit(ast_node) {
   var emit;

   ast_node.signal = get_signal(ast_node, ast_node.signal_name);
   ast_node.store_dependencies();
   emit = new net.Emit(ast_node);
   ast_node.signal.add_emitter(ast_node);
...
function make_atom(ast_node) {
   ...
   /* and door avoid to give `1` to atom net if it depends of signal (with the
      expression) that is not ready */
   if (ast_node.signal_accessors) {
      go = net.make_and(ast_node);
      go.connect_to(atom, false);
      dep = [go];
      k = [atom];
   } else {
      go = atom;
   ...
   return new Interface(go, undefined, undefined, undefined, undefined, k, dep);
}
...
function instantiate_signal(ast_node, input) {
   if (ast_node.valued)
      return new signal.ValuedSignal(ast_node,
				     ...
				     ast_node.init_value,
				     ast_node.type);
   return new signal.Signal(ast_node, ast_node.signal_name, input);
}
...
function get_signal(ast_node, name) {
   ...
   sig = machine.input_signals[name]
      || machine.output_signals[name];

   if (!sig) {
      let max_incar = machine.local_signals[name].length;
      let incar_node = ast_node.incarnation;

      if (incar_node >= max_incar)
	 incar_node = max_incar - 1;
      sig = machine.local_signals[name][incar_node];
   }
   return sig;
}
...
ast.LocalSignal.prototype.make_circuit = function() {
   ...
   for (let i = 0; i < this.max_incarnation + 1; i++)
      this.machine.local_signals[this.signal_name]
      .push(instantiate_signal(this, false));
   this.children[0].make_circuit();
...
/* This visitor connect circuit. At this state, lot on circuit are already
   connected, but it remains to connect dependencies of signals to statement */

function ConnectCircuitVisitor() {}

ConnectCircuitVisitor.prototype.visit = function(ast_node) {
   if (ast_node.has_signal_dependencies()) {
      let connectors = ast_node.circuit.deps_connectors;

      for (let i in ast_node.deps_test) {
	 let sig = ast_node.deps_test[i];
	 let test = make_signal_test(ast_node, sig);

	 if (connectors[0])
	    test.connect_to(connectors[0], false);
	 if (connectors[1])
	    test.connect_to(connectors[1], true);
      }

      for (let i in ast_node.deps_access) {
	 let sig = ast_node.deps_access[i];
	 let access = make_signal_access(ast_node, sig);

	 if (connectors[0])
	    access.connect_to(connectors[0], false);
	 if (connectors[1])
	    access.connect_to(connectors[1], true);
      }
   }
}
...
PrintTreeVisitor.prototype.visit = function(node) {
   ...
   console.log(buf
		  ...
		  "max: " + node.max_incarnation
		  : "lvl: " +  node.incarnation) + "]");
...
/* Compute incarnation level of statements */
...
IncarnationVisitor.prototype.visit = function(ast_node) {
   var ast_node_instanceof_loop = ast_node instanceof ast.Loop;

   if (ast_node_instanceof_loop) {
      this.in_loop++;
      this.loop_stack.push(ast_node);
      ast_node.locals_to_reincar = [];
   } else if (ast_node instanceof ast.LocalSignal) {
      if (this.in_loop > 0) {
   	 this.lvl++;
   	 this.down_at_loop_out++;
      }
      ast_node.max_incarnation = this.lvl;
      for (let i in this.loop_stack)
    	this.loop_stack[i].locals_to_reincar.push(ast_node);
   } else if (ast_node instanceof ast.Pause && this.lvl > 0) {
      ast_node.new_incarnation_on_k0 = true;
   }

   ast_node.incarnation = this.depth;
   for (var i in ast_node.children)
      ast_node.children[i].accept(this);
