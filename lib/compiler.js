...
ast.Exec.prototype.make_circuit = function() {
   let exec_status = {
      ...
      start: false,
      signal: this.machine.input_signal_map[this.signal_name],
      exec_interface: this.exec_interface,
      ast_node: this
    };
   this.machine.exec_state_map[this.signal_name] = exec_status;

   /* make_await taking account incarnation levels */
...
ast.InputSignal.prototype.make_circuit = function() {
   ...
   sig.task_signal = this.task_signal;
...
CheckSignalTypeVisitor.prototype.visit = function(ast_node) {
   if (ast_node instanceof ast.Signal) {
	 ...
	 if (s.task_signal)
	    throw new error.SignalError("Can't emit a TaskSignal.",
					ast_node.signal_name, ast_node.loc);
      } else if (ast_node instanceof ast.Exec) {
	 let s = this.signal_map[ast_node.signal_name];

	 if (!s.task_signal)
	    throw new error.SignalError("Exec statement must use only" +
					" TaskSignal.",
					ast_node.signal_name, ast_node.loc);
	 if (this.used_task_signal[ast_node.signal_name])
	    throw new error.SignalError("TaskSignal can be used by only one" +
					" Exec statement.",
					ast_node.signal_name, ast_node.loc);
	 this.used_task_signal[ast_node.signal_name] = true;
      }
...
CheckNameVisitor.prototype.visit = function(ast_node) {
   ...
   if (instanceof_local_signal) {
	 ...
	 throw new error.SyntaxError("Local signal name embedded in another" +
      ...
      /* this case happens only for Input, Output and Task Signal */
