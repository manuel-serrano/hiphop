...
function make_loop(ast_node, child_interface) {
   ...
   child_interface.k_matrix[0][depth].connect_to(go_list[ast_node.depth],
						 net.FAN.STD);
...
function make_abort(ast_node, child_interface) {
   ...
   for (let i = 0; i <= ast_node.depth; i++) {
	 ...
	 go_list[i].connect_to(init_net, net.FAN.STD);
      ...
      go_list[i].connect_to(child_interface.go_list[i], net.FAN.STD);
   }
   ...
   /* connect K0 on depth */
   child_interface.k_matrix[0][ast_node.depth].connect_to(or, net.FAN.STD);
