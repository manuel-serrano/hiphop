...
function make_parallel(ast_node, children_interface) {
   ...
   /* max completion code of the parallel */
   let max = 0;

   for (let c in children_interface)
      if (children_interface[c].k_matrix.length - 1 > max)
	 max = children_interface[c].k_matrix.length - 1;
   console.log("max completion code:", max);
