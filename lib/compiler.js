...
/*    Last change :  Sun Dec 14 10:30:03 2025 (serrano)                */
...
/*    connMode ...                                                     */
/*---------------------------------------------------------------------*/
const connMode = { GO: 101, K: 102, KGO: 103, KK: 104, FORK: 105, MERGE: 106, BRANCH: 107, CONNECT: 108, RNCAGO: 109, RNCAKILL: 110, RNCADOWN: 111 };
...
function makeForkCircuit_new(env, astNode, childCircuits) {
   ...
   if (astNode.machine.forkOrKill || true) {
...
function makeTrapCircuit_new(env, astNode, childCircuit) {
   ...
   connectToList(childCircuit.kMatrix[0], k0_or_k2, connMode.CONNECT);
   connectToList(childCircuit.kMatrix[2], k0_or_k2, connMode.CONNECT);
   connectToList(k0_or_k2, kMatrix[0], connMode.CONNECT);
...
function makeLoopCircuit_new(env, astNode, childCircuit) {
   ...
   // c.GO[l] <- GO v c.k0[l]
   const go_or_k0 = net.makeOr(astNode, "govk0", l);
   connectTo(goList[l], go_or_k0);
...
/*    safeLoopBody ...                                                 */
/*    -------------------------------------------------------------    */
/*    A loop body is safe if it contains no fork and no local          */
/*    definition.                                                      */
/*---------------------------------------------------------------------*/
function safeLoopBody(nodes) {

   function unsafe(n) {
      if (n instanceof ast.Fork || n instanceof ast.Local) {
	 return true;
      } else if (n instanceof ast.Sequence
	 || n instanceof ast.Abort
	 || n instanceof ast.Every
	 || n instanceof ast.Trap
