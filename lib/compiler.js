...
function make_abort(ast_node) {
   ...
   let or = net.make_or(ast_node, "or_and3_kO");
   let go_list = [];
   let k_matrix = [[]];
   let test = null;
   ...
   for (let i = 0; i < this.depth; i++) {
      ...
      if (ast_node.func || ast_node.args.length > 0) {
	 ...
	 let decr_cnt_net = new net.TestExpressionNetNet(ast_node,
							 "decr_counter___" + i,
							 decr_counter_func,
							 [], i);
	 let init_counter_func = function(args) {
	    let init_val = ast_node.func.apply(null, args);
	 ...
	 let init_cnt_net = new net.ActionNet(ast_node, "init_counter___" + i,
					      init_counter_func,
					      ast_node.args, i);

	 /* decrement counter when signal is set and res is set */
	 sig_gate.connect_to(decr_net, net.FAN.STD);
	 and1.connect_to(decr_net, net.FAN.STD);
	 test = decr_net;

	 /* reset counter when GO is set */
	 go_list[i].connect_to(init_cnt_net, net.FAN.STD);
      } else if (ast_node.func || ast_node.args_list.length > 0) {
	 test = new net.TestExpressionNet(ast_node, "test_expr___" + i,
					  ast_node.func, ast_node.args_list, i);
      } else if (ast_node.signal_name) {
	 if (ast_node.test_pre)
	    test = get_signal_pre_gate(ast_node.machine, ast_node.signal_name)
	 else
	    test = get_signal_gate(ast_node.machine, ast_node.signal_name, i)
      } else {
	 throw new error.InternalError("Unexpected case.", ast_node.loc);
      }
      test.connect_to(and2, net.FAN.NEG);
      test.connect_to(and3, net.FAN.STD);
   }
   ...
   if (child.cinterface.sel)
      child.cinterface.sel.connect_to(and2, net.FAN.STD);

   res_list = child.get_res_list();
   for (let i in res_list)
      and2.connect_to(res_list[i], net.FAN.STD);

   child.cinterface.k[0][this.depth].connect_to(or, net.FAN.STD);

   k_matrix[0][this.depth] = or;
...
ast.Emit.prototype.make_circuit = function() {
   ...
   for (let i = 0; i <= this.depth; i++) {
	 ...
	 go.connect_to(new net.SignalExpressionNet(this,
						   "signal_expr___" + i,
						   i),
		       net.FAN.STD);
   }
