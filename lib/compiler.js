/*=====================================================================*/
/*    serrano/prgm/project/hiphop/1.3.x/lib/compiler.js                */
/*    -------------------------------------------------------------    */
/*    Author      :  Colin Vidal, Manuel Serrano                       */
/*    Creation    :  Mon Jul 16 18:06:28 2018                          */
<<<<<<< HEAD
/*    Last change :  Tue Jan 30 08:09:35 2024 (serrano)                */
=======
/*    Last change :  Tue Jan 30 11:57:20 2024 (serrano)                */
>>>>>>> c7c0eca249f06524b86e53be9c1a315e475b6ac2
...
const fs = config.isServer ? await import("fs") : {};
=======
//import * as causality from "./bourdoncle.js";
>>>>>>> c7c0eca249f06524b86e53be9c1a315e475b6ac2
...
function makeCounterNet(env, ast_node, type,
   ...
   ///*
   let count_counter_func = function() {
      return counter;
   }
   //*/
   ...
   ///*           
   init_net.decrCounter = res;
   res.initCounter = init_net; // for verification
   res.countFunc = count_counter_func; // for verification
   try{
...
function makeSuspendCircuit(env, ast_node, childCircuit) {
   ...
   for (let k = 0; k < childCircuit.k_matrix.length; k++) {
      k_matrix[k] = ccutils.getKListChild(ast_node, ast.Suspend, childCircuit, k);
   }
...
function makeAbortCircuit(env, ast_node, childCircuit, force_non_immediate) {
   ...
   function makeAbortInner(env, ast_node, childCircuit) {
	 ...
	 k_matrix[0][l] = childCircuit.k_matrix[0][l];
      ...
      for (let k = 1; k < childCircuit.k_matrix.length; k++)
	 k_matrix[k] = ccutils.getKListChild(ast_node, ast.Abort, childCircuit, k);
...
ast.Module.prototype.makeCircuit = function(env) {
   ...
   for (let i in this.sigDeclList) {
	 ...
	 null, null, null);
	  ...
	  sigdecl.accessibility == ast.INOUT) {
...
function compile(machine, ast_node) {
   ...
   if (config.isServer && machine.dumpNets) {
      dumpNets(machine, false, ".nets-.json");
   }
   if (machine.sweep) {
      sweep.sweep(machine);
      if (config.isServer && machine.dumpNets) {
...
/*    (see machine.js).                                                */
/*---------------------------------------------------------------------*/
function dumpNets(machine, sweep, suffix) {
