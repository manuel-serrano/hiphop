...
ast.CountExpressionNode.prototype.make_counter = function(type,
   ...
   if (this.func_count || this.args_list_count.length > 0) {
	 ...
	 if (init_val < 1)
	    error.RuntimeError("Assert counter expression > 0 failed.", loc);
...
function _make_abort(ast_node, child_interface) {
   ...
   } else if (ast_node.func_count || ast_node.args_list_count.length > 0) {
      let decr_list = [];
...
function make_emit(ast_node) {
   ...
   for (let i = 0; i <= ast_node.depth; i++) {
	 ...
	 go.connect_to(new net.ActionNet(ast_node, ast.Emit, "_exec_return_sig",
		       ...
		       net.FAN.STD);
      } else if (ast_node.func || ast_node.args_list.length > 0) {
...
ast.Exec.prototype.make_circuit = function() {
   let exec_status = {
      ...
      start: false,
      exec_interface: this.exec_interface,
      ast_node: this,
...
CheckSignalTypeVisitor.prototype.visit = function(ast_node) {
   if (ast_node instanceof ast.Signal) {
	 ...
	 if ((ast_node.args_list.length > 0 || ast_node.func) && !s.valued)
	 ...
	 for (let i in ast_node.args_list) {
		...
		arg.get_value &&
		!this.signal_map[arg.signal_name].valued)
...
CheckNameVisitor.prototype.visit = function(ast_node) {
   function check_args_list(sigs, sigslocal, sigslocalremaned, list) {
      for (let i = 0; i < list.length; i++) {
	 let arg  = list[i];

	 if (arg instanceof lang.SignalAccessor) {
	    let local_id = sigslocal.lastIndexOf(arg.signal_name);

	    if (local_id > -1) {
	       arg.signal_name = sigslocalremaned[local_id];
	    } else if (sigslocalremaned.indexOf(arg.signal_name) == -1 &&
		       sigs.indexOf(arg.signal_name) == -1) {
   ...
   if (ast_node.args_list) {
      check_args_list(this.signal_names,
		      this.local_stack,
		      this.local_stack_renamed,
		      ast_node.args_list);
   }
   ...
   if (ast_node.start_args_list)
      check_args_list(this.signal_names,
		      this.local_stack,
		      this.local_stack_renamed,
		      ast_node.start_args_list);
