...
function make_present(ast_node, circuit_then, circuit_else) {
   ...
   sig.connect_to(and_else, net.CONNECT.NEG_STATE);
...
/* TODO
   Make a causal dependency between any emit of this local signal (in this
   scope, it canno't be outside this scope). We have to do this to force the
   scheduling of emit statement *after* the reset of the signal (in case or
   there is a selected pause and an emit after). If we don't do it, we run into
   a causality exception...

   (00:01) Anyways, it still a bug...

   +-- MODULE  [parent: N/A]
   +-- OUTPUTSIGNAL O  [parent: MODULE]
   +-- LOOP  [parent: MODULE]
      +-- LOCALSIGNAL S  [parent: LOOP]
         +-- SEQUENCE  [parent: LOCALSIGNAL]
            +-- PRESENT S  [parent: SEQUENCE]
               +-- EMIT O  [parent: PRESENT]
               +-- NOTHING  [parent: PRESENT]
            +-- PAUSE  [parent: SEQUENCE]
            +-- EMIT S  [parent: SEQUENCE]

    When PAUSE resumes, EMIT S will sent 1 in dependency of PRESENT, so when
    GO of sequence will propage to present, it will take the THEN branch, even
    if the signal is fully resetted...
*/
...
ast.LocalSignal.prototype.make_circuit = function() {
   ...
   for (let i in this.circuit.go)
      reset_signal.connect_to(this.circuit.go[i], net.CONNECT.STATE)

   reset_signal.action = function() {
      if (!this.state)
	 return;
      let s = get_signal(this.ast_node, this.ast_node.signal_name).reset(true);
   }
...
ast.OutputSignal.prototype.make_circuit = function() {
   this.machine.output_signals[this.signal_name] =
      instantiate_signal(this, net.CONNECT.STATE);
}
