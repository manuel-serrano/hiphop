...
function compute_register_id(ast_node, depth, seq) {
   ast_node.instr_depth = depth;
   ...
   seq = 0;
   for (let i in ast_node.children)
      compute_register_id(ast_node.children[i], depth, seq++);
}
...
function compiler_format_pretty_print(indent, ast_node, sel) {
   ...
   let instruction = ast_node.constructor.name + ast_node.pretty_print_param();
	 ...
	 buf +=  instruction + "*";
      	 ...
      	 buf += instruction;
      buf += "   " + ast_node.instr_depth + ast_node.instr_seq + "\n";
      return buf;
   }
...
function compile(machine, ast_node) {
   ...
   compute_register_id(ast_node, 0, 0);
