...
function _make_abort(ast_node, child_interface) {
   ...
   if (ast_node.func_count || ast_node.args_list_count.length > 0) {
      ...
      decr_list[0] = ast_node.make_expression(ast.Abort, [and1], [], [],
					      ast_node.depth);
      decr_list[1] = and1;
      ast_node.make_counter(ast.Abort, decr_list, child_interface.go_list,
			    [and3], [and2]);
   } else {
   ...
   return new Interface(ast_node, ast.Abort, child_interface.go_list, and1,
			child_interface.susp, child_interface.kill_list,
			child_interface.sel, k_matrix);
}
...
/* TODO: can be simplified (see p.150, and Suspend above, expr test
   only in the depth)
   TOTO2: rezrite code for counter creation. */
...
//    let go_list = [];
//    let k_matrix = [[]];
//    let depth_test_expr_list = [];

//    let has_counter = ast_node.func_count || ast_node.args_list_count.length > 0;
...
//       if (has_counter)
// 	 test_expr = ast_node.make_expression(ast.Abort, [and1], [], [], i);
//       else
// 	 ast_node.make_expression(ast.Abort, [and1], [and3], [and2], i);

//       /* keeps test_expr of depth to connect it to counter if exists */
//       if (i > 0)
// 	 depth_test_expr_list.push(test_expr);
//    }

//    depth_test_expr_list.push(and1);
//    ast_node.make_counter(ast.Abort, depth_test_expr_list, go_list,
// 			 [and3], [and2]);
...
RenameLocalSignalVisitor.prototype.visit = function(ast_node) {
   ...
   if (ast_node instanceof ast.LocalSignal && !ast_node.renamed) {
