...
function make_if(ast_node, circuit_then, circuit_else) {
   ...
   for (let i in circuit_then.go)
      test.connect_to(circuit_then.go[i], net.CONNECT.STATE)
   for (let i in circuit_else.go)
      test.connect_to(circuit_else.go[i], net.CONNECT.NEG_STATE);
   ...
   return new Interface(deps, res, susp, kill, sel, k);
}
