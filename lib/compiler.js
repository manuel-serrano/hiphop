...
/*    Last change :  Wed Oct  8 17:02:33 2025 (serrano)                */
...
function makeForkCircuit_new(env, astNode, childCircuits) {
   ...
   childCircuits.forEach((c, n, _) => {
	 ...
	 const andReg = net.makeAnd(astNode, "syncreg[" + n + "]", l);
...
/*---------------------------------------------------------------------*/
function makeLoopCircuit_sans_dup(env, astNode, childCircuit) {
   // circuits common interface
   const { l, goList, res, susp, killList, kMatrix, sel } =
      makeCircuitInterface(astNode);
   ...
   // instantaenous check (GO vxor k0)
   if (l === 0) {
      const instantaneousAnd = net.makeAnd(astNode, "iAnd", 0);
/*       const instantaneousOr = net.makeAnd(astNode, "iOr", 0);       */
...
/*       connectTo(instantaneousOr, instantaneousAnd);                 */
      ...
      connectTo(instantaneousAnd,
		new net.ActionNet(astNode, "error", 0,
				  function() {
				     throw error.TypeError(`Instantaneous loop detected ${astNode.loc}.`, astNode.loc) },
				  []));
/* 				  []));                                */
...
function makeLoopCircuit(env, astNode, childCircuit) {
   if (astNode.machine.loopDup) {
      ...
      return makeLoopCircuit_sans_dup(env, astNode, childCircuit);
   }
}
...
function compile(machine, astNode) {
   ...
   if ("HIPHOP_SYNCREG" in process.env) {
      machine.syncReg = (process.env.HIPHOP_SYNCREG === "true");
   }
   ...
   // assertion
   if (!machine.loopUnroll && !machine.reincarnation && !machine.syncReg && !machine.loopDup) {
      console.error('*** WARNING: suspicious compilation mode, use "unroll" or "reincarnation".');
   }
   if (machine.syncReg && !machine.loopUnroll) {
      console.error('*** WARNING: suspicious compilation mode, use "syncReg" and "!loopUnroll".');
   }
