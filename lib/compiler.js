...
function make_parallel(ast_node, children_interface) {
   ...
   kill_list = killListChildren(ast_node, ast.Parallel, children_interface);
...
function make_sequence(ast_node, children_interface) {
   ...
   for (i = 0; i < len; i++) {
	 ...
	 for (let l in kList) {
	    if (!k_matrix[j][l]) {
	       k_matrix[j][l] = net.make_or(ast_node, ast.Sequence, "buf_kil", l);
	    }
...
function make_suspend(ast_node, child_interface) {
   ...
   let kill_list = killListChildren(ast_node, ast.Suspend, [child_interface]);

   return new Interface(ast_node, ast.Suspend, child_interface.go_list,
			and1, or1, kill_list, child_interface.sel, k_matrix);
}
   ...
   for (let c in children_interface) {
	 ...
	 if (childDepth > ast_node.depth) {
	    if (childDepth != ast_node.depth + 1) {
	       throw new error.InternalError("killListChildren: level error");
	    }
