...
/*    Last change :  Fri Sep 28 16:02:07 2018 (serrano)                */
...
function _make_emit( ast_node, signal_name ) {
   ...
   for (let i = 0; i <= ast_node.depth; i++) {
	 ...
	 let exec_emission_func = function() {
	    sig.set_value( ast_node.exec_status.value, 0, ast_node.loc )
...
ast.Run.prototype.make_circuit = function() {
   // MS 28sep1028, start a fresh signal environment for the module
   const oenv = this.machine.cc_local_signal_stack;
   this.machine.cc_local_signal_stack = [];
   
   this.children[0].make_circuit();
   this.cinterface = this.children[0].cinterface;
   this.push_oneshot_register();
   
   // MS 28sep1028, restore the lexical environment
   this.machine.cc_local_signal_stack = oenv;
}
...
ast.Module.prototype.make_circuit = function() {
   ...
   //const oenv = this.cc_local_signal_stack;
