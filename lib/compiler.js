...
/*    Last change :  Fri Feb 14 18:15:05 2025 (serrano)                */
   ...
   if (astNode instanceof ast.Loop) {
	 ...
	 throw new Error("unrollLoops: bad formed loop");
      ...
      const seq = new ast.Sequence("SEQUENCE", null, astNode.loc, ast.nodebug,
				   [ body, body.clone() ]);
      const loop = new ast.Loop("LOOP", astNode.loc, ast.nodebug, seq);
      loop.machine = seq.machine = astNode.machine;
      loop.machine= seq.depth = 0;
      return seq;
   } else {
      return astNode;
   }
...
function compile(machine, astNode) {
   ...
   if (!(machine.unrollLoops || newCompiler)) {
