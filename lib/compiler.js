...
function make_abort(ast_node) {
   ...
   for (let i = 0; i < ast_node.depth; i++) {
      ...
      /* gate for `test` expression (evaluated each tine RES=1) */
      if (ast_node.func || ast_node.args_list.length > 0) {
	 test = new net.TestExpressionNet(ast_node, "test___" + i,
					  ast_node.func, ast_node.args_list, i);
      }

      /* gates for initialization of count (evaluated each time GO=1),
	 and decrementation of count (each time RES=1 and test=1) */
	 ...
	 test.connect_to(decr_net, net.FAN.STD);
