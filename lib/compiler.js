...
/*    Last change :  Thu Feb 20 18:18:16 2025 (serrano)                */
...
function traceNode(astNode) {
   if (debug) {
      return `${astNode.constructor.name}[${astNode.depth}:${astNode.returnCode}]@${astNode.loc.pos}`
   }
}
...
function makeCircuitInterface(astNode) {
   ...
   const kMatrix = makeKMatrix(l + 1, astNode, astNode.returnCode + 1);
...
ast.Fork.prototype.makeCircuit = function(env, sigtable) {
   ...
   traceExit(traceNode(this), this.circuit.kMatrix.length,
...
function makeForkCircuit_new(env, astNode, childCircuits) {
   ...
   const kmaxlen = astNode.returnCode + 1;
...
function makeTrapCircuit_new(env, astNode, childCircuit) {
   ...
   if (childCircuit.kMatrix.length <= 2) {
	 ...
	 net.makeOr(astNode, "k0vk2", i));
      ...
      // Ki < c.Ki-1
      for (let k = 2; k < kMatrix.length; k++) {
	 connectToList(childCircuit.kMatrix[k + 1], kMatrix[k]);
      }
...
function makePauseCircuit_orig(env, ast_node) {
   return makePauseCircuit_colin(env, ast_node);
}
...
function makeNothingCircuit_new(env, astNode) {
   ...
   const { l, goList, res, susp, killList, kMatrix, sel } =
      makeCircuitInterface(astNode);
   const const0 = net.makeOr(astNode, "const0_k1", 0);
...
ast.Module.prototype.makeCircuit = function(env, sigtable) {
   traceEnter(`${this.constructor.name}[${this.depth}:${this.returnCode}]@${this.loc.pos}...`);
