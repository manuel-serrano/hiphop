...
function Interface(ast_node, go_list, res, susp, kill_list, sel, k_matrix) {
   if (!(go_list instanceof Array))
      throw new error.InternalError("`go_list` must be an array.", ast_node.loc);
   ...
   if (!(k_matrix instanceof Array))
      throw new error.InternalError("`k_matrix must be a matrix.", ast_node.loc);
...
ast.Signal.prototype.make_circuit = function(env) {
   ...
   if (this.valued)
      sig = new signal.ValuedSignal(this.signal_name, gate_list,
				    this.combine_with, this.init_value)
   else
      sig = new signal.Signal(this.signal_name, gate_list);
   return sig;
}
...
ast.Module.prototype.make_circuit = function(env) {
   ...
   console.log(child.cinterface.k_matrix)
