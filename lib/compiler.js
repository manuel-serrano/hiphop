   ...
   for (let c in children_interface) {
	 ...
	 if (!union[k]) {
	    ...
	    for (let i = 0; i <= ast_node.depth; i++) {
	       union[k][i] = net.make_or(ast_node, ast.Parallel, "union_k" + k, i);
	    }
	 }
	 let kList = get_k_list_child(ast_node, ast.Parallel, iChild, k);
	 for (let i = 0; i <= ast_node.depth; i++) {
	    if (kList[i]) {
...
ast.Local.prototype.make_circuit = function() {
   ...
   for (let i = 0; i <= this.depth; i++) {
      k_matrix[1][i] = net.make_or(this, ast.Local, "k1_buf", i);
      kill_list[i] = net.make_or(this, ast.Local, "kill_buf", i);
   ...
   for (let i in this.signal_declaration_list) {
	 ...
	 for (let l = 0; l <= this.depth; l++) {
	    let action_init = new net.SignalInitNet(
	       this, ast.Local, s, "init", l, function() {
	    ...
	    let action_reinit = new net.SignalInitNet(
	       this, ast.Local, s, "reinit", l, function() {
   ...
   for (let i in kill_list) {
      let childKillList = child.cinterface.kill_list;
      if (childKillList && childKillList[i]) {
	 kill_list[i].connect_to(childKillList[i], net.FAN.STD);
      }
   }
   ...
   //
   // Environment translation of KILL wires
   //
   kill_list[this.depth - 1].connect_to(kill_list[this.depth], net.FAN.STD);
