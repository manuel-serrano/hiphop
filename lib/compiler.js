...
/*    Last change :  Wed Feb 12 15:37:09 2025 (serrano)                */
...
function makeForkCircuit_new(env, astNode, childCircuits) {
   ...
   // c.RES <- RES
   childCircuits.forEach(c => res.connectTo(c.res));
   
   // c.SUSP <- SUSP
   childCircuits.forEach(c => susp.connectTo(c.susp));
   ...
   childCircuits.forEach(c => c.sel.connectTo(or_sel));
   or_sel.connectTo(sel);
   ...
   childCircuits.forEach((c, n, _) => {
      ...
      goList[l].connectTo(sel_not);
      c.sel.connectTo(sel_not);
      sel_not.connectTo(child_min_matrix[0][l], net.FAN.NEG);

      // connect all incarnations of child min Kn-1 to child min Kn
      for (let k = 1; k < kmax; k++) {
	 for (let i = 0; i <= l; i++) {
	    child_min_matrix[k - 1][i].connectTo(child_min_matrix[k][i]);
	    c.kMatrix[k][i].connectTo(child_min_matrix[k][i]);
	 }
      }      
   });
...
function makeAbortNonImmediateCircuit_new(env, astNode, childCircuit) {
   ...
   if (astNode instanceof ast.Exec) {
      ...
      res_and_sel.connectTo(e);
      
      const { cnt, reset } = makeCounterNet(env, astNode);
      goList.forEach(n => n.connectTo(reset));
      e.connectTo(cnt);
      res_and_sel.connect(cnt);
      s = cnt;
   } else {
...
function makeIfCircuit_new(env, astNode, childCircuits) {
   ...
   // circuits common interface
   const { l, goList, res, susp, killList, kMatrix, sel } =
      makeCircuitInterface(astNode, childCircuits[0].kMatrix.length);
   ...
   for (let i = 0; i <= l; i++) {
      ...
      goList[i].connectTo(go_and_s);
      goList[i].connectTo(go_and_non_s);

      s.connectTo(go_and_s, astNode.not ? net.FAN.NEG : net.FAN.STD);
      s.connectTo(go_and_non_s, astNode.not ? net.FAN.STD : net.FAN.NEG);

      go_and_s.connectTo(childCircuits[0].goList[i]);
      go_and_non_s.connectTo(childCircuits[1].goList[i]);
   }
...
function makeLocalCircuit_new(env, astNode, sigtable) {
   ...
   astNode.sigDeclList.forEach(sigprop => {
	 ...
	 for (let i = 0; i <= l; i++) {
	    ...
	    goList[i].connectTo(action_init);
	    initList.push(action_init);
