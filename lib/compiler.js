...
function make_parallel(ast_node, children_interface) {
   ...
   for (let c in children_interface) {
      ...
      for (let k in child_interface.k_matrix) {
	 let k_lvl = child_interface.k_matrix[k];
	 ...
	 if (k_lvl.length > 0) {
	    for (let l = 0; l <= ast_node.depth; l++) {
	       if (!union[k][l])
		  union[k][l] = net.make_or(ast_node, ast.Parallel,
					    "union_k" + k, l);
	       k_lvl[l].connect_to(union[k][l], net.FAN.STD);
	    }

	    if (k_lvl.length > ast_node.depth + 1) {
	       console.log("yes");
	       for (let l = ast_node.depth + 1; l < k_lvl.length; l++)
		  k_lvl[l].connect_to(union[k][ast_node.depth], net.FAN.STD);
	    }
	 }
      }
   }
   ...
   for (let c in children_interface) {
	 ...
	 for (let l = 0; l <= ast_node.depth; l++) {
	    if (k_lvl[l]) {
	       let or = net.make_or(ast_node, ast.Parallel, "or" + c, l);

	       k_lvl[l].connect_to(or, net.FAN.STD);
	       min[c][k][l] = or;

	       if (k == 0) {
		  let orneg = net.make_or(ast_node, ast.Parallel, "orneg" + c,
					  l);
		  if (child_interface.sel)
		     child_interface.sel.connect_to(orneg, net.FAN.STD);
		  go_list[ast_node.depth].connect_to(orneg, net.FAN.STD);
		  orneg.connect_to(or, net.FAN.NEG);
	       } else if (min[c][k - 1][l]) {
		  min[c][k - 1][l].connect_to(or, net.FAN.STD);
	       }
	    }
	 }
      }
...
function make_trap(ast_node, child_interface) {
   ...
   for (let l = 0; l <= ast_node.depth; l++) {
      ...
      kill_list[l] = net.make_or(ast_node, ast.Trap, "or_kill", l);

      if (child_interface.kill_list && child_interface.kill_list[l])
	 kill_list[l].connect_to(child_interface.kill_list[l], net.FAN.STD);

      if (k_matrix_child[0][l])
	 k_matrix_child[0][l].connect_to(k0_list[l], net.FAN.STD);

      if (k_matrix_child[2] && k_matrix_child[2][l]) {
	 let k2 = k_matrix_child[2][l];

	 k2.connect_to(k0_list[l], net.FAN.STD);
	 k2.connect_to(kill_list[l], net.FAN.STD);
      }
   }

   for (let k = 3; k < k_matrix_child.length; k++)
      k_matrix[k - 1] = k_matrix_child[k];
...
function make_sequence(ast_node, children_interface) {
   ...
   k_matrix[0] = children_interface[len - 1].k_matrix[0];
...
function make_loop(ast_node, child_interface) {
   ...
   for (let k = 1; k < child_interface.k_matrix.length; k++)
      connect_k_min_incarnation(ast_node, ast.Loop, k_matrix,
				child_interface, k);
...
ast.LocalSignal.prototype.make_circuit = function() {
   ...
   for (let k in child.cinterface.k_matrix)
      connect_k_min_incarnation(this, ast.LocalSignal, k_matrix,
				child.cinterface, k);
...
/* Helper function to connect a return code from a substatement to a statement
   using min(m, n) incarnation indice (useful for local, parallel and loop).
   The parallel actually didn't use this function because of the coding
   of the parallel. If could be a good thing to work to make it use this */
function connect_k_min_incarnation(ast_node, type, k_matrix1,
				   child_interface, k) {
   let depth1 = ast_node.depth;
   let depth2 = child_interface.ast_node.depth;
   let k_matrix2 = child_interface.k_matrix;

   if (!k_matrix2[k])
      throw new error.InternalError("Can't use `connect_k_min_incarnation` " +
				    "with undefined K code on child.",
				    cinterface.ast_node.loc);
   if (!k_matrix1[k])
      k_matrix1[k] = [];
   if (k_matrix2[k].length == 0)
      return;
   for (let l = 0; l <= depth1; l++) {
      if (!k_matrix1[k][l])
	 k_matrix1[k][l] = net.make_or(ast_node, type, "or_k" + k, l);
      k_matrix2[k][l].connect_to(k_matrix1[k][l], net.FAN.STD);
   }

   /* The loop is probably useless: it should make only one iteration... */
   for (let l = ast_node.depth + 1; l <= depth2; l++)
      k_matrix2[k][l].connect_to(k_matrix1[k][depth1], net.FAN.STD);
}
...
function get_kill_list_children(ast_node, type, children_interface) {
   ...
   for (let i = 0; i <= ast_node.depth; i++)
	 ...
	 let child = children_interface[j];
