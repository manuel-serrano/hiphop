...
ast.Pause.prototype.make_circuit = function(env) {
   ...
   reg.connect_to(and_reg_res, net.FAN.STD);
   reg.connect_to(and_susp_sel, net.FAN.STD);
...
ast.Emit.prototype.make_circuit = function(env) {
   ...
   for (let i = 0; i < this.depth; i++) {
      let go = net.make_or(this, "go___" + i);
      let sig_gate = env.get_signal_gate(env, this.signal_name, i);

      go.connect_to(sig_gate, net.FAN.STD);
      go_list[i] = go;
   }

   this.cinterface = new Interface(go_list, null, null, null, null, k_matrix);
}

ast.LocalSignal.prototype.make_circuit = function(env) {
   let sig = Signal.prototype.make_circuit.call(this, env);
...
ast.InputSignal.prototype.make_circuit = function(env) {
   ...
   env.input_signal_gate_map[name] = sig.gate_list;
}
...
ast.OutputSignal.prototype.make_circuit = function(env) {
   ...
   env.output_signal_gate_map[name] = sig.gate_list;
}
...
ast.Signal.prototype.make_circuit = function(env) {
   ...
   for (let i in this.depth)
      gate_list[i] = net.make_or(this, this.signal_name);

   if (this.valued)
      sig = new signal.ValuedSignal(this.signal_name, this.gate_list,
				    this.combine_with, this.init_value)
   else
      sig = new signal.Signal(this.signal_name, this.gate_list);
   return sig;
}

ast.Module.prototype.make_circuit = function(env) {
   let boot_reg = new net.Register(this, "global_boot_register");
   let const0 = net.make_or(this, "global_const0");

   const0.connect_to(boot_register, net.FAN.STD);

   /* build environment */
   env.input_signal_gate_map = {};
   env.output_signal_gate_map = {};
   ...
   boot_reg.connect_to(child.cinterface.go[0], net.FAN.STD);
   ...
   this.machine.k0 = child.cinterface.k[0][0];
   this.machine.k1 = child.cinterface.k[1][0];
}

/* Helper function that give a signal at a specific incarnation level */

function get_signal_gate(env, signal_name, lvl) {
   let gate_list = (env.input_signal_gate_map[signal_name] ||
		    env.output_signal_gate_map[signal_name]);
