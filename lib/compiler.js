...
function compile( machine, ast_node ) {
   ...
   let { loc, size, signals } = causality.findCausalityErrorDeep( machine );
   if( size >=0 ) {
      console.log( `Warning: causality cycle of length ${size} detected, involving signals ${signals.toString() }`, loc );
   }
}
