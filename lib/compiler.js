...
/*    Last change :  Mon Feb 10 07:39:24 2025 (serrano)                */
...
function assertCircuitInterfaceConnections(c) {
   if (assert) {
	 ...
	 if (w.fanoutList.length === 0)
	    throw new Error(`Bad "kill[${i}]" connection for ${c.astNode.constructor.name}`);
      });
	 ...
	 if (w.faninList.length === 0)
	    throw new Error(`Bad "kMatrix[${i}][${j}]" for ${c.astNode.constructor.name}`);
      }));
   }
   return c;
}
...
/*---------------------------------------------------------------------*/
function makeCircuitInterface(astNode, klength = 2) {
...
function makeExpressionNet(env, astNode, level) {
   ...
   if (astNode.func || astNode.accessor_list.length > 0) {
      ...
      res = new net.TestExpressionNet(astNode, "testexpr", level,
...
ast.Trap.prototype.makeCircuit = function(env, sigtable) {
   ...
   function makeTrapCircuit(env, astNode, child_interface) {
	 ...
	 kMatrix[k - 1] = ccutils.getKListChild(astNode, ast.Trap,
      ...
      return new Circuit(astNode,
			 child_interface.goList,
			 child_interface.res,
			 child_interface.susp,
			 ccutils.killListChildren(astNode, ast.Trap, [child_interface]),
			 child_interface.sel,
			 kMatrix);
   }
...
ast.Exit.prototype.makeCircuit = function(env, sigtable) {
   ...
   function makeExitCircuit(env, astNode) {
	 ...
	 kMatrix[astNode.returnCode][i] = go;
      ...
      return new Circuit(astNode, goList,
			 null, null, null, null,
			 kMatrix);
   }
...
function makeSequenceCircuit_orig(env, astNode, childCircuits) {
   ...
   return new Circuit(astNode, childCircuits[0].goList,
		      res, susp, killList, sel, kMatrix);
}
...
function makeSequenceCircuit_new(env, astNode, childCircuits) {
   ...
   // k n > 0 connections
   for (let j = 1; j < kMatrix.length; j++) {
	 const kj_list = Array.from({length: l}, (_, i) =>
	    net.makeOr(astNode, `k${j}/${i}`));

      childCircuits.forEach(c => connectToList(c.kMatrix[j], kj_list));
      connectToList(kj_list, kMatrix[j]);
   }
...
function makeLoopCircuit_orig(env, astNode, childCircuit) {
   ...
   for (let l = 0; l <= depth2; l++) {
      if (childCircuit.kMatrix[0][l] && (depth2 == 0 || l < depth2))
   ...
   if (childCircuit.kMatrix[0][depth2]) {
...
function makeLoopCircuit_new(env, astNode, childCircuit) {
   ...
   kMatrix[0].forEach(k0 => const0.connectTo(k0));
   // Kn <- c.Kn
   for (let k = 1; k < childCircuit.kMatrix.length; k++) {
...
function makeIfCircuit(env, astNode, childCircuits) {
   ...
   for (let i = 0; i <= astNode.depth; i++) {
      ...
      if (astNode.not) {
 	 const e = makeExpressionNet(env, astNode, i);
	 go.connectTo(e);
