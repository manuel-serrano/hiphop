...
/*    Last change :  Sun Feb 16 14:20:39 2025 (serrano)                */
...
/*    unrollChild ...                                                  */
/*---------------------------------------------------------------------*/
function unrollChild(astNode, child) {
   if (astNode.depth === 0) {
      // depth === 0 <=> unroll required
      console.log("UNROLL CH=", child.machine === null, child.loc, child.constructor.name);
      const clone = child.clone();
      console.log("       CL=", clone.machine === null, clone.loc);
      const seq = new ast.Sequence("SEQUENCE", null,
				   astNode.loc, ast.nodebug,
				   [ child, clone ]);
      seq.machine = astNode.machine;
      return seq;
   } else {
      return child;
   }
...
ast.Loop.prototype.makeCircuit = function(env, sigtable) {
   ...
   const cc0 = unrollChild(this, this.children[0]).makeCircuit(env, sigtable);
...
function unrollLoops(astNode) {
   ...
   if (astNode instanceof ast.Loop) {
	 ...
	 throw new Error("unrollLoops: bad formed loop");
      ...
      const clone = astNode.clone();
      const seq = new ast.Sequence("SEQUENCE", null, astNode.loc, ast.nodebug,
...
function compile(machine, astNode) {
   ...
   machine.unrollLoops |= newCompiler;
