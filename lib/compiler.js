...
ast.Sequence.prototype.make_circuit = function() {
   ...
   if (len == 1) {
      res = this.children[i].cinterface.res;
   } else {
      res = net.make_or(this, ast.Sequence, "buffer_res");
      for (let i in this.children) {
	 let res_child = this.children[i].cinterface.res;
   ...
   if (len == 1) {
      susp = this.children[i].cinterface.susp;
   } else {
      susp = net.make_or(this, ast.Sequence, "buffer_susp");
      for (let i in this.children) {
	 let susp_child = this.children[i].cinterface.susp;
