...
/*    Last change :  Wed Dec 17 08:26:42 2025 (serrano)                */
...
function makeSynchronizer(astNode, cLength, rLevel) {
   ...
   const unions = Array.from({length: kLength}, (_, k) =>
	 ...
	 net.makeOr(syncNode, `UNION[${k},${i}]`, i)));
...
function makeForkCircuit_new(env, astNode, childCircuits) {
   ...
   childCircuits.forEach((c, n, _) => {
	 ...
	 connectTo(c.sel, cmin[0][l+1], net.FAN.NEG);
      }
...
function makeTrapCircuit_new(env, astNode, childCircuit) {
   ...
   if (suspicious_k2_kill_connection) {
      if (childCircuit.astNode.depth >= 1) {
	 // experimental
	 connectToList(killList, childCircuit.killList, connMode.RNCAGO);
	 connectToWire(childCircuit.kMatrix[2][l + 1], childCircuit.killList[l + 1]);
      } else {
...
function makePauseCircuit(env, astNode) {
   ...
   // K[0,l] <= REG ^ RES
   const res_and_reg = net.makeAnd(astNode, `K[0,${l}]`, l);
