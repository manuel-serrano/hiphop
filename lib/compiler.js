...
/*    Last change :  Thu Jun  5 08:17:37 2025 (serrano)                */
...
function makeForkCircuit_int(env, astNode, childCircuits) {
   ...
   if (kMatrix.length !== astNode.trapDepth + 2) {
      console.error("makeForkCircuit PAS KLENGTH", kLength, astNode.trapDepth + 2);
   }
   ...
   childCircuits.forEach(c => {
	 ...
	 for (let i = 0; i <= l; i++) {
	    connectTo(ckList[i], sync.union[k][i], net.FAN.STD);
	 }
      }
   });
...
function compile(machine, astNode) {
   ...
   if (machine.reincarnation === undefined) {
      machine.reincarnation = !machine.unrollLoop;
   }

   // assertion
   if (!machine.unrollLoop && !machine.reincarnation && !machine.syncReg) {
      console.error('*** WARNING: suspicious compilation mode, use "unroll" or "reincarnation".');
   }
   if (machine.syncReg && !machine.unrollLoop) {
