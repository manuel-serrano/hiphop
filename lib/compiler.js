...
/*    Last change :  Sun Mar  2 06:49:48 2025 (serrano)                */
...
/*    connMode ...                                                     */
/*---------------------------------------------------------------------*/
const connMode = { GO: 101, K: 102, KGO: 103, FORK: 104, MERGE: 105, BRANCH: 106, CONNECT: 107 };
...
function makeForkCircuit_int(env, astNode, childCircuits) {
   ...
   // c.GO <- GO
   childCircuits.forEach(c => connectToList(goList, c.goList));
...
function makeForkCircuit(env, astNode, childCircuits) {
   ...
   } else if (false && intCompiler && origCompilerFuns.indexOf("makeForkCircuit") < 0) {
...
function makeAbortNonImmediateCircuit_int(env, astNode, childCircuit) {
   ...
   connectToList(killList, childCircuit.killList, connMode.K);
   connectTo(susp, childCircuit.susp);

   // K0 <- c.K0, for i < l
   connectToList(childCircuit.kMatrix[0], kMatrix[0], connMode.KGO);
/*    for (let i = 0; i < l; i++) {                                    */
...
function makeEmitCircuit_new(env, astNode, signame) {
   ...
   connectTo(new net.makeOr(astNode, "false"), sel);
