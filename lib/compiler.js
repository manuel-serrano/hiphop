...
/*    Last change :  Tue Jul 31 14:32:38 2018 (serrano)                */
...
function getSignalObject( machine, tag, loc, signal_name, scope_offset=0 ) {
   ...
   if( !sig ) {
      throw new TypeError( tag + ": unknown signal `" + signal_name + "'.", loc);
   }
...
InitVisitor.prototype.visit = function(ast_node) {
   ...
   if (ast_node.id) {
	 ...
	 throw new SyntaxError("id `" + ast_node.id + "' must be unique", ast_node.loc);
...
SignalVisitor.prototype.visit = function( ast_node ) {
   function _error_already_used( name, loc ) {
      throw new SyntaxError( "Signal name `" + name + "' already used.",
			     loc );
   }
	 ...
	 if( prop.bound ) {
	    ...
	    if( prop.bound == -1 ) {
		  ...
		  prop.bound = prop.name;
	       ...
	       throw new SyntaxError( "Signal `" + prop.name + "' is " +
