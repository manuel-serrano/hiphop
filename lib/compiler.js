...
function Interface(ast_node, type, go_list, res, susp, kill_list, sel,
   ...
   if (kill_list && !(kill_list instanceof Array))
      throw new error.InternalError("`kill_list` must be an array.",
				    ast_node.loc);
   this.kill_list = kill_list || this.get_kill_list(ast_node.lvl);
...
Interface.prototype.get_res = function() {
   for (let i in this.ast_node.children) {
      let res_child = this.ast_node.children[i].cinterface.get_res();
...
Interface.prototype.get_susp = function() {
   ...
   for (let i in this.ast_node.children) {
      let susp_child = this.ast_node.children[i].cinterface.get_susp();
...
Interface.prototype.get_kill_list = function(level) {
   ...
   for (let i in this.ast_node.children) {
      let kill_child =
	  this.ast_node.children[i].cinterface.get_kill_list(level);
