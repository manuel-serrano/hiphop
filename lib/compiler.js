...
/*    Last change :  Tue Feb 18 13:42:05 2025 (serrano)                */
...
ast.Trap.prototype.makeCircuit = function(env, sigtable) {
   traceEnter(`${this.constructor.name}[${this.depth}]@${this.loc.pos}, ${this.trapName}...`);
...
function makeSequenceCircuit_new(env, astNode, childCircuits) {
   ...
   for (let i = 0; i < len - 1; i++) {
      ...
      const curDepth = cur.astNode.depth;
      const curK0 = cur.kMatrix[0];
      const next = childCircuits[i + 1];
      const nextDepth = next.astNode.depth;
      const nextGo = next.goList;
      const commondepth = Math.min(curDepth, nextDepth);

      if (curDepth === nextDepth) {
	 connectToList(curK0, nextGo);
      } else if (curDepth < nextDepth) {
	 for (let l = 0; l <= curDepth; l++) {
	    connectTo(curK0[l], nextGo[l]);
	 }
      } else {
	 for (let l = 0; l <= curDepth - 2; l++) {
	    connectTo(curK0[l], nextGo[l]);
	 }
	 const k0_to_go = net.makeOr(astNode, "k0_go_go", curDepth - 1);
	 connectTo(curK0[curDepth - 1], k0_to_go);
	 connectTo(curK0[curDepth], k0_to_go);
	 connectTo(k0_to_go, nextGo[nextDepth]);
      }
   }
...
function makePauseCircuit_orig(env, astNode) {
   ...
   let res_and_reg = net.makeAnd(astNode, "res^reg", astNode.depth);
...
function makeLoopCircuit_orig(env, astNode, childCircuit) {
   ...
   for (let l = 0; l <= depth1; l++) {
      let govk0 = net.makeOr(astNode, "go_or_k0", l);
...
function makeLoopCircuit_new(env, astNode, childCircuit) {
   ...
   const govk0 = net.makeOr(astNode, "go_or_k0", l); 
   connectTo(goList[l], govk0);
   connectTo(childCircuit.kMatrix[0][l], govk0);
...
function makeLocalCircuit_new(env, astNode, sigtable) {
   ...
   astNode.sigDeclList.forEach(sigprop => {
	 ...
	 const s = new signal.Signal(
	    astNode, sigprop, ast.Local,
	    pre_reg,
	    kMatrix[1], killList, susp, l);
...
ast.Module.prototype.makeCircuit = function(env, sigtable) {
   ...
   for (let i = 0; i < this.sigDeclList.length; i++) {
	 ...
	 : null;
      const s = new signal.Signal(this, sigdecl, ast.Module, pre_reg, null, null, null, this.depth);
...
function unrollLoops(astNode) {
   ...
   if (astNode instanceof ast.Loop) {
	 ...
	 throw new Error("unrollLoops: bad formed loop");
      ...
      const body = astNode.children[0];
      const clone = astNode.duplicate().setMachine(astNode.machine);
      const seq = new ast.Sequence("SEQUENCE", null, astNode.loc,
				   ast.nodebug,
				   [ body, clone.children[0] ]);
      clone.children = [ seq ];
      return clone;
   } else {
      return astNode;
...
/*    computeNodeDepth ...                                             */
/*---------------------------------------------------------------------*/
function computeNodeDepth_orig(astNode, depth, in_loop) {
   ...
   astNode.children.forEach((c, i, a) => { 
      computeNodeDepth(c, depth, in_loop) ;
   });
}

function computeNodeDepth_new(astNode, depth, inloop) {
   
   function rec(astNode, depth, inloop) {
      astNode.depth = depth;
	 ...
	 if (inloop) {
	    ...
	    astNode.children.forEach(c => rec(c, depth, inloop));
	 }
      } else {
	 astNode.children.forEach(c => rec(c, depth, inloop));
      }
   }
   rec(astNode, depth, inloop);
}

function computeNodeDepth(astNode, depth, inloop) {
   if (newCompiler && origCompilerFuns.indexOf("makeForkCircuit") < 0) {
      // MS18feb2025, substract 1, because in _orig mode, computeNodeDepth
      // is called with an initial value of 1. When the swap to the new
      // compiler is complete, change the extern call
      return computeNodeDepth_new(astNode, depth - 1, inloop);
   } else {
      return computeNodeDepth_orig(astNode, depth, inloop);
   }
}
...
function compile(machine, astNode) {
   ...
   astNode.acceptAuto({visit: astNode => {
      if (!astNode.machine) {
	 console.log("ERROR PAS MACHINE", astNode.loc);
      }}});
