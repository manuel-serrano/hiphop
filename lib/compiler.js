/*=====================================================================*/
/*    serrano/prgm/project/hiphop/hiphop/lib/c.js                      */
/*    -------------------------------------------------------------    */
/*    Author      :  Colin Vidal, Manuel Serrano                       */
/*    Creation    :  Mon Jul 16 18:06:28 2018                          */
/*    Last change :  Sat Feb  8 06:47:41 2025 (serrano)                */
...
/*---------------------------------------------------------------------*/
function makeCircuitInterface(ast_node, type) {
   const l = ast_node.depth;
   const go_list = makeWireList(l + 1, ast_node, type, "go");
   const res = new net.WireNet(ast_node, type, "res");
   const susp = new net.WireNet(ast_node, type, "susp");
   const kill_list = makeWireList(l + 1, ast_node, type, "kill");
   const k_matrix = [makeWireList(l + 1, ast_node, type, "k0"),
		     makeWireList(l + 1, ast_node, type, "k1")];
   const sel = new net.WireNet(ast_node, type, "sel");

   return { l, go_list, res, susp, kill_list, k_matrix, sel };
}
...
function assertCircuitInterface(c) {
   ...
   function checkNetList(list, name) {
      if (!c.go_list)
	 throw new Error(`bad circuit "${c.type}", missing ${mame}`);
      if (!c.go_list.length === l)
	 throw new Error(`bad circuit "${c.type}", wrong ${name} length`);
      c.go_list.forEach((e, i, _) => {
	 if (!(e instanceof net.Net)) {
	    throw new Error(`bad circuit "${c.type}", wrong net ${name}[${i}]`);
	 }
      });
   }
      
   const ast_node = c.astNode;
   const l = ast_node.depth;

   checkNetList(c.go_list, "go_list");
   checkNetList(c.kill_list, "kill_list");
   if (!c.k_matrix)
      throw new Error(`bad circuit "${c.type}", wrong k_matrix[${i}]`);
   for (let k = 0; k < c.k_matrix.length; k++) {
      checkNetList(c.k_matrix[k], `k_matrix[${k}]`);
   }

   if (!(c.res instanceof net.Net))
      throw new Error(`bad circuit "${c.type}", wrong net res`);
   if (!(c.susp instanceof net.Net))
      throw new Error(`bad circuit "${c.type}", wrong net susp`);
   if (!(c.sel instanceof net.Net))
      throw new Error(`bad circuit "${c.type}", wrong net sel`);
}
...
function linkNode(ast_node) {
   ...
   if (ast_node.dynamic) {
      ...
      reg.connectTo(ast_node.circuit.go_list[ast_node.depth]);
      reg.dynamic = true;
...
/*    -------------------------------------------------------------    */
/*    If the FROM list is smaller than the TO list, connect the        */
/*    last gate of FROM to the last gate to TO.                        */
/*---------------------------------------------------------------------*/
function connectToList(l, from_list, to_list) {
   // assert
   if (!(typeof l === "number")) {
      throw new Error(`connectToList l is not a number`);
   }
   ...
   if (to_list.length > from_list.length) {
      from_list[l].connectTo(to_list[l + 1]);
   }
}
...
/*    makeWireList ...                                                 */
/*---------------------------------------------------------------------*/
function makeWireList(length, ast_node, type, name) {
   return Array.from({length}, (_, i) => {
      return new net.WireNet(ast_node, type, name + "[" + i + "]");
   });
}
...
ast.Fork.prototype.makeCircuit = function(env, sigtable) {
   ...
   function makeParallelCircuit(env, ast_node, childCircuits) {
	 ...
	 for (let k = 2; k < union.length; k++) {
	    for (let i = union[k].length - 1; i >= 0; i--) {
		  ...
		  kill_list[i] = net.makeOr(ast_node, ast.Fork, "pkill", i);
	       ...
	       union[k][i].connectTo(kill_list[i]);
	    }
	 }
      }
	 ...
	 go_list[ast_node.depth].connectTo(sel_not);
	 ...
	 for (let l = union[k].length - 1; l >= 0; l--) {
	    ...
	    union[k][l].connectTo(k_matrix[k][l]);
	 }
      }
	 ...
	 for (let k = min[c].length - 1; k >= 0; k--) {
	    for (let l = min[c][k].length - 1; l >= 0; l--) {
	       min[c][k][l].connectTo(k_matrix[k][l]);
	    }
	 }
      }
...
/*---------------------------------------------------------------------*/
function makeSequenceCircuit_orig(env, ast_node, childCircuits) {
   ...
   for (let i = 0; i < len; i++) {
	 ...
	 for (let l = 0; l < kList.length; l++) {
	    ...
	    kList[l].connectTo(k_matrix[j][l]);
	 }
      }
   }
...
function makeSequenceCircuit_new(env, ast_node, childCircuits) {
   // circuits common interface
   const { l, go_list, res, susp, kill_list, k_matrix, sel } =
      makeCircuitInterface(ast_node, ast.Pause);
   const orSel = net.makeOr(ast_node, ast.Sequence, "sel");
   const len = childCircuits.length;

   // debug assertion
   childCircuits.foreach(assertCircuitInterface);

   for (let i = 0; i < len; i++) {
      const c = childCircuits[i];

      // connect children SEL to the sequence SEL
      c.sel.connectTo(orSel);
	 ...
	 for (let l = 0; l <= c.astNode.depth; l++) {
	    const  next_l = Math.min(next, next_depth);
	    
	    c.k_matrix[0][l].connectTo(next_go_list[next_l]);
	 }
      }

      // connects Kn where n > 0
      for (let j = 1; j < c.k_matrix.length; j++) {
	 const kList = ccutils.getKListChild(ast_node, ast.Sequence, c, j);
	 connectToList(XXX, kList, k_matrix[j]);
   ...
   // get K0 of last child
   connectToList(XXX, child.Cricuits[len - 1].k_matrix[0], k_matrix[0]);

   // children connections
   childCircuits.forEach(c => {
      // RES
      res.connectTo(c.susp);
      // SUSP
      susp.connectTo(c.susp);
      // KILL
      connectToList(l, kill_list, c.kill_list);
      if (c.ast_node.depth > l) {
	 kill_list[l].connectTo(c.kill_list[l + 1]);
      }
   });
   orSel.connectTo(sel);
...
function makeSequenceCircuit(env, ast_node, childCircuits) {
   if (process.env.NEW === "true") {
      return makeSequenceCircuit_new(env, ast_node, childCircuits);
   } else {
      return makeSequenceCircuit_orig(env, ast_node, childCircuits);
   }
}
...
function makePauseCircuit_new(env, ast_node) {
   // circuits common interface
   const { l, go_list, res, susp, kill_list, k_matrix, sel } =
      makeCircuitInterface(ast_node, ast.Pause);

   const reg = new net.RegisterNet(ast_node, ast.Pause, "reg", 0);
   const reglhs = net.makeAnd(ast_node, ast.Pause, "reg_or_lhs");
   const regrhs = net.makeOr(ast_node, ast.Pause, "reg_or_rhs");

   // reglhs = (SUS ^ REG ^ !KILL[l])
   susp.connectTo(reglhs);
   reg.connectTo(reglhs);
   kill_list[l].connectTo(reglhs, net.FAN.NEG);

   // regrhs = V (GO[i] ^ !KILL[i])
   for (let i = 0; i <= l; i++) {
      // GO[i] ^ !KILL[i]
      const and = net.makeAnd(ast_node, ast.Pause, "reg_or_rhs_and", i);
      go_list[i].connectTo(and);
      kill_list[i].connectTo(and, net.FAN.NEG);
      and.connectTo(regrhs);
   }

   // REG := reglhs v regrhs
   const or = net.makeOr(ast_node, ast.Pause, "reg_or")
   reglhs.connectTo(or);
   regrhs.connectTo(or);
   or.connectTo(reg);
   ...
   // K[1, i] <= GO[i]
   connectToList(l, go_list, k_matrix[1]);
   
   // K[0,l] <= REG ^ RES
   const and = net.makeAnd(ast_node, ast.Pause, "and_k0");
   reg.connectTo(and);
   res.connectTo(and);
   and.connectTo(k_matrix[0][l]);
...
function makePauseCircuit(env, ast_node) {
   if (process.env.NEW === "true") {
      return makePauseCircuit_new(env, ast_node);
   } else {
      return makePauseCircuit_orig(env, ast_node);
   }
}
...
function makeLoopCircuit(env, ast_node, childCircuit) {
   ...
   for (let l = 0; l <= depth1; l++) {
      ...
      or.connectTo(childCircuit.go_list[l]);
      go_list[l] = or;
   }
...
/*    makeAtomCircuit ...                                              */
/*---------------------------------------------------------------------*/
function makeAtomCircuit_orig(env, ast_node, sigtable) {
   const go_list = [];
   const k_matrix = [[]];
   const atomFunc = ast_node.func;
   const frame = ast_node.frame;
   ...
   for (let i = 0; i <= ast_node.depth; i++) {
      ...
      const goNet = net.makeOr(ast_node, ast.Atom, "go", i);
      const actionNet = new net.ActionNet(ast_node, ast.Atom, "action", i,
					  func, ast_node.accessor_list);
      goNet.connectTo(actionNet);
      go_list[i] = goNet;
      k_matrix[0][i] = actionNet
   }

   return new Circuit(ast_node, ast.Atom,
		      // --
		      go_list,   // GO
		      null,      // RES
		      null,      // SUSP
...
function makeAtomCircuit_new(env, ast_node, sigtable) {
   // circuits common interface
   const { l, go_list, res, susp, kill_list, k_matrix, sel } =
      makeCircuitInterface(ast_node, ast.Atom);
   const atomFunc = ast_node.func;
   const frame = ast_node.frame;
   const func = frame ? function() { atomFunc(frame) } : atomFunc;
   ...
   for (let i = 0; i <= l; i++) {
      ...
      const actionNet = new net.ActionNet(ast_node, ast.Atom, "action", i,
					  func, ast_node.accessor_list);
      go_list[i].connectTo(actionNet);
      actionNet.connectTo(k_matrix[0][i]);
   }
...
function makeAtomCircuit(env, ast_node, sigtable) {
   if (process.env.NEW === "true") {
      return makeAtomCircuit_new(env, ast_node, sigtable);
   } else {
      return makeAtomCircuit_orig(env, ast_node, sigtable);
   }
}
...
function makeSuspendCircuit(env, ast_node, childCircuit) {
   ...
   if (k_matrix.length > 1) {
      if (k_matrix[1][ast_node.depth]) {
      	 or2.connectTo(k_matrix[1][ast_node.depth]);
      }
   }
...
/*---------------------------------------------------------------------*/
function makeAbortNonImmediateCircuit_orig(env, ast_node, childCircuit) {
   const and1 = net.makeAnd(ast_node, ast.Abort, "and1_sel_res");
   ...
   for (let l = 0; l <= ast_node.depth; l++) {
      ...
      go.connectTo(childCircuit.go_list[l]);
      go_list[l] = go;
   }
   ...
   if (ast_node instanceof ast.Exec) {
      ast_node.exec_status.callback_wire.connectTo(and3);
      ...
      go_list.forEach(n => n.connectTo(reset));
...
function makeAbortNonImmediateCircuit_new(env, ast_node, childCircuit) {
   // circuits common interface
   const { l, go_list, res, susp, kill_list, k_matrix, sel } =
      makeCircuitInterface(ast_node, ast.Abort);
   let e;

   // debug assertion
   assertCircuitInterface(childCircuit);
   
   if (ast_node instanceof ast.Exec) {
      e = ast_node.exec_status.callback_wire;
   } else if (ast_node.func_count) {
      // counter test expression
      // when a counter must be created, AND and AND-not gates must be
      // connected only on the counter output (and not on expr test)
      e = makeExpressionNet(env, ast_node, ast.Abort, l);
      const { cnt, reset } = makeCounterNet(env, ast_node, ast.Abort);
   ...
   const and = net.makeAnd(ast_node, ast.Abort, "res_and_sel");
   ...
   // NRES = RES ^ !S[l(s)]
   const nres = net.makeAnd(ast_node, ast.Abort, "nres");
   and.connectTo(nres);
   e.connectTo(nres, net.FAN.NEG);

   nres.connectTo(childCircuit.res);
   
   // K[0,l] <= RES ^ S[l(s)]
   const k0l = net.makeAnd(ast_node, ast.Abort, "k0l");
   const k0 = net.makeOr(ast_node, ast.Abort, "k0");
   and.connectTo(k0l);
   e.connectTo(k0l);

   k0l.connectTo(k0);
   childCircuit.k_matrix[0][l].connectTo(k0);
   
   k0.connectTo(k_matrix[0][l]);

   // direct connections from the parent to the child circuit
   connectToList(l, go_list, childCircuit.go_list);
   connectToList(l, kill_list, childCircuit.kill_list);
   susp.connectTo(childCircuit.susp);

   connectToList(l - 1, childCircuit.k_matrix[0], k_matrix[0]);
   for (let k = 1; k < childCircuit.k_matrix.length; k++) {
      connectToList(l, childCircuit.k_matrix[k], k_matrix[k]);
   }
...
function makeAbortNonImmediateCircuit(env, ast_node, childCircuit) {
   if (process.env.NEW === "true") {
      return makeAbortNonImmediateCircuit_new(env, ast_node, childCircuit);
   } else {
      return makeAbortNonImmediateCircuit_orig(env, ast_node, childCircuit);
   }
}
...
function makeAbortCircuit(env, ast_node, childCircuit, force_non_immediate) {
   if (!force_non_immediate && ast_node.immediate) {
      const abortc = makeAbortNonImmediateCircuit(env, ast_node, childCircuit);
      const ifc = [ makeNothingCircuit(env, ast_node), abortc ];
      
      return makeIfCircuit(env, ast_node, ifc);
   } else {
      return makeAbortNonImmediateCircuit(env, ast_node, childCircuit);
   }
}
...
function makeIfCircuit(env, ast_node, childCircuits) {
   ...
   for (let i = 0; i <= ast_node.depth; i++) {
	 ...
	 go.connectTo(e);
	 e.connectTo(and_else);
	 e.connectTo(and_then, net.FAN.NEG);
      } else {
	 const e = makeExpressionNet(env, ast_node, ast.If, i);
	 go.connectTo(e);
	 e.connectTo(and_then);
	 e.connectTo(and_else, net.FAN.NEG);
      ...
      and_then.connectTo(childCircuits[0].go_list[i]);
      and_else.connectTo(childCircuits[1].go_list[i]);
...
ast.Exec.prototype.makeCircuit = function(env, sigtable) {
   ...
   for (let l = 0; l <= this.depth; l++) {
	 ...
	 this, ast.Exec, "start", l, function() {
      ...
      await_node.go_list[l].connectTo(start);
	 ...
	 this, ast.Exec, "kill", l, function() {
      ...
      await_node.kill_list[l].connectTo(kill);
...
ast.Local.prototype.makeCircuit = function(env, sigtable) {
   ...
   this.sigDeclList.forEach(sigprop => {
	 ...
	 for (let l = 0; l <= this.depth; l++) {
	    ...
	    go_list[l].connectTo(action_init);
   ...
   if (nestedKillList) {
	 ...
	 kill_list[k].connectTo(nestedKillList[k]);
      }
   }
   ...
   for (let l = 0; l <= this.depth; l++) {
      ...
      go.connectTo(child.circuit.go_list[l]);
   }
...
ast.Module.prototype.makeCircuit = function(env, sigtable) {
   ...
   const const1 = net.makeOr(this, ast.Module, "global_const0");
   ...
   // signals connections
   signalsReady.connectTo(child.circuit.go_list[0]);
   ...
   if (child.circuit.kill_list) {
      const0.connectTo(child.circuit.kill_list[0]);
      const0.connectTo(child.circuit.kill_list[1]);
   }

   if (child.circuit.go_list) {
      const0.connectTo(child.circuit.go_list[1]);
   }
...
function compile(machine, ast_node) {
   ...
   if (machine.dumpNets) {
      machine.dumpNets(machine, false, ".nets0.json");
   }
   // remove all temporary wire nets
   sweep.unwire(machine);
