...
/*    Last change :  Wed Jan 26 09:03:34 2022 (serrano)                */
...
function compile(machine, ast_node) {
   ...
   if (size > 0) {
       if (posFlag) {
	  //console.log(`Warning: causality cycle of length ${size} detected, involving signals ${signals.toString() }`);
       }
       else {
	  //console.log(`Warning (No location available): causality cycle of length ${size} detected, involving signals ${signals.toString() }`);
       }
   } else if (size < 0) {
