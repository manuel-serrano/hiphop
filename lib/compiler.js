...
/*    Last change :  Wed Dec 17 10:12:22 2025 (serrano)                */
...
function bindSigAccessorList(env, accessorList, astNode, warningp) {
   accessorList.forEach(sigprop => {
      if (warningp && !sigprop.get_value && astNode.machine.verbose >= 0) {
	 ...
	 console.error(`*** WARNING: mixing signal value and signal status`);
...
/*    connMode ...                                                     */
/*---------------------------------------------------------------------*/
const connMode = { GO: 101, K: 102, KGO: 103, KK: 104, FORK: 105, MERGE: 106, BRANCH: 107, CONNECT: 108, RNCAGO: 109, RNCAKILL: 110, RNCADOWN: 111, RNCADROPDOWN: 112, RNCADROPUP: 113 };
...
function connectToList(from_list, to_list, mode = connMode.BRANCH) {
   ...
   switch (mode) {
	 ...
	 if (fl > tl) {
	    ...
	    if (fl < tl) {
		  ...
		  throw new Error(`connectToList(RNCAGO), from list too small ${fl}/${tl}`);
	       ...
	       connectTo(makeUnconnectedNet(from_list[fl - 1], tl - 1), to_list[tl - 1], net.FAN.STD);
	    }
	 }
	 return;
	 ...
	 return;
      ...
      case connMode.RNCADOWN:
	 if (fl < tl) {
	    throw new Error(`connectToList(RNCADOWN), from list too big ${fl}/${tl}`);
	 } else {
	    for (let i = 0; i < tl; i++) {
	       connectTo(from_list[i], to_list[i], connType);
	    }
	    if (fl > tl) {
	       if (fl - 1 !== tl) {
		  throw new Error(`connectToList(RNCADOWN), from list too small ${fl}/${tl}`);
	       } else {
		  const to = to_list[tl - 1];
		  const nto = connectToWire(from_list[fl - 1], to, net.FAN.STD);
		  to_list[tl - 1] = nto;
	       }
	    }
	 }
	 return;
      ...
      case connMode.RNCADROPDOWN:
	 if (fl - 1 !== tl) {
	    throw new Error(`connectToList(RNCADROPDOWN), wrong lists ${fl}/${tl}`);
	 } else {
	    for (let i = 0; i < tl; i++) {
	       connectTo(from_list[i], to_list[i], connType);
	    }
	 ...
	 return;
      ...
      case connMode.RNCADROPUP:
	 if (fl + 1 !== tl) {
	    throw new Error(`connectToList(RNCADROPUP), wrong lists ${fl}/${tl}`);
	 } else {
	    for (let i = 0; i < fl; i++) {
	       connectTo(from_list[i], to_list[i], connType);
	    }
...
function makeForkCircuit_new(env, astNode, childCircuits) {
   ...
   const { l, goList, res, susp, killList, kMatrix, sel } =
      makeCircuitInterface(astNode);
   const kLength = kMatrix.length;
   const lc = childCircuits[0].astNode.depth;

   let orKillList = [];

   // c.GO <- GO
   childCircuits.forEach(c => connectToList(goList, c.goList, connMode.RNCAGO));
   ...
   if (kLength > 2) {
	 ...
	 net.makeOr(astNode, "Vkill", i));
      connectToList(killList, orKillList, connMode.RNCAKILL);
  
      childCircuits.forEach(c =>
	 connectToList(orKillList, c.killList, connMode.RNCAKILL));
   } else {
      childCircuits.forEach(c =>
   ...
   // build and connect the synchronizer
   const sync = makeSynchronizer(astNode, childCircuits.length, lc);
   
   // Union K of children
   childCircuits.forEach(c => {
      for (let k = 0; k < kLength; k++) {
	 connectToList(c.kMatrix[k], sync.unions[k], connMode.CONNECT);
      }
   });
...
/*       console.log("D=", c.astNode.depth, "l=", l, "lc=", lc);       */
      if (c.astNode.depth === 0) {
	 // DO NOT CONNECT OR-NOT gate from LEM = !(GO ^ c.SEL)
	 ...
	 const xem = net.makeAnd(astNode, "xem[" + n + "]", l);
	 ...
	 connectTo(c.sel, cmin[0][lc], net.FAN.NEG);

/* 	 const xem = net.makeOr(astNode, "xem[" + n + "]", l);         */
/* 	 connectTo(c.sel, xem, net.FAN.STD);                           */
/* 	 connectTo(goList[lc], xem);                                   */
/* 	 connectTo(xem, cmin[0][l], net.FAN.NEG);                      */
      }
      
      // connect all incarnations of K0 of child
      connectToList(c.kMatrix[0], cmin[0], connMode.CONNECT);
   ...
   // Fig 11.10 and seem to have no reason. The new compiler no longer
   ...
   if (astNode.machine.forkOrKill || false) {
      //console.error("THERE IS SOMETHING WRONG HERE, TO BE FIXED");
      // connect K to KILL
      for (let k = 2; k < sync.unions.length; k++) {
	 const l = orKillList.length;
	 for (let i = l - 1; i >= 0; i--) {
   ...
   // connect the synchronizer to the cicruit kMatrix
   sync.kMatrix.forEach((m, k, _) =>
      connectToList(m, kMatrix[k], connMode.RNCADOWN));
...
function makeTrapCircuit_new(env, astNode, childCircuit) {
   ...
   // debug
   if (childCircuit.kMatrix.length <= 2) {
      throw new TypeError("makeTrapCircuit: illegal child circuit K length " +
	 childCircuit.kMatrix.length);
   }
   
   // c.GO <- GO
   //connectToList(goList, childCircuit.goList, connMode.GO);
   connectToList(goList, childCircuit.goList, connMode.RNCAGO);
   ...
   // Otherwise, the following program would execute only at the first
   // instant:
   ...
   if (suspicious_k2_kill_connection) {
      if (true) {
	 // experimental
	 const nkill = Array.from({length: l + 1}, (_, i) =>
            net.makeOr(astNode, `NKILL[${i}]`, i));

	 connectToList(killList, nkill, connMode.CONNECT);
	 childCircuit.kMatrix[2][l].connectTo(nkill[l], net.FAN.STD);
	 connectToList(nkill, childCircuit.killList, connMode.GO);
      } else {
	 const nkill = Array.from({length: l + 1}, (_, i) =>
            net.makeOr(astNode, `NKILL[${i}]`, i));

	 connectToList(killList, nkill, connMode.CONNECT);
	 connectToList(childCircuit.kMatrix[2], nkill, connMode.CONNECT);
	 connectToList(nkill, childCircuit.killList, connMode.CONNECT);
      }
   } else {
      connectToList(killList, childCircuit.killList, connMode.CONNECT);
   }
   ...
   // K0 <- c.K0 V c.K2
   const k0_or_k2 = Array.from({length: l + 1}, (_, i) =>
      net.makeOr(astNode, `K[0,${i}]`, i));

   const kmode = childCircuit.astNode.depth >= 1
      ? connMode.RNCADOWN : connMode.CONNECT;
   
   connectToList(childCircuit.kMatrix[0], k0_or_k2, kmode);
   connectToList(childCircuit.kMatrix[2], k0_or_k2, kmode);

   connectToList(k0_or_k2, kMatrix[0], kmode);
   
   // K1 < c.K1
   connectToList(childCircuit.kMatrix[1], kMatrix[1], kmode);
   
   // Ki < c.Ki-1
   for (let k = 2; k < astNode.trapDepth + 2; k++) {
      connectToList(childCircuit.kMatrix[k + 1], kMatrix[k], kmode);
   }
...
function makePauseCircuit(env, astNode) {
   ...
   const or = net.makeOr(astNode, "REG")
   const reglhs = net.makeAnd(astNode, "REG.lhs");
   const regrhs = net.makeOr(astNode, "REG.rhs");
...
function makeLoopCircuit_int(env, astNode, childCircuit) {
   ...
   // c.GO[l] <- GO v c.k0[l]
   const go_or_k0 = net.makeOr(astNode, "govk0", l2); 
...
function makeLoopCircuit_new(env, astNode, childCircuit) {
   ...
   if (!astNode.machine.loopSafe) {
      for (let i = 0; i < l; i++) {
	 if (childCircuit.kMatrix[0].length > i) {
	    connectTo(
	       childCircuit.kMatrix[0][l],
	       new net.ActionNet(astNode, "error", i,
				 function() {
				    throw error.TypeError("Instantaneous loop.", astNode.loc) },
				 []));
	 }
   ...
   // c.GO[l] <- GO v c.k0[l]
   const go_or_k0 = net.makeOr(astNode, "INLOOP", l);
   connectTo(goList[l], go_or_k0);
   connectTo(childCircuit.kMatrix[0][l], go_or_k0);
   connectTo(go_or_k0, childCircuit.goList[l]);
   ...
   // Kn <- c.Kn
   for (let k = 1; k < childCircuit.kMatrix.length; k++) {
      connectToList(childCircuit.kMatrix[k], kMatrix[k], connMode.CONNECT);
   }
...
function makeLocalCircuit(env, astNode, sigtable) {
   ...
   astNode.sigDeclList.forEach(sigprop => {
	 ...
	 for (let i = 0; i <= lc; i++) {
	    if (s.init_func) {
	       bindSigAccessorList(env, s.init_accessor_list, astNode, false);
	    ...
	    if (s.reinit_func) {
	       bindSigAccessorList(env, s.reinit_accessor_list, astNode, false);
...
function safeLoopBody(nodes) {
   ...
   function unsafe(n) {
	 ...
	 || n instanceof ast.Abort
	 || n instanceof ast.Every
	 || n instanceof ast.Suspend) {
...
/*---------------------------------------------------------------------*/
/*    computeReincarnationLevel ...                                    */
/*---------------------------------------------------------------------*/
function computeReincarnationLevel_int(astNode, depth, inloop) {
...
function computeReincarnationLevel_new(astNode, depth, inloop) {
...
function computeReincarnationLevel(astNode) {
   try {
      if (compiler(astNode) === "new") {
	 return computeReincarnationLevel_new(astNode, 0, false);
      } else {
	 return computeReincarnationLevel_int(astNode, 1, false);
      }
   } finally {
...
function compile(machine, astNode) {
   ...
   if (machine.reincarnation) {
      computeReincarnationLevel(astNode);
   }
