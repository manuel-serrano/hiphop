...
/*    Last change :  Thu Feb 13 07:25:08 2025 (serrano)                */
...
function connectToList(from_list, to_list, connType = net.FAN.STD) {
   ...
   for (let i = 0; i < l - 2; i++) {
      from_list[i].connectTo(to_list[i], connType);
   }

   if (to_list.length === from_list.length) {
      from_list[l - 2].connectTo(to_list[l - 2], connType);
      from_list[l - 1].connectTo(to_list[l - 1], connType);
   } else if (to_list.length === from_list.length + 1) {
      // to_list.length > from_list.length
      from_list[l - 2].connectTo(to_list[l - 2], connType);
      from_list[l - 1].connectTo(to_list[l - 1], connType);
      from_list[l - 1].connectTo(to_list[l], connType);
   } else if (to_list.length === from_list.length - 1) {
      // to_list.length < from_list.length
      const n = from_list[l - 2].debugName + "v" + to_list[l - 2].debugName;
      const or = net.makeOr(from_list[l - 2].astNode, n, l - 2);
      from_list[l - 2].connectTo(or, connType);
      from_list[l - 1].connectTo(or, connType);
      or.connectTo(to_list[l - 2], connType);
   } else {
...
function makeTrapCircuit_new(env, astNode, childCircuit) {
   ...
   if (childCircuit.kMatrix.length <= 2) {
	 ...
	 return net.makeOr(astNode, "k0vk2");
      });
      const kill_or_k2 = Array.from({length: l + 1}, (_, i) => {
	 return net.makeOr(astNode, "killvk2");
      });
...
function makeSequenceCircuit_orig(env, astNode, childCircuits) {
   ...
   for (let i = 0; i < len; i++) {
	 ...
	 for (let l = 0; l <= childCircuit.astNode.depth; l++) {
	    ...
	    childCircuit.kMatrix[0][l]
	       .connectTo(next_goList[next_l], net.FAN.STD);
	 }
      }
	 ...
	 for (let l = 0; l < kList.length; l++) {
	    ...
	    kList[l].connectTo(kMatrix[j][l], net.FAN.STD);
	 }
      }
   }
...
function makePauseCircuit_orig(env, astNode) {
   ...
   let and_to_k0 = net.makeAnd(astNode, "res^reg");
...
function makePauseCircuit_new(env, astNode) {
   ...
   const regrhs = net.makeOr(astNode, "go^!kill");
   ...
   // K[0,l] <= REG ^ RES
   const and = net.makeAnd(astNode, "res^reg");
   connectTo(reg, and);
   connectTo(res, and);
   connectTo(and, kMatrix[0][l]);
...
function makeLoopCircuit_orig(env, astNode, childCircuit) {
   ...
   for (let l = 0; l <= depth1; l++) {
      let or = net.makeOr(astNode, "govk0/l", l);

      or.connectTo(childCircuit.goList[l], net.FAN.STD);
      goList[l] = or;
   }
   ...
   if (childCircuit.kMatrix[0] && childCircuit.kMatrix[0][depth2]) {
	 ...
	 .connectTo(goList[depth1], net.FAN.STD);
   }
...
function makeLoopCircuit_new(env, astNode, childCircuit) {
   ...
   // c.GO[l] <- GO v c.k0[l]
   const or = net.makeOr(astNode, "govk0/l", l);
   connectTo(goList[l], or);
   connectTo(childCircuit.kMatrix[0][l2], or);
   connectTo(or, childCircuit.goList[l]);
