...
/*    Last change :  Sun Mar  2 19:42:31 2025 (serrano)                */
...
/*    assert & debug                                                   */
/*    -------------------------------------------------------------    */
/*    HIPHOPTRACE=...,compile,connect,trace     compiler debug         */
/*    HIPHOPTRACE_CIRCUIT=Pause,Fork,Trap,...   propagate debug        */
/*    ORIG=makePauseCircuit,makeForkCircuit,... compiler version       */
/*---------------------------------------------------------------------*/
const assert = (process.env.NEW === "true");
const newCompiler = process.env.NEW === "true";
const intCompiler = process.env.NEW === "int";
...
const debugConnect = process.env.HIPHOPTRACE
   && process.env.HIPHOPTRACE.split(",").find(n => n === "connect");
const debugTrace = process.env.HIPHOPTRACE
   && process.env.HIPHOPTRACE.split(",").find(n => n === "trace");
const debugTraceCircuit = process.env.HIPHOPTRACE_CIRCUIT?.split(",") || [];
...
function traceNode(astNode) {
   if (debug) {
      return `${astNode.constructor.name}[${astNode.depth}:${astNode.trapDepth + 2}/${astNode.circuit ? astNode.circuit.kMatrix.length : "?"}]@${astNode.loc.pos}`
   }
}
...
function makeForkCircuit_int(env, astNode, childCircuits) {
   ...
   childCircuits.forEach(c => {
      if (c.killList) connectToList(orKillList, c.killList, connMode.GO);
   });
...
/*    }                                                                */
   ...
   childCircuits.forEach((c, n, _) => {
      ...
      connectToList(c.kMatrix[0], cmin[0]);
   ...
   for (let k = 0; k < kmaxlen; k++) {
      ...
      childCircuits.forEach((c, n, _) =>
	 connectToList(mins[n][k], akmatrix[k]));
   }
...
function makeForkCircuit(env, astNode, childCircuits) {
   ...
   } else if (intCompiler && origCompilerFuns.indexOf("makeForkCircuit") < 0) {
      return makeForkCircuit_int(env, astNode, childCircuits);
   } else {
...
function makeTrapCircuit(env, astNode, childCircuit) {
   ...
   } else if (intCompiler && origCompilerFuns.indexOf("makeTrapCircuit") < 0) {
      return makeTrapCircuit_int(env, astNode, childCircuit);
   } else {
...
function makeExitCircuit(env, astNode) {
   ...
   } else if (intCompiler && origCompilerFuns.indexOf("makeExitCircuit") < 0) {
      return makeExitCircuit_new(env, astNode);
   } else {
...
function makeSequenceCircuit_int(env, astNode, childCircuits) {
   ...
   childCircuits.forEach(c => {
      // c.RES <- RES
      if (c.res) connectTo(res, c.res);
      // c.SUSP <- SUSP
      if (c.susp) connectTo(susp, c.susp);
      // c.KILL <- KILL
      if (c.killList) connectToList(killList, c.killList, connMode.GO);
   });
   ...
   childCircuits.forEach(c => {
      if (c.sel) connectTo(c.sel, orSel, net.FAN.STD);
   });
...
function makeSequenceCircuit_new(env, astNode, childCircuits) {
   ...
   for (let i = 0; i < len - 1; i++) {
      const cur = childCircuits[i];
      const next = childCircuits[i + 1];
      connectToList(cur.kMatrix[0], next.goList);
   }
   
   // get K0 of last child
   connectToList(childCircuits[len - 1].kMatrix[0], kMatrix[0]);
...
function makeSequenceCircuit(env, astNode, childCircuits) {
   ...
   } else if (intCompiler && origCompilerFuns.indexOf("makeSequenceCircuit") < 0) {
      return makeSequenceCircuit_int(env, astNode, childCircuits);
   } else {
...
function makePauseCircuit(env, astNode) {
   ...
   } else if (intCompiler && origCompilerFuns.indexOf("makePauseCircuit") < 0) {
      return makePauseCircuit_new(env, astNode);
   } else {
...
function makeLoopCircuit(env, astNode, childCircuits) {
   ...
   } else if (intCompiler && origCompilerFuns.indexOf("makeLoopCircuit") < 0) {
      return makeLoopCircuit_new(env, astNode, childCircuits);
   } else {
...
function makeNothingCircuit(env, astNode) {
   ...
   } else if (intCompiler && origCompilerFuns.indexOf("makeNothingCircuit") < 0) {
      return makeNothingCircuit_new(env, astNode);
   } else {
...
function makeAtomCircuit(env, astNode, sigtable) {
   ...
   } else if (intCompiler && origCompilerFuns.indexOf("makeAtomCircuit") < 0) {
      return makeAtomCircuit_new(env, astNode);
   } else {
...
function makeSuspendCircuit(env, astNode, childCircuit) {
   ...
   } else if (intCompiler && origCompilerFuns.indexOf("makeSuspendCircuit") < 0) {
      return makeSuspendCircuit_new(env, astNode, childCircuit);
   } else {
...
function makeAbortNonImmediateCircuit(env, astNode, childCircuit) {
   ...
   } else if (intCompiler && origCompilerFuns.indexOf("makeAbortCircuit") < 0) {
      return makeAbortNonImmediateCircuit_int(env, astNode, childCircuit);
   } else {
...
function makeEmitCircuit(env, astNode, signame) {
   ...
   } else if (intCompiler && origCompilerFuns.indexOf("makeEmitCircuit") < 0) {
      return makeEmitCircuit_int(env, astNode, signame);
   } else {
...
function makeIfCircuit(env, astNode, childCircuits) {
   ...
   } else if (intCompiler && origCompilerFuns.indexOf("makeIfCircuit") < 0) {
      return makeIfCircuit_new(env, astNode, childCircuits);
   } else {
...
function makeLocalCircuit(env, astNode, sigtable) {
   ...
   } else if (intCompiler && origCompilerFuns.indexOf("makeLocalCircuit") < 0) {
      return makeLocalCircuit_new(env, astNode, sigtable);
   } else {
...
function computeNodeDepth_new(astNode, depth, inloop) {
   ...
   if (origCompilerFuns.indexOf("makeForkCircuit") < 0) {
...
function compile(machine, astNode) {
   ...
   machine.unrollLoops |= (newCompiler && process.env.UNROLL === "true");
