...
function make_pause(ast_node) {
   ...
   for (let i = 0; i <= ast_node.depth; i++) {
	 ...
	 //
	 // Useless connection? value is anyway propagated directly
	 // via and_to_reg
	 //
...
function _make_emit(ast_node, signal_name) {
   ...
   for (let i = 0; i <= ast_node.depth; i++) {
      ...
      let sig_gate = get_signal_gate(ast_node.machine, ast_node.loc,
				     signal_name, i);
      ...
      //
      if (ast_node instanceof ast.Exec) {
	 let sig = get_signal_object(ast_node.machine, ast_node.loc,
				     signal_name);
...
function _make_emit_if(ast_node) {
   ...
   //
   // That burn the eyes and that will probably brake one day, but it
   ...
   if (ast_node.if_func) {
      let if_circuit;
      let nothing_circuit;
      let if_ast = new ast.If(undefined, ast_node.loc, [], false,
...
ast.Exec.prototype.make_circuit = function() {
   ...
   for(let l = 0; l <= this.depth; l++) {
	 ...
	 exec_status.lvl = l;
      ...
      signal.runtime_signal_accessor(this, this.accessor_list, l);
   }
...
ast.Let.prototype.make_circuit = function() {
   ...
   let res = net.make_or(this, ast.Let, "res_buf");

   //
   // As child circuit can use signal declared in this let, we need to
   // build them first. As we don't know yet if the child circuit uses
   // k1, kill or susp, we have to use buffers...
   //
   let k_matrix = [[], []];
   let kill_list = [];
   let susp = net.make_or(this, ast.Let, "susp_buf");

   for (let i = 0; i <= this.depth; i++) {
      k_matrix[1][i] = net.make_or(this, ast.Let, "k1_buf", i);
      kill_list[i] = net.make_or(this, ast.Let, "kill_buf", i);
      go_list[i] = net.make_or(this, ast.Let, "go", i);
   }

   //
   // Generation of signal in the Let scope.  For now, Only not
   ...
   for (let i in this.signal_declaration_list) {
      ...
      if (!sigdecl.bound) {
	 let s = new signal.Signal(this, sigdecl, ast.Let, k_matrix[1],
				   kill_list, susp);
	 ...
	 //
	 for (let l = 0; l <= this.depth; l++) {
	    let action_init = new net.SignalInitNet(
	       this, ast.Let, s, "init", l, function() {
		  signal.create_scope.call(
		     this, s);
	       }, s.init_accessor_list);
	    go_list[l].connect_to(action_init, net.FAN.STD);

	    let action_reinit = new net.SignalInitNet(
	       this, ast.Let, s, "reinit", l, function() {
   ...
   k_matrix[0] = get_k_list_child(this, ast.Let, child.cinterface, 0);
   for (let i = 2; i < child.cinterface.k_matrix.length; i++)
      k_matrix[i] = get_k_list_child(this, ast.Let, child.cinterface, i)
   ...
   //
   for (let l = 0; l <= this.depth; l++)
      go_list[l].connect_to(child.cinterface.go_list[l], net.FAN.STD);

   this.cinterface = new Interface(this, ast.Let, go_list, res, susp,
...
ast.Module.prototype.make_circuit = function() {
   ...
   for (let i in this.signal_declaration_list) {
	 ...
	 this.machine.output_signal_map[s.name] = s;
      ...
      //
      // For global signals, reinit_func must be call since the first
      // reaction, therefore, init_func is override
      //
      if (s.reinit_func) {
	 let action_reinit = new net.SignalInitNet(
	    this, ast.Module, s, "reinit", 0, function() {
	       signal.resume_scope.call(this, s);
	    }, s.reinit_accessor_list);
	 const0.connect_to(action_reinit, net.FAN.NEG);
      } else if (s.init_func) {
	 let action_init = new net.SignalInitNet(
	    this, ast.Module, s, "init", 0, function() {
...
// substatement, taking account a possible decrementation of
// incarnation
//
...
// level
//
function get_signal_gate(machine, loc, signal_name, lvl) {
   let gate_list = get_signal_object(machine, loc, signal_name).gate_list;
...
//
function get_signal_pre_gate(machine, loc, signal_name) {
   return get_signal_object(machine, loc, signal_name).pre_gate;
}
...
//
// Frist, it try to get a signal in the current scope. Otherwise, it
// goes on the previous scope, to previous scopre, until it comes to
// the global scope. If no signal is found, it is an arror. If a
// global signal is found, it is returned.
//
// If a local signal is found, it looks at the bound propertie of the
// signal. If empty, the local signal is returned. If it is not empty,
// the function is called recursivly to find and return the bounding
// signal.
//
// Therefore, this function use machine.input/output_signal_map and
// machine.cc_local_signal_stack.
//
function get_signal_object(machine, loc, signal_name, scope_offset=0) {
   let scope_len = machine.cc_local_signal_stack.length - 1;
   let i = scope_len - scope_offset;
   ...
   for (; i >= 0; i--) {
	 ...
	 if (prop.name == signal_name) {
	    if (!prop.bound)
	       ...
	       return get_signal_object(machine, loc, prop.bound,
					scope_offset + 1);
	 }
      }
   }
   ...
   if (!sig)
      throw new error.SyntaxError("Unknown signal " + signal_name + ".", loc);
...
// in XML : if more than one attribute is given with the same name,
...
SignalVisitor.prototype.visit = function(ast_node) {
   ...
   let instanceof_let = ast_node instanceof ast.Let;
	 ...
	 if (prop.bound) {
	    ...
	    if (prop.bound == -1) {
		  ...
		  prop.bound = prop.name;
	       ...
	       throw new error.SyntaxError("Signal " + prop.name + " is " +
...
TrapVisitor.prototype.visit = function(ast_node) {
   ...
   if (ast_node_instanceof_trap) {
	 ...
	 throw new error.SyntaxError("Unknown trap name " +
