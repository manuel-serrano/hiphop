...
const _window_viewer = function(d_name, wd, stepper) {
   ...
   return <html>
     <head css=${tr.css} jscript=${tr.jscript}>
		...
		if (stepper_state) {
		   send({ d_name: ${d_name}, type: ${dcommon.STEPPER_NEXT} })
		}
	     }
...
ServerDebugger.prototype.send_md = function(msg) {
   ...
   //
   // WebSocketServer doesn't supports OPEN/CLOSE/etc. properties ?
   //
   if (state != "open") {
...
ServerDebugger.prototype.send_view = function(view_ws, msg) {
   ...
   if (state != "open") {
      //
      // Following code seems to have issues. (removing view_ws while
      // forEach in the liste?)
      //
...
ServerDebugger.prototype.stepper_next = function() {
   this.send_md({ type: dcommon.STEPPER_NEXT });
   this.stepper_steps++;
   this.stepper_update();
}

ServerDebugger.prototype.stepper_enable = function() {
   this.stepper = true;
   this.stepper_remains = 0;
   this.stepper_steps = 0;
...
sdebugger_ws.onconnection = function(evt) {
   ...
   mdebugger_ws.onmessage = function(evt) {
	 ...
	 if (sd) {
	    //
	    // WebSocketServer doesn't supports
	    // OPEN/CLOSE/etc. properties ?
	    //
	    if (sd.mdebugger_ws.readyState == "open") {
	       //
