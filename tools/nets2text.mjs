...
/*    Last change :  Tue Apr  8 08:06:54 2025 (serrano)                */
...
function main(argv) {
   ...
   info.nets.sort((x, y) => x.id < y.id).forEach(n => {
      console.log(`[${TYPES[n.type]}] ${padding(n.$ast.ctor + "@" + n.$ast.loc.pos, 12, false)} | ${padding(n.$name, 40)} | ${num(n.id, 5)}${expr(n)} => {${n.fanout.map(f => f.id + "").join(", ")}} <= {${n.fanin.map(f => f.id + "").join(", ")}}`);
   });
}
